...
export async function qualifyAbi(wasm) {
  for (const exportName of ["__wbindgen_malloc", "__wbindgen_free",
  ...
  // wasm-bindgen 0.2.126 is pinned by the release build. The fixture owns this
  // allocation and frees it; it never borrows a live simulator dispatch frame.
  const frame = wasm.__wbindgen_malloc(FRAME_BYTES, 8) >>> 0;
  if (!frame || frame % 8) throw new Error("Invalid ABI fixture allocation.");
  let checks = 0;
  ...
  try {
    ...
    wasm.__wbindgen_free(frame, FRAME_BYTES, 8);
  }
}
  ...
  self.addEventListener("message", async ({ data }) => {
    try {
      ...
      await rejectMutation({ ...wasm, rspice_ui_wasm_jit_eval_op_v1: () => 0 },
        "an incorrect helper value", (error) => error.message.includes("integer truncation"));
