...
/// Reachable, decoded members and HDL entry points presented to the compiler adapter.
pub struct HdlSourceInputs<'a> {
    pub sources: &'a BTreeMap<String, String>,
    pub roots: &'a BTreeSet<String>,
}

/// One include edge captured while the adapter validates the HDL entry points.
pub struct HdlSourceInclude {
...
pub fn import(
    ...
    for (name, bytes) in files {
        let safe_name = normalize_member_path(&name)
    ...
    let selected_root = if let Some(requested_root) = requested_root {
        ...
        let display_root = normalize_member_path(display_name)
            .ok()
