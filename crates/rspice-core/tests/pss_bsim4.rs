//! BSIM4 shooting state, coupled charge, and ordinary transient handoff.
...
#[test]
fn bsim4_pss_overlap_storage_matches_rc_waveform_and_physical_mode() {
    let netlist = Netlist::parse(
        "BSIM4 overlap RC\nVIN in 0 SIN(0 .1 1meg)\nR1 in out 1k\nM1 0 out 0 0 mm L=1u W=10u M=2 OFF\n.model mm NMOS LEVEL=54 VTH0=3 CAPMOD=0 XPART=-1 CGSO=1e-5 CGDO=0 CGBO=0 CF=0\n.end\n"
    ).unwrap();
...
#[test]
fn bsim4_pss_coupled_native_charge_matches_hb_and_resumes() {
    for (kind, polarity, capmod, nqs, rgate, rbody, rds, method) in [
        ("NMOS", 1.0, 0, 0, 0, 0, 0, IntegrationMethod::Trapezoidal),
        ("PMOS", -1.0, 2, 0, 3, 2, 0, IntegrationMethod::Gear2),
        ("NMOS", 1.0, 2, 1, 0, 0, 1, IntegrationMethod::Trapezoidal),
        ("PMOS", -1.0, 2, 1, 3, 1, 1, IntegrationMethod::Gear2),
    ] {
        let storage = if nqs == 1 && polarity > 0.0 {
            "CGDO=0 CGSO=0 CGBO=0 CF=0 CGDL=0 CGSL=0 CJS=0 CJD=0 CJSWS=0 CJSWD=0 CJSWGS=0 CJSWGD=0"
        } else {
            "CGDO=7.9e-10 CGSO=6.3e-10 CJ=9.5e-4 CJSW=2.4e-10"
        };
        let netlist = Netlist::parse(&format!(
            "BSIM4 PSS\nVDD supply 0 {}\nVIN in 0 SIN({} {} 1G)\nRD supply out 500\nRG in gate 100\nRS source 0 10\nM1 out gate source 0 mm L=.18u W=10u AD=4p AS=4p PD=20u PS=20u M=2 OFF\n.model mm {kind}(LEVEL=54 TOXE=4.1n VTH0={} U0=270 K1=.59 K2=.0026 CAPMOD={capmod} TRNQSMOD={nqs} RGATEMOD={rgate} RBODYMOD={rbody} RDSMOD={rds} RSHG=100 RSH=10 {storage})\n.options hbint tahb=0\n.end\n",
        ...
        let (analysis, state) = engine
            ...
            .unwrap_or_else(|error| panic!("{kind} CAPMOD={capmod} TRNQSMOD={nqs}: {error}"));
