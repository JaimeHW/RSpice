...
#[test]
fn bsim4_checkpoints_preserve_qs_and_nqs_accepted_trajectories() {
    for (nqs, kind, polarity, method) in [
        (0, "NMOS", 1.0, IntegrationMethod::Trapezoidal),
        (0, "PMOS", -1.0, IntegrationMethod::Gear2),
        (1, "NMOS", 1.0, IntegrationMethod::Gear2),
        ...
        let label = format!("BSIM4 {kind} NQS={nqs} {method:?}");
        let deck = format!(
            "BSIM4 accepted state continuation\nVDD supply 0 {}\nVIN in 0 SIN({} {} 1G)\nRD supply out 500\nRG in gate 100\nRS source 0 10\nM1 out gate source 0 mm L=.18u W=10u AD=4p AS=4p PD=20u PS=20u M=2 OFF\n.model mm {kind}(LEVEL=54 TOXE=4.1n VTH0={} U0=270 K1=.59 K2=.0026 CAPMOD=2 TRNQSMOD={nqs} RGATEMOD=3 RBODYMOD=1 RDSMOD={nqs} RSHG=100 RSH=10 CGDO=7.9e-10 CGSO=6.3e-10 CJ=9.5e-4 CJSW=2.4e-10)\n.end\n",
            polarity * 1.8,
            polarity * 0.7,
            polarity * 0.05,
            polarity * 0.37,
...
fn vbic13_clipped_thermal_state_with_tcrth_resumes_exactly() {
    for level in [11, 12] {
        ...
        for method in [
            ...
            let full = assert_scheduled_deck_resumes_exactly(
                &format!("VBIC13 LEVEL={level} clipped thermal state {method:?}"),
                &deck,
                0.5e-6,
                0.237e-6,
                1e-8,
            ...
            let values = &full.branch_currents[branch_index(&full, "Vth")];
            assert!(
                values.iter().any(|value| (value - values[0]).abs() > 1e-4),
                "the thermal sink must vary along the clipped trajectory"
            );
        }
    }
}
