...
mod tests {
    ...
    #[test]
    fn test_digital_state_logic() {
        assert!(DigitalState::One.is_high());
        assert!(DigitalState::OneR.is_high());
        assert!(!DigitalState::Zero.is_high());

        assert!(DigitalState::Zero.is_low());
        assert!(!DigitalState::One.is_low());

        assert!(DigitalState::Unknown.is_unknown());
        assert!(DigitalState::HighZ.is_high_z());
    }

    #[test]
    fn test_digital_state_invert() {
        assert_eq!(DigitalState::Zero.invert(), DigitalState::One);
        assert_eq!(DigitalState::One.invert(), DigitalState::Zero);
        assert_eq!(DigitalState::Unknown.invert(), DigitalState::Unknown);
    }

    #[test]
    fn test_digital_state_and() {
        assert_eq!(DigitalState::One.and(&DigitalState::One), DigitalState::One);
        assert_eq!(
            DigitalState::One.and(&DigitalState::Zero),
            DigitalState::Zero
        );
        assert_eq!(
            DigitalState::Zero.and(&DigitalState::One),
            DigitalState::Zero
        );
        assert_eq!(
            DigitalState::Zero.and(&DigitalState::Zero),
            DigitalState::Zero
        );
        assert_eq!(
            DigitalState::One.and(&DigitalState::Unknown),
            DigitalState::Unknown
        );
        // Zero AND anything is zero
        assert_eq!(
            DigitalState::Zero.and(&DigitalState::Unknown),
            DigitalState::Zero
        );
    }

    #[test]
    fn test_digital_state_or() {
        assert_eq!(DigitalState::One.or(&DigitalState::One), DigitalState::One);
        assert_eq!(DigitalState::One.or(&DigitalState::Zero), DigitalState::One);
        assert_eq!(
            DigitalState::Zero.or(&DigitalState::Zero),
            DigitalState::Zero
        );
        // One OR anything is one
        assert_eq!(
            DigitalState::One.or(&DigitalState::Unknown),
            DigitalState::One
        );
    }

    #[test]
    fn test_digital_state_xor() {
        assert_eq!(
            DigitalState::One.xor(&DigitalState::One),
            DigitalState::Zero
        );
        assert_eq!(
            DigitalState::One.xor(&DigitalState::Zero),
            DigitalState::One
        );
        assert_eq!(
            DigitalState::Zero.xor(&DigitalState::One),
            DigitalState::One
        );
        assert_eq!(
            DigitalState::Zero.xor(&DigitalState::Zero),
            DigitalState::Zero
        );
    }

    #[test]
    fn test_digital_value_resolve() {
        let strong_one = DigitalValue::one();
        let weak_zero = DigitalValue::new(DigitalState::ZeroR, DigitalStrength::Resistive);

        // Strong wins over weak
        let resolved = strong_one.resolve(&weak_zero);
        assert_eq!(resolved.state, DigitalState::One);

        // Same strength, same state = no contention
        let strong_zero = DigitalValue::zero();
        let resolved = strong_zero.resolve(&DigitalValue::zero());
        assert_eq!(resolved.state, DigitalState::Zero);

        // Same strength, different state = contention
        let resolved = strong_one.resolve(&strong_zero);
        assert!(resolved.state.is_unknown());
    }

    #[test]
    fn test_digital_state_from_char() {
        assert_eq!(DigitalState::from_char('0'), Some(DigitalState::Zero));
        assert_eq!(DigitalState::from_char('1'), Some(DigitalState::One));
        assert_eq!(DigitalState::from_char('X'), Some(DigitalState::Unknown));
        assert_eq!(DigitalState::from_char('Z'), Some(DigitalState::HighZ));
        assert_eq!(DigitalState::from_char('?'), None);
    }

    #[test]
    fn test_voltage_conversion() {
        let state = DigitalState::from_voltage(4.5, 0.8, 2.0);
        assert_eq!(state, DigitalState::One);

        let state = DigitalState::from_voltage(0.3, 0.8, 2.0);
        assert_eq!(state, DigitalState::Zero);

        let state = DigitalState::from_voltage(1.5, 0.8, 2.0);
        assert_eq!(state, DigitalState::Unknown);

        let v = DigitalState::One.to_voltage(0.0, 5.0, 2.5);
