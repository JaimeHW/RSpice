...
mod tests {
    ...
    fn test_context_creation() {
        ...
        assert!(ctx.is_init() || ctx.call_type == CallType::Init);
    }
    ...
    fn test_input_output() {
        ...
        assert!((ctx.input("in") - 5.0).abs() < 1e-10);
    ...
    fn test_state_variables() {
        ...
        assert!((ctx.state(0) - 1.5).abs() < 1e-10);
        assert!((ctx.state(1) - 2.5).abs() < 1e-10);

        ctx.advance_state();
        assert!((ctx.state_prev(0) - 1.5).abs() < 1e-10);
    }

    #[test]
    fn test_thermal_voltage() {
        let ctx = CmContext::new();
        let vt = ctx.thermal_voltage();
        // At 300K, Vt ≈ 25.9mV
        assert!(vt > 0.025 && vt < 0.027);
    }

    #[test]
    fn test_digital_event_scheduling() {
        let mut ctx = CmContext::new();
        use super::super::digital::{DigitalState, DigitalStrength};

        let high = DigitalValue {
            state: DigitalState::One,
            strength: DigitalStrength::Strong,
        };

        ctx.set_output_digital("out", high, 1e-9);
