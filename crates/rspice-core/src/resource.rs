...
pub enum ResourceKind {
    ...
    NetlistLines,
    /// Bytes materialized by source expansion, including retained multi-run decks.
    ExpandedSourceBytes,
...
/// Read bytes from an arbitrary stream without ever retaining more than the
/// configured resource budget.
pub(crate) fn read_bytes_limited(
    ...
    loop {
        ...
        ResourceLimitError::ensure(resource, requested, limit)?;
        if bytes.capacity().saturating_sub(bytes.len()) < count {
            bytes.try_reserve(count).map_err(|error| {
                std::io::Error::other(format!(
                    "unable to grow bounded byte buffer to {requested} bytes: {error}"
                ))
            })?;
        }
...
/// Read one file as bytes with metadata and streaming checks so a concurrently
/// growing file cannot bypass the initial size validation.
pub(crate) fn read_file_bytes_limited(
    ...
    read_bytes_limited(file, resource, limit)
}
...
pub struct ResourceLimits {
    ...
    /// Maximum bytes materialized by include/library or multi-run source expansion.
