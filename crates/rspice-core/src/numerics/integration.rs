...
//!
//! They lived in `analysis::core::transient`, which put a numerics primitive
//! above the circuit store, the device models and the XSPICE subsystem that
//! consume it — all three reached upward for a struct describing how to
//! discretize a derivative. Analysis *results* belong above those layers;
//! the discretization rule does not.
