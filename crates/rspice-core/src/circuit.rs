...
mod construction;
mod model_safety;
pub use model_safety::{DeviceModelSafety, ModelSafetyValue};
mod terminals;
...
pub struct CircuitData {
    ...
    pub(crate) device_model_safety: HashMap<String, DeviceModelSafety>,
    ...
    /// Freshly stamped LEVEL=2 current-coordinate rounding bounds. None
    /// refuses a winding-voltage constraint that does not close. This is
    /// assembly scratch, not accepted device or checkpoint state.
    pub(crate) xyce_core_transient_roundoff: Vec<(usize, Option<Value>)>,
