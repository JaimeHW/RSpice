...
mod tests {
    ...
    fn test_diagnostic_creation() {
        let d = Diagnostic::warning(DiagnosticCategory::Convergence, "Test warning")
            .at_time(1.0e-6)
            .at_location("node1");

        assert_eq!(d.level, DiagnosticLevel::Warning);
        assert_eq!(d.category, DiagnosticCategory::Convergence);
        assert_eq!(d.time, Some(1.0e-6));
        assert_eq!(d.location.as_deref(), Some("node1"));
    }

    #[test]
    fn test_diagnostic_display() {
        let d = Diagnostic::error(DiagnosticCategory::Solver, "Matrix singular").at_location("M1");

        let s = format!("{}", d);
        assert!(s.contains("ERROR"));
        assert!(s.contains("SOLVER"));
        assert!(s.contains("Matrix singular"));
    ...
    fn test_diagnostics_container() {
        ...
        diags.info(DiagnosticCategory::General, "Starting simulation");
        diags.warn(DiagnosticCategory::Convergence, "Slow convergence");
        diags.error(DiagnosticCategory::Solver, "Failed to converge");
    ...
    fn test_convergence_quality() {
        let mut quality = ConvergenceQuality::new();

        quality.record_solve(5, 1e-10);
        quality.record_solve(3, 1e-12);
        quality.record_gmin_stepping();
        quality.record_force_accept(42);
        quality.finalize(2);

        assert_eq!(quality.total_iterations, 8);
        assert_eq!(quality.gmin_stepping_count, 1);
        assert_eq!(quality.force_accepted_points, 1);
        assert_eq!(quality.force_accepted_indices, vec![42]);
        assert!((quality.avg_iterations_per_solve - 4.0).abs() < 0.01);
        assert!(quality.has_issues());
    }

    #[test]
    fn test_clean_convergence() {
        let quality = ConvergenceQuality::new();
        assert!(!quality.has_issues());
    ...
    fn test_diagnostics_merge() {
        let mut d1 = SimulationDiagnostics::new();
        d1.info(DiagnosticCategory::General, "Info 1");
        d1.convergence.record_gmin_stepping();

        let mut d2 = SimulationDiagnostics::new();
        d2.warn(DiagnosticCategory::Convergence, "Warning 1");
        d2.convergence.record_source_stepping();

        d1.merge(d2);

        assert_eq!(d1.count(), 2);
        assert_eq!(d1.convergence.gmin_stepping_count, 1);
        assert_eq!(d1.convergence.source_stepping_count, 1);
    }
    ...
    fn test_filter_by_category() {
        ...
        diags.info(DiagnosticCategory::Convergence, "Conv info");
        diags.warn(DiagnosticCategory::Solver, "Solver warning");
        diags.info(DiagnosticCategory::Convergence, "Conv info 2");

        let conv = diags.by_category(DiagnosticCategory::Convergence);
        assert_eq!(conv.len(), 2);

        let solver = diags.by_category(DiagnosticCategory::Solver);
    ...
    fn test_report_generation() {
        let mut diags = SimulationDiagnostics::new();
        diags.warn(DiagnosticCategory::Convergence, "Test warning");
        diags.convergence.total_iterations = 100;
        diags.convergence.finalize(10);

        let report = diags.report();
        assert!(report.contains("Simulation Diagnostics Report"));
