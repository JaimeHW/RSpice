...
mod tests {
    ...
    fn test_drive_strength_ordering() {
        assert!(DriveStrength::HighZ < DriveStrength::Weak);
    ...
    #[test]
    fn test_drive_strength_resistance() {
        // HighZ should have very high resistance
        assert!(DriveStrength::HighZ.to_resistance() > 1e9);

        // Strong should have low resistance
        assert!(DriveStrength::Strong.to_resistance() < 1000.0);

        // Resistance should decrease with increasing strength
        assert!(DriveStrength::Pull.to_resistance() > DriveStrength::Strong.to_resistance());
    }

    #[test]
    fn test_drive_strength_from_resistance() {
        assert_eq!(DriveStrength::from_resistance(1e15), DriveStrength::HighZ);
        assert_eq!(DriveStrength::from_resistance(1e6), DriveStrength::Weak);
        assert_eq!(DriveStrength::from_resistance(10e3), DriveStrength::Pull);
        assert_eq!(DriveStrength::from_resistance(100.0), DriveStrength::Strong);
        assert_eq!(DriveStrength::from_resistance(0.5), DriveStrength::Supply);
    }

    // =========== LogicValue Tests ===========

    #[test]
    fn test_logic_value_invert() {
        assert_eq!(LogicValue::Low.invert(), LogicValue::High);
        assert_eq!(LogicValue::High.invert(), LogicValue::Low);
        assert_eq!(LogicValue::Unknown.invert(), LogicValue::Unknown);
        assert_eq!(LogicValue::HighZ.invert(), LogicValue::HighZ);
    }

    #[test]
    fn test_logic_value_from_voltage() {
        assert_eq!(LogicValue::from_voltage(0.0, 0.8, 2.0), LogicValue::Low);
        assert_eq!(LogicValue::from_voltage(3.3, 0.8, 2.0), LogicValue::High);
        assert_eq!(LogicValue::from_voltage(1.5, 0.8, 2.0), LogicValue::Unknown);
    }

    #[test]
    fn test_logic_value_to_voltage() {
        assert_eq!(LogicValue::High.to_voltage(3.3, 0.0), Some(3.3));
        assert_eq!(LogicValue::Low.to_voltage(3.3, 0.0), Some(0.0));
        assert_eq!(LogicValue::HighZ.to_voltage(3.3, 0.0), None);
    }

    // =========== DrivenSignal Tests ===========

    #[test]
    fn test_driven_signal_creation() {
        let sig = DrivenSignal::strong_high();
        assert_eq!(sig.value, LogicValue::High);
        assert_eq!(sig.strength, DriveStrength::Strong);

        let sig = DrivenSignal::pull_down();
        assert_eq!(sig.value, LogicValue::Low);
        assert_eq!(sig.strength, DriveStrength::Pull);
    }

    #[test]
    fn test_driven_signal_high_z() {
        let sig = DrivenSignal::high_z();
        assert!(sig.is_high_z());
    ...
    #[test]
    fn test_driven_signal_analog() {
        let sig = DrivenSignal::strong_high();
        let (v, r) = sig.to_analog(3.3, 0.0);
        assert_eq!(v, Some(3.3));
        assert!(r < 1000.0); // Low resistance for strong drive

        let sig = DrivenSignal::high_z();
        let (v, r) = sig.to_analog(3.3, 0.0);
        assert!(v.is_none());
        assert!(r > 1e9); // High resistance for Z
    }

    // =========== BusResolver Tests ===========

    #[test]
    fn test_resolver_strong_wins_over_z() {
        let resolver = BusResolver::logic_3v3();

        let result = resolver.resolve(&[DrivenSignal::strong_high(), DrivenSignal::high_z()]);

        assert_eq!(result.value, LogicValue::High);
        assert_eq!(result.strength, DriveStrength::Strong);
    }

    #[test]
    fn test_resolver_strong_wins_over_pull() {
        let resolver = BusResolver::logic_3v3();

        let result = resolver.resolve(&[DrivenSignal::strong_low(), DrivenSignal::pull_up()]);

        assert_eq!(result.value, LogicValue::Low);
        assert_eq!(result.strength, DriveStrength::Strong);
    }

    #[test]
    fn test_resolver_pull_wins_over_z() {
        let resolver = BusResolver::logic_3v3();

        let result = resolver.resolve(&[DrivenSignal::pull_up(), DrivenSignal::high_z()]);

        assert_eq!(result.value, LogicValue::High);
        assert_eq!(result.strength, DriveStrength::Pull);
    }

    #[test]
    fn test_resolver_conflict_same_strength() {
        let resolver = BusResolver::logic_3v3();

        let result = resolver.resolve(&[DrivenSignal::strong_high(), DrivenSignal::strong_low()]);

        // Same strength, different values = conflict
        assert_eq!(result.value, LogicValue::Unknown);
        assert_eq!(result.strength, DriveStrength::Strong);
    }

    #[test]
    fn test_resolver_no_conflict_same_value() {
        let resolver = BusResolver::logic_3v3();

        let result = resolver.resolve(&[DrivenSignal::strong_high(), DrivenSignal::strong_high()]);

        // Same strength, same value = no conflict
        assert_eq!(result.value, LogicValue::High);
        assert_eq!(result.strength, DriveStrength::Strong);
    }

    #[test]
    fn test_resolver_all_z() {
        let resolver = BusResolver::logic_3v3();

        let result = resolver.resolve(&[DrivenSignal::high_z(), DrivenSignal::high_z()]);

        assert!(result.is_high_z());
    }
    ...
    #[test]
    fn test_resolver_analog_thevenin() {
        let resolver = BusResolver::logic_3v3();

        // Single strong driver to VDD
        let (v, r) = resolver.resolve_analog(&[DrivenSignal::strong_high()]);
        assert!((v - 3.3).abs() < 0.01);
        assert!(r < 1000.0);

        // Pull-up and pull-down should give roughly mid-rail
        let (v, _r) =
            resolver.resolve_analog(&[DrivenSignal::pull_up(), DrivenSignal::pull_down()]);
        assert!((v - 1.65).abs() < 0.1); // Mid-rail for equal pulls
    }

    // =========== TristateBuffer Tests ===========

    #[test]
    fn test_tristate_buffer_enabled() {
        let mut buf = TristateBuffer::new("U1".to_string(), 1, 2, 3);

        // Enable with high input
        buf.update(3.3, 3.3, 1.5);

        assert!(buf.is_enabled());
        assert_eq!(buf.output_signal().value, LogicValue::High);
    }

    #[test]
    fn test_tristate_buffer_disabled() {
        let mut buf = TristateBuffer::new("U1".to_string(), 1, 2, 3);

        // Disable (low enable)
        buf.update(0.0, 3.3, 1.5);

        assert!(!buf.is_enabled());
        assert!(buf.output_signal().is_high_z());
    }

    #[test]
    fn test_tristate_buffer_resistance() {
        let buf = TristateBuffer::new("U1".to_string(), 1, 2, 3);

        // When disabled, should have very high resistance
        assert!(buf.output_resistance() > 1e9);
    }

    // =========== PullResistor Tests ===========

    #[test]
    fn test_pull_up_resistor() {
        let pull = PullResistor::pull_up("R1".to_string(), 1, 10e3);

        assert_eq!(pull.direction, LogicValue::High);
        assert_eq!(pull.resistance, 10e3);
        assert_eq!(pull.strength, DriveStrength::Pull);
    }

    #[test]
    fn test_pull_down_resistor() {
        let pull = PullResistor::pull_down("R2".to_string(), 1, 10e3);

        assert_eq!(pull.direction, LogicValue::Low);
        assert_eq!(pull.resistance, 10e3);
    }
    ...
    fn test_bus_creation() {
        let bus = Bus::new("DATA".to_string(), 1, 3.3, 0.0);
        assert_eq!(bus.name, "DATA");
        assert!(bus.value().is_high_z());
    }

    #[test]
    fn test_bus_with_pull_up() {
        let mut bus = Bus::new("SDA".to_string(), 1, 3.3, 0.0);
        bus.add_pull(DrivenSignal::pull_up());

        let resolved = bus.resolve();
        assert_eq!(resolved.value, LogicValue::High);
        assert_eq!(resolved.strength, DriveStrength::Pull);
    }

    #[test]
    fn test_bus_driver_overrides_pull() {
        let mut bus = Bus::new("SDA".to_string(), 1, 3.3, 0.0);
        bus.add_pull(DrivenSignal::pull_up());
        bus.add_driver(DrivenSignal::strong_low());

        let resolved = bus.resolve();
        assert_eq!(resolved.value, LogicValue::Low);
        assert_eq!(resolved.strength, DriveStrength::Strong);
    }

    #[test]
    fn test_bus_conflict_detection() {
        let mut bus = Bus::new("DATA".to_string(), 1, 3.3, 0.0);
        bus.add_driver(DrivenSignal::strong_high());
        bus.add_driver(DrivenSignal::strong_low());

        bus.resolve();
        assert!(bus.has_conflict());
    }

    #[test]
    fn test_bus_thevenin() {
        let mut bus = Bus::new("DATA".to_string(), 1, 3.3, 0.0);
        bus.add_driver(DrivenSignal::strong_high());

        let (v, r) = bus.thevenin_equivalent();
        assert!((v - 3.3).abs() < 0.01);
        assert!(r < 1000.0);
    }

    #[test]
    fn test_bus_active_driver_count() {
        let mut bus = Bus::new("DATA".to_string(), 1, 3.3, 0.0);
        assert_eq!(bus.active_driver_count(), 0);

        bus.add_driver(DrivenSignal::high_z());
        assert_eq!(bus.active_driver_count(), 0);

        bus.add_driver(DrivenSignal::strong_high());
        assert_eq!(bus.active_driver_count(), 1);

        bus.add_driver(DrivenSignal::strong_low());
        assert_eq!(bus.active_driver_count(), 2);
    }

    #[test]
    fn test_bus_clear_drivers() {
        let mut bus = Bus::new("DATA".to_string(), 1, 3.3, 0.0);
        bus.add_driver(DrivenSignal::strong_high());
        bus.add_driver(DrivenSignal::strong_low());
