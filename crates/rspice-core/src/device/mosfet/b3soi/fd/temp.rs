//! Size- and temperature-dependent parameter evaluation for B3SOIFD.
//!
//! Faithful port of ngspice-46 `b3soifdtemp.c` (`B3SOIFDtemp`). One
...
/// pParam + per-instance derived quantities (ngspice `b3soifdSizeDependParam`
/// plus the instance-level fields filled in by `B3SOIFDtemp`).
#[derive(Debug, Clone, Default)]
#[allow(clippy::too_many_arguments)]
pub struct B3SoiFdSized {
    // Model-level temperature data (b3soifdtemp.c lines 50-77).
    pub vtm: Value,
    pub eg0: Value,
    pub ni: Value,
    pub factor1: Value,
...
impl B3SoiFdSized {
    /// Port of `B3SOIFDtemp` (b3soifdtemp.c lines 38-813).
    ///
    /// `temp` is the device operating temperature in Kelvin (ngspice
    /// `CKTtemp`).
    pub fn new(model: &B3SoiFdModel, geom: &B3SoiFdGeometry, temp: Value) -> Result<Self, String> {
        ...
        if p.leff <= 0.0 {
            return Err("B3SOIFD: effective channel length <= 0".to_string());
        }
        p.weff = geom.w - 2.0 * dw;
        if p.weff <= 0.0 {
            return Err("B3SOIFD: effective channel width <= 0".to_string());
        }
        p.leff_cv = geom.l - 2.0 * dlc;
        if p.leff_cv <= 0.0 {
            return Err("B3SOIFD: effective channel length for C-V <= 0".to_string());
        }
        p.weff_cv = geom.w - 2.0 * dwc;
        if p.weff_cv <= 0.0 {
            return Err("B3SOIFD: effective channel width for C-V <= 0".to_string());
        }
        ...
        // (B3SOIFDcheckModel intentionally not ported: paramchk=0 in the
        ...
        if m.k1_given || m.k2_given {
            if !m.k1_given {
                log::warn!("B3SOIFD: k1 should be specified with k2; using 0.53");
                p.k1 = 0.53;
            }
            if !m.k2_given {
                log::warn!("B3SOIFD: k2 should be specified with k1; using -0.0186");
                p.k2 = -0.0186;
            }
        } else {
            if !m.vbx_given {
