//! B3SOIFD model card: full parameter set with ngspice defaults.
//!
//! Parameter list transcribed from ngspice-46 `b3soifd.c` (`B3SOIFDmPTable`),
//! defaults from `b3soifdset.c` (`B3SOIFDsetup`, lines 52-873).
...
///
/// `b3soifdtemp.c` evaluates every binned family as
...
/// Full B3SOIFD `.model` card (one per model statement, shared by instances).
#[derive(Debug, Clone)]
pub struct B3SoiFdModel {
    /// +1.0 for NMOS, -1.0 for PMOS (ngspice `B3SOIFDtype`).
    pub mtype: Value,
    ...
    // Binned families (defaults per b3soifdset.c).
    pub npeak: Binned,
    ...
    // Derived constants (computed in `finish_setup`, ngspice b3soifdset.c
    // lines 970-997).
    pub cox: Value,
    pub cbox: Value,
    pub csi: Value,
...
impl B3SoiFdModel {
    ...
    pub fn from_params(
        ...
        Self::try_from_params(params, is_pmos, nominal_temp_k)
            .expect("B3SOIFD model selectors are valid")
    }
    ...
    pub fn try_from_params(
        ...
        // the XBJT slot (`IOPR("xdif", B3SOIFD_MOD_XBJT, ...)` in b3soifd.c),
        ...
        let mut model = Self {
            ...
            // NOTE: lkb3/wkb3/pkb3 default to 1.0 in b3soifdset.c (lines
    ...
    /// Derived film/oxide constants (b3soifdset.c lines 970-997).
    fn finish_setup(&mut self) {
        ...
        if tmp2 < tmp1 {
            log::warn!(
                "B3SOIFD: vbsa = {:.3} is too large for tsi = {:.3e}; treated as zero",
                self.vbsa.v,
                self.tsi
            );
