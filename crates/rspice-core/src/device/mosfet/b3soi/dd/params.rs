//! B3SOIDD model card: full parameter set with ngspice defaults.
//!
...
///
/// `b3soiddtemp.c` evaluates every binned family as
...
/// Full B3SOIDD `.model` card (one per model statement, shared by instances).
#[derive(Debug, Clone)]
pub struct B3SoiDdModel {
    /// +1.0 for NMOS, -1.0 for PMOS (ngspice `B3SOIDDtype`).
    pub mtype: Value,
    ...
    // Binned families (defaults per b3soiddset.c).
    pub npeak: Binned,
    ...
    // Derived constants (computed in `finish_setup`, ngspice b3soiddset.c
    // lines 970-997).
    pub cox: Value,
    pub cbox: Value,
    pub csi: Value,
...
impl B3SoiDdModel {
    ...
    pub fn try_from_params(
        ...
        let mut model = Self {
            ...
            beta0: binned(p, "BETA0", 30.0, 0.0, 0.0, 0.0),
    ...
    fn finish_setup(&mut self) {
        ...
        if tmp2 < tmp1 {
            log::warn!(
                "B3SOIDD: vbsa = {:.3} is too large for tsi = {:.3e}; treated as zero",
                self.vbsa.v,
                self.tsi
            );
