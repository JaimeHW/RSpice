...
mod tests {
    ...
    #[test]
    fn test_level1_cutoff() {
        let params = test_params();
        let vth = calculate_vth(&params, 0.0);
        let (id, region) = calculate_id_level1(&params, 0.3, 1.0, 0.0, vth);

        assert!(id.abs() < 1e-9, "Cutoff current should be near zero");
        assert_eq!(region, MosRegion::Cutoff);
    }

    #[test]
    fn test_level1_saturation() {
        let params = test_params();
        let vth = calculate_vth(&params, 0.0);
        let (id, region) = calculate_id_level1(&params, 2.0, 3.0, 0.0, vth);
    ...
    fn test_gm_positive() {
        let params = test_params();
        let vth = calculate_vth(&params, 0.0);
        let gm = calculate_gm(&params, 2.0, 1.0, vth);

        assert!(gm > 0.0, "Transconductance should be positive");
    }

    #[test]
    fn test_gds_positive() {
        let params = test_params();
        let vth = calculate_vth(&params, 0.0);
        let gds = calculate_gds(&params, 2.0, 1.0, vth);

        assert!(gds > 0.0, "Output conductance should be positive");
    }
}
