...
//! - the CAPMOD=3 charge-thickness intrinsic charge model (1955-2239), the
//!   junction depletion charges (2253-2431), the capMod>=2 smoothed overlap
//!   charges (2533-2557), and the mode-dependent node-charge assembly for
...
//!
//! Scope notes:
//! - `capMod` 0/1/2 charge models are not ported; requesting charges under
//!   them is a typed error (the DC path is capMod-independent).
//! - NQS (`nqsMod`/`acnqsMod` = 1) and ACM (`acmMod` > 0) are rejected at
//!   device construction; every `nqsMod` branch here is the `== 0` arm.
...
///
/// Errors only when charges are requested with an unported `capMod` (0/1/2).
...
pub fn eval(
    ...
    if compute_charges && model.cap_mod != 3 && model.xpart >= 0.0 {
        return Err(format!(
            "BSIM3: CAPMOD={} charge model is not implemented (only CAPMOD=3)",
            model.cap_mod
        ));
    }
    ...
    if model.xpart < 0.0 {
        ...
        // capMod == 3 — the Charge-Thickness Model (b3ld.c:1955-2239).
        // (capMod 0/1/2 were rejected at the top of this function.)
    ...
    // --- Overlap charges, capMod >= 2 smoothing (b3ld.c:2533-2557) ---
    let t0 = vgd + DELTA_1;
    ...
    // --- Node-charge assembly, nqsMod = 0 (b3ld.c:2560-2594 / 2676-2710) ---
    let qgd = qgdo;
    let qgs = qgso;
    let qgb = p.cgbo * vgb;
    if mode > 0 {
