...
impl Instance {
    pub fn stamp(&mut self, ctx: &GeneratedEvalContext<'_>, stamper: &mut GeneratedStamper<'_>) {
        ...
        let f9=eval_ddt(ddt_state_current, ddt_state_previous, ddt_state_older, ddt_state_initialized, ddt_derivative_current, ddt_derivative_previous, ddt_active, ddt_scale, ddt_previous_value_scale, ddt_older_value_scale, ddt_previous_derivative_scale, 1, f8);let fe=ctx.node_voltage(n[5]);let fi=eval_ddt(ddt_state_current, ddt_state_previous, ddt_state_older, ddt_state_initialized, ddt_derivative_current, ddt_derivative_previous, ddt_active, ddt_scale, ddt_previous_value_scale, ddt_older_value_scale, ddt_previous_derivative_scale, 2, f8);let fo=eval_ddt(ddt_state_current, ddt_state_previous, ddt_state_older, ddt_state_initialized, ddt_derivative_current, ddt_derivative_previous, ddt_active, ddt_scale, ddt_previous_value_scale, ddt_older_value_scale, ddt_previous_derivative_scale, 3, (fe*sf[76]));let fu=ctx.simparam_or("gmin", n_);let fw=(eO>sf[72]);let fx=(if fw{eO}else{sf[72]});let fA=(eT>sf[72]);let fB=(if fA{eT}else{sf[72]});let fL=(if k{n_}else{(if i{m_}else{n_})});let fM=(t*fL);let fN=(fL/sf[5]);let fO=(fN/v);let g8=(fL/a2);let gx=((ar*(-(fM+fM)))+(al*((am*(g8/a4))+(ap*(((ag*(((aa*((a7*fL)+(l*(a6*fL))))-(a8*fL))/(aa*aa)))-(ad*(ae*(fL+fL))))/(ag*ag))))));let gz=((-gx)/sf[18]);let gE=(av*av);let gP=(gx+((av*g8)+(a4*gz)));let h9=(if ((b2)!=0.0){((-(aU*(sf[24]*fM)))/(b4*b4))}else{n_});let ha=(if ((b2)!=0.0){(sf[23]/b4)}else{n_});let hb=(if ((b2)!=0.0){(sf[77]/b4)}else{n_});let hd=(sf[25]*fM);let he=(ba*(-(sf[13]*(sf[14]*fN))));let hh=(ba*ba);let hl=(if ((b2)!=0.0){((he-(b8*hd))/hh)}else{n_});let hm=(if ((b2)!=0.0){(sf[77]/ba)}else{n_});let hn=(if ((b2)!=0.0){(sf[23]/ba)}else{n_});let hr=(if ((b2)!=0.0){((he-(bd*hd))/hh)}else{n_});let hv=(if bj{n_}else{h9});let hw=(if bj{n_}else{ha});let hx=(if bj{n_}else{hb});let hy=(if bp{n_}else{(if bj{h9}else{n_})});let hz=(if bp{n_}else{(if bj{ha}else{n_})});let hA=(if bp{n_}else{(if bj{hb}else{n_})});let hN=(if ((b2)!=0.0){((br_*hy)+(bq*(br_*hv)))}else{hy});let hO=(if ((b2)!=0.0){((br_*hz)+(bq*(br_*hw)))}else{hz});let hP=(if ((b2)!=0.0){((br_*hA)+(bq*(br_*hx)))}else{hA});let hQ=(bB*hl);let hR=(bB*hm);let hS=(bB*hn);let i5=(bN*hr);let iF=(if c7{n_}else{(if ((b2)!=0.0){(((bW*(sf[9]*(H*((sf[6]*fO)+(((u*(sf[7]*fN))-(B*fM))/(u*u))))))+(I*hN))-(((c3*((bV*(sf[11]*(sf[12]*fN)))+(Q*(if ((b2)!=0.0){((if bA{(hQ/bC)}else{(if bE{hQ}else{(if bv{hl}else{n_})})})-(if bM{(i5/bO)}else{(if bQ{i5}else{(if bI{hr}else{n_})})}))}else{n_}))))-(bY*(sf[26]*((sf[15]*(sf[16]*fN))*(c1*(c0).ln())))))/(c3*c3)))}else{n_})});let iG=(if c7{n_}else{(if ((b2)!=0.0){((I*hO)-((Q*(if ((b2)!=0.0){(if bA{(hR/bC)}else{(if bE{hR}else{(if bv{hm}else{n_})})})}else{n_}))/c3))}else{n_})});let iH=(if c7{n_}else{(if ((b2)!=0.0){((I*hP)-((Q*(if ((b2)!=0.0){(if bA{(hS/bC)}else{(if bE{hS}else{(if bv{hn}else{n_})})})}else{n_}))/c3))}else{n_})});let iU=(cl*cl);let j4=(if ((ca)!=0.0){((-(ci*(cg*(sf[28]*fM))))/iU)}else{hv});let j5=(if ((ca)!=0.0){(((cl*sf[78])-(ci*(ck*(if ((ca)!=0.0){(if ce{sf[77]}else{n_})}else{n_}))))/iU)}else{hw});let j6=(if ((ca)!=0.0){(((cl*sf[79])-(ci*(ck*(if ((ca)!=0.0){(if ce{sf[23]}else{n_})}else{n_}))))/iU)}else{hx});let jd=(if cw{n_}else{(if cq{j4}else{hN})});let je=(if cw{n_}else{(if cq{j5}else{hO})});let jf=(if cw{n_}else{(if cq{j6}else{hP})});let jM=(cZ*(sf[33]*(cW*(sf[34]*fO))));let jX=(sf[52]*(-gP));let k7=(aH*aH);let li=ddt_scale;let ln=(sf[75]*li);
        ...
        stamper.stamp_current_node3_local(
            ...
            4,
            multiplicity * ((if ((sf[49])!=0.0){(dw*(-iH))}else{n_})),
        );
        ...
        stamper.stamp_current_node1_local(
            ...
            6,
            multiplicity * ((if ((sf[49])!=0.0){(dw*li)}else{n_})),
        );
        ...
        stamper.stamp_current_node1_local(
            ...
            2,
            multiplicity * ((if ((sf[62])!=0.0){ln}else{n_})),
        );
        stamper.stamp_potential_branch_local(
            Some(5),
            None,
        ...
        stamper.stamp_current_node1_local(
            ...
            2,
            multiplicity * ((if sb[17]{ln}else{n_})),
        );
        stamper.stamp_current_node1_local(
            Some(5),
            None,
        ...
        stamper.stamp_current_node1_local(
            ...
            5,
            multiplicity * ((if sb[17]{(sf[76]*li)}else{n_})),
        );
        stamper.stamp_current_const_local(
            Some(2),
            None,
        ...
        stamper.stamp_current_node3_local(
            Some(0),
            Some(3),
            multiplicity * ((if ((sf[73])!=0.0){(aW/fx)}else{n_})),
            0,
            multiplicity * ((if ((sf[73])!=0.0){(m_/fx)}else{n_})),
            2,
            multiplicity * ((if ((sf[73])!=0.0){((-(aW*(if fw{((if ((sf[49])!=0.0){(jM/dH)}else{jM})/sf[3])}else{n_})))/(fx*fx))}else{n_})),
            3,
            multiplicity * ((if ((sf[73])!=0.0){(ch/fx)}else{n_})),
        );
        stamper.stamp_current_const_local(
            Some(0),
            Some(3),
        ...
        stamper.stamp_current_node3_local(
            Some(1),
            Some(4),
            multiplicity * ((if ((sf[74])!=0.0){(aZ/fB)}else{n_})),
            1,
            multiplicity * ((if ((sf[74])!=0.0){(m_/fB)}else{n_})),
            2,
            multiplicity * ((if ((sf[74])!=0.0){((-(aZ*(if fA{((d7*(sf[36]*(d4*(sf[37]*fO))))/sf[3])}else{n_})))/(fB*fB))}else{n_})),
            4,
            multiplicity * ((if ((sf[74])!=0.0){(ch/fB)}else{n_})),
        );
        stamper.stamp_current_const_local(
            Some(1),
            Some(4),
        ...
        stamper.stamp_current_node3_local(
            ...
            2,
            multiplicity * ((sf[3]*(sf[23]*(iF-(if cE{n_}else{(if ((ca)!=0.0){((cB*(sf[10]*(K*(sf[8]*fO))))+(L*(if ((ca)!=0.0){((cy*jd)+(cx*(cy*(if cq{n_}else{j4}))))}else{jd})))}else{n_})}))))),
            3,
            multiplicity * ((sf[3]*(sf[23]*(iG-(if cE{n_}else{(if ((ca)!=0.0){(L*(if ((ca)!=0.0){((cy*je)+(cx*(cy*(if cq{n_}else{j5}))))}else{je}))}else{n_})}))))),
            4,
            multiplicity * ((sf[3]*(sf[23]*(iH-(if cE{n_}else{(if ((ca)!=0.0){(L*(if ((ca)!=0.0){((cy*jf)+(cx*(cy*(if cq{n_}else{j6}))))}else{jf}))}else{n_})}))))),
        );
        let fH_ddt=eval_ddt(ddt_state_current, ddt_state_previous, ddt_state_older, ddt_state_initialized, ddt_derivative_current, ddt_derivative_previous, ddt_active, ddt_scale, ddt_previous_value_scale, ddt_older_value_scale, ddt_previous_derivative_scale, 4, (sf[3]*(sf[23]*(aP*em))));
        stamper.stamp_current_node3_local(
            Some(3),
            Some(4),
            multiplicity * (fH_ddt),
            2,
            multiplicity * ((((sf[3]*(sf[23]*((em*((aO*((-(sf[17]*(sf[20]*(-(((av*(-gz))-(aw*gz))/gE)))))/(aE*aE)))+(aF*(sf[20]*((az*fL)-(((av*(gP-gz))-(aI*gz))/gE))))))+(aP*((if eb{(((eh*gP)+(aH*(-(eg*(sf[58]*((-((-(aU*gP))/k7))/ed))))))/sf[58])}else{(if ((dP)!=0.0){((dY*gP)/sf[58])}else{n_})})+(if eb{n_}else{(if ((dP)!=0.0){(dV*((e7*jX)+(dN*(((aH*(sf[59]*jX))-(e5*gP))/k7))))}else{n_})}))))))) * ddt_scale)),
            3,
        ...
        let fJ_ddt=eval_ddt(ddt_state_current, ddt_state_previous, ddt_state_older, ddt_state_initialized, ddt_derivative_current, ddt_derivative_previous, ddt_active, ddt_scale, ddt_previous_value_scale, ddt_older_value_scale, ddt_previous_derivative_scale, 5, (sf[3]*(sf[23]*(c8*dw))));
        stamper.stamp_current_node3_local(
            Some(3),
            Some(4),
            multiplicity * (fJ_ddt),
            ...
            4,
            multiplicity * ((((sf[3]*(sf[23]*(dw*iH)))) * ddt_scale)),
        );
        stamper.stamp_current_const_local(
            Some(3),
            Some(4),
    ...
    pub fn stamp_reactive(&mut self, ctx: &GeneratedEvalContext<'_>, stamper: &mut GeneratedReactiveStamper<'_>) {
        ...
        let sf=&self.scalar_static_f64;
        let sb=&self.scalar_static_bool;
        let b=ctx.node_voltage(n[2]);let e=((ctx.temperature()+b)+sf[0]);let g=1300.0;let h=173.14999999999998;let i=(e>h);let j=(if i{e}else{h});let k=(g<j);let l=(if k{g}else{j});let m_=1.0;let n_=0.0;let t=8.6170869e-5;let u=(l*t);let v=(l/sf[5]);let A=(v-m_);let B=(sf[7]*A);let H=((((v).ln()*sf[6])+(B/u))).exp();let I=(sf[9]*H);let Q=(sf[11]*(m_+(A*sf[12])));let V=(sf[13]*(m_+(A*sf[14])));let a2=300.15;let a4=(l/a2);let a6=0.000702;let a7=(l*a6);let a8=(l*a7);let aa=(l+1108.0);let ad=(-(1.16-(a8/aa)));let ae=1.3806226e-23;let ag=(ae*(l+l));let al=(-(u+u));let am=1.5;let ap=1.6021918e-19;let ar=((am*(a4).ln())+(((ad/ag)+1.3454442398941469e20)*ap));let as_=(al*ar);let av=((sf[19]-as_)/sf[18]);let aw=(sf[19]-av);let az=0.0004;let aE=(m_+(sf[20]*(sf[22]-(aw/av))));let aF=(sf[17]/aE);let aH=(as_+(a4*av));let aI=(aH-av);let aO=(m_+(sf[20]*((az*(l-a2))-(aI/av))));let aP=(aF*aO);let aU=(sf[23]*(ctx.node_voltage(n[3])-ctx.node_voltage(n[4])));let b2=(if (I>n_){m_}else{n_});let b4=(u*sf[24]);let b6=(if ((b2)!=0.0){(aU/b4)}else{n_});let b8=((-aU)-V);let ba=(u*sf[25]);let bc=(if ((b2)!=0.0){(b8/ba)}else{n_});let bd=(-V);let bf=(if ((b2)!=0.0){(bd/ba)}else{n_});let bg=80.0;let bi=(if (b6>bg){m_}else{n_});let bj=(((b2)!=0.0)&&((bi)!=0.0));let bp=(((b2)!=0.0)&&(!((bi)!=0.0)));let bq=(if bp{m_}else{(if bj{(m_+(b6-bg))}else{n_})});let br_=((if bj{bg}else{b6})).exp();let bu=37.0;let bv=(bc>=bu);let bw=(!bv);let bx=-37.0;let by=(bc<=bx);let bA=(bw&&(!by));let bB=(bc).exp();let bC=(m_+bB);let bE=(bw&&by);let bI=(bf>=bu);let bJ=(!bI);let bK=(bf<=bx);let bM=(bJ&&(!bK));let bN=(bf).exp();let bO=(m_+bN);let bQ=(bJ&&bK);let bV=(if ((b2)!=0.0){((if bA{(bC).ln()}else{(if bE{bB}else{(if bv{bc}else{n_})})})-(if bM{(bO).ln()}else{(if bQ{bN}else{(if bI{bf}else{n_})})}))}else{n_});let bW=((if ((b2)!=0.0){(bq*br_)}else{bq})-m_);let bY=(Q*bV);let c0=(aU).abs();let c1=f64::powf(c0,(sf[15]*(m_+(A*sf[16]))));let c3=(m_+(sf[26]*c1));let c7=(!((b2)!=0.0));let dw=(sf[46]*(m_+((f64::powf((m_+f64::powf((((ctx.node_voltage(n[0])-ctx.node_voltage(n[1]))/sf[43])).abs(),sf[44])),sf[45])-m_)*sf[47])));let dN=(aU+((-aH)*sf[52]));let dP=(if (dN>n_){m_}else{n_});let dV=(if ((dP)!=0.0){sf[57]}else{n_});let dY=(m_-(sf[54]*(sf[54]*dV)));let e5=(dN*sf[59]);let e7=(sf[54]+(e5/aH));let eb=(!((dP)!=0.0));let ed=(m_-(aU/aH));let eg=((sf[58]*(ed).ln())).exp();let eh=(m_-eg);let em=((if eb{((aH*eh)/sf[58])}else{(if ((dP)!=0.0){((aH*dY)/sf[58])}else{n_})})+(if eb{n_}else{(if ((dP)!=0.0){(dV*(dN*e7))}else{n_})}));let eY=0.0;let f8=(b*sf[75]);let f9=0.0;let fi=0.0;let fo=0.0;let fL=(if k{n_}else{(if i{m_}else{n_})});let fM=(t*fL);let fN=(fL/sf[5]);let g8=(fL/a2);let gx=((ar*(-(fM+fM)))+(al*((am*(g8/a4))+(ap*(((ag*(((aa*((a7*fL)+(l*(a6*fL))))-(a8*fL))/(aa*aa)))-(ad*(ae*(fL+fL))))/(ag*ag))))));let gz=((-gx)/sf[18]);let gE=(av*av);let gP=(gx+((av*g8)+(a4*gz)));let h9=(if ((b2)!=0.0){((-(aU*(sf[24]*fM)))/(b4*b4))}else{n_});let ha=(if ((b2)!=0.0){(sf[23]/b4)}else{n_});let hb=(if ((b2)!=0.0){(sf[77]/b4)}else{n_});let hd=(sf[25]*fM);let he=(ba*(-(sf[13]*(sf[14]*fN))));let hh=(ba*ba);let hl=(if ((b2)!=0.0){((he-(b8*hd))/hh)}else{n_});let hm=(if ((b2)!=0.0){(sf[77]/ba)}else{n_});let hn=(if ((b2)!=0.0){(sf[23]/ba)}else{n_});let hr=(if ((b2)!=0.0){((he-(bd*hd))/hh)}else{n_});let hy=(if bp{n_}else{(if bj{h9}else{n_})});let hz=(if bp{n_}else{(if bj{ha}else{n_})});let hA=(if bp{n_}else{(if bj{hb}else{n_})});let hQ=(bB*hl);let hR=(bB*hm);let hS=(bB*hn);let i5=(bN*hr);let jX=(sf[52]*(-gP));let k7=(aH*aH);let li=1.0;let ln=(sf[75]*li);
        ...
        stamper.stamp_current_reactive_node3_local(
            ...
            Some(4),
            2,
            multiplicity * ((sf[3]*(sf[23]*((em*((aO*((-(sf[17]*(sf[20]*(-(((av*(-gz))-(aw*gz))/gE)))))/(aE*aE)))+(aF*(sf[20]*((az*fL)-(((av*(gP-gz))-(aI*gz))/gE))))))+(aP*((if eb{(((eh*gP)+(aH*(-(eg*(sf[58]*((-((-(aU*gP))/k7))/ed))))))/sf[58])}else{(if ((dP)!=0.0){((dY*gP)/sf[58])}else{n_})})+(if eb{n_}else{(if ((dP)!=0.0){(dV*((e7*jX)+(dN*(((aH*(sf[59]*jX))-(e5*gP))/k7))))}else{n_})}))))))),
            3,
        ...
        stamper.stamp_current_reactive_node3_local(
            ...
            Some(4),
            2,
            multiplicity * ((sf[3]*(sf[23]*(dw*(if c7{n_}else{(if ((b2)!=0.0){(((bW*(sf[9]*(H*((sf[6]*(fN/v))+(((u*(sf[7]*fN))-(B*fM))/(u*u))))))+(I*(if ((b2)!=0.0){((br_*hy)+(bq*(br_*(if bj{n_}else{h9}))))}else{hy})))-(((c3*((bV*(sf[11]*(sf[12]*fN)))+(Q*(if ((b2)!=0.0){((if bA{(hQ/bC)}else{(if bE{hQ}else{(if bv{hl}else{n_})})})-(if bM{(i5/bO)}else{(if bQ{i5}else{(if bI{hr}else{n_})})}))}else{n_}))))-(bY*(sf[26]*((sf[15]*(sf[16]*fN))*(c1*(c0).ln())))))/(c3*c3)))}else{n_})}))))),
            3,
            ...
            4,
            multiplicity * ((sf[3]*(sf[23]*(dw*(if c7{n_}else{(if ((b2)!=0.0){((I*(if ((b2)!=0.0){((br_*hA)+(bq*(br_*(if bj{n_}else{hb}))))}else{hA}))-((Q*(if ((b2)!=0.0){(if bA{(hS/bC)}else{(if bE{hS}else{(if bv{hn}else{n_})})})}else{n_}))/c3))}else{n_})}))))),
        );
    }
}
