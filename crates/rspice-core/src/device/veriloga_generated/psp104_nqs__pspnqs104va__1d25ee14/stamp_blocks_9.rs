...
impl Instance {
    ...
    pub(super) fn stamp_transient_block_144(
        ...
        s.store_sub(2165, 2133, 2153);s.store_scalar(2166, 0.0);s.store_mul(2167, 2129, 2165);s.store_scalar(2168, 1.0);s.store_scalar(2169, 1.0);s.store_scalar(2173, 1.0);s.store_scalar(2174, 1.0);s.store_scalar(2176, 1.0);s.b[2279] = (s.v[2133] > 0.0);s.store_scalar(2279, if s.b[2279] { 1.0 } else { 0.0 });
    ...
    pub(super) fn stamp_transient_block_146(
        ...
        s.store_scalar(2229, 0.0);s.copy_ad(2230, 2167);s.store_scalar(2235, 0.0);s.store_scalar(2234, 1.0);s.copy_ad(2237, 2109);s.copy_ad(2236, 2232);s.b[2288] = (s.v[2133] > 0.0);s.store_scalar(2288, if s.b[2288] { 1.0 } else { 0.0 });s.b[2289] = (s.v[2160] > 1e-100);s.store_scalar(2289, if s.b[2289] { 1.0 } else { 0.0 });
    ...
    pub(super) fn stamp_transient_block_158(
        ...
        if ((s.b[2340] && (!s.b[2344])) && s.b[2346]) {s.store_offset_add(2093, 2092, 725, (-1.0));s.store_scaled_add_sqrt_square_offset_rhs(2094, 2093, 2093, 10.0, 0.5);s.store_add_mixed_ai(2095, A::add_scaled_product(s.ad_value(2092), 1.0, s.ad_value(2090), A::ln(s.ad_value(2094)), (-1.0)), 725);s.store_scaled_add_sqrt_square_offset_rhs(2096, 2095, 2095, 2.0, 0.5);}
