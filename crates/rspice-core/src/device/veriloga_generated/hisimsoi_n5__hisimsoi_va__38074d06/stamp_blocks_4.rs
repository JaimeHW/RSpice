...
impl Instance {
    ...
    pub(super) fn stamp_transient_block_72(
        ...
        let mut tf: usize = 0;
        while {
            let tc: f64 = (2.0 * 20.0);let td: f64 = (tc + 1.0);let te: f64 = if ((((((p[24] != 0.0) && s.b[1604]) && (!s.b[1605])) && (!s.b[1628])) && s.b[1635]) && (s.v[167] <= td)) { 1.0 } else { 0.0 };
            te != 0.0
        } {
    ...
    if s.b[1775] {
        ...
        if (s.b[1775] && s.b[1795]) {s.store_scalar(1785, p[258]);s.store_scaled_voltage(1783, ctx, nodes, Some(7), Some(2), p[50]);}
        ...
        if s.b[1775] {s.store_scalar(1792, ((((p[271] * p[271]) + (p[56] * p[56]))) as f64).sqrt());s.store_primal_scale(1794, 105, p[9]);s.store_primal_scale(1779, 1779, 0.0001);s.store_primal_scale(1780, 1780, 0.01);s.store_scale(1784, 429, 1.0 / (s.v[81]));s.store_powf(328, 1784, p[269]);s.store_div(1787, 1779, 328);}
    }
}
