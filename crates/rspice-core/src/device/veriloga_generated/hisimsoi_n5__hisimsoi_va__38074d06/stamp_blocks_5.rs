...
impl Instance {
    ...
    if s.b[1803] {
        ...
        if (s.b[1803] && s.b[1823]) {s.store_scalar(1813, p[258]);s.store_scaled_voltage(1811, ctx, nodes, Some(7), Some(2), p[50]);}
        ...
        if s.b[1803] {s.store_scalar(1820, ((((p[271] * p[271]) + (p[56] * p[56]))) as f64).sqrt());s.store_primal_scale(1822, 105, p[9]);s.store_primal_scale(1807, 1807, 0.0001);s.store_primal_scale(1808, 1808, 0.01);s.store_scale(1812, 429, 1.0 / (s.v[81]));s.store_powf(328, 1812, p[269]);s.store_div(1815, 1807, 328);s.store_sub_ad(327, A::add_scaled_product(A::scale_offset(s.ad_value(1812), 0.4, 1.8), 1.0, s.ad_value(1812), s.ad_value(1812), 0.1), A::scale_offset(s.ad_value(1812), (-p[270]), p[270]));}
    }
    #[inline(never)]
    pub(super) fn stamp_transient_block_81(
        ...
        if s.b[1803] {s.store_div(1816, 1808, 327);s.store_add_mixed_ia(1809, 1809, A::scaled_offset(s.ad_value(429), (-s.v[81]), p[274]));s.store_scalar(1804, (1.0 + (p[279] / ((s.v[100]) as f64).powf(p[280]))));s.store_scalar(1806, (1.0 + (p[277] / ((s.v[100]) as f64).powf(p[278]))));s.store_scalar(1805, (1.0 + (p[275] / ((s.v[109]) as f64).powf(p[276]))));s.store_mul(1815, 1815, 1804);s.store_offset_product3(1816, s.ad_value(1816), s.ad_value(1805), s.ad_value(1806), 1.0, 1e-50);s.store_div(1817, 1811, 1813);s.store_mul(1818, 1815, 1817);}
    ...
    pub(super) fn stamp_transient_block_82(
        ...
        s.b[1849] = (p[43] == 1.0);s.store_scalar(1849, if s.b[1849] { 1.0 } else { 0.0 });s.b[1850] = ((p[37] != 0.0) || ((p[25] == 1.0) && (p[26] == 2.0)));s.store_scalar(1850, if s.b[1850] { 1.0 } else { 0.0 });s.b[1851] = (p[43] == 0.0);s.store_scalar(1851, if s.b[1851] { 1.0 } else { 0.0 });
    }
    #[inline(never)]
    ...
    pub(super) fn stamp_reactive_block_2(
        ...
        s.store_scalar(101, (p[1] / p[9]));s.store_scalar(102, p[60]);s.store_scalar(103, (if (s.v[56] < 1.0) { 0.0 } else { p[295] }));s.store_scalar(104, (if (s.v[56] < 1.0) { p[60] } else { p[61] }));s.b[634] = (p[43] == 0.0);s.store_scalar(634, if s.b[634] { 1.0 } else { 0.0 });
    ...
    pub(super) fn stamp_reactive_block_3(
        ...
        let mut t3: usize = 0;
        while {
            let t2: f64 = if (s.b[639] && (s.v[562] < p[9])) { 1.0 } else { 0.0 };
            t2 != 0.0
        } {
        ...
        if (s.b[652] && (p[24] == 0.0)) {s.store_scalar(534, 0.0);s.store_scalar(532, 0.0);}
    }
    #[inline(never)]
    pub(super) fn stamp_reactive_block_4(
        ...
        if (s.b[652] && (p[24] == 0.0)) {s.store_scalar(533, 0.0);s.store_scalar(531, 0.0);}
