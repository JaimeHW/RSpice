...
impl Instance {
    ...
    pub(super) fn stamp_transient_block_80(
        ...
        if ((s.b[1620] && (!s.b[1685])) && (!s.b[1692])) {s.store_div_scaled_inputs3_indices(168, 206, -1.0, 899, (-1.0), 219, 1.0, 167, 1.0);s.store_scaled_add_sqrt_square_offset_rhs(168, 168, 168, ((4.0 * 0.01) * 0.01), 0.5);s.store_div_scaled_value_offset_denominator(169, s.ad_value(661), 1.0, s.ad_value(168), 0.001, 1.0);}
        ...
        s.b[1694] = (p[44] == 0.0);s.store_scalar(1694, if s.b[1694] { 1.0 } else { 0.0 });s.b[1695] = ((s.v[865] <= 0.0) || (s.v[659] <= 0.0));s.store_scalar(1695, if s.b[1695] { 1.0 } else { 0.0 });
        ...
        if (((s.b[1620] && (!s.b[1694])) && s.b[1697]) && (!s.b[1698])) {s.store_add_scaled_product_mixed_iia(370, 869, (-1.0 / (s.v[184])), 874, A::scale_offset(s.ad_value(639), p[600], (((((-1.0)) * (p[600]))) + (1.0))), 1.0);s.store_scale(167, 875, s.v[184]);s.store_div_scaled_product_offset_denominator_indices(168, 870, 167, 1.0, 167, 1.0, 1.0);}
    }
    #[inline(never)]
    ...
    pub(super) fn stamp_transient_block_82(
        ...
        if s.b[1620] {s.store_mul(1095, 373, 379);s.store_mul(502, 666, 463);s.store_mul(505, 667, 494);s.store_scale(508, 671, (s.v[189] * p[2]));}
    }
    #[inline(never)]
    ...
    pub(super) fn stamp_transient_block_88(
        ...
        if s.b[1620] {s.store_scaled_add_sqrt_square_offset_rhs(246, 1075, 1075, ((0.25 * 0.1) * 0.1), 0.5);s.store_add(245, 1076, 1077);s.store_add_scaled_inputs(167, 245, 1.0 / (p[230]), 246, (p[231] * 1.0 / (p[230])));s.store_scaled_add_sqrt_square_offset_rhs(167, 167, 167, ((4.0 * 0.001) * 0.001), 0.5);s.store_offset_powf_ad(168, s.ad_value(167), (0.7 * p[229]), 1.0);s.store_div_from_scalar(427, (p[228] * 1.9e-9), 168);s.store_div_from_scalar_ad(428, (3.9 * 8.8541878128e-12), A::add_scaled_inputs(s.ad_value(429), (3.9 * 1.0 / (p[110])), s.ad_value(427), 1.0 / (s.v[200])));s.store_mul_scale_offset_mixed_ia(387, 1075, A::div_from_scalar((8.8541878128e-12 * p[110]), s.ad_value(429)), (-(((p[2] * s.v[187]) * s.v[188]) + p[1379])), 0.0);s.store_scale(391, 428, (((p[2] * s.v[187]) * s.v[188]) + p[1379]));}
    ...
    pub(super) fn stamp_transient_block_89(
        ...
        if s.b[1620] {s.store_sub(1032, 235, 215);s.store_scalar(1034, (3.453133e-11 / p[75]));s.store_primal_mul_ad_affine_product_rhs(1037, 909, s.ad_value(1034), A::scale_offset(s.ad_value(1036), ((s.v[187] / p[1373]) * p[2]), p[1382]), p[1388], 0.0);s.store_mul_sub_rhs(1038, 1037, 1032, 1033);s.copy_ad(1039, 1038);}
        ...
        if (s.b[1620] && s.b[1737]) {s.store_primal_scale(1042, 1034, p[17]);s.store_scalar(1043, (p[1396] * p[17]));s.store_primal_scale(1044, 1034, p[18]);s.store_scalar(1045, (p[1396] * p[18]));s.store_mul_scale_offset_indices(177, 236, 379, -1.0, 0.0);}
    }
    #[inline(never)]
    pub(super) fn stamp_transient_block_90(
        ...
        if (s.b[1620] && s.b[1737]) {s.store_mul_scale_offset_indices(178, 237, 379, -1.0, 0.0);}
    ...
    pub(super) fn stamp_transient_block_92(
        ...
        let (t5,) = {
    if (s.b[1620] && s.b[1741]) {
        let t4: f64 = (0.001 * s.v[95]);
        (t4,)
    } else {
        (s.v[101],)
    }
};
        s.store_scalar(101, t5);
        if (s.b[1620] && s.b[1741]) {s.store_div_scaled_inputs_indices(167, 726, (-s.v[184]), 300, 1.0);s.store_add_scaled_inputs_product_mixed_aaii(4, A::div_scaled_product(A::scale_offset(s.ad_value(743), 1.0 / (s.v[184]), 1.0), s.ad_value(706), ((1.602176462e-19 * p[74]) * p[74]), s.ad_value(271), (2.0 * s.v[180])), 1.0, A::div_from_scalar(p[294], s.ad_value(271)), 1.0, 3, 216, (-1.0));}
        let (td,) = {
    if (s.b[1620] && s.b[1741]) {
        let t6: f64 = (-s.v[4]);let t7: f64 = { let limited_exp_arg = t6; if limited_exp_arg > 80.0 { LIMEXP_MAX * (1.0 + limited_exp_arg - 80.0) } else if limited_exp_arg < -80.0 { 1.804851387e-35 } else { limited_exp_arg.exp() } };let t8: f64 = (t7 + s.v[4]);let t9: f64 = (t8 - 1.0);let ta: f64 = (t9).sqrt();let tb: f64 = (s.v[294] * ta);let tc: f64 = (s.v[4] + tb);
        (tc,)
    } else {
        (s.v[104],)
    }
};
        s.store_scalar(104, td);s.b[1745] = (s.v[4] < s.v[97]);s.store_scalar(1745, if s.b[1745] { 1.0 } else { 0.0 });s.b[1746] = (s.v[214] < s.v[104]);s.store_scalar(1746, if s.b[1746] { 1.0 } else { 0.0 });s.b[1747] = (((s.v[214]) as f64).abs() <= s.v[101]);s.store_scalar(1747, if s.b[1747] { 1.0 } else { 0.0 });
