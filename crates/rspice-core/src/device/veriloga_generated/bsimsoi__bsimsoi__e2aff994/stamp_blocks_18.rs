...
impl Instance {
    ...
    pub(super) fn stamp_reactive_block_72(
        ...
        if (s.b[1620] && (!s.b[1676])) {s.store_scalar(167, (((((-0.5) * s.v[184]) * s.v[184]) / p.p595) / p.p595));s.store_limited_exp(603, 167);s.store_sub_from_scalar(169, 1.0, 603);s.store_scale(167, 601, ((1.0 / s.v[184]) + (1.0 / p.p595)));}
    }
    #[inline(never)]
    ...
    pub(super) fn stamp_reactive_block_75(
        ...
        s.b[1694] = (p.p44 == 0.0);s.store_scalar(1694, if s.b[1694] { 1.0 } else { 0.0 });s.b[1695] = ((s.v[865] <= 0.0) || (s.v[659] <= 0.0));s.store_scalar(1695, if s.b[1695] { 1.0 } else { 0.0 });s.b[1696] = (s.v[355] > (s.v[659] / 80.0));s.store_scalar(1696, if s.b[1696] { 1.0 } else { 0.0 });
        ...
        if (((s.b[1620] && (!s.b[1694])) && (!s.b[1697])) && (!s.b[1699])) {s.store_add_scaled_product_mixed_iia(370, 869, (-1.0 / (s.v[184])), 874, A::scale_offset(s.ad_value(639), p.p600, (((((-1.0)) * (p.p600))) + (1.0))), 1.0);s.store_scale(167, 875, s.v[184]);s.store_div_scaled_product_offset_denominator_indices(168, 870, 167, 1.0, 167, 1.0, 1.0);}
    }
    #[inline(never)]
