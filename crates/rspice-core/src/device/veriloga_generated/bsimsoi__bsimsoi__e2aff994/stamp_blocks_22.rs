...
impl Instance {
    ...
    pub(super) fn stamp_reactive_block_123(
        ...
        if ((s.b[1620] && s.b[1775]) && s.b[1782]) {s.store_offset_mul(169, 818, 228, 1.0);s.store_mul3_lhs(170, 491, 168, 169);s.store_limited_exp(171, 170);s.store_sub(169, 204, 219);s.store_sqrt_square_offset(229, 169, 0.0001);}
    ...
    pub(super) fn stamp_reactive_block_124(
        ...
        if ((s.b[1620] && s.b[1775]) && s.b[1782]) {s.store_offset_mul(169, 821, 229, 1.0);s.store_mul3_lhs(170, 491, 168, 169);s.store_limited_exp(171, 170);}
    ...
    pub(super) fn stamp_reactive_block_125(
        ...
        if (s.b[1620] && s.b[1791]) {s.store_add_scaled_product(614, A::div_scaled_product3_by_product(s.ad_value(1005), s.ad_value(610), s.ad_value(1006), 1.0, s.ad_value(171), s.ad_value(1007), 1.0), 1.0, A::div(s.ad_value(1004), s.ad_value(167)), A::add_scaled_inputs3(s.ad_value(168), 1.0, s.ad_value(169), 1.0, s.ad_value(170), 1.0), 1.0);s.store_mul3_affine_lhs(172, 608, 613, ((s.v[183] * p[2]) * 10000000000.0), 0.0, 613);s.store_mul_ad_product_lhs_mixed_ai(615, A::div(s.ad_value(1008), s.ad_value(172)), 380, 380);s.store_add(173, 615, 614);}
        if s.b[1620] {s.store_scaled_div(167, 243, 607, 1.0 / (s.v[184]));s.store_square(168, 167);s.store_offset_scaled(170, 168, (((p[1022] * s.v[184])) * (p[1019])), p[1019]);s.store_offset_scaled(171, 168, (((p[1023] * s.v[184])) * (p[1020])), p[1020]);s.store_offset_scaled(172, 168, (((p[1298] * s.v[184])) * (p[1297])), p[1297]);s.store_square(633, 172);s.store_square(632, 171);s.copy_ad(345, 343);}
    ...
    pub(super) fn stamp_reactive_block_127(
        ...
        if (s.b[1620] && s.b[1799]) {s.store_mul_scale_offset(293, A::add_scaled_inputs_product(s.ad_value(843), 1.0, s.ad_value(844), 1.0 / (s.v[184]), s.ad_value(845), s.ad_value(218), 1.0), A::pow(s.ad_value(639), s.ad_value(846)), 1.0, (-1.0));s.store_mul_scale_offset_rhs(946, 300, 218, p[1264], 1.0);}
        ...
        if (s.b[1620] && s.b[1799]) {s.store_add_div_rhs_mixed_ia(171, 290, 284, A::pow_from_scalar(s.v[184], s.ad_value(286)));s.store_add_scaled_product_mixed_iia(278, 278, 1.0, 171, A::tanh(A::mul(s.ad_value(288), s.ad_value(227))), (-1.0));s.store_add_scaled_inputs3_mixed_aii(242, A::offset(A::add(A::add_scaled_inputs4(s.ad_value(291), 1.0, s.ad_value(278), 1.0, s.ad_value(944), 1.0, s.ad_value(293), -1.0), s.ad_value(945)), p[1151]), 1.0, 956, 1.0, 932, 1.0);s.store_add_scaled_inputs_product_indices(213, 222, 1.0, 212, (-1.0), 242, 270, (-1.0));s.store_add_scaled_inputs_product_mixed_aiii(367, A::add_scaled_product(s.ad_value(222), 1.0, s.ad_value(218), s.ad_value(270), (-1.0)), 1.0, 212, (-1.0), 242, 270, (-1.0));s.store_add_scaled_inputs_product_indices(214, 222, 1.0, 212, (-1.0), 242, 270, (-1.0));s.store_sub(216, 238, 215);s.store_scaled_sqrt_mul_scaled_lhs(294, 953, ((2.0 * 1.602176462e-19) * s.v[180]), 270, 1.0 / (s.v[199]));s.store_scalar(947, (p[1148] * (1.0 + (p[1149] * ((s.v[184]) as f64).powf((-p[1150]))))));s.store_mul_scale_offset_indices(294, 294, 947, 1.0, 1.0);s.store_div_from_scalar(295, 1.0, 294);s.store_square(296, 294);s.store_div_from_scalar(297, 1.0, 296);s.store_scalar(5, (s.v[180] / p[74]));s.store_scalar(7, (s.v[181] / p[75]));s.store_primal_div_scaled_inputs2_indices(3, 7, 1.0, 728, 1.0, 5, 1.0);s.store_scalar(2, (p[76] / p[75]));s.store_div(124, 294, 2);s.store_offset_scaled(125, 124, 0.7071067811865475, 1.0);s.store_scale(126, 125, 1e-7);s.store_scalar(127, (5.0 / 4.0));s.store_div_from_scalar(128, 1.0, 124);s.store_square(129, 124);s.store_div_from_scalar_ad(130, 1.0, A::add_scaled_inputs(s.ad_value(127), 1.0, s.ad_value(124), 0.7324648775608221));}
