...
impl Instance {
    ...
    pub(super) fn stamp_transient_block_32(
        ...
        if s.b[1431] {s.store_scaled_offset_ad(340, A::div_scaled_offset_numerator(s.ad_value(182), 1.0, (-1.0), s.ad_value(782), 1.0), 1.0, 0.5);s.store_scaled_add_mixed_ai(182, A::offset(s.ad_value(182), (-1.0)), 782, 0.5);}
        ...
        s.store_scaled_mul(335, 1436, 790, 0.5);s.store_scale(781, 335, (2.0 * 1.0 / (p[262])));s.store_offset_mul_offset_rhs_mixed_ia(782, 781, A::mul_offset_rhs(s.ad_value(781), A::mul_offset_rhs(s.ad_value(781), A::mul_offset_rhs(s.ad_value(781), A::mul(s.ad_value(781), A::scale_offset(s.ad_value(781), (1.0 / 5040.0), (1.0 / 720.0))), (1.0 / 120.0)), (1.0 / 24.0)), (1.0 / 6.0)), (1.0 / 2.0), 1.0);s.store_offset_mul_offset_rhs_mixed_ia(783, 781, A::mul_offset_rhs(s.ad_value(781), A::mul_offset_rhs(s.ad_value(781), A::mul(s.ad_value(781), A::scale_offset(s.ad_value(781), (1.0 / 840.0), (1.0 / 144.0))), (1.0 / 30.0)), (1.0 / 8.0)), (1.0 / 3.0), (1.0 / 2.0));s.store_div_from_scalar(1437, p[262], 782);s.store_div_scaled_inputs_square_rhs(336, 783, (-2.0), 782, 1.0);s.b[1442] = (s.v[1437] < 1e-12);s.store_scalar(1442, if s.b[1442] { 1.0 } else { 0.0 });
        if s.b[1442] {s.store_scalar(1437, 1e-12);}
        s.store_add(1438, 1435, 1437);s.store_add_scaled_inputs(1439, 790, 1.0, 1437, 2.0);s.store_add(1440, 791, 1437);s.store_add_scaled_inputs3_offset_indices(85, 791, 1.0, 120, 1.0, 182, -1.0, (-s.v[160]));
    }
    #[inline(never)]
    pub(super) fn stamp_transient_block_33(
        ...
        let t0: f64 = (s.v[160] - s.v[120]);let t1: f64 = (t0 + s.v[182]);let t2: f64 = (t1 + s.v[1435]);s.store_scalar(86, t2);s.b[1443] = (s.v[963] != 0.0);s.store_scalar(1443, if s.b[1443] { 1.0 } else { 0.0 });s.b[1444] = (p[42] == 1.0);s.store_scalar(1444, if s.b[1444] { 1.0 } else { 0.0 });s.b[1445] = (p[42] == 2.0);s.store_scalar(1445, if s.b[1445] { 1.0 } else { 0.0 });s.b[1446] = (p[42] == 3.0);s.store_scalar(1446, if s.b[1446] { 1.0 } else { 0.0 });
