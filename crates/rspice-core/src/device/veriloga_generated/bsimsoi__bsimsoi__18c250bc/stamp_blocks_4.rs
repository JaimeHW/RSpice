...
impl Instance {
    ...
    pub(super) fn stamp_transient_block_65(
        ...
        s.b[1552] = ((s.v[399] == 0.0) || (s.v[399] == 2.0));s.store_scalar(1552, if s.b[1552] { 1.0 } else { 0.0 });s.b[1553] = (p[39] == 3.0);s.store_scalar(1553, if s.b[1553] { 1.0 } else { 0.0 });s.b[1554] = ((p[39] == 0.0) || (p[39] == 2.0));s.store_scalar(1554, if s.b[1554] { 1.0 } else { 0.0 });s.b[1555] = ((p[39] == 0.0) || (p[39] == 1.0));s.store_scalar(1555, if s.b[1555] { 1.0 } else { 0.0 });s.b[1556] = (p[39] == 2.0);s.store_scalar(1556, if s.b[1556] { 1.0 } else { 0.0 });s.b[1558] = (s.v[37] == 2.0);s.store_scalar(1558, if s.b[1558] { 1.0 } else { 0.0 });s.b[1559] = ((p[36] == 1.0) && (p[14] != 0.0));s.store_scalar(1559, if s.b[1559] { 1.0 } else { 0.0 });s.b[1560] = ((p[35] != 0.0) && (!true));s.store_scalar(1560, if s.b[1560] { 1.0 } else { 0.0 });s.b[1561] = true;s.store_scalar(1561, if s.b[1561] { 1.0 } else { 0.0 });s.b[1562] = true;s.store_scalar(1562, if s.b[1562] { 1.0 } else { 0.0 });s.b[1563] = (p[430] == 2.0);s.store_scalar(1563, if s.b[1563] { 1.0 } else { 0.0 });s.b[1564] = (p[430] == 2.0);s.store_scalar(1564, if s.b[1564] { 1.0 } else { 0.0 });s.b[1565] = ((p[35] != 0.0) && (!true));s.store_scalar(1565, if s.b[1565] { 1.0 } else { 0.0 });s.b[1566] = true;s.store_scalar(1566, if s.b[1566] { 1.0 } else { 0.0 });s.b[1567] = true;s.store_scalar(1567, if s.b[1567] { 1.0 } else { 0.0 });
    }
    #[inline(never)]
    pub(super) fn stamp_transient_block_66(
        s: &mut Scratch,
    ) {
        s.copy_ad(426, 916);s.copy_ad(427, 918);s.copy_ad(428, 919);s.store_add(425, 896, 895);s.store_sub(918, 427, 895);s.store_sub(919, 428, 896);s.store_add(916, 426, 425);
    }
    #[inline(never)]
    ...
    pub(super) fn stamp_reactive_block_12(
        ...
        let mut t45: usize = 0;
        while {
            let t2: f64 = (s.v[526] - s.v[527]);let t15: f64 = (s.dn[526][0] - s.dn[527][0]);let t16: f64 = (s.dn[526][1] - s.dn[527][1]);let t1b: f64 = (s.dn[526][2] - s.dn[527][2]);let t1c: f64 = (s.dn[526][3] - s.dn[527][3]);let t1d: f64 = (s.dn[526][4] - s.dn[527][4]);let t1e: f64 = (s.dn[526][5] - s.dn[527][5]);let t1f: f64 = (s.dn[526][6] - s.dn[527][6]);let t20: f64 = (s.dn[526][7] - s.dn[527][7]);let t21: f64 = (s.dn[526][8] - s.dn[527][8]);let t22: f64 = (s.dn[526][9] - s.dn[527][9]);let t17: f64 = (s.dn[526][10] - s.dn[527][10]);let t18: f64 = (s.dn[526][11] - s.dn[527][11]);let t19: f64 = (s.dn[526][12] - s.dn[527][12]);let t1a: f64 = (s.dn[526][13] - s.dn[527][13]);let t3: f64 = (s.db[526][0] - s.db[527][0]);let t4: f64 = (s.db[526][1] - s.db[527][1]);let td: f64 = (s.db[526][2] - s.db[527][2]);let te: f64 = (s.db[526][3] - s.db[527][3]);let tf: f64 = (s.db[526][4] - s.db[527][4]);let t10: f64 = (s.db[526][5] - s.db[527][5]);let t11: f64 = (s.db[526][6] - s.db[527][6]);let t12: f64 = (s.db[526][7] - s.db[527][7]);let t13: f64 = (s.db[526][8] - s.db[527][8]);let t14: f64 = (s.db[526][9] - s.db[527][9]);let t5: f64 = (s.db[526][10] - s.db[527][10]);let t6: f64 = (s.db[526][11] - s.db[527][11]);let t7: f64 = (s.db[526][12] - s.db[527][12]);let t8: f64 = (s.db[526][13] - s.db[527][13]);let t9: f64 = (s.db[526][14] - s.db[527][14]);let ta: f64 = (s.db[526][15] - s.db[527][15]);let tb: f64 = (s.db[526][16] - s.db[527][16]);let tc: f64 = (s.db[526][17] - s.db[527][17]);let t23: f64 = (t2).abs();let t36: f64 = if t2 >= 0.0 { t15 } else { (-t15) };let t37: f64 = if t2 >= 0.0 { t16 } else { (-t16) };let t3c: f64 = if t2 >= 0.0 { t1b } else { (-t1b) };let t3d: f64 = if t2 >= 0.0 { t1c } else { (-t1c) };let t3e: f64 = if t2 >= 0.0 { t1d } else { (-t1d) };let t3f: f64 = if t2 >= 0.0 { t1e } else { (-t1e) };let t40: f64 = if t2 >= 0.0 { t1f } else { (-t1f) };let t41: f64 = if t2 >= 0.0 { t20 } else { (-t20) };let t42: f64 = if t2 >= 0.0 { t21 } else { (-t21) };let t43: f64 = if t2 >= 0.0 { t22 } else { (-t22) };let t38: f64 = if t2 >= 0.0 { t17 } else { (-t17) };let t39: f64 = if t2 >= 0.0 { t18 } else { (-t18) };
            let t3a: f64 = if t2 >= 0.0 { t19 } else { (-t19) };let t3b: f64 = if t2 >= 0.0 { t1a } else { (-t1a) };let t24: f64 = if t2 >= 0.0 { t3 } else { (-t3) };let t25: f64 = if t2 >= 0.0 { t4 } else { (-t4) };let t2e: f64 = if t2 >= 0.0 { td } else { (-td) };let t2f: f64 = if t2 >= 0.0 { te } else { (-te) };let t30: f64 = if t2 >= 0.0 { tf } else { (-tf) };let t31: f64 = if t2 >= 0.0 { t10 } else { (-t10) };let t32: f64 = if t2 >= 0.0 { t11 } else { (-t11) };let t33: f64 = if t2 >= 0.0 { t12 } else { (-t12) };let t34: f64 = if t2 >= 0.0 { t13 } else { (-t13) };let t35: f64 = if t2 >= 0.0 { t14 } else { (-t14) };let t26: f64 = if t2 >= 0.0 { t5 } else { (-t5) };let t27: f64 = if t2 >= 0.0 { t6 } else { (-t6) };let t28: f64 = if t2 >= 0.0 { t7 } else { (-t7) };let t29: f64 = if t2 >= 0.0 { t8 } else { (-t8) };let t2a: f64 = if t2 >= 0.0 { t9 } else { (-t9) };let t2b: f64 = if t2 >= 0.0 { ta } else { (-ta) };let t2c: f64 = if t2 >= 0.0 { tb } else { (-tb) };let t2d: f64 = if t2 >= 0.0 { tc } else { (-tc) };let t44: f64 = if ((!s.b[629]) && ((s.v[525] <= 4.0) && (t23 > 1e-12))) { 1.0 } else { 0.0 };
            t44 != 0.0
        } {
            t45 += 1;
            if t45 > Self::MAX_ANALOG_LOOP_ITERATIONS { ctx.report_analog_loop_limit("reactive stamp", t45, Self::MAX_ANALOG_LOOP_ITERATIONS); break; }
            ...
            if (!s.b[629]) {s.store_div_from_scalar(528, (p[58] * 1.9e-9), 639);s.store_add_scaled_product_indices(526, 415, 1.0, 416, 528, (-1.0 / (p[47])));s.store_primal_offset(525, 525, 1.0);}
        }
