...
impl Instance {
    ...
    pub(super) fn stamp_transient_block_34(
        ...
        let ctx_temp = ctx.temperature();s.store_scaled_voltage(729, ctx, nodes, Some(5), Some(7), p[87]);s.store_scaled_voltage(731, ctx, nodes, Some(6), Some(7), p[87]);s.store_scaled_voltage(728, ctx, nodes, Some(8), Some(7), p[87]);s.store_scaled_voltage(733, ctx, nodes, Some(0), Some(2), p[87]);s.store_scaled_voltage(734, ctx, nodes, Some(6), Some(2), p[87]);s.store_scaled_voltage(735, ctx, nodes, Some(8), Some(2), p[87]);s.store_scaled_voltage(799, ctx, nodes, Some(0), Some(5), p[87]);s.store_scaled_voltage(804, ctx, nodes, Some(7), Some(2), p[87]);s.store_scaled_voltage(857, ctx, nodes, Some(10), Some(2), p[87]);s.store_scaled_voltage(858, ctx, nodes, Some(9), Some(0), p[87]);s.store_scaled_voltage(865, ctx, nodes, Some(8), Some(7), p[87]);s.store_scaled_voltage(866, ctx, nodes, Some(8), Some(5), p[87]);s.copy_ad(859, 857);s.copy_ad(860, 858);s.copy_ad(867, 865);s.copy_ad(868, 866);s.store_scalar(798, 0.0);
        ...
        if (s.b[1348] && s.b[1351]) {s.store_sqrt_mul_scaled_lhs(209, 964, ((2.0 * 1.034943e-10) * 1.6021918e-19), 155);s.store_div_scaled_product_by_product_indices(210, 394, 394, 1.0, 964, 964, 1.0);}
    }
    #[inline(never)]
    ...
    pub(super) fn stamp_transient_block_37(
        ...
        if ((s.b[1348] && s.b[1374]) && s.b[1379]) {s.store_sqrt_square_add(782, 781, 782);s.store_offset_scaled_div(340, 781, 782, 0.5, 0.5);}
    }
    #[inline(never)]
    pub(super) fn stamp_transient_block_38(
        ...
        if ((s.b[1348] && s.b[1374]) && s.b[1379]) {s.store_scaled_add(336, 781, 782, 0.5);}
    ...
    if (s.b[1397] && s.b[1400]) {
        ...
        if s.b[1401] {s.store_scale(781, 376, (0.5 * (2.0 * 1.0 / (p[262]))));s.store_offset_mul_offset_rhs_mixed_ia(782, 781, A::mul_offset_rhs(s.ad_value(781), A::mul_offset_rhs(s.ad_value(781), A::mul_offset_rhs(s.ad_value(781), A::mul(s.ad_value(781), A::scale_offset(s.ad_value(781), (1.0 / 5040.0), (1.0 / 720.0))), (1.0 / 120.0)), (1.0 / 24.0)), (1.0 / 6.0)), (1.0 / 2.0), 1.0);s.store_offset_mul_offset_rhs_mixed_ia(783, 781, A::mul_offset_rhs(s.ad_value(781), A::mul_offset_rhs(s.ad_value(781), A::mul(s.ad_value(781), A::scale_offset(s.ad_value(781), (1.0 / 840.0), (1.0 / 144.0))), (1.0 / 30.0)), (1.0 / 8.0)), (1.0 / 3.0), (1.0 / 2.0));}
    }
    #[inline(never)]
    pub(super) fn stamp_transient_block_40(
        ...
        if s.b[1401] {s.store_div_from_scalar(108, p[262], 782);s.store_div_scaled_inputs_square_rhs(336, 783, (-2.0), 782, 1.0);}
    ...
    pub(super) fn stamp_transient_block_47(
        ...
        s.store_add(1434, 1431, 1433);s.store_add_scaled_inputs(1435, 790, 1.0, 1433, 2.0);s.store_add(1436, 791, 1433);s.store_add_scaled_inputs3_offset_indices(85, 791, 1.0, 120, 1.0, 182, -1.0, (-s.v[160]));let ta: f64 = (s.v[160] - s.v[120]);let tb: f64 = (ta + s.v[182]);let tc: f64 = (tb + s.v[1431]);s.store_scalar(86, tc);s.b[1439] = (s.v[963] != 0.0);s.store_scalar(1439, if s.b[1439] { 1.0 } else { 0.0 });s.b[1440] = (p[42] == 1.0);s.store_scalar(1440, if s.b[1440] { 1.0 } else { 0.0 });s.b[1441] = (p[42] == 2.0);s.store_scalar(1441, if s.b[1441] { 1.0 } else { 0.0 });s.b[1442] = (p[42] == 3.0);s.store_scalar(1442, if s.b[1442] { 1.0 } else { 0.0 });
        if (s.b[1439] && s.b[1440]) {s.copy_ad(1459, 960);}
    }
}
