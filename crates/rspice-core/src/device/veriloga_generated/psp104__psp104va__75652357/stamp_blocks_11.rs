...
impl Instance {
    ...
    pub(super) fn stamp_reactive_block_35(
        ...
        if ((s.b[2180] && (!s.b[2192])) && (!s.b[2194])) {s.store_add_scaled_product_mixed_iia(2101, 1919, 1.0, 2091, A::add(A::offset(s.ad_value(2092), 1.0), s.ad_value(2097)), (-1.0));}
    ...
    pub(super) fn stamp_reactive_block_36(
        ...
        s.copy_ad(1843, 2069);s.copy_ad(1845, 2087);s.copy_ad(1846, 2088);s.copy_ad(1847, 2093);s.copy_ad(1848, 2096);s.copy_ad(1850, 2103);s.copy_ad(1849, 2102);s.copy_ad(1851, 2109);s.copy_ad(1852, 2111);s.copy_ad(1853, 2120);s.copy_ad(1854, 2121);s.copy_ad(1855, 2122);s.copy_ad(1856, 2124);s.copy_ad(1857, 2126);s.copy_ad(1859, 2127);
    }
    #[inline(never)]
    pub(super) fn stamp_reactive_block_37(
        ...
        s.copy_ad(1858, 2129);s.copy_ad(1860, 2128);s.store_scalar(1861, 1.0);s.store_scalar(1862, 1.0);s.store_scalar(1864, 1.0);s.store_scalar(1865, 1.0);s.store_scalar(832, 0.0);s.b[2201] = (s.v[1817] > 0.0);s.store_scalar(2201, if s.b[2201] { 1.0 } else { 0.0 });
    ...
    pub(super) fn stamp_reactive_block_38(
        ...
        if s.b[2203] {s.store_add_mixed_ai(1935, A::add_scaled_inputs_product(s.ad_value(1933), -1.0, s.ad_value(779), (-0.5), s.ad_value(777), A::sqrt(A::add_scaled_inputs3(s.ad_value(1933), 1.0, s.ad_value(779), 0.25, s.ad_value(787), 1.0)), 1.0), 788);s.store_scaled_add(1866, 1805, 1934, (-s.v[348]));s.store_scaled_add(1867, 1806, 1935, (-s.v[348]));}
        s.b[2204] = (p.p40 != 0.0);s.store_scalar(2204, if s.b[2204] { 1.0 } else { 0.0 });s.b[2205] = (s.v[232] > 0.0);s.store_scalar(2205, if s.b[2205] { 1.0 } else { 0.0 });
        ...
        if (s.b[2204] && s.b[2209]) {s.store_mul_scale_offset_mixed_ia(1919, 793, A::mul(s.ad_value(1936), A::add_scaled_product(s.ad_value(239), 1.0, s.ad_value(240), s.ad_value(1936), 1.0)), 1.0, (-1.5));s.store_offset(1938, 1935, 3.0);s.store_primal_sub_from_scalar(1939, (-3.0), 230);s.store_scale(1940, 831, 30.0);s.store_scalar(812, (4.0 - 0.9));s.store_add(813, 1938, 1940);s.store_mul_div_from_scalar_lhs_ad_mixed_ia(1919, 2.0, 812, A::sub(s.ad_value(813), A::sqrt(A::sub(A::square(s.ad_value(813)), A::mul3(s.ad_value(812), s.ad_value(1938), s.ad_value(1940))))));s.store_scalar(812, (4.0 - 0.3));s.store_add(813, 1939, 1919);}
        ...
        if ((s.b[2204] && s.b[2213]) && s.b[2214]) {s.store_offset(1919, 771, 1.0);s.store_div_scaled_product_mixed_aii(1920, A::sqrt(s.ad_value(1919)), 820, 1.0, 1843, 1.0);}
    }
    #[inline(never)]
    pub(super) fn stamp_reactive_block_39(
        ...
        if ((s.b[2204] && s.b[2213]) && s.b[2214]) {s.store_add_mixed_ai(1921, A::square(s.ad_value(1920)), 1919);s.store_scale(1919, 1920, 2.0);s.store_div_scaled_product3_mixed_iiia(1846, 1843, 1813, 1919, 1.0, A::add(A::sqrt(A::sub(s.ad_value(1921), s.ad_value(1919))), A::sqrt(A::add(s.ad_value(1921), s.ad_value(1919)))), 1.0);}
        ...
        if (s.b[2204] && s.b[2213]) {s.store_add_scaled_product_mixed_iai(1946, 1850, 1.0, A::add_scaled_inputs3(s.ad_value(1945), 1.0, s.ad_value(736), (-1.0), s.ad_value(1942), -1.0), 1813, 1.0);s.store_mul_scale_offset_mixed_ia(1946, 1813, A::add_scaled_inputs3(s.ad_value(819), 1.0, s.ad_value(1922), 1.0, s.ad_value(1942), -1.0), -1.0, 0.0);}
        ...
        if (s.b[2204] && s.b[2213]) {s.store_mul_scale_offset_mixed_ia(1919, 791, A::mul(s.ad_value(1936), A::add_scaled_product(s.ad_value(235), 1.0, s.ad_value(236), s.ad_value(1936), 1.0)), 1.0, (-1.5));}
        s.b[2223] = ((s.v[1817] <= 0.0) || ((s.v[235] == 0.0) && (s.v[236] == 0.0)));s.store_scalar(2223, if s.b[2223] { 1.0 } else { 0.0 });
        if ((s.b[2204] && s.b[2213]) && (!s.b[2223])) {s.store_add_scaled_product_indices(1919, 235, 1.0, 236, 1936, 2.0);s.store_div_scaled_value_by_product_indices(1950, 241, 1.0, 1919, 791, 1.0);s.store_scaled_div(1951, 1848, 1950, 0.5);}
        s.b[2224] = (s.v[1951] < 0.001);s.store_scalar(2224, if s.b[2224] { 1.0 } else { 0.0 });s.b[2225] = (((s.v[1951]) as f64).abs() < 230.25850929940458);s.store_scalar(2225, if s.b[2225] { 1.0 } else { 0.0 });
        ...
        if (((s.b[2204] && s.b[2213]) && (!s.b[2223])) && (!s.b[2224])) {s.store_div_from_scalar(1960, 1.0, 1959);s.store_sub(1919, 1959, 1960);s.store_add(1921, 1959, 1960);}
        s.b[2227] = (p.p42 != 0.0);s.store_scalar(2227, if s.b[2227] { 1.0 } else { 0.0 });s.b[2228] = ((s.v[243] > 0.0) && (s.v[1867] < 0.0));s.store_scalar(2228, if s.b[2228] { 1.0 } else { 0.0 });
    ...
    pub(super) fn stamp_reactive_block_40(
        ...
        if (s.b[2232] && s.b[2233]) {s.store_offset_add(1985, 1984, 719, (-1.0));s.store_scaled_add_mixed_ia(1986, 1985, A::sqrt_square_offset(s.ad_value(1985), 10.0), 0.5);}
    }
    #[inline(never)]
    pub(super) fn stamp_reactive_block_41(
        s: &mut ReactiveScratch,
    ) {
        if (s.b[2232] && s.b[2233]) {s.store_add_mixed_ai(1987, A::add_scaled_product(s.ad_value(1984), 1.0, s.ad_value(1982), A::ln(s.ad_value(1986)), (-1.0)), 719);s.store_scaled_add_mixed_ia(1988, 1987, A::sqrt_square_offset(s.ad_value(1987), 2.0), 0.5);}
        ...
        if ((s.b[2232] && (!s.b[2236])) && s.b[2238]) {s.store_mul(1990, 718, 1989);}
    }
    #[inline(never)]
    pub(super) fn stamp_reactive_block_42(
        ...
        if ((s.b[2232] && (!s.b[2236])) && s.b[2238]) {s.store_pow_indices(1991, 1990, 1983);s.store_add_scaled_square_product_mixed_iai(1992, 1982, 1.0, A::add_scaled_inputs3(s.ad_value(1988), 2.0, s.ad_value(1982), 2.0, s.ad_value(1991), -1.0), 1991, 1.0);s.store_mul_scale_offset_mixed_ia(1993, 1982, A::div_scaled_inputs2(A::sqrt(s.ad_value(1992)), 1.0, s.ad_value(1982), (-1.0), s.ad_value(1991), 1.0), 1.0, (-1.0));s.store_sub(1977, 1988, 1993);}
        ...
        s.b[2439] = (((p.p45 == 1.0) || (p.p47 > 0.0)) || (p.p48 > 0.0));s.store_scalar(2439, if s.b[2439] { 1.0 } else { 0.0 });
    }
    #[inline(never)]
    pub(super) fn stamp_reactive_block_43(
        ...
        s.b[2440] = ((p.p45 > 0.0) || (p.p47 > 0.0));s.store_scalar(2440, if s.b[2440] { 1.0 } else { 0.0 });
        ...
        if (s.b[2439] && s.b[2440]) {s.store_offset_mul(2302, 695, 2301, 1.0);s.store_scale(2303, 2302, s.v[709]);s.store_mul_ad_product_rhs(2304, 194, A::offset(A::mul(s.ad_value(196), s.ad_value(824)), 1.0), A::offset(A::mul(s.ad_value(195), s.ad_value(2289)), 1.0));s.store_mul_scale_offset_indices(2305, 2303, 2304, 1.0, 1.0);s.store_div_from_scalar(2306, 1.0, 2305);}
    }
    #[inline(never)]
    pub(super) fn stamp_reactive_block_44(
        ...
        if (s.b[2439] && s.b[2440]) {s.store_mul_mixed_ia(2290, 2282, A::sqrt_scaled_input(s.ad_value(2306), s.v[709]));s.store_square(2291, 2290);s.store_div_from_scalar(2307, 1.0, 2291);s.store_mul(2308, 2284, 2306);s.store_mul(2309, 2287, 2306);s.store_div_scaled_value_offset_denominator(2310, s.ad_value(824), 2.0, A::sqrt_product_offset(s.ad_value(192), s.ad_value(824), 1.0), 1.0, 1.0);s.store_mul_ad_product_rhs_mixed_ia(2311, 191, 2310, A::offset(A::mul(s.ad_value(193), s.ad_value(2289)), 1.0));s.store_mul(2312, 2280, 2306);s.store_sqrt_square_add(1920, 2283, 2281);s.store_sqrt_add_ad(1921, A::square(A::sub(s.ad_value(2283), s.ad_value(2311))), s.ad_value(2281));s.store_mul_add_scaled_inputs3_offset_rhs_indices(2313, 2306, 2311, 0.5, 1920, 0.5, 1921, ((-1.0) * (0.5)), 0.0);s.store_add(2314, 2312, 2308);s.store_sub(2315, 2314, 2313);}
    ...
    pub(super) fn stamp_reactive_block_45(
        ...
        if (s.b[2439] && s.b[2440]) {s.store_offset_scaled(2326, 2290, 0.7071067811865475, 1.0);s.store_scale(2327, 2326, 1e-5);s.store_div_from_scalar(2328, 1.0, 2326);s.store_scalar(2435, 0.0);s.store_scalar(2329, 0.0);}
