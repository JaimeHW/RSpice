...
impl Instance {
    ...
    pub(super) fn stamp_transient_block_64(
        ...
        if ((((p[24] != 0.0) && s.b[1606]) && (!s.b[1607])) && s.b[1608]) {s.store_primal_add_scaled_products_indices(1543, 1540, 462, 1.0, 1541, 461, 1.0);s.store_offset_ad(1548, A::add_scaled_products(s.ad_value(1542), s.ad_value(1554), 1.0, s.ad_value(1543), s.ad_value(1553), 1.0), (10.0 * 2.220446049250313e-16));}
    ...
    pub(super) fn stamp_transient_block_67(
        ...
        let mut t7: usize = 0;
        while {
            let t4: f64 = (2.0 * 20.0);let t5: f64 = (t4 + 1.0);let t6: f64 = if ((((((p[24] != 0.0) && s.b[1606]) && (!s.b[1607])) && (!s.b[1610])) && s.b[1617]) && (s.v[167] <= t5)) { 1.0 } else { 0.0 };
            t6 != 0.0
        } {
            t7 += 1;
    ...
    pub(super) fn stamp_transient_block_72(
        ...
        let mut tf: usize = 0;
        while {
            let tc: f64 = (2.0 * 20.0);let td: f64 = (tc + 1.0);let te: f64 = if ((((((p[24] != 0.0) && s.b[1606]) && (!s.b[1607])) && (!s.b[1630])) && s.b[1637]) && (s.v[167] <= td)) { 1.0 } else { 0.0 };
            te != 0.0
        } {
            tf += 1;
            if tf > Self::MAX_ANALOG_LOOP_ITERATIONS { ctx.report_analog_loop_limit("transient stamp", tf, Self::MAX_ANALOG_LOOP_ITERATIONS); break; }
            ...
            let (tb,) = {
    if (((((((p[24] != 0.0) && s.b[1606]) && (!s.b[1607])) && (!s.b[1630])) && s.b[1637]) && (!s.b[1640])) && s.b[1642]) {
        (1.0,)
    } else {
        (s.v[1546],)
    }
    ...
    pub(super) fn stamp_transient_block_73(
        ...
        if (((((p[24] != 0.0) && s.b[1606]) && (!s.b[1607])) && (!s.b[1630])) && s.b[1637]) {s.store_mul(458, 1556, 1605);s.store_div_from_scalar_add_ad(1524, 1.0, s.ad_value(1597), s.ad_value(1605));s.store_mul3_lhs(460, 1556, 1593, 1524);s.store_add(459, 458, 460);}
        if (((p[24] != 0.0) && s.b[1606]) && (!s.b[1607])) {s.store_sub(460, 459, 458);}
        if (((p[24] != 0.0) && s.b[1606]) && (!s.b[1607])) {
            if (p[43] == 1.0) {
                s.store_mul(1527, 287, 1536);
    ...
    pub(super) fn stamp_transient_block_74(
        ...
        if s.b[1656] {s.store_scaled_mul(1680, 285, 1674, p[237]);s.store_scaled_mul(1682, 285, 1675, p[237]);s.store_scale(1658, 429, 1.0 / (s.v[81]));s.store_square(1657, 1658);s.store_offset(1659, 1679, 1e-50);s.store_offset(1660, 1680, 1e-50);s.store_mul_div_from_scalar_lhs_ad_mixed_ia(1677, p[174], 225, A::ln_offset_div_scaled_inputs(s.ad_value(1657), p[177], s.ad_value(1659), 1.0, 1.0));s.store_mul_div_from_scalar_lhs_ad_mixed_ia(1678, p[174], 225, A::ln_offset_div_scaled_inputs(s.ad_value(1657), p[177], s.ad_value(1660), 1.0, 1.0));s.store_scale(1676, 227, p[174]);}
    ...
    pub(super) fn stamp_transient_block_76(
        ...
        if ((s.v[85] != 0.0) && s.b[1721]) {s.store_scalar(1716, p[233]);s.store_scalar(1717, p[234]);s.copy_ad(1718, 441);s.store_mul_product3_indices(1719, 1718, 1716, 1717, 1718, 1.0);s.store_offset_add_ad(1720, A::mul3(s.ad_value(250), s.ad_value(192), s.ad_value(1716)), A::mul3(s.ad_value(1717), s.ad_value(1718), s.ad_value(1718)), 1e-50);s.store_div(289, 1719, 1720);}
        if ((s.v[85] != 0.0) && (!s.b[1721])) {s.store_scalar(289, (p[233] + 1e-50));}
        if (s.v[85] != 0.0) {s.store_scalar(1719, p[235]);s.store_mul(290, 1719, 323);}
    ...
    if s.b[1775] {
        ...
        s.store_scalar(4, 0.0);s.store_scalar(5, 0.0);s.b[1777] = (p[259] == 1.0);s.store_scalar(1777, if s.b[1777] { 1.0 } else { 0.0 });
        let (t1c,) = {
    if s.b[1777] {
        (1.0,)
    } else {
        ...
        if (s.b[1777] && s.b[1797]) {s.store_scalar(1788, (p[264] / 1e-6));s.store_scalar(1781, p[266]);s.store_scalar(1782, p[268]);s.store_scalar(1783, p[273]);}
        if (s.b[1777] && s.b[1797]) {s.store_scalar(1784, (if (p[263] > 0.0) { (p[263] * p[255]) } else { 0.0 }));}
        if (s.b[1777] && s.b[1797]) {s.store_scalar(1787, p[258]);s.store_scaled_voltage(1785, ctx, nodes, Some(7), Some(2), p[50]);}
        if (s.b[1777] && (!s.b[1797])) {s.store_scalar(1788, (p[59] / 1e-6));s.store_scalar(1781, p[265]);s.store_scalar(1782, p[267]);s.store_scalar(1783, p[272]);}
        if (s.b[1777] && (!s.b[1797])) {s.store_scalar(1784, (if (p[263] > 0.0) { (p[263] * p[256]) } else { 0.0 }));}
        if (s.b[1777] && (!s.b[1797])) {s.store_scalar(1787, p[257]);s.store_scaled_voltage(1785, ctx, nodes, Some(0), Some(6), p[50]);}
        if s.b[1777] {s.store_scalar(1794, ((((p[271] * p[271]) + (p[56] * p[56]))) as f64).sqrt());s.store_primal_scale(1796, 105, p[9]);s.store_primal_scale(1781, 1781, 0.0001);s.store_primal_scale(1782, 1782, 0.01);s.store_scale(1786, 429, 1.0 / (s.v[81]));s.store_powf(328, 1786, p[269]);s.store_div(1789, 1781, 328);}
    }
}
