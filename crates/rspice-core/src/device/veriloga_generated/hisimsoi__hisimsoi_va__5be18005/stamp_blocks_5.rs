...
impl Instance {
    ...
    pub(super) fn stamp_transient_block_80(
        ...
        let mut t6: usize = 0;
        while {
            let t3: f64 = (2.0 * 20.0);let t4: f64 = (t3 + 1.0);let t5: f64 = if ((((((p[24] != 0.0) && s.b[1606]) && (!s.b[1607])) && (!s.b[1630])) && s.b[1637]) && (s.v[167] <= t4)) { 1.0 } else { 0.0 };
            t5 != 0.0
        } {
            t6 += 1;
    ...
    if s.b[1777] {
        ...
        s.store_scalar(3, t13);s.b[1797] = (s.v[3] == 1.0);s.store_scalar(1797, if s.b[1797] { 1.0 } else { 0.0 });
        ...
        if (s.b[1777] && s.b[1797]) {s.store_scalar(1787, p[258]);}
    }
    #[inline(never)]
    pub(super) fn stamp_transient_block_89(
        ...
        if (s.b[1777] && s.b[1797]) {s.store_scaled_voltage(1785, ctx, nodes, Some(7), Some(2), p[50]);}
    ...
    if s.b[1805] {
        ...
        s.store_scalar(3, t14);s.b[1825] = (s.v[3] == 1.0);s.store_scalar(1825, if s.b[1825] { 1.0 } else { 0.0 });
        ...
        if (s.b[1805] && s.b[1825]) {s.store_scalar(1815, p[258]);}
    }
    #[inline(never)]
    pub(super) fn stamp_transient_block_91(
        ...
        if s.b[1805] {s.store_scalar(1822, ((((p[271] * p[271]) + (p[56] * p[56]))) as f64).sqrt());s.store_primal_scale(1824, 105, p[9]);s.store_primal_scale(1809, 1809, 0.0001);s.store_primal_scale(1810, 1810, 0.01);s.store_scale(1814, 429, 1.0 / (s.v[81]));s.store_powf(328, 1814, p[269]);s.store_div(1817, 1809, 328);s.store_sub_ad(327, A::add_scaled_product(A::scale_offset(s.ad_value(1814), 0.4, 1.8), 1.0, s.ad_value(1814), s.ad_value(1814), 0.1), A::scale_offset(s.ad_value(1814), (-p[270]), p[270]));s.store_div(1818, 1810, 327);s.store_add_mixed_ia(1811, 1811, A::scaled_offset(s.ad_value(429), (-s.v[81]), p[274]));s.store_scalar(1806, (1.0 + (p[279] / ((s.v[100]) as f64).powf(p[280]))));s.store_scalar(1808, (1.0 + (p[277] / ((s.v[100]) as f64).powf(p[278]))));s.store_scalar(1807, (1.0 + (p[275] / ((s.v[109]) as f64).powf(p[276]))));s.store_mul(1817, 1817, 1806);s.store_offset_product3(1818, s.ad_value(1818), s.ad_value(1807), s.ad_value(1808), 1.0, 1e-50);s.store_div(1819, 1813, 1815);s.store_mul(1820, 1817, 1819);}
    ...
    pub(super) fn stamp_transient_block_92(
        ...
        if ((!s.b[1833]) && (s.v[85] == 0.0)) {s.store_scalar(574, 0.0);s.store_scalar(575, 0.0);s.store_scalar(583, 0.0);s.store_scalar(584, 0.0);s.store_scalar(585, 0.0);s.store_scalar(586, 0.0);s.store_scalar(581, 0.0);}
        ...
        s.b[1851] = (p[43] == 1.0);s.store_scalar(1851, if s.b[1851] { 1.0 } else { 0.0 });s.b[1852] = ((p[37] != 0.0) || ((p[25] == 1.0) && (p[26] == 2.0)));s.store_scalar(1852, if s.b[1852] { 1.0 } else { 0.0 });
    }
    #[inline(never)]
    pub(super) fn stamp_transient_block_93(
        ...
        s.b[1853] = (p[43] == 0.0);s.store_scalar(1853, if s.b[1853] { 1.0 } else { 0.0 });
    }
    #[inline(never)]
