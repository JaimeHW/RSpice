...
impl Instance {
    ...
    pub(super) fn stamp_transient_block_33(
        ...
        s.b[1074] = ((p[25] == 1.0) && (p[26] == 2.0));s.store_scalar(1074, if s.b[1074] { 1.0 } else { 0.0 });
    ...
    if (!s.b[737]) {
        ...
        if (!s.b[737]) {s.copy_ad(349, 161);s.store_scale(419, 229, ((p[237] * (p[237] * 0.5)) * 9662367879.197212));s.store_sqrt_mul_scaled_lhs(327, 225, 2.0, 419);s.store_scaled_add_ad(328, A::exp(s.ad_value(327)), A::exp_scaled_input(s.ad_value(327), -1.0), 0.5);s.store_div_ln_lhs(420, 328, 419);s.store_scalar(167, 1.0);}
    }
    #[inline(never)]
    ...
    if ((s.b[1133] && s.b[1220]) && (!s.b[1223])) {
        ...
        s.store_scalar(146, t45);s.copy_ad(202, 199);s.store_scalar(204, 0.0);s.b[1224] = ((p[281] > 0.0) && (p[285] > 0.0));s.store_scalar(1224, if s.b[1224] { 1.0 } else { 0.0 });
    ...
    if (s.b[1224] && s.b[1270]) {
        let t55: f64 = (s.v[123] - s.v[185]);let t56: f64 = (t55 + s.v[320]);let t57: f64 = (t56 + s.v[1271]);let t58: f64 = (t57 + p[286]);
        (t58,)
    } else {
        (s.v[160],)
    }
