    ...
    Scratch<VARIABLE_COUNT, NODE_COUNT, BRANCH_COUNT>
{
    ...
    #[inline]
    pub(crate) fn store_scaled_voltage(
        &mut self,
        index: usize,
        ctx: &GeneratedEvalContext<'_>,
        nodes: &[usize; NODE_COUNT],
        ...
        if let Some(node) = pos {
            self.dn[index][node] += scale;
        }
        if let Some(node) = neg {
            self.dn[index][node] -= scale;
        }
    }

    #[inline]
    pub(crate) fn store_offset_voltage(
        &mut self,
        index: usize,
        ctx: &GeneratedEvalContext<'_>,
        nodes: &[usize; NODE_COUNT],
    ...
    #[inline]
    pub(crate) fn store_abs_voltage(
        &mut self,
        index: usize,
        ctx: &GeneratedEvalContext<'_>,
        nodes: &[usize; NODE_COUNT],
        ...
        self.db[index] = [0.0; BRANCH_COUNT];
        if let Some(node) = pos {
            self.dn[index][node] += derivative_scale;
        }
        if let Some(node) = neg {
            self.dn[index][node] -= derivative_scale;
        }
    ...
    #[inline]
    pub(crate) fn store_sub_voltage_abs_voltage(
        &mut self,
        index: usize,
        ctx: &GeneratedEvalContext<'_>,
        nodes: &[usize; NODE_COUNT],
        left_pos: Option<usize>,
        left_neg: Option<usize>,
        abs_pos: Option<usize>,
        abs_neg: Option<usize>,
    ) {
        let left_pos_value = left_pos
            .map(|node| ctx.node_voltage(nodes[node]))
            .unwrap_or(0.0);
        let left_neg_value = left_neg
            .map(|node| ctx.node_voltage(nodes[node]))
            .unwrap_or(0.0);
        let abs_pos_value = abs_pos
            .map(|node| ctx.node_voltage(nodes[node]))
            .unwrap_or(0.0);
        let abs_neg_value = abs_neg
            .map(|node| ctx.node_voltage(nodes[node]))
            .unwrap_or(0.0);
        ...
        self.dn[index] = [0.0; NODE_COUNT];
        self.db[index] = [0.0; BRANCH_COUNT];
        if let Some(node) = left_pos {
            self.dn[index][node] += 1.0;
        }
        if let Some(node) = left_neg {
            self.dn[index][node] -= 1.0;
        }
        if let Some(node) = abs_pos {
            self.dn[index][node] -= abs_derivative_scale;
        }
        if let Some(node) = abs_neg {
            self.dn[index][node] += abs_derivative_scale;
        }
    }

    #[inline]
    pub(crate) fn store_mul_voltage_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ctx: &GeneratedEvalContext<'_>,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = value.dn[axis] * voltage;
        }
        if let Some(node) = pos {
            self.dn[index][node] += value.value;
        }
        if let Some(node) = neg {
            self.dn[index][node] -= value.value;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = value.db[axis] * voltage;
        }
    }

    #[inline]
    pub(crate) fn store_div_voltage_by_ad(
        &mut self,
        index: usize,
        ctx: &GeneratedEvalContext<'_>,
        nodes: &[usize; NODE_COUNT],
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = right.dn[axis] * right_scale;
        }
        if let Some(node) = pos {
            self.dn[index][node] += reciprocal;
        }
        if let Some(node) = neg {
            self.dn[index][node] -= reciprocal;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = right.db[axis] * right_scale;
        }
    }
    ...
    pub(crate) fn store_add_ad(
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = left.dn[axis] + right.dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = left.db[axis] + right.db[axis];
        }
    }
    ...
    pub(crate) fn store_mul_ad(
        ...
        self.v[index] = left.value * right.value;
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = left.dn[axis] * right.value + left.value * right.dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = left.db[axis] * right.value + left.value * right.db[axis];
        }
    }
    ...
    pub(crate) fn store_div_ad(
        ...
        self.v[index] = quotient;
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = left.dn[axis] * reciprocal + right.dn[axis] * right_scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = left.db[axis] * reciprocal + right.db[axis] * right_scale;
        }
    }
    ...
    pub(crate) fn store_pow_ad(
        ...
        self.v[index] = output;
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = AdValue::<NODE_COUNT, BRANCH_COUNT>::pow_derivative(
                output,
                base,
                exponent,
                left.dn[axis],
        ...
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = AdValue::<NODE_COUNT, BRANCH_COUNT>::pow_derivative(
                output,
                base,
                exponent,
                left.db[axis],
    ...
    pub(crate) fn store_min_ad(
        ...
        let selected = if left.value <= right.value {
            left
        } else {
            right
        };
    ...
    pub(crate) fn store_scale_ad(
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = value.dn[axis] * scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = value.db[axis] * scale;
        }
    }
    ...
    #[inline]
    pub(crate) fn store_offset_ad_value(
        &mut self,
        index: usize,
        mut value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
    ) {
        value.value += offset;
    ...
    #[inline]
    pub(crate) fn store_offset_scaled_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        scale: f64,
    ...
    #[inline]
    pub(crate) fn store_offset_add_ad(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_offset_sub_ad(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_offset_mul_ad(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_offset_div_ad(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_offset_exp_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = value.dn[axis] * output;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = value.db[axis] * output;
        }
    }

    #[inline]
    pub(crate) fn store_offset_sqrt_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = value.dn[axis] * derivative_scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = value.db[axis] * derivative_scale;
        }
    }

    #[inline]
    pub(crate) fn store_offset_ln_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
    ...
    #[inline]
    pub(crate) fn store_offset_limited_exp_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
        ...
        if value.value > 80.0 {
            ...
            for axis in 0..NODE_COUNT {
                self.dn[index][axis] = value.dn[axis] * LIMEXP_MAX;
            }
            for axis in 0..BRANCH_COUNT {
                self.db[index][axis] = value.db[axis] * LIMEXP_MAX;
            }
    ...
    #[inline]
    pub(crate) fn store_offset_powf_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        exponent: f64,
    ...
    #[inline]
    pub(crate) fn store_offset_sub_from_scalar_ad(
        &mut self,
        index: usize,
        scalar: f64,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = -value.dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = -value.db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_offset_div_from_scalar_ad(
        &mut self,
        index: usize,
        scalar: f64,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_offset_pow_ad(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_offset_min_ad(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_offset_max_ad(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_offset_rem_ad(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_offset_hypot_ad(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_offset_atan2_ad(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_offset_rem_from_scalar_ad(
        &mut self,
        index: usize,
        scalar: f64,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_offset_pow_from_scalar_ad(
        &mut self,
        index: usize,
        scalar: f64,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_offset_min_from_scalar_ad(
        &mut self,
        index: usize,
        scalar: f64,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_offset_max_from_scalar_ad(
        &mut self,
        index: usize,
        scalar: f64,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_offset_rem_with_scalar_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        scalar: f64,
    ...
    #[inline]
    pub(crate) fn store_offset_min_with_scalar_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        scalar: f64,
    ...
    #[inline]
    pub(crate) fn store_offset_max_with_scalar_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        scalar: f64,
    ...
    #[inline]
    pub(crate) fn store_offset_abs_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
    ...
    #[inline]
    pub(crate) fn store_offset_square_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
    ...
    #[inline]
    pub(crate) fn store_offset_limexp_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
    ...
    #[inline]
    pub(crate) fn store_offset_log10_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
    ...
    #[inline]
    pub(crate) fn store_offset_sin_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
    ...
    #[inline]
    pub(crate) fn store_offset_cos_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
    ...
    #[inline]
    pub(crate) fn store_offset_tan_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
    ...
    #[inline]
    pub(crate) fn store_offset_atan_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
    ...
    #[inline]
    pub(crate) fn store_offset_sinh_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
    ...
    #[inline]
    pub(crate) fn store_offset_cosh_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
    ...
    #[inline]
    pub(crate) fn store_offset_tanh_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
    ...
    #[inline]
    pub(crate) fn store_offset_asinh_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
    ...
    #[inline]
    pub(crate) fn store_offset_acosh_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
    ...
    #[inline]
    pub(crate) fn store_offset_atanh_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
    ...
    #[inline]
    pub(crate) fn store_offset_floor_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
    ...
    #[inline]
    pub(crate) fn store_offset_ceil_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
    ...
    #[inline]
    pub(crate) fn store_scaled_offset_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
    ...
    #[inline]
    pub(crate) fn store_scaled_add_ad(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = (left.dn[axis] + right.dn[axis]) * scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = (left.db[axis] + right.db[axis]) * scale;
        }
    }

    #[inline]
    pub(crate) fn store_scaled_sub_ad(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_scaled_mul_ad(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                (left.dn[axis] * right.value + left.value * right.dn[axis]) * scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                (left.db[axis] * right.value + left.value * right.db[axis]) * scale;
        }
    }

    #[inline]
    pub(crate) fn store_scaled_div_ad(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                (left.dn[axis] * reciprocal + right.dn[axis] * right_scale) * scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                (left.db[axis] * reciprocal + right.db[axis] * right_scale) * scale;
        }
    }

    #[inline]
    pub(crate) fn store_scaled_exp_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        scale: f64,
    ...
    #[inline]
    pub(crate) fn store_scaled_ln_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        scale: f64,
    ...
    #[inline]
    pub(crate) fn store_scaled_sqrt_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        scale: f64,
    ...
    #[inline]
    pub(crate) fn store_scaled_limexp_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        scale: f64,
    ...
    #[inline]
    pub(crate) fn store_scaled_limited_exp_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        scale: f64,
    ...
    #[inline]
    pub(crate) fn store_scaled_abs_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        scale: f64,
    ...
    #[inline]
    pub(crate) fn store_scaled_powf_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        exponent: f64,
    ...
    #[inline]
    pub(crate) fn store_scaled_sub_from_scalar_ad(
        &mut self,
        index: usize,
        scalar: f64,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_scaled_div_from_scalar_ad(
        &mut self,
        index: usize,
        scalar: f64,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_sqrt_offset_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
    ...
    #[inline]
    pub(crate) fn store_sqrt_scaled_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        scale: f64,
    ...
    #[inline]
    pub(crate) fn store_sqrt_add_ad(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        self.v[index] = root;
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = (left.dn[axis] + right.dn[axis]) * derivative_scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = (left.db[axis] + right.db[axis]) * derivative_scale;
        }
    }

    #[inline]
    pub(crate) fn store_sqrt_sub_ad(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_sqrt_mul_ad(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                (left.dn[axis] * right.value + left.value * right.dn[axis]) * derivative_scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                (left.db[axis] * right.value + left.value * right.db[axis]) * derivative_scale;
        }
    }

    #[inline]
    pub(crate) fn store_sqrt_div_ad(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                (left.dn[axis] * reciprocal + right.dn[axis] * right_scale) * derivative_scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                (left.db[axis] * reciprocal + right.db[axis] * right_scale) * derivative_scale;
        }
    }

    #[inline]
    pub(crate) fn store_sqrt_abs_ad(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ) {
        ...
        let root = raw.sqrt();
        let derivative_scale = if value.value >= 0.0 {
            1.0 / (2.0 * root)
        } else {
            -1.0 / (2.0 * root)
        };
    ...
    #[inline]
    pub(crate) fn store_sqrt_sub_from_scalar_ad(
        &mut self,
        index: usize,
        scalar: f64,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_sqrt_div_from_scalar_ad(
        &mut self,
        index: usize,
        scalar: f64,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_div_from_scalar_offset_ad(
        &mut self,
        index: usize,
        scalar: f64,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_div_from_scalar_scaled_ad(
        &mut self,
        index: usize,
        scalar: f64,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_div_from_scalar_add_ad(
        &mut self,
        index: usize,
        scalar: f64,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        self.v[index] = quotient;
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = (left.dn[axis] + right.dn[axis]) * denominator_scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = (left.db[axis] + right.db[axis]) * denominator_scale;
        }
    }

    #[inline]
    pub(crate) fn store_div_from_scalar_sub_ad(
        &mut self,
        index: usize,
        scalar: f64,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_div_from_scalar_mul_ad(
        &mut self,
        index: usize,
        scalar: f64,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                (left.dn[axis] * right.value + left.value * right.dn[axis]) * denominator_scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                (left.db[axis] * right.value + left.value * right.db[axis]) * denominator_scale;
        }
    }

    #[inline]
    pub(crate) fn store_div_from_scalar_div_ad(
        &mut self,
        index: usize,
        scalar: f64,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        self.v[index] = quotient;
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = (left.dn[axis] * right_reciprocal
                + right.dn[axis] * right_scale)
                * denominator_scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = (left.db[axis] * right_reciprocal
                + right.db[axis] * right_scale)
                * denominator_scale;
        }
    }

    #[inline]
    pub(crate) fn store_div_from_scalar_sqrt_ad(
        &mut self,
        index: usize,
        scalar: f64,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_div_from_scalar_square_ad(
        &mut self,
        index: usize,
        scalar: f64,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_div_from_scalar_sub_from_scalar_ad(
        &mut self,
        index: usize,
        scalar: f64,
        denominator_scalar: f64,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ) {
    ...
    #[inline]
    pub(crate) fn store_div_from_scalar_div_from_scalar_ad(
        &mut self,
        index: usize,
        scalar: f64,
        denominator_scalar: f64,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = value.dn[axis] * quotient_scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = value.db[axis] * quotient_scale;
        }
    }

    #[inline]
    pub(crate) fn store_div_from_scalar_pow_ad(
        &mut self,
        index: usize,
        scalar: f64,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_div_from_scalar_powf_ad(
        &mut self,
        index: usize,
        scalar: f64,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_div_from_scalar_exp_ad(
        &mut self,
        index: usize,
        scalar: f64,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_div_from_scalar_sin_ad(
        &mut self,
        index: usize,
        scalar: f64,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_div_from_scalar_sinh_ad(
        &mut self,
        index: usize,
        scalar: f64,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_scaled_add_ad_rhs(
        &mut self,
        index: usize,
        left: usize,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = (left_dn[axis] + right.dn[axis]) * scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = (left_db[axis] + right.db[axis]) * scale;
        }
    }

    #[inline]
    pub(crate) fn store_scaled_add_ad_lhs(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: usize,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = (left.dn[axis] + right_dn[axis]) * scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = (left.db[axis] + right_db[axis]) * scale;
        }
    }

    #[inline]
    pub(crate) fn store_scaled_sub_ad_rhs(
        &mut self,
        index: usize,
        left: usize,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_scaled_sub_ad_lhs(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: usize,
    ...
    #[inline]
    pub(crate) fn store_add_scaled_ad_rhs(
        &mut self,
        index: usize,
        left: usize,
        scale: f64,
        ...
        self.v[index] = left_value + right.value;
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = left_dn[axis] * scale + right.dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = left_db[axis] * scale + right.db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_add_scaled_ad_lhs(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: usize,
    ...
    #[inline]
    pub(crate) fn store_sub_scaled_ad_rhs(
        &mut self,
        index: usize,
        left: usize,
        scale: f64,
    ...
    #[inline]
    pub(crate) fn store_sub_scaled_ad_lhs(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: usize,
    ...
    pub(crate) fn store_mul3_lhs(
        ...
        for axis in 0..NODE_COUNT {
            let product_derivative = left_dn[axis] * middle_value + left_value * middle_dn[axis];
            self.dn[index][axis] =
                product_derivative * right_value + product_value * right_dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            let product_derivative = left_db[axis] * middle_value + left_value * middle_db[axis];
            self.db[index][axis] =
                product_derivative * right_value + product_value * right_db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_mul3_rhs(
        &mut self,
        index: usize,
        left: usize,
        middle: usize,
        ...
        for axis in 0..NODE_COUNT {
            let product_derivative = middle_dn[axis] * right_value + middle_value * right_dn[axis];
            self.dn[index][axis] = left_dn[axis] * product_value + left_value * product_derivative;
        }
        for axis in 0..BRANCH_COUNT {
            let product_derivative = middle_db[axis] * right_value + middle_value * right_db[axis];
            self.db[index][axis] = left_db[axis] * product_value + left_value * product_derivative;
        }
    }

    #[inline]
    pub(crate) fn store_mul_ad_product_lhs(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        for axis in 0..NODE_COUNT {
            let product_derivative = left.dn[axis] * right.value + left.value * right.dn[axis];
            self.dn[index][axis] =
                product_derivative * source_value + product_value * source_dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            let product_derivative = left.db[axis] * right.value + left.value * right.db[axis];
            self.db[index][axis] =
                product_derivative * source_value + product_value * source_db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_mul_ad_product_rhs(
        &mut self,
        index: usize,
        source: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        for axis in 0..NODE_COUNT {
            let product_derivative = left.dn[axis] * right.value + left.value * right.dn[axis];
            self.dn[index][axis] =
                source_dn[axis] * product_value + source_value * product_derivative;
        }
        for axis in 0..BRANCH_COUNT {
            let product_derivative = left.db[axis] * right.value + left.value * right.db[axis];
            self.db[index][axis] =
                source_db[axis] * product_value + source_value * product_derivative;
        }
    }

    #[inline]
    pub(crate) fn store_mul3_affine_lhs(
        &mut self,
        index: usize,
        left: usize,
        middle: usize,
        ...
        for axis in 0..NODE_COUNT {
            let product_derivative = left_dn[axis] * middle_value + left_value * middle_dn[axis];
            let affine_derivative = product_derivative * scale;
            self.dn[index][axis] = affine_derivative * right_value + affine_value * right_dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            let product_derivative = left_db[axis] * middle_value + left_value * middle_db[axis];
            let affine_derivative = product_derivative * scale;
            self.db[index][axis] = affine_derivative * right_value + affine_value * right_db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_mul3_affine_rhs(
        &mut self,
        index: usize,
        left: usize,
        middle: usize,
        ...
        for axis in 0..NODE_COUNT {
            let product_derivative = middle_dn[axis] * right_value + middle_value * right_dn[axis];
            let affine_derivative = product_derivative * scale;
            self.dn[index][axis] = left_dn[axis] * affine_value + left_value * affine_derivative;
        }
        for axis in 0..BRANCH_COUNT {
            let product_derivative = middle_db[axis] * right_value + middle_value * right_db[axis];
            let affine_derivative = product_derivative * scale;
            self.db[index][axis] = left_db[axis] * affine_value + left_value * affine_derivative;
        }
    }

    #[inline]
    pub(crate) fn store_mul_ad_affine_product_lhs(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        for axis in 0..NODE_COUNT {
            ...
            let affine_derivative = product_derivative * scale;
            self.dn[index][axis] =
                affine_derivative * source_value + affine_value * source_dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            let product_derivative = left.db[axis] * right.value + left.value * right.db[axis];
            let affine_derivative = product_derivative * scale;
            self.db[index][axis] =
                affine_derivative * source_value + affine_value * source_db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_mul_ad_affine_product_rhs(
        &mut self,
        index: usize,
        source: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        for axis in 0..NODE_COUNT {
            ...
            let affine_derivative = product_derivative * scale;
            self.dn[index][axis] =
                source_dn[axis] * affine_value + source_value * affine_derivative;
        }
        for axis in 0..BRANCH_COUNT {
            let product_derivative = left.db[axis] * right.value + left.value * right.db[axis];
            let affine_derivative = product_derivative * scale;
            self.db[index][axis] =
                source_db[axis] * affine_value + source_value * affine_derivative;
        }
    }

    #[inline]
    pub(crate) fn store_mul_add_lhs(
        &mut self,
        index: usize,
        left: usize,
        middle: usize,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                (left_dn[axis] + middle_dn[axis]) * right_value + sum * right_dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                (left_db[axis] + middle_db[axis]) * right_value + sum * right_db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_mul_add_rhs(
        &mut self,
        index: usize,
        left: usize,
        middle: usize,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                left_dn[axis] * sum + left_value * (middle_dn[axis] + right_dn[axis]);
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                left_db[axis] * sum + left_value * (middle_db[axis] + right_db[axis]);
        }
    }

    #[inline]
    pub(crate) fn store_mul_sub_lhs(
        &mut self,
        index: usize,
        left: usize,
        middle: usize,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                (left_dn[axis] - middle_dn[axis]) * right_value + difference * right_dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                (left_db[axis] - middle_db[axis]) * right_value + difference * right_db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_mul_sub_rhs(
        &mut self,
        index: usize,
        left: usize,
        middle: usize,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                left_dn[axis] * difference + left_value * (middle_dn[axis] - right_dn[axis]);
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                left_db[axis] * difference + left_value * (middle_db[axis] - right_db[axis]);
        }
    }

    #[inline]
    pub(crate) fn store_mul_add_ad_lhs(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                (left.dn[axis] + right.dn[axis]) * source_value + sum * source_dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                (left.db[axis] + right.db[axis]) * source_value + sum * source_db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_mul_add_ad_rhs(
        &mut self,
        index: usize,
        source: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                source_dn[axis] * sum + source_value * (left.dn[axis] + right.dn[axis]);
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                source_db[axis] * sum + source_value * (left.db[axis] + right.db[axis]);
        }
    }

    #[inline]
    pub(crate) fn store_mul_sub_ad_lhs(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                (left.dn[axis] - right.dn[axis]) * source_value + difference * source_dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                (left.db[axis] - right.db[axis]) * source_value + difference * source_db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_mul_sub_ad_rhs(
        &mut self,
        index: usize,
        source: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                source_dn[axis] * difference + source_value * (left.dn[axis] - right.dn[axis]);
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                source_db[axis] * difference + source_value * (left.db[axis] - right.db[axis]);
        }
    }

    #[inline]
    pub(crate) fn store_mul_sub_from_scalar_lhs(
        &mut self,
        index: usize,
        scalar: f64,
        value: usize,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = -value_dn[axis] * source_value + left_value * source_dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = -value_db[axis] * source_value + left_value * source_db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_mul_sub_from_scalar_rhs(
        &mut self,
        index: usize,
        source: usize,
        scalar: f64,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = source_dn[axis] * right_value - source_value * value_dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = source_db[axis] * right_value - source_value * value_db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_mul_sub_from_scalar_ad_lhs(
        &mut self,
        index: usize,
        scalar: f64,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        self.v[index] = left_value * source_value;
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = -value.dn[axis] * source_value + left_value * source_dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = -value.db[axis] * source_value + left_value * source_db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_mul_sub_from_scalar_ad_rhs(
        &mut self,
        index: usize,
        source: usize,
        scalar: f64,
        ...
        self.v[index] = source_value * right_value;
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = source_dn[axis] * right_value - source_value * value.dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = source_db[axis] * right_value - source_value * value.db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_mul_div_from_scalar_lhs(
        &mut self,
        index: usize,
        scalar: f64,
        value: usize,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                value_dn[axis] * derivative_scale * source_value + quotient * source_dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                value_db[axis] * derivative_scale * source_value + quotient * source_db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_mul_div_from_scalar_rhs(
        &mut self,
        index: usize,
        source: usize,
        scalar: f64,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                source_dn[axis] * quotient + source_value * value_dn[axis] * derivative_scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                source_db[axis] * quotient + source_value * value_db[axis] * derivative_scale;
        }
    }

    #[inline]
    pub(crate) fn store_mul_div_from_scalar_ad_lhs(
        &mut self,
        index: usize,
        scalar: f64,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                value.dn[axis] * derivative_scale * source_value + quotient * source_dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                value.db[axis] * derivative_scale * source_value + quotient * source_db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_mul_div_from_scalar_ad_rhs(
        &mut self,
        index: usize,
        source: usize,
        scalar: f64,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                source_dn[axis] * quotient + source_value * value.dn[axis] * derivative_scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                source_db[axis] * quotient + source_value * value.db[axis] * derivative_scale;
        }
    }

    #[inline]
    pub(crate) fn store_mul_pow_ad_lhs(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        for axis in 0..NODE_COUNT {
            let derivative = AdValue::<NODE_COUNT, BRANCH_COUNT>::pow_derivative(
                output,
                base,
                exponent,
                left.dn[axis],
            ...
            self.dn[index][axis] = derivative * source_value + output * source_dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            let derivative = AdValue::<NODE_COUNT, BRANCH_COUNT>::pow_derivative(
                output,
            ...
            self.db[index][axis] = derivative * source_value + output * source_db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_mul_pow_ad_rhs(
        &mut self,
        index: usize,
        source: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        for axis in 0..NODE_COUNT {
            ...
            self.dn[index][axis] = source_dn[axis] * output + source_value * derivative;
        }
        for axis in 0..BRANCH_COUNT {
            let derivative = AdValue::<NODE_COUNT, BRANCH_COUNT>::pow_derivative(
                output,
            ...
            self.db[index][axis] = source_db[axis] * output + source_value * derivative;
        }
    }

    #[inline]
    pub(crate) fn store_mul_powf_ad_lhs(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        exponent: f64,
    ...
    #[inline]
    pub(crate) fn store_mul_powf_ad_rhs(
        &mut self,
        index: usize,
        source: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_mul_neg_ad_lhs(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: usize,
        ...
        self.v[index] = left_value * right_value;
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = -left.dn[axis] * right_value + left_value * right_dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = -left.db[axis] * right_value + left_value * right_db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_mul_neg_ad_rhs(
        &mut self,
        index: usize,
        left: usize,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        self.v[index] = left_value * right_value;
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = left_dn[axis] * right_value - left_value * right.dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = left_db[axis] * right_value - left_value * right.db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_mul_div_lhs(
        &mut self,
        index: usize,
        numerator: usize,
        denominator: usize,
        ...
        for axis in 0..NODE_COUNT {
            let quotient_derivative =
                numerator_dn[axis] * reciprocal + denominator_dn[axis] * denominator_scale;
            self.dn[index][axis] = quotient_derivative * source_value + quotient * source_dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            let quotient_derivative =
                numerator_db[axis] * reciprocal + denominator_db[axis] * denominator_scale;
            self.db[index][axis] = quotient_derivative * source_value + quotient * source_db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_mul_div_rhs(
        &mut self,
        index: usize,
        source: usize,
        numerator: usize,
        ...
        for axis in 0..NODE_COUNT {
            let quotient_derivative =
                numerator_dn[axis] * reciprocal + denominator_dn[axis] * denominator_scale;
            self.dn[index][axis] = source_dn[axis] * quotient + source_value * quotient_derivative;
        }
        for axis in 0..BRANCH_COUNT {
            let quotient_derivative =
                numerator_db[axis] * reciprocal + denominator_db[axis] * denominator_scale;
            self.db[index][axis] = source_db[axis] * quotient + source_value * quotient_derivative;
        }
    }

    #[inline]
    pub(crate) fn store_mul_div_ad_lhs(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        for axis in 0..NODE_COUNT {
            let quotient_derivative =
                left.dn[axis] * reciprocal + right.dn[axis] * denominator_scale;
            self.dn[index][axis] = quotient_derivative * source_value + quotient * source_dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            let quotient_derivative =
                left.db[axis] * reciprocal + right.db[axis] * denominator_scale;
            self.db[index][axis] = quotient_derivative * source_value + quotient * source_db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_mul_div_ad_rhs(
        &mut self,
        index: usize,
        source: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    pub(crate) fn store_mul_square_lhs(&mut self, index: usize, value: usize, source: usize) {
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                value_dn[axis] * derivative_scale * source_value + square * source_dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                value_db[axis] * derivative_scale * source_value + square * source_db[axis];
        }
    }
    ...
    pub(crate) fn store_mul_square_rhs(&mut self, index: usize, source: usize, value: usize) {
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                source_dn[axis] * square + source_value * value_dn[axis] * derivative_scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                source_db[axis] * square + source_value * value_db[axis] * derivative_scale;
        }
    }

    #[inline]
    pub(crate) fn store_mul_unary_lhs(
        &mut self,
        index: usize,
        value_source: usize,
        source: usize,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                value_dn[axis] * derivative_scale * source_value + unary_value * source_dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                value_db[axis] * derivative_scale * source_value + unary_value * source_db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_mul_unary_rhs(
        &mut self,
        index: usize,
        source: usize,
        value_source: usize,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                source_dn[axis] * unary_value + source_value * value_dn[axis] * derivative_scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                source_db[axis] * unary_value + source_value * value_db[axis] * derivative_scale;
        }
    }

    #[inline]
    pub(crate) fn store_mul_unary_ad_lhs(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        source: usize,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                value.dn[axis] * derivative_scale * source_value + unary_value * source_dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                value.db[axis] * derivative_scale * source_value + unary_value * source_db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_mul_unary_ad_rhs(
        &mut self,
        index: usize,
        source: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                source_dn[axis] * unary_value + source_value * value.dn[axis] * derivative_scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                source_db[axis] * unary_value + source_value * value.db[axis] * derivative_scale;
        }
    }
    ...
    #[inline]
    pub(crate) fn store_mul_exp_ad_lhs(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        source: usize,
    ...
    #[inline]
    pub(crate) fn store_mul_exp_ad_rhs(
        &mut self,
        index: usize,
        source: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_mul_ln_ad_lhs(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        source: usize,
    ...
    #[inline]
    pub(crate) fn store_mul_ln_ad_rhs(
        &mut self,
        index: usize,
        source: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_mul_sqrt_ad_lhs(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        source: usize,
    ...
    #[inline]
    pub(crate) fn store_mul_sqrt_ad_rhs(
        &mut self,
        index: usize,
        source: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_mul_abs_ad_lhs(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        source: usize,
    ...
    #[inline]
    pub(crate) fn store_mul_abs_ad_rhs(
        &mut self,
        index: usize,
        source: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_mul_cos_ad_lhs(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        source: usize,
    ...
    #[inline]
    pub(crate) fn store_mul_cos_ad_rhs(
        &mut self,
        index: usize,
        source: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_mul_tanh_ad_lhs(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        source: usize,
    ...
    #[inline]
    pub(crate) fn store_mul_tanh_ad_rhs(
        &mut self,
        index: usize,
        source: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    #[inline]
    pub(crate) fn store_mul_limexp_lhs(
        &mut self,
        index: usize,
        value_source: usize,
        source: usize,
        ...
        if raw < 80.0 {
            let value = raw.exp();
            self.store_mul_unary_lhs(index, value_source, source, value, value);
        } else {
            self.store_mul_unary_lhs(
                index,
                value_source,
                source,
                LIMEXP_MAX * (1.0 + raw - 80.0),
                LIMEXP_MAX,
            );
        }
    }
    ...
    #[inline]
    pub(crate) fn store_mul_limexp_rhs(
        &mut self,
        index: usize,
        source: usize,
        value_source: usize,
        ...
        if raw < 80.0 {
            let value = raw.exp();
            self.store_mul_unary_rhs(index, source, value_source, value, value);
        } else {
            self.store_mul_unary_rhs(
                index,
                source,
    ...
    #[inline]
    pub(crate) fn store_mul_limexp_ad_lhs(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        source: usize,
        ...
        if raw < 80.0 {
            let output = raw.exp();
            self.store_mul_unary_ad_lhs(index, value, source, output, output);
        } else {
            self.store_mul_unary_ad_lhs(
                index,
                value,
    ...
    #[inline]
    pub(crate) fn store_mul_limexp_ad_rhs(
        &mut self,
        index: usize,
        source: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        if raw < 80.0 {
            let output = raw.exp();
            self.store_mul_unary_ad_rhs(index, source, value, output, output);
        } else {
            self.store_mul_unary_ad_rhs(
                index,
                source,
    ...
    #[inline]
    pub(crate) fn store_mul_limited_exp_lhs(
        &mut self,
        index: usize,
        value_source: usize,
        source: usize,
        ...
        if raw > 80.0 {
            ...
            self.store_mul_unary_lhs(index, value_source, source, 1.804851387e-35, 0.0);
        } else {
            let value = raw.exp();
            self.store_mul_unary_lhs(index, value_source, source, value, value);
        }
    ...
    #[inline]
    pub(crate) fn store_mul_limited_exp_rhs(
        &mut self,
        index: usize,
        source: usize,
        value_source: usize,
        ...
        if raw > 80.0 {
            ...
            self.store_mul_unary_rhs(index, source, value_source, 1.804851387e-35, 0.0);
        } else {
            let value = raw.exp();
            self.store_mul_unary_rhs(index, source, value_source, value, value);
        }
    ...
    #[inline]
    pub(crate) fn store_mul_limited_exp_ad_lhs(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        source: usize,
        ...
        if raw > 80.0 {
            ...
            self.store_mul_unary_ad_lhs(index, value, source, 1.804851387e-35, 0.0);
        } else {
            let output = raw.exp();
            self.store_mul_unary_ad_lhs(index, value, source, output, output);
        }
    ...
    #[inline]
    pub(crate) fn store_mul_limited_exp_ad_rhs(
        &mut self,
        index: usize,
        source: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        if raw > 80.0 {
            ...
            self.store_mul_unary_ad_rhs(index, source, value, 1.804851387e-35, 0.0);
        } else {
            let output = raw.exp();
            self.store_mul_unary_ad_rhs(index, source, value, output, output);
        }
    ...
    #[inline]
    pub(crate) fn store_mul_offset_lhs(
        &mut self,
        index: usize,
        left: usize,
        offset: f64,
    ...
    #[inline]
    pub(crate) fn store_mul_offset_rhs(
        &mut self,
        index: usize,
        left: usize,
        right: usize,
    ...
    #[inline]
    pub(crate) fn store_mul_offset_ad_lhs(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        offset: f64,
    ...
    #[inline]
    pub(crate) fn store_mul_offset_ad_rhs(
        &mut self,
        index: usize,
        left: usize,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
    ...
    pub(crate) fn store_mul_scale_ad_lhs(
        &mut self,
        index: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        scale: f64,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                value.dn[axis] * scale * source_value + left_value * source_dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                value.db[axis] * scale * source_value + left_value * source_db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_mul_scale_ad_rhs(
        &mut self,
        index: usize,
        source: usize,
        value: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                source_dn[axis] * right_value + source_value * value.dn[axis] * scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                source_db[axis] * right_value + source_value * value.db[axis] * scale;
        }
    }

    #[inline]
    pub(crate) fn store_mul_scaled_ad_rhs(
        &mut self,
        index: usize,
        left: usize,
        scale: f64,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                left_dn[axis] * scale * right.value + left_value * right.dn[axis];
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                left_db[axis] * scale * right.value + left_value * right.db[axis];
        }
    }

    #[inline]
    pub(crate) fn store_mul_scaled_ad_lhs(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: usize,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                left.dn[axis] * right_value + left.value * right_dn[axis] * scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                left.db[axis] * right_value + left.value * right_db[axis] * scale;
        }
    }
    ...
    #[inline]
    pub(crate) fn store_scaled_div_ad_rhs(
        &mut self,
        index: usize,
        left: usize,
        right: AdValue<NODE_COUNT, BRANCH_COUNT>,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                (left_dn[axis] * reciprocal + right.dn[axis] * right_scale) * scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                (left_db[axis] * reciprocal + right.db[axis] * right_scale) * scale;
        }
    }

    #[inline]
    pub(crate) fn store_scaled_div_ad_lhs(
        &mut self,
        index: usize,
        left: AdValue<NODE_COUNT, BRANCH_COUNT>,
        right: usize,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                (left.dn[axis] * reciprocal + right_dn[axis] * right_scale) * scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                (left.db[axis] * reciprocal + right_db[axis] * right_scale) * scale;
        }
    }
    ...
    #[inline]
    pub(crate) fn store_add_scaled_inputs(
        &mut self,
        index: usize,
        left: usize,
        left_scale: f64,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = left_dn[axis] * left_scale + right_dn[axis] * right_scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = left_db[axis] * left_scale + right_db[axis] * right_scale;
        }
    }

    #[inline]
    pub(crate) fn store_sub_scaled_inputs(
        &mut self,
        index: usize,
        left: usize,
        left_scale: f64,
    ...
    #[inline]
    pub(crate) fn store_offset_scaled_add(
        &mut self,
        index: usize,
        left: usize,
        right: usize,
    ...
    #[inline]
    pub(crate) fn store_offset_scaled_sub(
        &mut self,
        index: usize,
        left: usize,
        right: usize,
    ...
    #[inline]
    pub(crate) fn store_offset_scaled_mul(
        &mut self,
        index: usize,
        left: usize,
        right: usize,
    ...
    #[inline]
    pub(crate) fn store_offset_scaled_div(
        &mut self,
        index: usize,
        left: usize,
        right: usize,
    ...
    pub(crate) fn store_min3(&mut self, index: usize, first: usize, second: usize, third: usize) {
        ...
        if self.v[second] < self.v[selected] {
            selected = second;
        }
        if self.v[third] < self.v[selected] {
            selected = third;
        }
    ...
    #[inline]
    pub(crate) fn store_div_from_scalar_offset_input(
        &mut self,
        index: usize,
        scalar: f64,
        source: usize,
    ...
    #[inline]
    pub(crate) fn store_div_from_scalar_scaled_input(
        &mut self,
        index: usize,
        scalar: f64,
        source: usize,
    ...
    #[inline]
    pub(crate) fn store_div_from_scalar_offset_scaled_input(
        &mut self,
        index: usize,
        scalar: f64,
        source: usize,
    ...
    #[inline]
    pub(crate) fn store_sqrt_offset_scaled_input(
        &mut self,
        index: usize,
        source: usize,
        scale: f64,
    ...
    #[inline]
    pub(crate) fn store_sqrt_square_add(
        &mut self,
        index: usize,
        square_source: usize,
        add_source: usize,
        ...
        self.v[index] = value;
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = square_dn[axis] * square_scale + add_dn[axis] * add_scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = square_db[axis] * square_scale + add_db[axis] * add_scale;
        }
    }
    ...
    #[inline]
    pub(crate) fn store_limited_exp_scaled_input(
        &mut self,
        index: usize,
        source: usize,
        scale: f64,
    ...
    #[inline]
    pub(crate) fn store_scaled_sqrt_scaled_input(
        &mut self,
        index: usize,
        source: usize,
        input_scale: f64,
        ...
        self.store_unary_scaled(
            ...
            source,
            value * output_scale,
            output_scale * input_scale / (2.0 * value),
        );
    }

    #[inline]
    pub(crate) fn store_scaled_exp_scaled_input(
        &mut self,
        index: usize,
        source: usize,
        input_scale: f64,
        ...
        self.store_unary_scaled(
            ...
            source,
            value * output_scale,
            value * output_scale * input_scale,
        );
    }

    #[inline]
    pub(crate) fn store_scaled_limexp_scaled_input(
        &mut self,
        index: usize,
        source: usize,
        input_scale: f64,
        ...
        if raw < 80.0 {
            ...
            self.store_unary_scaled(
                index,
                source,
                LIMEXP_MAX * (1.0 + (raw - 80.0)) * output_scale,
                LIMEXP_MAX * output_scale * input_scale,
            );
        }
    }

    #[inline]
    pub(crate) fn store_scaled_limited_exp_scaled_input(
        &mut self,
        index: usize,
        source: usize,
        input_scale: f64,
    ...
    #[inline]
    pub(crate) fn store_scaled_ln_scaled_input(
        &mut self,
        index: usize,
        source: usize,
        input_scale: f64,
        ...
        self.store_unary_scaled(
            ...
            source,
            raw.ln() * output_scale,
            output_scale * input_scale / raw,
        );
    }

    #[inline]
    pub(crate) fn store_scaled_sin_scaled_input(
        &mut self,
        index: usize,
        source: usize,
        input_scale: f64,
        ...
        self.store_unary_scaled(
            index,
            source,
            raw.sin() * output_scale,
            raw.cos() * output_scale * input_scale,
        );
    }
    ...
    pub(crate) fn store_offset_limexp(&mut self, index: usize, source: usize, offset: f64) {
        ...
        if raw < 80.0 {
            ...
            self.store_unary_scaled(
                index,
                source,
                LIMEXP_MAX * (1.0 + (raw - 80.0)) + offset,
                LIMEXP_MAX,
            );
        }
    }
    ...
    #[inline]
    pub(crate) fn store_unary_add_scaled(
        &mut self,
        index: usize,
        left: usize,
        right: usize,
        ...
        self.v[index] = value;
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] = (left_dn[axis] + right_dn[axis]) * derivative_scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] = (left_db[axis] + right_db[axis]) * derivative_scale;
        }
    }

    #[inline]
    pub(crate) fn store_unary_sub_scaled(
        &mut self,
        index: usize,
        left: usize,
        right: usize,
    ...
    #[inline]
    pub(crate) fn store_unary_mul_scaled(
        &mut self,
        index: usize,
        left: usize,
        right: usize,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                (left_dn[axis] * right_value + left_value * right_dn[axis]) * derivative_scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                (left_db[axis] * right_value + left_value * right_db[axis]) * derivative_scale;
        }
    }

    #[inline]
    pub(crate) fn store_unary_div_scaled(
        &mut self,
        index: usize,
        left: usize,
        right: usize,
        ...
        for axis in 0..NODE_COUNT {
            self.dn[index][axis] =
                (left_dn[axis] * reciprocal + right_dn[axis] * right_scale) * derivative_scale;
        }
        for axis in 0..BRANCH_COUNT {
            self.db[index][axis] =
                (left_db[axis] * reciprocal + right_db[axis] * right_scale) * derivative_scale;
        }
    }
...
impl<const NODE_COUNT: usize, const BRANCH_COUNT: usize> AdValue<NODE_COUNT, BRANCH_COUNT> {
    ...
    pub(crate) fn add(left: Self, right: Self) -> Self {
        ...
        for index in 0..NODE_COUNT {
            value.dn[index] += right.dn[index];
        }
        for index in 0..BRANCH_COUNT {
            value.db[index] += right.db[index];
        }
        value
    }
    ...
    pub(crate) fn mul(left: Self, right: Self) -> Self {
        ...
        for index in 0..NODE_COUNT {
            value.dn[index] = value.dn[index] * right.value + left_value * right.dn[index];
        }
        for index in 0..BRANCH_COUNT {
            value.db[index] = value.db[index] * right.value + left_value * right.db[index];
        }
        value
    }
    ...
    pub(crate) fn div(left: Self, right: Self) -> Self {
        ...
        for index in 0..NODE_COUNT {
            value.dn[index] = value.dn[index] * reciprocal + right.dn[index] * right_scale;
        }
        for index in 0..BRANCH_COUNT {
            value.db[index] = value.db[index] * reciprocal + right.db[index] * right_scale;
        }
        value
    }
    ...
    pub(crate) fn limexp(arg: Self) -> Self {
        ...
        if raw < 80.0 {
            ...
            Self::unary_intrinsic(arg, LIMEXP_MAX * (1.0 + (raw - 80.0)), LIMEXP_MAX)
        }
    }
    #[inline]
    ...
    pub(crate) fn powf(left: Self, exponent: f64) -> Self {
        ...
        result.value = value;
        for index in 0..NODE_COUNT {
            result.dn[index] = Self::pow_derivative(value, base, exponent, result.dn[index], 0.0);
        }
        for index in 0..BRANCH_COUNT {
            result.db[index] = Self::pow_derivative(value, base, exponent, result.db[index], 0.0);
        }
        result
    }
    #[inline]
    pub(crate) fn pow_from_scalar(base: f64, right: Self) -> Self {
        ...
        result.value = value;
        for index in 0..NODE_COUNT {
            result.dn[index] = Self::pow_derivative(value, base, exponent, 0.0, result.dn[index]);
        }
        for index in 0..BRANCH_COUNT {
            result.db[index] = Self::pow_derivative(value, base, exponent, 0.0, result.db[index]);
        }
        result
    }
    #[inline]
    pub(crate) fn pow(left: Self, right: Self) -> Self {
        ...
        for index in 0..NODE_COUNT {
            result.dn[index] =
                Self::pow_derivative(value, base, exponent, result.dn[index], right.dn[index]);
        }
        for index in 0..BRANCH_COUNT {
            result.db[index] =
                Self::pow_derivative(value, base, exponent, result.db[index], right.db[index]);
        }
        result
    }
    #[inline]
    ...
    pub(crate) fn hypot(left: Self, right: Self) -> Self {
        ...
        for index in 0..NODE_COUNT {
            result.dn[index] =
                (left_value * result.dn[index] + right_value * right.dn[index]) / value;
        }
        for index in 0..BRANCH_COUNT {
            result.db[index] =
                (left_value * result.db[index] + right_value * right.db[index]) / value;
        }
        result
    }
    #[inline]
    pub(crate) fn atan2(y: Self, x: Self) -> Self {
        ...
        for index in 0..NODE_COUNT {
            result.dn[index] = (x_value * result.dn[index] - y_value * x.dn[index]) / denominator;
        }
        for index in 0..BRANCH_COUNT {
            result.db[index] = (x_value * result.db[index] - y_value * x.db[index]) / denominator;
        }
        result
    }
