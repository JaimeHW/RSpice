...
impl Instance {
    ...
    pub(super) fn stamp_transient_block_36(
        ...
        let (td,) = {
    if s.b[781] {
        let tb: f64 = (4.0 * 1.3806488e-23);let tc: f64 = (tb * s.v[213]);
        (tc,)
    } else {
        (s.v[302],)
    }
};
        s.store_scalar(302, td);
        let (tf,) = {
    if s.b[781] {
        let te: f64 = (s.v[171] * s.v[302]);
        (te,)
    } else {
        (s.v[303],)
    }
};
        s.store_scalar(303, tf);s.b[787] = (p[14] == 1.0);s.store_scalar(787, if s.b[787] { 1.0 } else { 0.0 });
    }
    #[inline(never)]
    ...
    pub(super) fn stamp_transient_block_40(
        ...
        if ((s.b[1055] && (!s.b[1061])) && (!s.b[1062])) {s.store_div_from_scalar_offset_square(838, 1.0, 864, 2.0);s.store_mul_square_lhs(853, 864, 838);s.store_mul3_affine_lhs(854, 864, 838, 4.0, 0.0, 838);s.store_mul_ad_product_lhs_mixed_ai(855, A::sub_scaled_inputs(s.ad_value(838), 8.0, s.ad_value(853), 12.0), 838, 838);s.store_sub(838, 841, 864);s.store_add_scaled_product_mixed_iia(856, 838, 2.0, 257, A::add_scaled_inputs_product(A::sub_from_scalar(1.0, s.ad_value(852)), 1.0, s.ad_value(851), 1.0, s.ad_value(842), A::offset(s.ad_value(854), 1.0), (-1.0)), 1.0);}
    }
    #[inline(never)]
    ...
    pub(super) fn stamp_transient_block_42(
        ...
        s.store_offset_sqrt_ad(897, A::offset(A::div_scaled_inputs2(s.ad_value(250), 2.0, s.ad_value(896), (-2.0), s.ad_value(251), 1.0), 1.0), (-1.0));s.store_add_scaled_product_indices(898, 896, 1.0, 251, 897, 1.0);s.store_scaled_add_offset_sqrt_square_offset_ad(0, A::mul(s.ad_value(30), s.ad_value(875)), ((1.0) + (0.5)), ((1.0) + ((-0.5))), 0.01, 0.5);s.store_div_from_scalar_offset_product(899, 1.0, 867, 0, 1.0);s.store_div_from_scalar_offset_product(900, 1.0, 868, 0, 1.0);s.store_mul_scale_offset(0, A::mul3_scaled_output(s.ad_value(325), A::offset(A::sqrt(A::offset(A::div(s.ad_value(336), s.ad_value(325)), 1.0)), (-1.0)), A::offset(A::mul(s.ad_value(36), s.ad_value(897)), 1.0), 2.0), A::mul(s.ad_value(37), s.ad_value(875)), 1.0, 1.0);s.store_mul(901, 869, 0);s.store_mul(902, 870, 0);s.store_add_mixed_ai(903, A::add_scaled_product(s.ad_value(898), 1.0, A::add_scaled_inputs3(s.ad_value(874), 1.0, s.ad_value(898), (-1.0), s.ad_value(901), 1.0), s.ad_value(899), 1.0), 337);s.store_add_mixed_ai(904, A::add_scaled_product(s.ad_value(898), 1.0, A::add_scaled_inputs3(s.ad_value(885), 1.0, s.ad_value(898), (-1.0), s.ad_value(902), 1.0), s.ad_value(900), 1.0), 337);s.store_add_scaled_inputs3_sqrt_third_mixed_aia(905, A::add_scaled_product(s.ad_value(904), 1.0, s.ad_value(25), A::sub(s.ad_value(903), s.ad_value(904)), 1.0), 0.5, 221, 0.5, A::offset(A::square(A::sub(A::add_scaled_product(s.ad_value(904), 1.0, s.ad_value(25), A::sub(s.ad_value(903), s.ad_value(904)), 1.0), s.ad_value(221))), 0.01), (-0.5));s.store_add_scaled_inputs3_sqrt_third_mixed_aia(906, A::add_scaled_product(s.ad_value(903), 1.0, s.ad_value(26), A::sub(s.ad_value(904), s.ad_value(903)), 1.0), 0.5, 221, 0.5, A::offset(A::square(A::sub(A::add_scaled_product(s.ad_value(903), 1.0, s.ad_value(26), A::sub(s.ad_value(904), s.ad_value(903)), 1.0), s.ad_value(221))), 0.01), (-0.5));s.store_div(907, 890, 899);s.store_div(908, 891, 900);s.store_div_from_scalar(881, 1.0, 907);s.store_div_from_scalar(882, 1.0, 908);
    }
    #[inline(never)]
    pub(super) fn stamp_transient_block_43(
        s: &mut Scratch,
    ) {
        s.store_div_from_scalar_add_ad(909, 1.0, A::offset(s.ad_value(881), 1.0), s.ad_value(882));s.store_div_square_rhs(880, 249, 894);s.store_div_scaled_offset_numerator_mixed_ia(877, 907, 1.0, 1.0, A::offset(s.ad_value(908), 1.0), 1.0);s.store_ln(878, 877);s.b[1071] = (s.v[878] > 1e-8);s.store_scalar(1071, if s.b[1071] { 1.0 } else { 0.0 });
        ...
        s.store_sub_scaled_inputs(797, 793, 1.0, 791, 3.0);s.store_sub(912, 905, 796);s.store_sub(916, 906, 797);s.store_scalar(803, 0.0);s.store_scalar(806, 0.0);s.store_mul(798, 907, 912);s.b[1076] = ((s.v[905] - s.v[912]) < 80.0);s.store_scalar(1076, if s.b[1076] { 1.0 } else { 0.0 });
    ...
    if s.b[1083] {
        ...
        if ((!s.b[1085]) && (!s.b[1086])) {s.store_offset_ad(804, A::mul_sub_from_scalar_rhs_scaled_output(s.ad_value(800), 1.0, A::mul_sub_from_scalar_rhs_scaled_output(s.ad_value(800), 1.0, A::scale(s.ad_value(800), 0.0238095238095), 0.0166666666667), 0.1666666666667), 2.0);s.store_offset_scaled_mul_sub_from_scalar_scaled_sub_rhs_scaled_output(805, 800, 1.0, 800, 1.0, 800, 0.0333333333333, 0.0357142857143, ((0.0333333333333) * ((-0.1666666666667))), 0.1666666666667);}
