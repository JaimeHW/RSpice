...
impl Instance {
    ...
    pub(super) fn stamp_transient_block_132(
        ...
        s.b[1747] = (p[9] > 0.0);s.store_scalar(1747, if s.b[1747] { 1.0 } else { 0.0 });
    ...
    pub(super) fn stamp_transient_block_133(
        ...
        if s.b[1608] {s.store_mul3_lhs(1573, 1544, 1572, 0);}
    }
    #[inline(never)]
    pub(super) fn stamp_transient_block_134(
        ...
        s.b[1755] = (p[13] > 0.0);s.store_scalar(1755, if s.b[1755] { 1.0 } else { 0.0 });
    ...
    pub(super) fn stamp_transient_block_137(
        ...
        if s.b[1763] {s.store_offset(0, 254, (2.0 * 0.6931471805599));s.store_add(1330, 460, 0);s.store_add(1331, 463, 0);s.store_add_scaled_inputs3_sqrt_third_sub_square_offset(1332, 1330, 0.5, 254, 0.5, 1330, 254, 9.0, (-0.5));s.store_add_scaled_inputs4_mixed_iiia(1333, 1331, 0.5, 254, 0.5, 339, 0.5, A::sqrt_square_offset(A::add_scaled_inputs3(s.ad_value(1331), 1.0, s.ad_value(254), -1.0, s.ad_value(339), -1.0), 9.0), (-0.5));s.store_mul_sqrt_mixed_ia(1334, 294, A::mul_offset_rhs(s.ad_value(445), s.ad_value(444), 0.5));s.store_mul_sqrt_mixed_ia(1335, 294, A::mul_offset_rhs(A::mul3(s.ad_value(445), s.ad_value(456), s.ad_value(444)), s.ad_value(443), 0.5));s.store_mul_square_lhs(1336, 1334, 291);s.store_mul_square_lhs(1337, 1335, 291);s.store_sub(2, 292, 1332);s.store_add_scaled_inputs3_indices(3, 292, 1.0, 339, 1.0, 1333, -1.0);s.store_scale(0, 1336, 2.0);s.store_add_scaled_offset_product_rhs_mixed_iia(1338, 1332, 1.0, 0, A::sqrt(A::offset(A::div(s.ad_value(2), s.ad_value(1336)), 1.0)), (-1.0), 1.0);s.store_add_scaled_offset_product_rhs_mixed_iia(1339, 1333, 1.0, 0, A::sqrt(A::offset(A::div(s.ad_value(3), s.ad_value(1336)), 1.0)), (-1.0), 1.0);s.store_scale(0, 1337, 2.0);s.store_add_scaled_offset_product_rhs_mixed_iia(1340, 1332, 1.0, 0, A::sqrt(A::offset(A::div(s.ad_value(2), s.ad_value(1337)), 1.0)), (-1.0), 1.0);s.store_add_scaled_offset_product_rhs_mixed_iia(1341, 1333, 1.0, 0, A::sqrt(A::offset(A::div(s.ad_value(3), s.ad_value(1337)), 1.0)), (-1.0), 1.0);s.store_mul(0, 293, 447);s.store_mul_product3_indices(2, 451, 0, 1334, 456, -1.0);s.store_mul_product3_indices(3, 452, 0, 1335, 457, -1.0);s.store_add_scaled_inputs3_sqrt_third_sub_square_offset(0, 1338, 0.5, 1330, ((-1.0) * 0.5), 1338, 1330, 1.0, 0.5);s.store_div_scaled_product3_mixed_iiia(379, 2, 0, 0, 1.0, A::sub(s.ad_value(1338), s.ad_value(1332)), 1.0);s.store_add_scaled_inputs3_sqrt_third_sub_square_offset(0, 1339, 0.5, 1331, ((-1.0) * 0.5), 1339, 1331, 1.0, 0.5);}
    }
    #[inline(never)]
    ...
    pub(super) fn stamp_transient_block_139(
        ...
        s.store_scaled_mul(374, 13, 374, p[32]);s.store_scaled_mul(377, 13, 377, p[32]);s.store_scaled_mul(376, 13, 376, p[32]);s.store_scaled_mul(375, 13, 375, p[32]);s.store_mul(378, 13, 378);s.b[1778] = (s.v[334] < 0.0);s.store_scalar(1778, if s.b[1778] { 1.0 } else { 0.0 });
        ...
        s.store_scaled_mul(1780, 390, 226, 1.0 / (1.602176565e-19));s.store_scaled_add(1781, 407, 432, (-0.5));s.store_add(1782, 415, 1781);s.store_div(0, 415, 1782);s.store_scaled_add_mixed_ia(1787, 0, A::sqrt_square_offset(s.ad_value(0), 1e-20), 0.5);s.store_scaled_mul(1788, 436, 435, (-0.1666666666667));s.store_square(1789, 1788);s.store_offset(1790, 429, (-1.0));s.store_max_with_scalar_ad(1791, A::sub_from_scalar(1.0, A::mul_scaled_lhs(s.ad_value(1790), 12.0, s.ad_value(1789))), 1e-20);s.store_div_from_scalar_square_ad(1792, 1.0, s.ad_value(1791));s.store_div_scaled_product3_by_product_mixed_aiiii(1793, A::mul3(s.ad_value(342), s.ad_value(390), s.ad_value(226)), 1782, 344, 1.0, 345, 346, 1.0);s.store_scale(1794, 1789, 12.0);s.store_add_scaled_inputs3_mixed_iia(2, 1787, 1.0, 1794, 1.0, A::mul3_scaled_output(A::offset(s.ad_value(1787), 1.0), s.ad_value(1794), s.ad_value(1790), 2.0), -1.0);s.store_max_with_scalar(3, 2, 1e-40);let t0: f64 = (s.v[1793] * s.v[1792]);let t1: f64 = (t0 * s.v[3]);s.store_scalar(1795, t1);s.b[1812] = (s.v[172] > 0.0);s.store_scalar(1812, if s.b[1812] { 1.0 } else { 0.0 });
        let (t3,) = {
    if s.b[1812] {
    ...
    if s.b[1812] {
        ...
        s.store_scalar(1795, tf);s.store_div_scaled_product3_indices(1799, 456, 447, 116, 1.0, 469, 1.0);s.store_mul_scale_offset_indices(1800, 1799, 468, 1.0, 1.0);s.store_mul_scale_offset_mixed_ia(1802, 1800, A::mul_scaled_lhs(s.ad_value(334), 0.25, s.ad_value(1788)), -1.0, 0.5);s.store_sub(1801, 1800, 1802);s.store_scalar(1805, 0.0);
    }
    #[inline(never)]
    pub(super) fn stamp_transient_block_140(
        ...
        s.b[1813] = (p[6] > 0.0);s.store_scalar(1813, if s.b[1813] { 1.0 } else { 0.0 });
    ...
    pub(super) fn stamp_transient_block_142(
        ...
        s.store_scalar(1839, 0.0);s.store_scalar(1832, 0.0);s.b[1884] = (p[9] > 0.0);s.store_scalar(1884, if s.b[1884] { 1.0 } else { 0.0 });
    ...
    pub(super) fn stamp_transient_block_143(
        ...
        s.store_add_scaled_product_mixed_iai(1846, 1844, (-1.0), A::sub_scaled_inputs(s.ad_value(337), -1.0, s.ad_value(1841), 1.0), 1831, 1.0);s.b[1887] = (p[2] > 0.0);s.store_scalar(1887, if s.b[1887] { 1.0 } else { 0.0 });
        ...
        s.store_add_scaled_inputs3_sqrt_third_sub_square_offset(1856, 1855, 0.5, 1834, 0.5, 1855, 1834, 4.0, (-0.5));s.store_offset_sqrt_ad(1857, A::offset(A::div_scaled_inputs2(s.ad_value(1834), 2.0, s.ad_value(1856), (-2.0), s.ad_value(1835), 1.0), 1.0), (-1.0));
    }
}
