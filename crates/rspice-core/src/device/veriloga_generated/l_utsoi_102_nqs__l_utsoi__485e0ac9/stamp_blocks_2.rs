...
impl Instance {
    ...
    pub(super) fn stamp_transient_block_32(
        ...
        s.store_primal_div_from_scalar(276, 1.0, 87);s.store_scaled_sqrt_scaled_input(277, 87, ((2.0 * 1.602176565e-19) * 9.10938291e-31), ((4.0 * 0.3333333333333) * 9.482522386533242e33));s.store_mul(278, 277, 18);s.store_mul(279, 277, 18);s.store_scalar(280, 0.0);s.b[676] = (s.v[79] < 0.0);s.store_scalar(676, if s.b[676] { 1.0 } else { 0.0 });
    ...
    pub(super) fn stamp_transient_block_33(
        ...
        s.store_scale(283, 233, 0.5);s.store_mul(284, 75, 226);s.store_mul(285, 75, 223);s.store_div_from_scalar_offset_product(286, 1.0, 88, 236, 1.0);s.store_div_from_scalar_square_ad(0, 4e-18, s.ad_value(18));s.store_mul(89, 89, 0);s.store_mul(90, 90, 0);s.store_scale(0, 18, 500000000.0);
    }
    #[inline(never)]
    pub(super) fn stamp_transient_block_34(
        ...
        s.store_scaled_add_sqrt_square_offset_ad(277, A::offset(A::mul(s.ad_value(93), s.ad_value(220)), 1.0), 0.01, 0.5);s.store_mul3_lhs(91, 208, 277, 0);s.store_scaled_add_sqrt_square_offset_ad(277, A::offset(A::mul(s.ad_value(94), s.ad_value(220)), 1.0), 0.01, 0.5);s.store_mul3_lhs(92, 209, 277, 0);s.store_mul_exp_mixed_ia(113, 210, A::mul_scaled_lhs(s.ad_value(114), -1.0, s.ad_value(295)));s.store_mul_scale_offset_mixed_ia(288, 223, A::mul(s.ad_value(99), s.ad_value(222)), 1.0, 1.0);s.store_div_from_scalar(289, 1.0, 288);s.store_mul3_affine_lhs(290, 252, 229, (2.0 * 1.602176565e-19), 0.0, 289);s.store_add_scaled_product_indices(0, 256, 1.0, 102, 220, p[14]);s.store_sub_offset_lhs_mixed_ai(100, A::add_scaled_inputs4(s.ad_value(211), p[14], s.ad_value(237), p[14], s.ad_value(243), p[14], s.ad_value(0), 1.0), p[34], 249);s.store_add_scaled_inputs4_indices(101, 212, p[14], 237, p[14], 244, p[14], 0, 1.0);s.store_scaled_exp_ad(0, A::mul(s.ad_value(111), s.ad_value(295)), p[35]);s.store_mul(110, 213, 0);s.store_mul(287, 116, 226);s.store_div_scaled_inputs_mixed_ia(291, 118, (0.25 * 1.602176565e-19), A::mul(s.ad_value(229), s.ad_value(226)), 1.0);s.store_ln_div(292, 118, 252);s.store_scaled_mul(293, 119, 226, 1.25e-6);
    }
    #[inline(never)]
    pub(super) fn stamp_transient_block_35(
        s: &mut Scratch,
    ) {
        s.store_primal_sqrt_ad(294, A::mul3_scaled_output(s.ad_value(229), s.ad_value(14), A::offset(s.ad_value(533), 4e-10), 1.0 / (3.45313e-11)));s.store_exp_mul(305, 169, 295);s.store_mul(168, 214, 305);let t0: f64 = (4.0 * 1.3806488e-23);let t1: f64 = (t0 * s.v[217]);s.store_scalar(306, t1);let t2: f64 = (s.v[171] * s.v[306]);s.store_scalar(307, t2);s.store_scalar(308, s.v[307]);
    }
    #[inline(never)]
    pub(super) fn stamp_transient_block_36(
        s: &mut Scratch,
    ) {
        let t3: f64 = (9.10938291e-31 * 1000000000000.0);let t4: f64 = (t3 * s.v[172]);s.store_scalar(309, t4);s.b[679] = (s.v[311] > 0.0);s.store_scalar(679, if s.b[679] { 1.0 } else { 0.0 });
    ...
    pub(super) fn stamp_transient_block_37(
        ...
        s.b[787] = (p[9] > 0.0);s.store_scalar(787, if s.b[787] { 1.0 } else { 0.0 });
        if (s.b[785] && s.b[787]) {s.store_mul_add_mixed_iai(249, 223, A::ln(A::div(s.ad_value(24), s.ad_value(251))), 235);}
        s.b[788] = (p[10] == 1.0);s.store_scalar(788, if s.b[788] { 1.0 } else { 0.0 });
        ...
        s.b[789] = (p[13] > 0.0);s.store_scalar(789, if s.b[789] { 1.0 } else { 0.0 });s.b[790] = (p[14] == 1.0);s.store_scalar(790, if s.b[790] { 1.0 } else { 0.0 });
        if ((s.b[785] && s.b[789]) && s.b[790]) {s.store_scale_ad(258, A::exp_scaled_input(A::ln(A::mul(s.ad_value(226), s.ad_value(259))), (-0.3333333333333)), ((0.4 * p[13]) * 1.27520989));}
        if ((s.b[785] && s.b[789]) && (!s.b[790])) {s.store_scale_ad(258, A::exp_scaled_input(A::ln(A::mul(s.ad_value(226), s.ad_value(259))), (-0.3333333333333)), ((0.4 * p[13]) * 1.5412087));}
    }
    #[inline(never)]
    ...
    pub(super) fn stamp_transient_block_43(
        ...
        s.store_mul_sub_rhs(0, 248, 878, 889);s.b[1071] = (p[13] > 0.0);s.store_scalar(1071, if s.b[1071] { 1.0 } else { 0.0 });
    ...
    pub(super) fn stamp_transient_block_44(
        ...
        s.store_offset_sqrt_ad(901, A::offset(A::div_scaled_inputs2(s.ad_value(254), 2.0, s.ad_value(900), (-2.0), s.ad_value(255), 1.0), 1.0), (-1.0));s.store_add_scaled_product_indices(902, 900, 1.0, 255, 901, 1.0);s.store_scaled_add_offset_sqrt_square_offset_ad(0, A::mul(s.ad_value(30), s.ad_value(879)), ((1.0) + (0.5)), ((1.0) + ((-0.5))), 0.01, 0.5);s.store_div_from_scalar_offset_product(903, 1.0, 871, 0, 1.0);s.store_div_from_scalar_offset_product(904, 1.0, 872, 0, 1.0);s.store_mul_scale_offset(0, A::mul3_scaled_output(s.ad_value(329), A::offset(A::sqrt(A::offset(A::div(s.ad_value(340), s.ad_value(329)), 1.0)), (-1.0)), A::offset(A::mul(s.ad_value(36), s.ad_value(901)), 1.0), 2.0), A::mul(s.ad_value(37), s.ad_value(879)), 1.0, 1.0);s.store_mul(905, 873, 0);s.store_mul(906, 874, 0);s.store_add_mixed_ai(907, A::add_scaled_product(s.ad_value(902), 1.0, A::add_scaled_inputs3(s.ad_value(878), 1.0, s.ad_value(902), (-1.0), s.ad_value(905), 1.0), s.ad_value(903), 1.0), 341);s.store_add_mixed_ai(908, A::add_scaled_product(s.ad_value(902), 1.0, A::add_scaled_inputs3(s.ad_value(889), 1.0, s.ad_value(902), (-1.0), s.ad_value(906), 1.0), s.ad_value(904), 1.0), 341);s.store_add_scaled_inputs3_sqrt_third_mixed_aia(909, A::add_scaled_product(s.ad_value(908), 1.0, s.ad_value(25), A::sub(s.ad_value(907), s.ad_value(908)), 1.0), 0.5, 225, 0.5, A::offset(A::square(A::sub(A::add_scaled_product(s.ad_value(908), 1.0, s.ad_value(25), A::sub(s.ad_value(907), s.ad_value(908)), 1.0), s.ad_value(225))), 0.01), (-0.5));s.store_add_scaled_inputs3_sqrt_third_mixed_aia(910, A::add_scaled_product(s.ad_value(907), 1.0, s.ad_value(26), A::sub(s.ad_value(908), s.ad_value(907)), 1.0), 0.5, 225, 0.5, A::offset(A::square(A::sub(A::add_scaled_product(s.ad_value(907), 1.0, s.ad_value(26), A::sub(s.ad_value(908), s.ad_value(907)), 1.0), s.ad_value(225))), 0.01), (-0.5));s.store_div(911, 894, 903);s.store_div(912, 895, 904);s.store_div_from_scalar(885, 1.0, 911);s.store_div_from_scalar(886, 1.0, 912);
    }
    #[inline(never)]
    pub(super) fn stamp_transient_block_45(
        s: &mut Scratch,
    ) {
        s.store_div_from_scalar_add_ad(913, 1.0, A::offset(s.ad_value(885), 1.0), s.ad_value(886));s.store_div_square_rhs(884, 253, 898);s.store_div_scaled_offset_numerator_mixed_ia(881, 911, 1.0, 1.0, A::offset(s.ad_value(912), 1.0), 1.0);s.store_ln(882, 881);s.b[1075] = (s.v[882] > 1e-8);s.store_scalar(1075, if s.b[1075] { 1.0 } else { 0.0 });
        ...
        s.store_sub_scaled_inputs(801, 797, 1.0, 795, 3.0);s.store_sub(916, 909, 800);s.store_sub(920, 910, 801);s.store_scalar(807, 0.0);s.store_scalar(810, 0.0);s.store_mul(802, 911, 916);s.b[1080] = ((s.v[909] - s.v[916]) < 80.0);s.store_scalar(1080, if s.b[1080] { 1.0 } else { 0.0 });
