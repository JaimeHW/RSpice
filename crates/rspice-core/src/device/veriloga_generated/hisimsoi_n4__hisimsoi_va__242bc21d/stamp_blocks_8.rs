...
impl Instance {
    ...
    pub(super) fn stamp_reactive_block_46(
        ...
        if (s.b[1220] && (0.0 != 0.0)) {s.store_offset_scaled(44, 166, (1.0 + 0.3), (((-p[287])) + ((-0.03))));s.store_scale(45, 166, ((1.0 + 0.3) * (4.0 * 0.03)));}
        if (s.b[1220] && (0.0 != 0.0)) {
            if (s.v[45] > 0.0) {
            } else {
                s.store_neg(45, 45);
        ...
        if (s.b[1220] && s.b[1266]) {s.copy_ad(378, 1230);s.copy_ad(1267, 1221);s.store_offset_add_scaled_inputs3_offset_indices(160, 185, (-1.0), 320, 1.0, 1267, 1.0, s.v[123], p[286]);}
    ...
    pub(super) fn stamp_reactive_block_47(
        ...
        if ((s.b[1220] && s.b[1266]) && (!s.b[1269])) {s.store_exp_ad(484, A::mul_offset_rhs(s.ad_value(225), s.ad_value(1267), (-p[287])));s.store_scalar(430, 0.0);s.copy_ad(1268, 378);s.store_scale(419, 229, ((p[237] * (p[237] * 0.5)) * 9662367879.197212));s.store_sqrt_mul_scaled_lhs(327, 225, 2.0, 419);s.store_scaled_add_ad(328, A::exp(s.ad_value(327)), A::exp_scaled_input(s.ad_value(327), -1.0), 0.5);s.store_div_ln_lhs(420, 328, 419);s.store_scalar(167, 1.0);}
    }
    #[inline(never)]
    ...
    pub(super) fn stamp_reactive_block_49(
        ...
        if s.b[1220] {s.store_sqrt_offset_input(1239, 328, (10.0 * 2.220446049250313e-16));s.store_mul(1240, 1238, 1239);}
    }
    #[inline(never)]
    pub(super) fn stamp_reactive_block_50(
        ...
        if s.b[1220] {s.store_mul_div_scaled_inputs_indices(1236, 1240, 227, 2.0, 1227, 1.0);s.store_mul_product3_indices(204, 337, 1236, 1235, 107, 1.0);s.store_add(199, 202, 204);}
    ...
    pub(super) fn stamp_reactive_block_52(
        ...
        s.store_scalar(1355, (s.v[88] * 100.0));s.store_scale(1356, 323, 0.0001);s.store_scalar(1357, (s.v[97] * 100.0));s.store_primal_scale(1358, 107, 100.0);s.store_scale(1359, 252, 0.01);s.store_scale(1360, 436, 0.0001);s.store_scale(1361, 238, 0.0001);s.b[1362] = (p[27] == 0.0);s.store_scalar(1362, if s.b[1362] { 1.0 } else { 0.0 });s.b[1363] = (s.v[145] == 0.0);s.store_scalar(1363, if s.b[1363] { 1.0 } else { 0.0 });
        ...
        if (!s.b[1362]) {s.store_offset_scaled(1343, 158, (-p[221]), p[222]);}
    }
    #[inline(never)]
    pub(super) fn stamp_reactive_block_53(
        ...
        if (!s.b[1362]) {s.store_exp_scaled_input(1345, 1343, s.v[1355]);s.store_scale(1343, 158, (1.0 / (s.v[1355]) * 1.0 / (s.v[1355])));s.store_mul(1346, 158, 1343);s.store_scale(1347, 1358, (p[220] / 1000000.0));s.store_sub(1344, 158, 157);s.store_offset_scaled(1343, 1344, (-p[221]), p[222]);s.store_exp_scaled_input(1345, 1343, s.v[1355]);s.store_scale(1343, 1344, (1.0 / (s.v[1355]) * 1.0 / (s.v[1355])));s.store_mul(1346, 1344, 1343);s.store_scale(1347, 1358, (p[220] / 1000000.0));s.store_offset_scaled_sub(1353, 513, 158, 1.0 / (s.v[1355]), ((((s.v[123]) + (p[225]))) * (1.0 / (s.v[1355]))));s.store_sqrt_square_offset(44, 1353, ((4.0 * 0.01) * 0.01));s.store_offset_add_scaled_inputs_indices(1353, 1353, 0.5, 44, 0.5, (1e-10 * 0.01));}
        ...
        if ((!s.b[1362]) && (!s.b[1370])) {s.store_exp(1345, 1344);s.store_scale(1346, 1358, (p[223] * s.v[1357]));}
        ...
        if ((!s.b[1378]) && (!s.b[1380])) {s.store_exp(1371, 1375);s.store_mul_scale_offset_mixed_ia(1372, 107, A::div_from_scalar(p[207], s.ad_value(302)), 1.6021918e-19, 0.0);}
        if (!s.b[1378]) {s.store_sub(1377, 157, 513);}
        s.b[1381] = (s.v[1377] > 0.0);s.store_scalar(1381, if s.b[1381] { 1.0 } else { 0.0 });
        if ((!s.b[1378]) && s.b[1381]) {s.store_square(1372, 1377);s.store_mul(331, 1372, 1377);s.store_offset(1375, 331, p[212]);}
        ...
        if ((!s.b[1389]) && (!s.b[1391])) {s.store_exp(1382, 1386);s.store_div_from_scalar(1385, 1.0, 302);s.store_scaled_mul(1383, 1385, 107, (p[207] * 1.6021918e-19));}
        if (!s.b[1389]) {s.store_neg(1388, 513);}
        s.b[1392] = (s.v[1388] > 0.0);s.store_scalar(1392, if s.b[1392] { 1.0 } else { 0.0 });
        if ((!s.b[1389]) && s.b[1392]) {s.store_square(1383, 1388);s.store_mul(331, 1383, 1388);s.store_offset(1386, 331, p[212]);}
        ...
        if (s.b[1393] && (p[24] != 0.0)) {s.copy_ad(521, 536);s.store_scalar(528, 0.0);}
        ...
        if ((s.b[1393] && (p[24] != 0.0)) && s.b[1473]) {s.store_primal_scale(522, 533, 0.5);s.store_scalar(523, p[292]);s.store_scalar(528, s.v[525]);}
    }
    #[inline(never)]
    pub(super) fn stamp_reactive_block_54(
        ...
        if ((s.b[1393] && (p[24] != 0.0)) && (s.b[1474] && (!s.b[1473]))) {s.store_primal_scale(522, 534, 0.5);s.store_scalar(523, p[68]);s.store_scalar(528, s.v[524]);s.store_scalar(528, 1.0);}
        s.b[1475] = (s.v[528] == 0.0);s.store_scalar(1475, if s.b[1475] { 1.0 } else { 0.0 });
        if ((s.b[1393] && (p[24] != 0.0)) && s.b[1475]) {s.store_mul_sqrt_mixed_ia(1423, 238, A::div(s.ad_value(521), s.ad_value(536)));s.store_scalar(1405, ((1.0 - -1.0) / 2.0));s.store_scalar(1406, ((1.0 + -1.0) / 2.0));s.store_add_scaled_products_mixed_iiia(1416, 461, 156, 1.0, 462, A::sub(s.ad_value(156), s.ad_value(157)), 1.0);s.store_add_scaled_products_indices(1417, 461, 157, 1.0, 462, 157, -1.0);s.store_add_scaled_products_mixed_iiia(1418, 461, 158, 1.0, 462, A::sub(s.ad_value(158), s.ad_value(157)), 1.0);s.store_add_scaled_products_mixed_iiia(1419, 462, 158, 1.0, 461, A::sub(s.ad_value(158), s.ad_value(157)), 1.0);s.store_sub(1420, 1417, 1416);s.store_neg(1421, 1416);s.store_primal_add_scaled_products_indices(1407, 1405, 461, 1.0, 1406, 462, 1.0);s.store_primal_add_scaled_products_indices(1408, 1405, 462, 1.0, 1406, 461, 1.0);s.store_add_scaled_products_indices(1422, 1407, 1418, 1.0, 1408, 1419, 1.0);s.store_offset_ad(1414, A::add_scaled_products(s.ad_value(1407), s.ad_value(1421), 1.0, s.ad_value(1408), s.ad_value(1420), 1.0), (10.0 * 2.220446049250313e-16));s.store_neg(1394, 1414);}
        ...
        if ((s.b[1393] && (p[24] != 0.0)) && s.b[1475]) {s.store_offset_scaled(1415, 1399, -1.0, (-1e-12));s.store_mul(1424, 1423, 1404);s.store_square(1425, 1424);s.store_sub(1426, 1422, 523);s.store_div(1394, 521, 230);s.store_mul_div_from_scalar_lhs_ad_mixed_ia(1427, 2.0, 225, A::ln(s.ad_value(1394)));s.store_neg(1428, 1415);}
        ...
        if (((s.b[1393] && (p[24] != 0.0)) && s.b[1475]) && s.b[1477]) {s.store_powf(1436, 1435, 0.3333333333333333);s.store_add_scaled_inputs_product_mixed_aiii(1437, A::sub_from_scalar(((-4.0) * 1.414213562373095), A::scale(s.ad_value(1402), 12.0)), 1.0, 1436, 2.0, 1436, 1436, 1.414213562373095);}
    }
    #[inline(never)]
    pub(super) fn stamp_reactive_block_55(
        ...
        if (((s.b[1393] && (p[24] != 0.0)) && s.b[1475]) && s.b[1477]) {s.store_div(1438, 1437, 1436);s.store_add_scaled_product_indices(1439, 1415, (-1.0), 1438, 227, 1.0);s.store_add(1395, 1439, 1415);s.store_div(1396, 1395, 1431);s.store_sqrt_square_offset(1397, 1396, 1.0);s.store_sub_div_lhs_indices(1440, 1395, 1397, 1415);s.store_sub(1396, 1426, 1440);s.store_mul(459, 1403, 1396);s.copy_ad(458, 459);}
        if (((s.b[1393] && (p[24] != 0.0)) && s.b[1475]) && (!s.b[1477])) {s.store_scalar(1438, 3.0);s.store_sub_div_lhs_indices(1441, 1438, 225, 1415);s.store_exp_neg_input(1402, 1438);s.store_offset_div_scaled_inputs2_mixed_aia(1401, A::offset(A::mul(s.ad_value(225), A::add(s.ad_value(1426), s.ad_value(1415))), (-1.0)), 4.0, 1402, 4.0, A::mul(s.ad_value(1425), s.ad_value(226)), 1.0, 1.0);}
        ...
        if (((s.b[1393] && (p[24] != 0.0)) && s.b[1475]) && (!s.b[1477])) {s.store_offset_add(1450, 1426, 1415, 0.1);}
    }
    #[inline(never)]
    pub(super) fn stamp_reactive_block_56(
        ...
        if ((((s.b[1393] && (p[24] != 0.0)) && s.b[1475]) && (!s.b[1477])) && s.b[1483]) {s.store_exp_ad(1457, A::mul_scaled_rhs(s.ad_value(225), s.ad_value(1415), -1.0));s.store_div(1394, 230, 521);s.store_square(1451, 1394);s.store_mul(1466, 1451, 1457);s.store_scalar(1411, 0.0);s.store_scalar(167, 1.0);}
    }
    #[inline(never)]
    pub(super) fn stamp_reactive_block_57(
        ...
        let mut tc: usize = 0;
        while {
            let t9: f64 = (2.0 * 20.0);let ta: f64 = (t9 + 1.0);let tb: f64 = if (((((s.b[1393] && (p[24] != 0.0)) && s.b[1475]) && (!s.b[1477])) && s.b[1483]) && (s.v[167] <= ta)) { 1.0 } else { 0.0 };
            tb != 0.0
        } {
            tc += 1;
            ...
            if ((((((s.b[1393] && (p[24] != 0.0)) && s.b[1475]) && (!s.b[1477])) && s.b[1483]) && (!s.b[1486])) && s.b[1488]) {s.store_scalar(1411, 1.0);}
        ...
        if ((((s.b[1393] && (p[24] != 0.0)) && s.b[1475]) && (!s.b[1477])) && s.b[1483]) {s.store_mul(458, 1423, 1472);}
    }
    #[inline(never)]
    pub(super) fn stamp_reactive_block_58(
        ...
        if ((((s.b[1393] && (p[24] != 0.0)) && s.b[1475]) && (!s.b[1477])) && s.b[1483]) {s.store_div_from_scalar_add_ad(1395, 1.0, s.ad_value(1464), s.ad_value(1472));s.store_mul3_lhs(460, 1423, 1460, 1395);s.store_add(459, 458, 460);}
        ...
        if ((((s.b[1393] && (p[24] != 0.0)) && s.b[1475]) && s.b[1495]) && (!s.b[1496])) {s.store_sqrt_add(1434, 1430, 1433);}
    }
    #[inline(never)]
    pub(super) fn stamp_reactive_block_59(
        ...
        if ((((s.b[1393] && (p[24] != 0.0)) && s.b[1475]) && s.b[1495]) && (!s.b[1496])) {s.store_add_scaled_offset_product_rhs_mixed_aii(1435, A::offset(s.ad_value(1434), ((-7.0) * 1.414213562373095)), 1.0, 1402, 1401, (-2.0), 9.0);}
    ...
    pub(super) fn stamp_reactive_block_60(
        ...
        if ((((s.b[1393] && (p[24] != 0.0)) && s.b[1475]) && (!s.b[1495])) && s.b[1499]) {s.store_neg_powf_add_input(1449, 1446, 1398, 0.3333333333333333);s.store_add_scaled_inputs3_div_scaled_third_indices(1401, 1448, 1.0, 1449, 1.0, 1443, 1.0, 1442, 3.0, -1.0);s.store_add_scaled_product_indices(1441, 1415, (-1.0), 1401, 227, 1.0);s.store_mul_add_rhs(1438, 225, 1441, 1415);}
        ...
        if ((((s.b[1393] && (p[24] != 0.0)) && s.b[1475]) && (!s.b[1495])) && s.b[1501]) {s.store_exp_ad(1457, A::mul_scaled_rhs(s.ad_value(225), s.ad_value(1415), -1.0));s.store_div(1394, 230, 521);s.store_square(1451, 1394);s.store_mul(1466, 1451, 1457);s.store_scalar(1411, 0.0);s.store_scalar(167, 1.0);}
    }
}
