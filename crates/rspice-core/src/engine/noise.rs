    ...
    fn vbic_tnf_noise_temperature_counts_the_instance_offset_once() {
        for (pnjmaxi, expected) in [
            ...
            (
                1e-9,
                [
                    (1.6655824176935968e-17, 8.00692488173953e-20),
                    (1.6655824049803437e-17, 8.006912168486638e-20),
                ],
            ),
        ] {
            for level in [11, 12] {
                ...
                "VBIC local temperature noise\nVcc vcc 0 3\nRc vcc c 1k\nVb b 0 DC 0.7 AC 1\nVth th 0 20\nQ1 c b 0{substrate} th vm SW_ET=0 M=3 TRISE=20\n\
                 .model vm NPN(LEVEL={level} IS=1e-16 NF=1.1 NR=1.2 ISRR=0.7 TNF=0.001 PNJMAXI={pnjmaxi} XISR=1.8 DEAR=0.1 IBEI=1e-18 IBCI=1e-18 IBEIP=0 ISP=0 RCX=10 RCI=2 RBX=5 RBI=3 RE=1 RBP=0 RS=0 GMIN=0 TNOM=27 KFN=1e-8 AFN=1.5 BFN=0.8)\n.temp 27\n.options gmin=0\n.end\n"
            )).unwrap();
                let engine = xyce_engine().resolved_for_netlist(&netlist);
                let circuit = engine.build_circuit(&netlist).unwrap();
                ...
                let results = engine
                    ...
                    .unwrap();
                for (point, (total, device)) in results.iter().zip(expected) {
                    let device_total: f64 = point
                        .contributions
                        .iter()
                        .filter(|source| source.identity.device.eq_ignore_ascii_case("Q1"))
