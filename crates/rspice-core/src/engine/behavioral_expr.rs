...
mod tests {
    ...
    #[test]
    fn test_prepare_behavioral_expression_expands_simple_function() {
        let mut params = ParamContext::new();
        params.define_function("bar1", vec!["p".to_string()], "p");
        let prepared = prepare_behavioral_expression("bar1(17.0)", &params).unwrap();
        let ast = parse_expression_strict(&prepared).unwrap();
        let program = compile(&ast);
        let mut vm = Vm::new();
        let out = vm.execute(&program, &Context::dc(&[], &[]));
        assert!((out - 17.0).abs() < 1e-12);
    }

    #[test]
    fn test_prepare_behavioral_expression_expands_function_with_voltage_probe() {
        let mut params = ParamContext::new();
        params.define_function("bar2", vec!["p".to_string()], "v(p)+p");
        let prepared = prepare_behavioral_expression("bar2(17.0)", &params).unwrap();
        assert!(
            prepared.to_ascii_uppercase().contains("V(P)"),
            "expected V(p) probe argument to remain symbolic, got {}",
            prepared
        );
        assert!(
            prepared.contains("17"),
            "expected numeric substitution outside probe references, got {}",
            prepared
        );
    }

    #[test]
    fn test_prepare_behavioral_expression_preserves_i_probe_symbolic_branch_arg() {
        let mut params = ParamContext::new();
        params.define_function(
            "baz1",
            vec!["n".to_string(), "vp".to_string()],
            "n+i(vp)+vp",
        );
        let prepared = prepare_behavioral_expression("baz1(17,10000)", &params).unwrap();
        let upper = prepared.to_ascii_uppercase();
        assert!(
            upper.contains("I(VP)"),
            "expected I(vp) probe argument to remain symbolic, got {}",
            prepared
        );
        assert!(
            upper.contains("10000"),
            "expected scalar argument substitution outside probes, got {}",
            prepared
        );
    }

    #[test]
    fn test_prepare_behavioral_expression_detects_recursive_functions() {
        let mut params = ParamContext::new();
        params.define_function("foo", vec!["x".to_string()], "foo(x)");
        let err = prepare_behavioral_expression("foo(1)", &params).unwrap_err();
        assert!(
