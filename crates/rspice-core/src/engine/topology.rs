...
use crate::execution::{TopologyComponent, TopologyFingerprint};
use crate::netlist::{
...
impl Engine {
    /// Fingerprint the solver topology produced by a parsed netlist.
    pub fn topology_fingerprint(
        &self,
        netlist: &Netlist,
    ) -> Result<TopologyFingerprint, SimulationError> {
        self.topology_fingerprint_with_abort(netlist, &NoAbort)
    }
    ...
    pub fn topology_fingerprint_with_abort(
        ...
        unknown_layout.extend(
            (0..circuit.hidden_state_count()).map(|index| format!("state-unknown-{index}")),
        );
...
mod tests {
    ...
    fn numeric_values_do_not_change_the_assembled_topology_fingerprint() {
        ...
        assert_eq!(
            engine.topology_fingerprint(&first).expect("first topology"),
            engine
    ...
    fn component_membership_and_connectivity_change_the_fingerprint() {
        ...
        let engine = Engine::default();
        let base = engine.topology_fingerprint(&base).expect("base topology");

        assert_ne!(
            base,
            engine
                .topology_fingerprint(&parallel)
                .expect("parallel topology")
        );
        assert_ne!(
            base,
            engine
                .topology_fingerprint(&extra_node)
                .expect("extra-node topology")
        );
    }
    ...
    fn terminal_polarity_order_is_part_of_the_fingerprint() {
        ...
        assert_ne!(
            engine
                .topology_fingerprint(&forward)
                .expect("forward topology"),
            engine
                .topology_fingerprint(&reverse)
                .expect("reverse topology"),
