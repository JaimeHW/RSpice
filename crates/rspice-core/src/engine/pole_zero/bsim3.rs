//! Explicit AC-only BSIM3 relaxation states for pole-zero extraction.
...
impl Engine {
    pub(super) fn bsim3_pz_response_state_count(circuit: &CircuitData) -> usize {
        circuit
            .bsim3v3
            .devices
            .iter()
    ...
    pub(super) fn stamp_bsim3_pz_descriptor_states(
        circuit: &CircuitData,
        op_voltages: &[Value],
        g: &mut Matrix,
        c: &mut Matrix,
    ) -> Result<(), SimulationError> {
        let count = Self::bsim3_pz_response_state_count(circuit);
        if count == 0 {
            return Ok(());
        }
        let start = Self::descriptor_expand_square(g, c, count);
        for (index, device) in circuit
            .bsim3v3
            .devices
            .iter()
            .filter(|device| device.uses_ac_nqs())
