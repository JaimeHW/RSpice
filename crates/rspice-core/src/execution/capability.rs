...
const ADAPTER_SECOND_DOCUMENT: &str = "the protocol-4 executor publishes one shared result document per authored card, and this \
     family is a second result produced beside another card's; core names and projects it, but \
     the adapter has no slot for it and refuses the deck rather than dropping it";
