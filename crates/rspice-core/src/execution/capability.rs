...
//
// A second reason applies to the surfaces that still lack it and is stated in
// each: the typed document, the rawfile and a VCD dump all carry a *bus
// declaration* beside the member traces, so a vector port's elements can be
// reassembled into the word they came from. A mixed Verilog-AMS module's
// vector discrete port now declares one, so this is a table a run really
// produces rather than one only a frontend could fill, and what separates the
// surfaces is whether each of their carriers forwards it.
const CLI_DIGITAL_FLATTENED_PLUS_EVENTS: &str = "every table format writes the flattened D(node) column (0, 1 or 0.5 per analysis time point, drive strength dropped); the exact twelve-state histories ride beside it only in the rawfile event plots --format raw/ascii append and in the typed document --format json publishes, and --format vcd keeps the timeline and four bit states but not the strength; a declared bus reaches the rawfile and the typed document, but --format vcd passes an empty table and writes the members as scalars, so one run's three exports do not yet agree on what is a word";
