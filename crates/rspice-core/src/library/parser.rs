...
pub fn parse_library_content(
    ...
    while i < lines.len() {
        ...
        if upper.starts_with(".SUBCKT") {
            ...
            if subckt_depth == 0 {
                if let Some(subckt) = parse_subckt_line(line, library_name, &last_comment) {
...
mod tests {
    ...
    fn test_parse_model_line() {
        ...
        assert_eq!(model.name, "1N4148");
        assert_eq!(model.model_type, ModelType::Diode);
        assert_eq!(model.description, Some("High-speed diode".to_string()));
    }

    #[test]
    fn test_parse_subckt_line() {
        let subckt = parse_subckt_line(
            ".SUBCKT LM741 INP INM OUT VCC VEE",
            "test.lib",
            "Classic opamp",
        );
        assert!(subckt.is_some());

        let subckt = subckt.unwrap();
        assert_eq!(subckt.name, "LM741");
        assert_eq!(subckt.pins.len(), 5);
        assert_eq!(subckt.pins[0], "INP");
    }

    #[test]
    fn test_parse_library_content() {
        let content = r#"
* Test Library
...
.ENDS LM741
"#;
        // Note: we're using a static str here for testing
        let content: &'static str = Box::leak(content.to_string().into_boxed_str());

        let (models, subcircuits) = parse_library_content(content, "test.lib");

        assert_eq!(models.len(), 1);
        assert_eq!(subcircuits.len(), 1);
        assert_eq!(models[0].name, "1N4148");
        assert_eq!(subcircuits[0].name, "LM741");
    }
}
