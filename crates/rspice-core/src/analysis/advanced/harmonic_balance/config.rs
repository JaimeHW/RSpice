...
mod config_tests {
    ...
    #[test]
    fn test_single_tone_config() {
        let config = HbConfig::new(1e9);
        assert_eq!(config.fundamental_freq, 1e9);
        assert_eq!(config.num_harmonics, 9);
        assert!(!config.is_multi_tone());
    }

    #[test]
    fn test_multi_tone_config() {
        let config = HbConfig::multi_tone(vec![HbTone::new(900e6, 5), HbTone::new(800e6, 3)]);
        assert!(config.is_multi_tone());
        assert_eq!(config.tones.len(), 2);
    }

    #[test]
    fn test_spectral_components_single_tone() {
        let config = HbConfig::new(1e9).with_harmonics(5);
        // DC + 5 harmonics = 6 components
        assert_eq!(config.num_spectral_components(), 6);
    }

    #[test]
    fn test_fft_size_power_of_two() {
        let config = HbConfig::new(1e9).with_harmonics(9).with_oversample(2);
        let fft_size = config.fft_size();
        assert!(fft_size.is_power_of_two());
        assert!(fft_size >= 20); // (9+1) * 2 = 20
    }

    #[test]
    fn test_harmonic_frequencies() {
        let config = HbConfig::new(100e6).with_harmonics(3);
        let freqs = config.harmonic_frequencies();
        assert_eq!(freqs.len(), 4);
        assert_eq!(freqs[0], 0.0); // DC
        assert_eq!(freqs[1], 100e6); // 1st
        assert_eq!(freqs[2], 200e6); // 2nd
        assert_eq!(freqs[3], 300e6); // 3rd
    }

    #[test]
    fn test_builder_chain() {
        let config = HbConfig::new(2e9)
            .with_harmonics(15)
            .with_tolerance(1e-8)
            .with_damping(0.7)
            .with_oversample(4)
    ...
    fn test_damping_clamp() {
        let config = HbConfig::new(1e9).with_damping(5.0);
        assert_eq!(config.damping, 1.0);

        let config2 = HbConfig::new(1e9).with_damping(0.01);
        assert_eq!(config2.damping, 0.1);
    }

    #[test]
    fn test_period() {
        let config = HbConfig::new(1e9);
        assert!((config.period() - 1e-9).abs() < 1e-20);
    }

    #[test]
    fn test_tone_with_name() {
        let tone = HbTone::new(2.4e9, 7).with_name("WiFi");
