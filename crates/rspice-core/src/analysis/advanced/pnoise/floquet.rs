...
mod floquet_tests {
    ...
    fn test_matrix_exponential_diagonal() {
        ...
        assert!((result[1][1].re - 2.0_f64.exp()).abs() < 0.1); // e^2 ≈ 7.389
    ...
    fn test_phase_noise_dbc_calculation() {
        ...
        // 1e-10 rad²/Hz should give -103 dBc/Hz (SSB)
        // L = S_φ/2 = 5e-11, 10*log10(5e-11) ≈ -103
    ...
    fn test_phase_noise_dbc_typical_values() {
        ...
        // -80 dBc/Hz corresponds to S_φ = 2e-8 rad²/Hz
    ...
    fn test_noise_to_phase_transfer_frequency_dependence() {
        ...
        // With 1/f² transfer, phase PSD ratio should be 100 per decade
    ...
    fn test_isf_samples_cosinusoidal() {
        ...
        // ISF should be cos(2πt/T)
        // At t=0 (sample 0): cos(0) = 1
        // At t=T/4 (sample 16 for 64 samples): cos(π/2) = 0
        // At t=T/2 (sample 32): cos(π) = -1
    ...
    fn test_flicker_noise_slope() {
        // With 1/f noise (PSD ∝ 1/f) added to 1/f² transfer function,
        // total phase noise ∝ 1/f³ near carrier (-30 dB/decade)
        let mut analyzer = FloquetAnalyzer::new(1e-9, 10);
        analyzer.compute_approximate_isf();
        ...
        // With 1/f noise × 1/f² TF = 1/f³
        // Ratio should be 10³ = 1000 per decade
        // In dB: 30 dB/decade
    ...
    fn test_white_noise_slope() {
        // With white noise (flat PSD) through 1/f² transfer function,
    ...
    fn test_set_multipliers() {
        let mut analyzer = FloquetAnalyzer::new(1e-9, 10);
        let multipliers = vec![
            Complex64::new(1.0, 0.0), // Phase mode (|μ| = 1)
            Complex64::new(0.5, 0.0), // Stable mode (|μ| < 1)
        ];
