...
mod tests {
    ...
    #[test]
    fn test_rate_class_multipliers() {
        assert!(RateClass::Fast.timestep_multiplier() < RateClass::Normal.timestep_multiplier());
        assert!(RateClass::Normal.timestep_multiplier() < RateClass::Slow.timestep_multiplier());
        assert!(RateClass::Slow.timestep_multiplier() < RateClass::VerySlow.timestep_multiplier());
    }

    #[test]
    fn test_rate_class_default() {
        assert_eq!(RateClass::default(), RateClass::Normal);
    }

    #[test]
    fn test_partition_creation() {
        let partition = CircuitPartition::new(0, RateClass::Fast);
        assert_eq!(partition.id, 0);
        assert_eq!(partition.rate_class, RateClass::Fast);
        assert!(partition.nodes.is_empty());
    }

    #[test]
    fn test_partition_add_nodes() {
        let mut partition = CircuitPartition::new(0, RateClass::Normal);
        partition.add_node(1);
        partition.add_node(2);
        partition.add_node(1); // Duplicate, should not add

        assert_eq!(partition.nodes.len(), 2);
        assert!(partition.nodes.contains(&1));
        assert!(partition.nodes.contains(&2));
    }

    #[test]
    fn test_partition_interface_nodes() {
        let mut partition = CircuitPartition::new(0, RateClass::Normal);
        partition.add_interface_node(5);
        partition.add_interface_node(5); // Duplicate

        assert_eq!(partition.interface_nodes.len(), 1);
    }

    #[test]
    fn test_config_default() {
        let config = MultiRateConfig::default();
        assert!(config.base_timestep > 0.0);
        assert!(config.max_timestep > config.base_timestep);
        assert!(config.min_timestep < config.base_timestep);
    }

    #[test]
    fn test_config_mixed_signal() {
        let config = MultiRateConfig::for_mixed_signal(1e9, 10e6);
        assert!(config.min_timestep < 1e-9); // Digital needs fine resolution
    }

    #[test]
    fn test_interface_history_record() {
        let mut history = InterfaceHistory::new(0, 10);
        ...
        assert_eq!(history.latest_time(), Some(2.0));
    }

    #[test]
    fn test_interface_history_interpolation() {
        let mut history = InterfaceHistory::new(0, 10);
        history.record(0.0, 0.0);
        history.record(1.0, 1.0);
        history.record(2.0, 2.0);

        assert!((history.interpolate(0.5).unwrap() - 0.5).abs() < 1e-10);
        assert!((history.interpolate(1.5).unwrap() - 1.5).abs() < 1e-10);
    }

    #[test]
    fn test_interface_history_clamp() {
        let mut history = InterfaceHistory::new(0, 10);
        history.record(1.0, 5.0);
        history.record(2.0, 10.0);

        // Before first point - returns first value
        assert_eq!(history.interpolate(0.0), Some(5.0));
        // After last point - returns last value
        assert_eq!(history.interpolate(3.0), Some(10.0));
    }

    #[test]
    fn test_interface_history_max_length() {
        let mut history = InterfaceHistory::new(0, 3);
        history.record(0.0, 0.0);
        history.record(1.0, 1.0);
        history.record(2.0, 2.0);
        history.record(3.0, 3.0); // Should evict first

        assert_eq!(history.time.len(), 3);
        assert_eq!(history.time[0], 1.0); // First point is now 1.0
    }

    #[test]
    fn test_partition_state_creation() {
        let state = PartitionState::new(0, 5);
        assert_eq!(state.partition_id, 0);
        assert_eq!(state.solution.len(), 5);
        assert_eq!(state.local_time, 0.0);
    }

    #[test]
    fn test_partition_state_step() {
        let mut state = PartitionState::new(0, 2);
        state.accept_step(vec![1.0, 2.0], 1e-9);

        assert_eq!(state.steps, 1);
        assert_eq!(state.subcycles, 1);
        assert!((state.local_time - 1e-9).abs() < 1e-15);
        assert_eq!(state.solution, vec![1.0, 2.0]);
    }

    #[test]
    fn test_multirate_state_creation() {
        let partitions = vec![
            CircuitPartition::new(0, RateClass::Fast),
            CircuitPartition::new(1, RateClass::Slow),
        ];
        let state = MultiRateState::new(&partitions);

        assert_eq!(state.partition_states.len(), 2);
        assert_eq!(state.time, 0.0);
    }

    #[test]
    fn test_multirate_result() {
        let config = MultiRateConfig::default();
        let mut result = MultiRateResult::new(config);

        result.record_sync_point(0.0, &[(0, 1.0), (1, 2.0)]);
        result.record_sync_point(1e-9, &[(0, 1.1), (1, 2.1)]);

        assert_eq!(result.time.len(), 2);
        assert!(result.get_voltage(0).is_some());
        assert_eq!(result.get_voltage(0).unwrap().len(), 2);
    }

    #[test]
    fn test_solver_creation() {
        let config = MultiRateConfig::default();
        let partitions = vec![CircuitPartition::new(0, RateClass::Normal)];
        let solver = MultiRateSolver::new(config, partitions);
    ...
    #[test]
    fn test_solver_single_rate() {
        let config = MultiRateConfig::default();
        let solver = MultiRateSolver::single_rate(config, 3);

        assert_eq!(solver.partitions().len(), 1);
        assert_eq!(solver.partitions()[0].nodes.len(), 3);
    }

    #[test]
    fn test_node_activity_from_bandwidth() {
        let activity = NodeActivity::with_bandwidth(0, 2e9);
        assert_eq!(activity.rate_class, RateClass::Fast);

        let activity = NodeActivity::with_bandwidth(1, 50e6);
        assert_eq!(activity.rate_class, RateClass::Slow);
    }

    #[test]
    fn test_node_activity_from_waveform() {
        // Use 300 MHz - clearly in Normal range (100 MHz - 1 GHz)
        // Avoid exact boundary values due to numerical precision in slew rate estimation
        let time: Vec<Value> = (0..1000).map(|i| i as Value * 0.1e-9).collect();
        let freq = 300e6;
        ...
        let activity = NodeActivity::from_waveform(0, &time, &values);

        // Should classify as Normal (100 MHz - 1 GHz range)
        assert_eq!(activity.rate_class, RateClass::Normal);
        // Bandwidth estimate should be reasonably close to actual frequency
        assert!(
            activity.bandwidth > 100e6,
            "Bandwidth {} should be > 100 MHz",
            activity.bandwidth
        );
    }

    #[test]
    fn test_auto_partitioner() {
        let activities = vec![
            NodeActivity::with_bandwidth(0, 2e9),   // Fast
            NodeActivity::with_bandwidth(1, 1e9),   // Fast
            NodeActivity::with_bandwidth(2, 100e6), // Slow
            NodeActivity::with_bandwidth(3, 1e3),   // VerySlow
        ];

        let partitions = AutoPartitioner::partition(&activities);

        // Should have 3 partitions (Fast, Slow, VerySlow - no Normal)
        assert_eq!(partitions.len(), 3);

        // Fast partition should be first
        assert_eq!(partitions[0].rate_class, RateClass::Fast);
        assert_eq!(partitions[0].nodes.len(), 2);
    }

    #[test]
    fn test_subcycle_partition() {
        let mut config = MultiRateConfig::default();
        config.sync_interval = 10e-9;
        config.t_stop = 100e-9;

        let mut partition = CircuitPartition::new(0, RateClass::Normal);
        partition.add_node(0);
        partition.timestep = 1e-9;

        let mut solver = MultiRateSolver::new(config, vec![partition]);

        // Simple decay: dv/dt = -v
        let subcycles = solver.subcycle_partition(0, |v, _t| vec![-v[0] * 1e8]);

        // Should have taken ~10 subcycles for 10ns interval
        assert!(subcycles > 0);
    }
    ...
    fn test_full_simulation() {
        ...
        solver.state.partition_states[0].solution = vec![1.0];

        // Simple decay
        let decay_fn = |v: &[Value], _t: Value| -> Vec<Value> { vec![-v[0] * 1e7] };
        let result = solver.run(&[decay_fn]);

        assert!(!result.time.is_empty());
        assert!(result.get_voltage(0).is_some());

        // Final value should be less than initial (decayed)
        let voltages = result.get_voltage(0).unwrap();
        assert!(voltages.last().unwrap().abs() < voltages[0].abs());
    }

    #[test]
    fn test_interface_coupling() {
        let mut config = MultiRateConfig::default();
        config.sync_interval = 10e-9;
        config.t_stop = 50e-9;

        // Create two partitions with shared interface node
        let mut fast_partition = CircuitPartition::new(0, RateClass::Fast);
        fast_partition.add_node(0);
        fast_partition.add_interface_node(1);
        fast_partition.timestep = 0.1e-9;

        let mut slow_partition = CircuitPartition::new(1, RateClass::Slow);
        slow_partition.add_node(1);
        slow_partition.add_interface_node(1);
        slow_partition.timestep = 10e-9;

        let solver = MultiRateSolver::new(config, vec![fast_partition, slow_partition]);

        // Interface histories should be set up for node 1
        assert!(solver.state.interface_histories.contains_key(&1));
    }
}
