...
mod config_tests {
    ...
    fn test_damping_clamp() {
        let config = PssConfig::new(1e9).with_damping(5.0);
        assert_eq!(config.damping_factor, 1.0);

        let config2 = PssConfig::new(1e9).with_damping(0.01);
        assert_eq!(config2.damping_factor, 0.1);
    }
}
