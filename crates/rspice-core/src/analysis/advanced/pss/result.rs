...
mod tests {
    ...
    #[test]
    fn test_pss_result_creation() {
        let result = PssResult::new(1e-9, 5, 256);
        assert_eq!(result.period, 1e-9);
        assert!((result.frequency - 1e9).abs() < 1.0);
        assert_eq!(result.num_nodes(), 5);
    }

    #[test]
    fn test_periodic_waveform_dc() {
        let time: Vec<Value> = (0..100).map(|i| i as f64 * 1e-9 / 99.0).collect();
        let values: Vec<Value> = time.iter().map(|_| 2.5).collect(); // Constant DC
        let wf = PeriodicWaveform::from_values(values);

        let dc = wf.dc(&time, 1e-9);
        assert!((dc - 2.5).abs() < 0.01);
    }

    #[test]
    fn test_periodic_waveform_sine() {
        // 1 GHz sine wave, 1V amplitude
        let freq = 1e9;
        let period = 1e-9;
        let n_points = 256;
        ...
        // DC should be ~0
        let dc = wf.dc(&time, period);
        assert!(dc.abs() < 0.01);

        // Peak-to-peak should be ~2
        let pp = wf.peak_to_peak();
        assert!((pp - 2.0).abs() < 0.02);

        // Harmonics
        let harmonics = wf.compute_harmonics(&time, freq, 5);
        assert_eq!(harmonics.len(), 6); // DC + 5 harmonics

        // DC ~ 0
        assert!(harmonics[0].magnitude.abs() < 0.01);

        // Fundamental ~ 1
        assert!((harmonics[1].magnitude - 1.0).abs() < 0.02);

        // Higher harmonics should be small (pure sine)
        for h in harmonics.iter().skip(2) {
            assert!(h.magnitude < 0.05);
        }
    }

    #[test]
    fn test_waveform_interpolation() {
        let time: Vec<Value> = vec![0.0, 0.25, 0.5, 0.75, 1.0];
        let values: Vec<Value> = vec![0.0, 1.0, 0.0, -1.0, 0.0];
        let wf = PeriodicWaveform::from_values(values);

        // At exact points
        assert!((wf.interpolate(&time, 0.25, 1.0) - 1.0).abs() < 1e-10);
        assert!((wf.interpolate(&time, 0.75, 1.0) - (-1.0)).abs() < 1e-10);

        // Interpolated
        let v_mid = wf.interpolate(&time, 0.125, 1.0);
        assert!((v_mid - 0.5).abs() < 1e-10);

        // Wraparound
        let v_wrap = wf.interpolate(&time, 1.25, 1.0);
        assert!((v_wrap - 1.0).abs() < 0.1);
    }

    #[test]
    fn test_square_wave_harmonics() {
        // Square wave: should have odd harmonics at 4/nπ amplitude
        let period = 1e-9;
        let freq = 1e9;
        let n_points = 1024;
        ...
        let wf = PeriodicWaveform::from_values(values);
        let harmonics = wf.compute_harmonics(&time, freq, 9);

        // DC should be ~0 (50% duty cycle)
        assert!(harmonics[0].magnitude.abs() < 0.05);

        // Fundamental: 4/π ≈ 1.273
        assert!((harmonics[1].magnitude - 4.0 / PI).abs() < 0.1);

        // 2nd harmonic (even): should be ~0
        assert!(harmonics[2].magnitude < 0.1);

        // 3rd harmonic: 4/(3π) ≈ 0.424
        assert!((harmonics[3].magnitude - 4.0 / (3.0 * PI)).abs() < 0.1);
    }

    #[test]
    fn test_floquet_stability() {
        let mut result = PssResult::new(1e-9, 2, 100);

        // Empty multipliers = assume stable
        assert!(result.is_stable());

        // All inside unit circle = stable
        result.floquet_multipliers = vec![
            num_complex::Complex64::new(0.5, 0.3),
            num_complex::Complex64::new(-0.2, 0.8),
        ];
        assert!(result.is_stable());

        // One outside = unstable
        result
            .floquet_multipliers
            .push(num_complex::Complex64::new(1.5, 0.0));
        assert!(!result.is_stable());
    }
}
