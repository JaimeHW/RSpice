...
impl Workspace<'_> {
    ...
    /// Apply d(F + dQ/dt)/dx to one real Fourier coordinate. Sampling the
    /// product before projection preserves couplings whose index difference
    /// is outside the retained state lattice, including sparse mixed grids.
    pub(super) fn jacobian_column(
        ...
        check_abort(abort)?;
        let entries = self.grid.len();
        let coordinate = column / entries;
        let basis = coordinates::basis(entries, column % entries);
        let perturbation = self.transform.to_real_samples_with_abort(&basis, abort)?;
        let count = self.grid.sample_count();
