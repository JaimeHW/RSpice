...
mod tests {
    ...
    #[test]
    fn test_standard_disciplines() {
        let db = DisciplineDb::with_standard();

        // Check electrical discipline exists
        let electrical = db.get_discipline("electrical");
        assert!(electrical.is_some());
        let electrical = electrical.unwrap();
        assert_eq!(electrical.potential, Some("Voltage".to_string()));
        assert_eq!(electrical.flow, Some("Current".to_string()));
    }

    #[test]
    fn test_nature_access() {
        let db = DisciplineDb::with_standard();

        // V() should resolve to Voltage nature
        let voltage = db.resolve_access("V");
        assert!(voltage.is_some());
        assert_eq!(voltage.unwrap().name, "Voltage");

        // I() should resolve to Current nature
        let current = db.resolve_access("I");
        assert!(current.is_some());
        assert_eq!(current.unwrap().name, "Current");
    }

    #[test]
    fn test_discipline_compatibility() {
        let db = DisciplineDb::with_standard();

        // electrical is compatible with itself
        assert!(db.are_compatible("electrical", "electrical"));

        // voltage is compatible with electrical (shares Voltage potential)
        assert!(db.are_compatible("voltage", "electrical"));

        // electrical is not compatible with thermal
        assert!(!db.are_compatible("electrical", "thermal"));
    }
}
