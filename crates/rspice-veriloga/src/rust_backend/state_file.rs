...
pub(super) fn generate_state_file_with_extensions(
    ...
    let parameter_fields = &ParameterBindings {
        ...
        simparams: parameter_simparam_queries(artifact),
    };
    ...
    if !parameter_fields.simparams.is_empty() {
        out.push_str(&format!(
            "    __simparam_values: [f64; {}],\n    __simparam_available: u64,\n",
            parameter_fields.simparams.len()
        ));
    }
    out.push_str("}\n\n");
    ...
    out.push_str("    fn new_box_with_simulation_parameters(simulation_parameters: &GeneratedSimulationParameters) -> Box<Self> {\n");
    if artifact.mir.parameters.is_empty() {
        out.push_str("        Box::new(Self { values: [] })\n");
    } else {
        out.push_str("        // SAFETY: all fields are f64 or u64, so zero bytes are valid; defaults and captured simulator queries initialize the complete allocation.\n");
        ...
        for (index, parameter) in parameter_fields.simparams.iter().enumerate() {
            out.push_str(&format!("            if let Some(value) = simulation_parameters.get({:?}) {{\n                (*ptr).__simparam_values[{index}] = value;\n                (*ptr).__simparam_available |= {};\n            }}\n", parameter.name(), 1u64 << index));
        }
    ...
    out.push_str("        let mut params = Self::new_box();\n");
    out.push_str("        let given = boxed_zero_bool_array::<{ Instance::PARAMETER_COUNT }>();\n");
    out.push_str("        Instance::finalize_parameter_vector(params.as_mut(), given.as_ref(), true).expect(\"invalid generated model parameter defaults\");\n");
    out.push_str("        Instance::finalize_parameter_vector(params.as_mut(), given.as_ref(), false).expect(\"invalid generated instance parameter defaults\");\n");
    out.push_str("        Instance::validate_parameter_vector(params.as_ref()).expect(\"invalid generated parameter defaults\");\n");
    out.push_str("        *params\n");
    out.push_str("    }\n");
    out.push_str("}\n\n");
    ...
    out.push_str("        Self::try_new_with_parameters(nodes, &[]).expect(\"invalid generated Verilog-A instance defaults\")\n");
    out.push_str("    }\n\n");
    out.push_str(
        "    /// Resolve overrides and dependent defaults before validating the instance.\n",
    );
    out.push_str("    pub fn try_new_with_parameters(nodes: &[usize], assignments: &[GeneratedParameterAssignment<'_>]) -> Result<Self, String> {\n        Self::try_new_with_simulation_parameters(nodes, assignments, &GeneratedSimulationParameters::default())\n    }\n\n");
    out.push_str(
        "    /// Capture simulator queries before resolving parameter defaults and ranges.\n",
    );
    out.push_str("    pub fn try_new_with_simulation_parameters(nodes: &[usize], assignments: &[GeneratedParameterAssignment<'_>], simulation_parameters: &GeneratedSimulationParameters) -> Result<Self, String> {\n");
    out.push_str("        if nodes.len() != Self::NODE_COUNT { return Err(format!(\"generated Verilog-A node count mismatch: expected {}, got {}\", Self::NODE_COUNT, nodes.len())); }\n");
    out.push_str("        let mut mapped = [0usize; Self::NODE_COUNT];\n");
    out.push_str("        mapped.copy_from_slice(nodes);\n");
    out.push_str("        let params = Parameters::new_box_with_simulation_parameters(simulation_parameters);\n");
    ...
    if artifact.mir.parameters.is_empty() {
        ...
        out.push_str("                    params.values[index] = value;\n");
        out.push_str("                    model_params.values[index] = value;\n");
...
pub(super) fn finalize_checkpoint_identity(
    ...
    // Historical migrations are qualified only for their recorded compiler
    // semantics. Source and state shape alone cannot authorize a new revision.
    let compatibility = compatibility.filter(|entry| {
        entry.target_compiler_semantics_version == GENERATED_MODEL_SEMANTICS_VERSION
    });
...
fn generated_model_semantic_identity(device: &GeneratedRustDevice) -> String {
    ...
    hasher.update(
        format!("rspice-generated-model-semantics-v{GENERATED_MODEL_SEMANTICS_VERSION}\0")
            .as_bytes(),
    );
...
/// Only queries used by defaults or ranges occupy per-instance storage.
struct ParameterBindings<'a> {
    fields: &'a HashMap<String, String>,
    simparams: Vec<rspice_veriloga_runtime::SimulationParameter>,
}

fn parameter_simparam_queries(
    artifact: &CanonicalIrArtifact,
) -> Vec<rspice_veriloga_runtime::SimulationParameter> {
    let mut pending = Vec::new();
    for parameter in &artifact.mir.parameters {
        if let Some(default) = &parameter.default_expr {
            pending.push(default.id);
        }
        if let Some(range) = &parameter.range {
            pending.extend(
                range
                    .min_expression
                    .iter()
                    .chain(range.max_expression.iter())
    ...
    let mut visited = vec![false; artifact.mir.expressions.len()];
    ...
    while let Some(id) = pending.pop() {
        ...
        match &artifact.mir.expressions[index].kind {
            HirExprKind::SystemFunction { name, args } | HirExprKind::Call { name, args } => {
                if name
                    .trim_start_matches('$')
                    .eq_ignore_ascii_case("simparam")
                    && let Some(id) = args.first()
                    && let Some(HirExprKind::StringLiteral { value }) = artifact
                        .mir
                        .expressions
                        .get(usize::from(*id))
                        .map(|expression| &expression.kind)
                    && let Some(parameter) =
                        rspice_veriloga_runtime::SimulationParameter::from_name(value)
                    && !queries.contains(&parameter)
                {
    ...
    queries.sort_by_key(|parameter| *parameter as u8);
    queries
}

// Only invariant defaults are installed here. Symbolic defaults run once,
// after overrides, through the bounded finalization helpers below.
fn emit_parameter_defaults(
    ...
    let defaults = artifact
        ...
        .collect::<Result<Vec<_>, _>>()?;
    emit_f64_const_array("DEFAULTS", &defaults, out);
    out.push_str("            std::ptr::copy_nonoverlapping(DEFAULTS.as_ptr(), (*ptr).values.as_mut_ptr(), DEFAULTS.len());\n");
    Ok(())
}
    ...
    match &expression.kind {
                ...
                .eq_ignore_ascii_case("simparam") =>
        {
            if !(1..=2).contains(&args.len()) {
                return Err(unsupported(
                    artifact,
                    "parameter $simparam requires one or two arguments",
                ));
            }
            let Some(HirExprKind::StringLiteral { value: name }) = artifact
                .mir
                .expressions
                ...
                return Err(unsupported(
                    artifact,
                    "parameter $simparam requires a literal query name",
                ));
            };
            let parameter = rspice_veriloga_runtime::SimulationParameter::from_name(name);
            let fallback = match args.get(1) {
                Some(fallback) => lower_parameter_default_expr(
                    artifact,
                    *fallback,
                    parameter_fields,
                    parameter_given,
                )?,
                None if parameter.is_some() => format!(
                    "return Err({:?}.to_owned())",
                    format!("simulation parameter '{name}' is unavailable and has no fallback")
                ),
                None => {
                    return Err(unsupported(
                        artifact,
                        format!("simulation parameter '{name}' has no fallback"),
                    ));
                }
            };
            let Some(parameter) = parameter else {
            ...
            let index = parameter_fields
                .simparams
                .iter()
                .position(|query| *query == parameter)
                .expect("parameter query discovery covers defaults and ranges");
            Ok(format!(
                "if params.__simparam_available & {}u64 != 0 {{ params.__simparam_values[{index}] }} else {{ {fallback} }}",
                1u64 << index
            ))
        }
...
mod tests {
    ...
    fn retained_semantic_identity_requires_the_exact_catalog_target_artifact() {
        ...
        let catalog = rspice_veriloga_runtime::GeneratedVerilogACompatibilityCatalogEntry {
            ...
            target_descriptor_abi_version:
                rspice_veriloga_runtime::GENERATED_VERILOGA_DESCRIPTOR_ABI_VERSION,
            target_compiler_semantics_version: super::GENERATED_MODEL_SEMANTICS_VERSION,
    ...
    #[test]
    fn historical_catalog_does_not_preserve_superseded_compiler_semantics() {
            ...
            .filter(|entry| entry.semantic_identity_override_artifact.is_none())
        {
            ...
            let expected = generated_model_semantic_identity(&device);
            assert!(device.files[0].contents.contains(&expected));
            if catalog.target_compiler_semantics_version != super::GENERATED_MODEL_SEMANTICS_VERSION
            {
                assert_ne!(
                    expected, catalog.semantic_identity,
                    "{} must not retain a historical identity after a semantic change",
