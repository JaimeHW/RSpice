...
mod tests {
    ...
    fn test_type_coercion() {
        assert!(ValueType::Integer.can_coerce_to(&ValueType::Real));
        assert!(ValueType::Boolean.can_coerce_to(&ValueType::Integer));
        assert!(ValueType::Boolean.can_coerce_to(&ValueType::Real));
        assert!(!ValueType::Real.can_coerce_to(&ValueType::Integer));
        assert!(!ValueType::String.can_coerce_to(&ValueType::Real));
    }

    #[test]
    fn test_common_type() {
        assert_eq!(
            ValueType::Integer.common_type(ValueType::Real),
            ValueType::Real
        );
        assert_eq!(
        ...
        assert_eq!(
            ValueType::Integer.common_type(ValueType::Integer),
            ValueType::Integer
        );
        assert_eq!(
    ...
    fn test_numeric_types() {
        ...
        assert!(!ValueType::Void.is_numeric());
    }

    #[test]
    fn test_parameter_range() {
        let range = ParameterRange::open(0.0, 100.0);
        assert!(!range.contains(0.0)); // exclusive
        assert!(range.contains(0.001));
        assert!(range.contains(50.0));
        assert!(range.contains(99.999));
        assert!(!range.contains(100.0)); // exclusive

        let closed = ParameterRange::closed(0.0, 100.0);
        assert!(closed.contains(0.0));
        assert!(closed.contains(100.0));

        let left_open = ParameterRange::left_open(0.0, 100.0);
        assert!(!left_open.contains(0.0));
        assert!(left_open.contains(100.0));
    }

    #[test]
    fn test_parameter_range_exclusions() {
        let mut range = ParameterRange::unrestricted();
        range.exclude = vec![0.0, 1.0, 2.0];
    ...
    fn test_function_registry() {
        ...
        assert!(registry.contains("ddt"));
        assert!(registry.contains("white_noise"));

        let sin = registry.get("sin").unwrap();
        assert_eq!(sin.return_type, ValueType::Real);
        assert_eq!(sin.min_args(), 1);
        assert!(!sin.is_analog_operator);

        let ddt = registry.get("ddt").unwrap();
        assert!(ddt.is_analog_operator);
    }

    #[test]
    fn test_function_signature_args() {
        let unary = FunctionSignature::math_unary("test");
        assert_eq!(unary.min_args(), 1);
        assert_eq!(unary.max_args(), 1);

        let binary = FunctionSignature::math_binary("test2");
        assert_eq!(binary.min_args(), 2);
        assert_eq!(binary.max_args(), 2);

        let idt = FunctionSignature::analog_operator("idt", 4);
        assert_eq!(idt.min_args(), 1); // Only first arg is required
        assert_eq!(idt.max_args(), 4);
    }

    #[test]
    fn test_array_dimension() {
        let dim = ArrayDimension { lower: 0, upper: 9 };
        assert_eq!(dim.size(), 10);

        let dim2 = ArrayDimension {
            lower: -5,
            upper: 5,
        };
    ...
    fn test_type_display() {
        ...
        assert_eq!(format!("{}", ValueType::Error), "<error>");
    }

    #[test]
    fn test_range_display() {
        let range = ParameterRange::open(0.0, 100.0);
        assert_eq!(format!("{}", range), "from (0:100)");
