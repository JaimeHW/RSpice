...
impl X64Encoder {
    ...
    pub(crate) fn and_r64_imm32(&mut self, reg: Gpr, value: i32) {
        self.emit_r64_imm32(0b100, reg, value);
    }

    pub(crate) fn or_r64_imm32(&mut self, reg: Gpr, value: i32) {
        self.emit_r64_imm32(0b001, reg, value);
    }

    pub(crate) fn xor_r64_imm32(&mut self, reg: Gpr, value: i32) {
        self.emit_r64_imm32(0b110, reg, value);
    }
    ...
    pub(crate) fn and_r64_r64(&mut self, dst: Gpr, src: Gpr) {
        self.emit_rex(true, src.code(), 0, dst.code());
        self.emit_u8(0x21);
        self.emit_modrm(0b11, src.code(), dst.code());
    }

    pub(crate) fn or_r64_r64(&mut self, dst: Gpr, src: Gpr) {
        self.emit_rex(true, src.code(), 0, dst.code());
        self.emit_u8(0x09);
        self.emit_modrm(0b11, src.code(), dst.code());
    }

    pub(crate) fn xor_r64_r64(&mut self, dst: Gpr, src: Gpr) {
        self.emit_rex(true, src.code(), 0, dst.code());
        self.emit_u8(0x31);
        self.emit_modrm(0b11, src.code(), dst.code());
    }
    ...
    pub(crate) fn minsd_xmm_m64_rip_disp32(&mut self, dst: Xmm, disp: i32) -> usize {
        self.emit_sse_reg_rip_disp32(0xF2, 0x5D, dst, disp)
    }
...
mod tests {
    ...
    fn encodes_r64_immediates_with_short_and_wide_forms() {
        ...
        encoder.and_r64_imm32(Gpr::R10, 7);
        encoder.or_r64_imm32(Gpr::R11, -1);
        encoder.xor_r64_imm32(Gpr::Rdx, 128);
        ...
        assert_eq!(
            ...
            [
                ...
                0xC7, 0xC2, 0xFE, 0xFF, 0xFF, 0xFF, 0x49, 0xC7, 0xC5, 0x7F, 0xFF, 0xFF, 0xFF, 0x49,
                0x83, 0xE2, 7, 0x49, 0x83, 0xCB, 0xFF, 0x48, 0x81, 0xF2, 128, 0, 0, 0,
            ]
        );
    }
    ...
    fn encodes_scalar_f64_rip_memory_ops() {
        ...
        let min_disp = encoder.minsd_xmm_m64_rip_disp32(Xmm::Xmm2, 24);
        let max_disp = encoder.maxsd_xmm_m64_rip_disp32(Xmm::Xmm4, -16);

        assert_eq!(mul_disp, 4);
        assert_eq!(div_disp, 12);
        assert_eq!(min_disp, 20);
        assert_eq!(max_disp, 28);
        assert_eq!(
            encoder.into_bytes(),
            [
                0xF2, 0x0F, 0x59, 0x0D, 16, 0, 0, 0, 0xF2, 0x0F, 0x5E, 0x2D, 0xF8, 0xFF, 0xFF,
                0xFF, 0xF2, 0x0F, 0x5D, 0x15, 24, 0, 0, 0, 0xF2, 0x0F, 0x5F, 0x25, 0xF0, 0xFF,
                0xFF, 0xFF,
            ]
        );
    }
