...
mod tests {
    ...
    fn make_ctx() -> ConversionContext {
        ...
        ctx.terminal_map.insert("p".into(), 0);
        ctx.terminal_map.insert("n".into(), 1);
        ctx.param_map.insert("r".into(), 0);
        ctx.var_map.insert("x".into(), 0);
        ctx
    }
    ...
    fn test_convert_parameter() {
        ...
        let result = conv.convert(&ident).unwrap();
        assert!(matches!(result, IrExpr::Param(name) if name == "r"));
    }
    ...
    fn test_convert_variable() {
        ...
        let result = conv.convert(&ident).unwrap();
        assert!(matches!(result, IrExpr::Var(name) if name == "x"));
    }
    ...
    fn test_convert_voltage() {
        ...
        let access = Expression::BranchAccess(BranchAccess::Nodes {
            access: "V".into(),
            pos: "p".into(),
            neg: Some("n".into()),
            span: dummy_span(),
        ...
        let result = conv.convert(&access).unwrap();
        assert!(matches!(result, IrExpr::Voltage(0, 1)));
    }
    ...
    fn test_convert_current() {
        ...
        let result = conv.convert(&access).unwrap();
        assert!(matches!(result, IrExpr::Current(0, 1)));
    }

    #[test]
    fn test_convert_system_function() {
        let ctx = make_ctx();
        let conv = ExprConverter::new(&ctx);

        let func = Expression::SystemFunction(SystemFunction {
            name: "$temperature".into(),
            args: vec![],
            span: dummy_span(),
        });

        let result = conv.convert(&func).unwrap();
        assert!(matches!(result, IrExpr::Temperature));
    }
    ...
    fn test_convert_vt() {
        ...
        let result = conv.convert(&func).unwrap();
        assert!(matches!(result, IrExpr::Vt));
    }
    ...
    fn test_convert_binary_expression() {
        ...
        let expr = Expression::Binary(crate::ast::BinaryExpr {
            op: BinaryOp::Add,
        ...
        assert!(matches!(result, IrExpr::Binary(BinaryOp::Add, _, _)));
    }
    ...
    fn test_convert_unary_expression() {
        ...
        let expr = Expression::Unary(crate::ast::UnaryExpr {
            op: UnaryOp::Neg,
            operand: Box::new(Expression::Number(NumberLit {
                value: 5.0,
                raw: "5.0".into(),
                span: dummy_span(),
            })),
        ...
        let result = conv.convert(&expr).unwrap();
        assert!(matches!(result, IrExpr::Unary(UnaryOp::Neg, _)));
    }
    ...
    fn test_convert_function_call() {
        ...
        let expr = Expression::Call(CallExpr {
            name: "exp".into(),
            args: vec![Expression::Number(NumberLit {
                value: 1.0,
                raw: "1.0".into(),
                span: dummy_span(),
            })],
        ...
        let result = conv.convert(&expr).unwrap();
        assert!(matches!(result, IrExpr::Call(IrFunction::Exp, _)));
    }
    ...
    fn test_convert_conditional() {
        ...
        let expr = Expression::Conditional(crate::ast::ConditionalExpr {
            condition: Box::new(Expression::Number(NumberLit {
                value: 1.0,
                raw: "1.0".into(),
                span: dummy_span(),
            })),
            then_expr: Box::new(Expression::Number(NumberLit {
                value: 2.0,
                raw: "2.0".into(),
                span: dummy_span(),
            })),
            else_expr: Box::new(Expression::Number(NumberLit {
                value: 3.0,
                raw: "3.0".into(),
                span: dummy_span(),
            })),
        ...
        let result = conv.convert(&expr).unwrap();
        assert!(matches!(result, IrExpr::Conditional(_, _, _)));
    }

    #[test]
    fn test_convert_math_constant() {
        let ctx = make_ctx();
        let conv = ExprConverter::new(&ctx);
        ...
        let result = conv.convert(&ident).unwrap();
        assert!(matches!(result, IrExpr::Const(v) if (v - std::f64::consts::PI).abs() < 1e-10));
    }

    #[test]
    fn test_context_from_module() {
        use crate::ast::{ParamType, PortDirection, VarType};
        use crate::semantic::{AnalyzedParameter, AnalyzedPort, AnalyzedVariable, SymbolTable};
        ...
        let module = AnalyzedModule {
            ...
            parameters: vec![AnalyzedParameter {
                ...
                value_type: crate::types::ValueType::Real,
                default: Some(1.0),
                range: None,
            }],
        ...
        assert_eq!(ctx.terminal_index("a"), Some(0));
        assert_eq!(ctx.terminal_index("b"), Some(1));
        assert_eq!(ctx.param_index("r"), Some(0));
        assert_eq!(ctx.var_index("x"), Some(0));
        assert_eq!(ctx.num_terminals(), 2);
    }

    #[test]
    fn test_convert_ddt_operator() {
        let ctx = make_ctx();
        let conv = ExprConverter::new(&ctx);

        let v_access = Expression::BranchAccess(BranchAccess::Nodes {
            access: "V".into(),
            pos: "p".into(),
            neg: Some("n".into()),
            span: dummy_span(),
        ...
        let expr = Expression::AnalogOperator(AnalogOperator::Ddt {
            expr: Box::new(v_access),
            abstol: None,
            span: dummy_span(),
        });

        let result = conv.convert(&expr).unwrap();
        assert!(matches!(result, IrExpr::Ddt(_)));
    }

    #[test]
    fn test_convert_limexp_operator() {
        let ctx = make_ctx();
        let conv = ExprConverter::new(&ctx);
        ...
        let expr = Expression::AnalogOperator(AnalogOperator::Limexp {
            expr: Box::new(num),
            span: dummy_span(),
        });

        let result = conv.convert(&expr).unwrap();
        assert!(matches!(result, IrExpr::Limexp(_)));
    }

    #[test]
    fn test_convert_idt_operator() {
        let ctx = make_ctx();
        let conv = ExprConverter::new(&ctx);
        ...
        let expr = Expression::AnalogOperator(AnalogOperator::Idt {
            expr: Box::new(num),
            ic: None,
            assert_val: None,
            abstol: None,
        ...
        let result = conv.convert(&expr).unwrap();
        assert!(matches!(result, IrExpr::Idt(_, None)));
    }
    ...
    fn test_error_unknown_identifier() {
        ...
        let result = conv.convert(&ident);
        assert!(result.is_err());
    }

    #[test]
    fn test_error_unknown_terminal() {
        let ctx = make_ctx();
        let conv = ExprConverter::new(&ctx);

        let access = Expression::BranchAccess(BranchAccess::Nodes {
            access: "V".into(),
            pos: "unknown_terminal".into(),
            neg: None,
            span: dummy_span(),
        });

        let result = conv.convert(&access);
        assert!(result.is_err());
    }

    #[test]
    fn test_convert_branch_ref() {
        let ctx = make_ctx();
        let conv = ExprConverter::new(&ctx);

        let access = BranchAccess::Nodes {
            access: "I".into(),
            pos: "p".into(),
            neg: Some("n".into()),
            span: dummy_span(),
        ...
        let result = conv.convert_branch_ref(&access).unwrap();
        assert_eq!(result.pos_terminal, 0);
        assert_eq!(result.neg_terminal, 1);
    }

    #[test]
    fn test_convert_hypot_function() {
        let ctx = make_ctx();
        let conv = ExprConverter::new(&ctx);
