...
fn rust_backend_auto_scalarizes_named_branch_current_probes() {
    ...
    .transpile(&artifact)
    .expect("transpile named branch current probe through auto backend");
    let stamp = generated
        .files
        .iter()
        .find(|file| file.relative_path == "stamp.rs")
...
fn rust_backend_auto_scalarizes_min_max_clipped_currents() {
    ...
    .transpile(&artifact)
    .expect("transpile min/max clipped current through auto backend");
    let stamp = generated
        .files
        .iter()
        .find(|file| file.relative_path == "stamp.rs")
...
#[test]
fn rust_backend_auto_scalarizes_pow_intrinsic_currents_with_derivatives() {
    let artifact = VerilogACompiler::default()
        .compile_canonical_ir(scalar_pow_intrinsic_source())
        .expect("canonical IR");
    ...
    .transpile(&artifact)
    .expect("transpile pow intrinsic current through auto backend");
    let stamp = generated
        .files
        .iter()
        .find(|file| file.relative_path == "stamp.rs")
...
fn rust_backend_uses_compact_integer_power_multiply_helpers() {
    ...
    assert!(
        support.contains("fn store_scaled_powi_ad")
            ...
            && support.contains("fn store_neg_powi_add_input")
    ...
    assert!(
        stamp.contains("s.store_div_from_scalar_powi_ad(")
            ...
            || stamp.contains("s.store_neg_powi_add_input(")
            || stamp.contains("s.store_mul_scaled_powi"),
        "integer power consumers should use fused powi helpers:\n{stamp}"
    );
    assert!(
        stamp.contains("s.store_neg_powi_add_input("),
        "negated integer powers of sums should use the fused powi add helper:\n{stamp}"
    );
    assert!(
...
fn rust_backend_uses_compact_scaled_mixed_multiply_store_helpers() {
    ...
    assert!(
        support.contains("fn store_mul_scaled_ln_ad_rhs(&mut self, index: usize, source: usize"),
        "{support}"
    );
    assert!(
        support
...
fn rust_backend_directly_stores_product_division_expression_helpers() {
    ...
    assert!(
        support.contains(
            "fn store_div_scaled_product_by_product_indices(&mut self, index: usize, product_left: usize, product_right: usize, product_scale: f64, denominator_left: usize, denominator_right: usize, denominator_scale: f64)"
        ),
        "{support}"
    );
    assert!(
        support.contains(
            "fn store_div_scaled_add_product_indices(&mut self, index: usize, value: usize, value_scale: f64, product_left: usize, product_right: usize, product_scale: f64, denominator: usize, denominator_scale: f64)"
        ),
        "{support}"
    );
    assert!(
        support.contains(
            "fn store_div_scaled_value_by_product_indices(&mut self, index: usize, input: usize, input_scale: f64, denominator_left: usize, denominator_right: usize, denominator_scale: f64)"
        ),
        "{support}"
    );
    assert!(
    ...
    assert!(
        support.contains(
            "fn store_div_scaled_inputs_product_indices(&mut self, index: usize, first: usize, first_scale: f64, second: usize, second_scale: f64, product_left: usize, product_right: usize, product_scale: f64, denominator: usize, denominator_scale: f64)"
        ),
        "{support}"
    );
    assert!(
    ...
    assert!(
        support.contains(
            "fn store_div_scaled_product_by_product3_indices(&mut self, index: usize, product_left: usize, product_right: usize, product_scale: f64, denominator_first: usize, denominator_second: usize, denominator_third: usize, denominator_scale: f64)"
        ),
        "{support}"
    );
    for helper in [
        ...
        "fn store_div_scaled_product_offset_denominator_indices(",
    ] {
    ...
    assert!(
        stamp.contains("s.store_div_scaled_product_by_product_indices("),
        "{stamp}"
    );
    assert!(
        stamp.contains("s.store_div_scaled_add_product_indices("),
        "{stamp}"
    );
    assert!(
        stamp.contains("s.store_div_scaled_value_by_product_indices("),
        "{stamp}"
    );
    assert!(
    ...
    assert!(
        stamp.contains("s.store_div_scaled_inputs_product_indices("),
        "{stamp}"
    );
    assert!(
    ...
    assert!(
        stamp.contains("s.store_div_scaled_product_by_product3_indices("),
        "{stamp}"
    );
    for helper in [
        "s.store_div_scaled_product_offset_lhs_indices(",
        "s.store_div_scaled_product_offset_rhs_indices(",
        "s.store_div_scaled_product_offset_denominator_indices(",
    ] {
    ...
    assert!(
        !stamp.contains("s.store_div_scaled_product_by_product("),
        "direct product-ratio root assignments should avoid by-value product-by-product stores:\n{stamp}"
    );
    assert!(
        !stamp.contains("s.store_div_scaled_add_product("),
        "direct add-product quotient root assignments should avoid by-value add-product stores:\n{stamp}"
    );
    assert!(
    ...
    assert!(
        !stamp.contains("s.store_div_scaled_inputs_product("),
        "direct affine-plus-product quotient root assignments should avoid by-value inputs-product stores:\n{stamp}"
    );
    assert!(
    ...
    assert!(
        !stamp.contains("s.store_div_scaled_product_by_product3("),
        "direct product-by-product3 root assignments should avoid by-value product-by-product3 stores:\n{stamp}"
    );
    assert!(
        !stamp.contains("s.store_div_scaled_product_offset_lhs("),
        "direct offset product-division root assignments should avoid by-value lhs-offset stores:\n{stamp}"
    );
    assert!(
        !stamp.contains("s.store_div_scaled_product_offset_rhs("),
        "direct offset product-division root assignments should avoid by-value rhs-offset stores:\n{stamp}"
    );
    assert!(
        !stamp.contains("s.store_div_scaled_product_offset_denominator("),
        "direct offset product-division root assignments should avoid by-value denominator-offset stores:\n{stamp}"
    );
    assert!(
        !stamp.contains("s.store_ad_value("),
...
fn rust_backend_directly_stores_mixed_index_product_helpers() {
    ...
    for helper in [
        ...
        "fn store_div_scaled_product_offset_lhs_mixed_aii(",
        "fn store_div_scaled_product_offset_rhs_mixed_iai(",
        ...
        "fn store_div_scaled_product_by_product3_mixed_iiaaa(",
    ] {
        assert!(support.contains(helper), "missing {helper}\n{support}");
    }
    ...
    for helper in [
        ...
        "s.store_div_scaled_product_offset_lhs_mixed_aii(",
        "s.store_div_scaled_product_offset_rhs_mixed_iai(",
        ...
        "s.store_div_scaled_product_by_product3_mixed_iiaaa(",
    ] {
        assert!(stamp.contains(helper), "missing {helper}\n{stamp}");
    }
    ...
    assert!(
        !stamp.contains("s.store_div_scaled_product_offset_lhs("),
        "mixed offset product-division root assignments should avoid by-value lhs-offset stores:\n{stamp}"
    );
    assert!(
        !stamp.contains("s.store_div_scaled_product_offset_rhs("),
        "mixed offset product-division root assignments should avoid by-value rhs-offset stores:\n{stamp}"
    );
    assert!(
        !stamp.contains("s.store_div_scaled_product_offset_denominator("),
        "mixed offset product-division root assignments should avoid by-value denominator-offset stores:\n{stamp}"
    );
    assert!(
        !stamp.contains("s.store_div_scaled_product_by_product("),
        "mixed product-ratio root assignments should avoid by-value product-by-product stores:\n{stamp}"
    );
    assert!(
        !stamp.contains("s.store_div_scaled_add_product("),
        "mixed add-product quotient root assignments should avoid by-value add-product stores:\n{stamp}"
    );
    assert!(
        !stamp.contains("s.store_div_scaled_value_by_product("),
    ...
    assert!(
        !stamp.contains("s.store_div_scaled_inputs_product("),
        "mixed affine-plus-product quotient root assignments should avoid by-value inputs-product stores:\n{stamp}"
    );
    assert!(
        !stamp.contains("s.store_div_scaled_product3_by_product("),
    ...
    assert!(
        !stamp.contains("s.store_div_scaled_product_by_product3("),
        "mixed product-by-product3 root assignments should avoid by-value product-by-product3 stores:\n{stamp}"
    );
    assert_generated_rust_compiles(&generated);
}
...
#[test]
fn rust_backend_fuses_expression_divided_product_division_chains() {
    let artifact = VerilogACompiler::default()
    ...
    .transpile(&artifact)
    .expect("transpile compact expression divided product division");
    let stamp = generated
        .files
        .iter()
        .find(|file| file.relative_path == "stamp.rs")
...
#[test]
fn rust_backend_fuses_expression_scalar_divided_product_division_chains() {
    let artifact = VerilogACompiler::default()
    ...
    .transpile(&artifact)
    .expect("transpile compact expression scalar divided product division");
    let stamp = generated
        .files
        .iter()
        .find(|file| file.relative_path == "stamp.rs")
    ...
    assert!(
        !stamp.contains("A::div_scaled_product(A::div_from_scalar("),
        "{stamp}"
    );
    assert!(
...
#[test]
fn rust_backend_fuses_expression_offset_denominator_division_chains() {
    let artifact = VerilogACompiler::default()
    ...
    .transpile(&artifact)
    .expect("transpile compact expression offset denominator division");
    let stamp = generated
        .files
        .iter()
        .find(|file| file.relative_path == "stamp.rs")
    ...
    assert!(
        support.contains(
            "fn div_scaled_value_offset_denominator(input: Self, input_scale: f64, denominator: Self, denominator_offset: f64, denominator_scale: f64) -> Self"
        ),
        "{support}"
    );
    assert!(
        support.contains(
            "fn div_scalar_offset_denominator(scalar: f64, denominator: Self, denominator_offset: f64, denominator_scale: f64) -> Self"
        ),
        "{support}"
    );
    assert!(
        stamp.contains("A::div_scaled_value_offset_denominator("),
        "{stamp}"
    );
    assert!(
        stamp.contains("A::div_scalar_offset_denominator("),
        "{stamp}"
    );
    assert!(!stamp.contains("A::div(s.ad_value("), "{stamp}");
    assert!(!stamp.contains("A::div_from_scalar("), "{stamp}");
    assert_generated_rust_compiles(&generated);
}
...
fn rust_backend_fuses_sub_from_scalar_mixed_multiply_store_helpers() {
    ...
    for legacy_helper in [
        ...
        "store_mul_sub_from_scalar_lhs_scaled_output",
        "store_mul_sub_from_scalar_lhs_scaled_ad_lhs",
        "store_mul_sub_from_scalar_lhs_scaled_ad_rhs",
        "store_mul_sub_from_scalar_lhs_scaled_ad",
    ] {
        assert!(
            !support.contains(&format!("pub(crate) fn {legacy_helper}(")),
            "legacy sub-from-scalar multiply helpers should fold into scale-offset helpers:\n{support}"
        );
        assert!(
            !stamp.contains(&format!("s.{legacy_helper}(")),
            "generated sub-from-scalar multiply calls should use scale-offset helpers:\n{stamp}"
        );
    }
    assert!(!stamp.contains("s.store_mul_ad_lhs("), "{stamp}");
    assert!(!stamp.contains("s.store_mul_ad_rhs("), "{stamp}");
...
fn rust_backend_fuses_scaled_value_mixed_multiply_store_helpers() {
    ...
    assert!(
        !support.contains("pub(crate) fn store_mul_scale_ad_lhs("),
        "{support}"
    );
    assert!(
        !support.contains("pub(crate) fn store_mul_scale_ad_rhs("),
        "{support}"
    );
    assert!(!stamp.contains("s.store_mul_scale_ad_lhs("), "{stamp}");
...
module compact_expression_direct_store_product_division_helpers(p, n);
    ...
    analog begin
        ...
        prod2 = (a * q) / den;
        prod3 = (a * q * r) / den;
        off_lhs = ((a + gain) * q) / den;
        off_rhs = (a * (q + gain)) / den;
        off_den = (a * q) / (r + gain);
        ratio = (a * q) / (r * den);
...
module compact_expression_direct_store_mixed_index_product_helpers(p, n);
    ...
    real div_aia;
    real div_aai;
    real div_offset_lhs_aii;
    real div_offset_rhs_iai;
    ...
    analog begin
        ...
        div_aia = ((a + t) * q) / (r + t);
        div_aai = ((a + t) * (q + t)) / r;
        div_offset_lhs_aii = ((sqrt(a) + product_scale) * q) / r;
        div_offset_rhs_iai = (a * (sqrt(q) + product_scale)) / r;
        ...
        div_product_ratio3_iiaaa = (a * q) / ((r + t) * (a + q) * sqrt(t));
...
fn compact_expression_divided_product_division_source() -> &'static str {
    r#"
module compact_expression_divided_product_division(p, n);
    inout p, n;
    electrical p, n;
    parameter real gain = 2.0;
    parameter real offset = 3.0;
...
fn compact_expression_scalar_divided_product_division_source() -> &'static str {
    r#"
module compact_expression_scalar_divided_product_division(p, n);
    inout p, n;
    electrical p, n;
    parameter real gain = 2.0;
    parameter real offset = 3.0;
...
fn compact_expression_offset_denominator_division_source() -> &'static str {
    r#"
module compact_expression_offset_denominator_division(p, n);
    inout p, n;
    electrical p, n;
    parameter real gain = 2.0;
    parameter real offset = 3.0;
...
fn scalar_min_max_clip_source() -> &'static str {
    r#"
module scalar_min_max_clip(p, n);
    inout p, n;
    electrical p, n;
    parameter real lo = -0.1;
    parameter real hi = 0.1;
    real clipped;
    analog begin
        clipped = max(min(V(p, n), hi), lo);
        I(p, n) <+ clipped;
    end
endmodule
"#
...
fn scalar_pow_intrinsic_source() -> &'static str {
    r#"
module scalar_pow_intrinsic(p, n);
    inout p, n;
    electrical p, n;
