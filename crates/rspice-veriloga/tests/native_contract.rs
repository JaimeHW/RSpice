...
fn canonical_device_from_source(instance: &str, source: &str) -> VerilogADevice {
    ...
    let model = compiler
        .compile(source)
        .expect("Verilog-A source must compile for bytecode model");
...
#[cfg(target_arch = "x86_64")]
#[test]
fn native_device_with_canonical_ir_executes_idtmod_without_modulus_as_idt_without_fallback() {
    let source = r#"
`include "disciplines.vams"
module native_canonical_idtmod_nomod(p, n);
    inout p, n;
    electrical p, n;
    analog I(p, n) <+ idtmod(1.0, 0.5);
endmodule
"#;
    let compiler = VerilogACompiler::new(CompilerOptions::default());
    let model = compiler.compile(source).expect("compile bytecode model");
    let artifact = compiler
        ...
        .expect("compile canonical IR");
    let mut device =
        VerilogADevice::try_new_with_canonical_ir("CIMODNOMOD1", model, &artifact, &[1, 0])
            .expect("canonical idtmod without modulus uses native JIT path");
    assert!(device.is_using_native());
    ...
    assert_eq!(
        device
            .try_evaluate()
            .expect("canonical idtmod-without-modulus DC evaluation succeeds")[0]
            .to_bits(),
        0.5_f64.to_bits()
    );
    device.advance_state();
    ...
    assert_eq!(
        device
            .try_evaluate()
            .expect("canonical idtmod-without-modulus transient evaluation succeeds")[0]
            .to_bits(),
        0.75_f64.to_bits()
    );
}
...
"#;
    let mut device = canonical_device_from_source("CDELAYMAX1", source);
...
#[cfg(target_arch = "x86_64")]
#[test]
fn native_device_with_canonical_ir_executes_ddx_current_without_fallback() {
    let source = r#"
`include "disciplines.vams"
module native_canonical_ddx_current(p, n);
    inout p, n;
    electrical p, n;
...
"#;
    ...
    let artifact = compiler
        ...
        .expect("compile canonical IR");
    let mut device =
        VerilogADevice::try_new_with_canonical_ir("DDXCANON1", model, &artifact, &[1, 0])
            .expect("canonical ddx current uses native JIT path");
    assert!(device.is_using_native());
...
fn native_device_with_canonical_ir_executes_limit_current_without_fallback() {
    let source = r#"
`include "disciplines.vams"
module native_canonical_limit_current(p, n);
    inout p, n;
    electrical p, n;
    analog I(p, n) <+ $limit(V(p, n), 0.5);
endmodule
"#;
    let compiler = VerilogACompiler::new(CompilerOptions::default());
    let model = compiler.compile(source).expect("compile bytecode model");
    let artifact = compiler
        ...
        .expect("compile canonical IR");
    let mut device =
        VerilogADevice::try_new_with_canonical_ir("LIMITCANON1", model, &artifact, &[1, 0])
            .expect("canonical limit current uses native JIT path");
    assert!(device.is_using_native());

    device.update_voltages(&[0.0]);
    let currents = device
        .try_evaluate()
        .expect("canonical first limit evaluation succeeds");
    assert_eq!(currents[0].to_bits(), 0.0_f64.to_bits());

    device.update_voltages(&[10.0]);
    let currents = device
        .try_evaluate()
        .expect("canonical upward limit evaluation succeeds");
    assert_eq!(currents[0].to_bits(), 0.5_f64.to_bits());

    let currents = device
        .try_evaluate()
        .expect("canonical repeated limit evaluation succeeds");
    assert_eq!(currents[0].to_bits(), 1.0_f64.to_bits());
}
...
"#;
    ...
    let artifact = compiler
        ...
        .expect("compile canonical IR");
    let mut device =
        VerilogADevice::try_new_with_canonical_ir("BITNOTCANON1", model, &artifact, &[1, 0])
    ...
    assert_eq!(currents[0].to_bits(), (-13.0_f64).to_bits());
}
