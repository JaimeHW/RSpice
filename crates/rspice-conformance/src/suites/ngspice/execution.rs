    ...
    pub fn run_test(&self, cir_path: &Path) -> TestResult {
        ...
        for analysis in &analysis_plan {
            let case = RegressionCase {
                name: &name,
                cir_path,
                source_path: &source_cir_path,
                source: &source,
                start: std::time::Instant::now(),
            };
            let mut result = match analysis {
                AnalysisSpec::DcOp => self.run_dc_op_test(&case),
                ...
                AnalysisSpec::Transient {
                    ...
                    tstart,
                    tmax,
                } => self.run_transient_test(&case, *tstep, *tstop, *tstart, *tmax),
                AnalysisSpec::Ac {
                    sweep_type,
                    points,
                    fstart,
                ...
                AnalysisSpec::PoleZero {
                    ...
                    &PoleZeroPorts {
                        input_pos,
                        input_neg: input_neg.as_deref(),
                        output_pos,
                        output_neg: output_neg.as_deref(),
                ...
                AnalysisSpec::Sensitivity {
                    ...
                    output_neg,
                    sweep,
                } => self.run_sensitivity_test(&case, output_pos, output_neg.as_deref(), *sweep),
                AnalysisSpec::TransferFunction {
                    output,
                    input_source,
                } => self.run_transfer_function_test(&case, output, input_source),
                AnalysisSpec::Unsupported { directive } => TestResult {
                    ...
                    mismatches: Vec::new(),
                    duration_ms: case.start.elapsed().as_millis(),
