...
use super::contracts_bug340::Bug340Role;
use super::contracts_bug352::Bug352Role;
use super::contracts_bug372::Bug372Role;
use super::contracts_bug389::Bug389Role;
use super::contracts_bug411::Bug411Role;
...
use super::contracts_bug1035_son::Bug1035SonRole;
use super::contracts_bug1152::Bug1152Role;
use super::contracts_bug1162::Bug1162Role;
use super::contracts_bug1190_son::Bug1190SonRole;
use super::contracts_bug1284::Bug1284Role;
use super::contracts_bug1398::Bug1398Role;
use super::contracts_bug1455::Bug1455Role;
use super::contracts_bug1692::Bug1692Role;
...
use super::contracts_issue565_566::Issue565566Role;
...
impl XyceTestRunner {
    ...
    fn run_discovered_test_unqualified(&self, deck: &XyceDeck) -> XyceTestResult {
        ...
        if let Some(role) = Bug113Role::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_bug113_oracle(deck, role, start) {
        ...
        if let Some(role) = Bug706Role::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_bug706_oracle(deck, role, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if let Some(role) = Bug141Role::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_bug141_oracle(deck, role, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if let Some(role) = Bug1190SonRole::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_bug1190_son_oracle(deck, role, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if let Some(role) = Bug1284Role::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_bug1284_oracle(deck, role, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if let Some(role) = Bug1152Role::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_bug1152_oracle(deck, role, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if Self::normalize_manifest_key(&deck.relative_path) == XYCE_BUG1025_RECORD {
            let result = match self.validate_bug1025_oracle(deck, start) {
                Ok(()) => self.passed_result(deck, start, XYCE_BUG1025_CONTRACT),
                Err(error) => {
                    self.failure_result(deck, start, XYCE_BUG1025_CONTRACT, error, Vec::new())
                }
            };
            if self.config.verbose {
                println!(
        ...
        if Self::normalize_manifest_key(&deck.relative_path) == XYCE_BUG636_RECORD {
            let result = match self.validate_bug636_oracle(deck, start) {
                Ok(()) => self.passed_result(deck, start, XYCE_BUG636_CONTRACT),
                Err(error) => {
                    self.failure_result(deck, start, XYCE_BUG636_CONTRACT, error, Vec::new())
                }
            };
            if self.config.verbose {
                println!(
        ...
        if Self::normalize_manifest_key(&deck.relative_path) == XYCE_BUG206_RECORD {
            let result = match self.validate_bug206_oracle(deck, start) {
                Ok(()) => self.passed_result(deck, start, XYCE_BUG206_CONTRACT),
                Err(error) => {
                    self.failure_result(deck, start, XYCE_BUG206_CONTRACT, error, Vec::new())
                }
            };
            if self.config.verbose {
                println!(
        ...
        if Self::normalize_manifest_key(&deck.relative_path) == XYCE_BUG1116_RECORD {
            let result = match self.validate_bug1116_oracle(deck, start) {
                Ok(()) => self.passed_result(deck, start, XYCE_BUG1116_CONTRACT),
                Err(error) => {
                    self.failure_result(deck, start, XYCE_BUG1116_CONTRACT, error, Vec::new())
                }
            };
            if self.config.verbose {
                println!(
        ...
        if Self::normalize_manifest_key(&deck.relative_path) == XYCE_ISSUE61_RECORD {
            let result = match self.validate_issue61_oracle(deck, start) {
                Ok(()) => self.passed_result(deck, start, XYCE_ISSUE61_CONTRACT),
                Err(error) => {
                    self.failure_result(deck, start, XYCE_ISSUE61_CONTRACT, error, Vec::new())
                }
            };
            if self.config.verbose {
                println!(
        ...
        if Self::normalize_manifest_key(&deck.relative_path) == XYCE_BUG784_RECORD {
            let result = match self.validate_bug784_oracle(deck, start) {
                Ok(()) => self.passed_result(deck, start, XYCE_BUG784_CONTRACT),
                Err(error) => {
                    self.failure_result(deck, start, XYCE_BUG784_CONTRACT, error, Vec::new())
                }
            };
            if self.config.verbose {
                println!(
        ...
        if let Some(kind) = XyceDiodeAnalyticKind::for_record(&deck.relative_path) {
            let contract = kind.result_contract();
            let result = match self.validate_diode_analytic_oracle(deck, kind, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if Self::normalize_manifest_key(&deck.relative_path) == XYCE_BUG302_RECORD {
            let result = match self.validate_bug302_oracle(deck, start) {
                Ok(()) => self.passed_result(deck, start, XYCE_BUG302_CONTRACT),
                Err(error) => {
                    self.failure_result(deck, start, XYCE_BUG302_CONTRACT, error, Vec::new())
                }
            };
            if self.config.verbose {
                println!(
        ...
        if let Some(role) = Bug1692Role::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_bug1692_oracle(deck, role, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if let Some(role) = Bug1035SonRole::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_bug1035_son_oracle(deck, role, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if let Some(role) = Issue451Role::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_issue451_oracle(deck, role, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if let Some(role) = Issue565566Role::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_issue565566_oracle(deck, role, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if let Some(role) = Bug1455Role::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_bug1455_oracle(deck, role, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if let Some(role) = TrTranRole::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_tr_tran_oracle(deck, role, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if let Some(role) = Bug689Role::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_bug689_oracle(deck, role, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if let Some(role) = Bug806Role::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_bug806_oracle(deck, role, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if let Some(role) = Bug805Role::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_bug805_oracle(deck, role, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if let Some(role) = Bug805SonRole::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_bug805_son_oracle(deck, role, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if let Some(role) = Bug1957Role::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_bug1957_oracle(deck, role, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if let Some(role) = Bug306SonRole::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_bug306_son_oracle(deck, role, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if let Some(role) = Bug308SonRole::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_bug308_son_oracle(deck, role, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if let Some(role) = Bug340Role::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_bug340_oracle(deck, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if let Some(role) = Bug389Role::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_bug389_oracle(deck, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if let Some(role) = Bug412Role::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_bug412_oracle(deck, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if let Some(role) = Bug440Role::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_bug440_oracle(deck, role, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if let Some(role) = Bug28Role::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_bug28_oracle(deck, role, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if let Some(role) = Bug1398Role::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_bug1398_oracle(deck, role, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if let Some(role) = Bug1162Role::for_record(&deck.relative_path) {
            let contract = role.contract();
            let result = match self.validate_bug1162_oracle(deck, role, start) {
                Ok(()) => self.passed_result(deck, start, contract),
                Err(error) => self.failure_result(deck, start, contract, error, Vec::new()),
            };
            if self.config.verbose {
        ...
        if Self::normalize_manifest_key(&deck.relative_path) == XYCE_BUG271_RECORD {
            let result = match self.validate_bug271_oracle(deck, start) {
                ...
                Err(error) => {
                    self.failure_result(deck, start, XYCE_BUG271_CONTRACT, error, Vec::new())
                }
            };
            if self.config.verbose {
                println!(
        ...
        if Self::normalize_manifest_key(&deck.relative_path) == XYCE_BUG1661_RECORD {
            let result = match self.validate_bug1661_oracle(deck, start) {
                Ok(()) => self.passed_result(deck, start, XYCE_BUG1661_CONTRACT),
                Err(error) => {
                    self.failure_result(deck, start, XYCE_BUG1661_CONTRACT, error, Vec::new())
                }
            };
            if self.config.verbose {
                println!(
        ...
        if Self::normalize_manifest_key(&deck.relative_path) == XYCE_BUG519_RECORD {
            let result = match self.validate_bug519_oracle(deck, start) {
                Ok(()) => self.passed_result(deck, start, XYCE_BUG519_CONTRACT),
                Err(error) => {
                    self.failure_result(deck, start, XYCE_BUG519_CONTRACT, error, Vec::new())
                }
            };
            if self.config.verbose {
                println!(
