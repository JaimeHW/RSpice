...
mod tests {
    ...
    fn test_exit_codes() {
        ...
        assert_eq!(ExitCode::MisuseOfCommand as u8, 2);
    }

    #[test]
    fn test_error_exit_code_mapping() {
        let err = CliError::parse_error("test");
        assert_eq!(err.exit_code(), ExitCode::InputError);

        let err = CliError::simulation_error("test");
    ...
    fn test_error_with_suggestion() {
        let err = CliError::ParseError {
            ...
            line: Some(5),
            suggestion: Some("Did you mean 'R1' instead of 'r1'?".to_string()),
        };
        assert_eq!(err.suggestion(), Some("Did you mean 'R1' instead of 'r1'?"));
    }
