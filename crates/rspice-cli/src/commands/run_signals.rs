...
mod tests {
    ...
    #[test]
    fn test_voltage_display_name_preserves_named_nodes() {
        assert_eq!(voltage_raw_name("OUT", 2), "OUT");
        assert_eq!(voltage_raw_name("V(OUT)", 2), "OUT");
        assert_eq!(voltage_display_name("OUT", 2), "V(OUT)");
        assert_eq!(voltage_display_name("", 2), "V(2)");
    }

    #[test]
    fn test_current_display_name_preserves_wrapped_branch_names() {
        assert_eq!(current_raw_name("V1", 1), "V1");
        assert_eq!(current_raw_name("I(V1)", 1), "V1");
        assert_eq!(current_display_name("V1", 1), "I(V1)");
    }

    #[test]
    fn test_dc_sweep_voltage_signals_export_all_named_nodes() {
        let mut first = SimulationResult::new(6, 0);
        first.node_names = vec![
            "0".to_string(),
            "IN".to_string(),
            "N1".to_string(),
            "N2".to_string(),
        ...
        first.node_voltages = vec![0.0, 6.0, 5.0, 4.0, 3.0, 2.0, 1.0];

        let mut second = first.clone();
        second.node_voltages = vec![0.0, 3.0, 2.5, 2.0, 1.5, 1.0, 0.5];

        let signals = dc_sweep_voltage_signals(&[(0.0, first), (1.0, second)]);
        assert_eq!(signals.len(), 6);
        assert_eq!(signals[0].display_name, "V(IN)");
        assert_eq!(signals[5].display_name, "V(N5)");
        assert_eq!(signals[5].values, vec![1.0, 0.5]);
    }

    #[test]
    fn test_ac_signals_include_branch_currents() {
        let first = AcResult {
            frequency: 1.0,
            node_names: vec!["IN".to_string(), "OUT".to_string()],
            branch_names: vec!["V1".to_string()],
            voltages: vec![Complex64::new(1.0, 0.0), Complex64::new(0.5, -0.25)],
            currents: vec![Complex64::new(-0.5, 0.25)],
        };
        let second = AcResult {
            frequency: 10.0,
            node_names: first.node_names.clone(),
            branch_names: first.branch_names.clone(),
            voltages: vec![Complex64::new(0.8, -0.1), Complex64::new(0.4, -0.2)],
            currents: vec![Complex64::new(-0.4, 0.2)],
        };

        let signals = ac_signals(&[first, second]);
        assert_eq!(signals.len(), 3);
        assert_eq!(signals[0].display_name, "V(IN)");
        assert_eq!(signals[1].display_name, "V(OUT)");
        assert_eq!(signals[2].display_name, "I(V1)");
    }

    #[test]
    fn test_insert_transient_measurement_aliases_supports_named_and_numeric_access() {
        ...
        assert_eq!(
            signals.get("V(OUT)").copied(),
            Some(result.voltages[1].as_slice())
        );
        assert_eq!(
            signals.get("V(out)").copied(),
            Some(result.voltages[1].as_slice())
        ...
        assert_eq!(
            signals.get("OUT").copied(),
            Some(result.voltages[1].as_slice())
        );
        assert_eq!(
            signals.get("V(2)").copied(),
            Some(result.voltages[1].as_slice())
        );
    }
}
