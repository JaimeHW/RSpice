//! Publication of the transient `.FFT` artifact in every format the CLI
//! writes, and the decoder that validates one back.
//!
//! `.FFT` is a post-process of a transient, so its artifact is published as
//! the second member of that transient's atomic pair: the two are staged
//! together and committed together, and a run never advertises a transient
//! whose declared spectrum is missing. The identities the artifact carries are
//! the ones the canonical plan minted; nothing here formats an `fft-NNN` tag
//! of its own.
//!
//! The typed JSON envelope, the delimited table, the RAW artifact's metadata
...
use crate::hdf5::{
    ...
    Hdf5SimulationData, write_hdf5_to_writer,
};
use std::path::Path;
...
pub(super) fn validate_fft_result_count(
    results: &[rspice_core::engine::TransientFftResult],
