...
**One sample is a result.** An operating point is a single point by construction, so every table format `run` writes it in is a format `convert` reads it back from; nothing here has a two-sample minimum. An *empty* coordinate is not a result — it has no row to write and nothing for `--start`/`--stop` to clip — and is refused when the file is read, naming the coordinate rather than blaming a flag that was not given.
