...
mod tests {
    ...
    fn test_format_voltage() {
        assert_eq!(format_voltage(3.3), "3.300V");
        assert_eq!(format_voltage(0.001), "1.000mV");
        assert_eq!(format_voltage(0.000001), "1.000µV");
        assert_eq!(format_voltage(-5.0), "-5.000V");
        assert_eq!(format_voltage(0.0), "0.000V");
    }

    #[test]
    fn test_format_current() {
        assert_eq!(format_current(1.0), "1.000A");
        assert_eq!(format_current(0.001), "1.000mA");
        assert_eq!(format_current(0.000001), "1.000µA");
        assert_eq!(format_current(0.000000001), "1.000nA");
        assert_eq!(format_current(-0.005), "-5.000mA");
    }

    #[test]
    fn test_format_power() {
        assert_eq!(format_power(1.0), "1.000W");
        assert_eq!(format_power(0.001), "1.000mW");
        assert_eq!(format_power(0.000001), "1.000µW");
    }

    #[test]
    fn test_annotation_mode_cycle() {
        assert_eq!(AnnotationMode::Hidden.cycle(), AnnotationMode::Voltages);
        assert_eq!(AnnotationMode::Voltages.cycle(), AnnotationMode::Currents);
        assert_eq!(AnnotationMode::Currents.cycle(), AnnotationMode::All);
        assert_eq!(AnnotationMode::All.cycle(), AnnotationMode::Hidden);
    }

    #[test]
    fn test_annotation_state_populate() {
        let mut state = DcAnnotationState::new();

        let mut net_positions = HashMap::new();
        net_positions.insert("N001".to_string(), Point::new(10, 20));
        net_positions.insert("N002".to_string(), Point::new(30, 40));

        let node_voltages = vec![
            ("N001".to_string(), 3.3),
            ("N002".to_string(), 1.65),
            ("0".to_string(), 0.0), // Ground - should be skipped
        ];

        state.populate_voltages(&node_voltages, &net_positions);

        assert_eq!(state.voltages.len(), 2);
        assert!(state.voltages.contains_key("N001"));
        assert!(state.voltages.contains_key("N002"));
        assert!(!state.voltages.contains_key("0")); // Ground skipped
    }

    #[test]
    fn test_visible_annotations() {
        let mut state = DcAnnotationState::new();

        let mut net_positions = HashMap::new();
        net_positions.insert("N001".to_string(), Point::new(10, 20));

        let node_voltages = vec![("N001".to_string(), 3.3)];
        state.populate_voltages(&node_voltages, &net_positions);

        // Hidden mode - no visible
        state.mode = AnnotationMode::Hidden;
        assert_eq!(state.visible_annotations().len(), 0);

        // Voltages mode - show voltages
        state.mode = AnnotationMode::Voltages;
        assert_eq!(state.visible_annotations().len(), 1);

        // Currents mode - no currents populated
        state.mode = AnnotationMode::Currents;
        assert_eq!(state.visible_annotations().len(), 0);
    }
    ...
    fn test_stale_marking() {
        ...
        // No annotations - marking stale should not set flag
        state.mark_stale();
        ...
        let mut net_positions = HashMap::new();
        net_positions.insert("N001".to_string(), Point::new(10, 20));
        state.populate_voltages(&[("N001".to_string(), 3.3)], &net_positions);

        // Now marking stale should set flag
        state.mark_stale();
        assert!(state.is_stale);
    }
}
