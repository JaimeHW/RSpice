...
mod tests {
    ...
    fn test_wire_contains_point_on_segment() {
        let wire = Wire::segment(1, Point::new(0, 0), Point::new(10, 0));
        assert!(wire.contains_point(Point::new(5, 0)));
        assert!(!wire.contains_point(Point::new(5, 1)));
    }

    #[test]
    fn test_wire_connects_to() {
        let wire1 = Wire::segment(1, Point::new(0, 0), Point::new(10, 0));
        let wire2 = Wire::segment(2, Point::new(10, 0), Point::new(10, 10));
        let wire3 = Wire::segment(3, Point::new(20, 0), Point::new(30, 0));

        assert!(wire1.connects_to(&wire2));
        assert!(!wire1.connects_to(&wire3));
    }
    ...
    fn test_wire_length() {
        ...
        assert_eq!(wire.length(), 15);
    }

    #[test]
    fn test_wire_is_empty() {
        let empty = Wire::new(1, vec![Point::new(0, 0)]);
        let valid = Wire::segment(2, Point::new(0, 0), Point::new(10, 0));
    ...
    fn test_wire_vertex_at() {
        ...
        assert_eq!(wire.vertex_at(5), None);
    }

    #[test]
    fn test_wire_insert_vertex() {
        let mut wire = Wire::segment(1, Point::new(0, 0), Point::new(10, 0));
        wire.insert_vertex(1, Point::new(5, 0));

        assert_eq!(wire.vertex_count(), 3);
        assert_eq!(wire.vertex_at(1), Some(Point::new(5, 0)));
    }
    ...
    #[test]
    fn test_wire_remove_vertex_prevents_invalid() {
        let mut wire = Wire::segment(1, Point::new(0, 0), Point::new(10, 0));

        // Shouldn't be able to remove - would make wire invalid
        let removed = wire.remove_vertex(0);
    ...
    fn test_wire_move_vertex() {
        ...
        assert!(wire.move_vertex(1, Point::new(15, 0)));
        assert_eq!(wire.end(), Some(Point::new(15, 0)));
    }
    ...
    fn test_wire_bounding_box() {
        let wire = Wire::new(
            1,
            vec![Point::new(5, 2), Point::new(10, 0), Point::new(0, 8)],
        );

        let (min, max) = wire.bounding_box().unwrap();
        assert_eq!(min, Point::new(0, 0));
        assert_eq!(max, Point::new(10, 8));
    }
    ...
    fn test_wire_hit_test_vertex() {
        ...
        let result = wire.hit_test(Point::new(10, 0), 5.0);
        assert!(matches!(result, WireHitResult::Vertex { index: 1, .. }));
    }

    #[test]
    fn test_wire_hit_test_segment() {
        let wire = Wire::segment(1, Point::new(0, 0), Point::new(20, 0));

        let result = wire.hit_test(Point::new(10, 2), 5.0);
        assert!(matches!(result, WireHitResult::Segment { index: 0, .. }));
    }
    ...
    fn test_wire_hit_test_none() {
        ...
        let result = wire.hit_test(Point::new(50, 50), 5.0);
        assert!(matches!(result, WireHitResult::None));
    }
    ...
    #[test]
    fn test_wire_translate() {
        let mut wire = Wire::segment(1, Point::new(0, 0), Point::new(10, 0));
        wire.translate(5, 10);

        assert_eq!(wire.start(), Some(Point::new(5, 10)));
        assert_eq!(wire.end(), Some(Point::new(15, 10)));
    }

    #[test]
    fn test_wire_translated() {
        let wire = Wire::segment(1, Point::new(0, 0), Point::new(10, 0));
        let translated = wire.translated(5, 10);

        // Original unchanged
        assert_eq!(wire.start(), Some(Point::new(0, 0)));
        // New wire translated
        assert_eq!(translated.start(), Some(Point::new(5, 10)));
    }

    #[test]
    fn test_wire_split_at() {
        let mut wire = Wire::segment(1, Point::new(0, 0), Point::new(10, 0));

        assert!(wire.split_at(Point::new(5, 0)));
        assert_eq!(wire.vertex_count(), 3);
        assert_eq!(wire.vertex_at(1), Some(Point::new(5, 0)));
    }

    #[test]
    fn test_wire_split_at_vertex() {
        let mut wire = Wire::segment(1, Point::new(0, 0), Point::new(10, 0));

        // Split at existing vertex should fail
        assert!(!wire.split_at(Point::new(0, 0)));
        assert_eq!(wire.vertex_count(), 2);
    }

    #[test]
    fn test_wire_simplify() {
        let mut wire = Wire::new(
            1,
            vec![
                Point::new(0, 0),
                Point::new(5, 0), // Collinear
                Point::new(10, 0),
            ],
        );
    ...
    #[test]
    fn test_wire_simplify_keeps_corners() {
        let mut wire = Wire::new(
            1,
            vec![
                Point::new(0, 0),
                Point::new(10, 0),
                Point::new(10, 10), // Corner - not collinear
            ],
        );
    ...
    fn test_wire_intersections_with_wire() {
        let wire1 = Wire::segment(1, Point::new(0, 5), Point::new(10, 5));
        let wire2 = Wire::segment(2, Point::new(5, 0), Point::new(5, 10));
    ...
    fn test_wire_is_orthogonal() {
        ...
        let diag = Wire::segment(2, Point::new(0, 0), Point::new(10, 10));

        assert!(ortho.is_orthogonal());
        assert!(!diag.is_orthogonal());
    }
}
