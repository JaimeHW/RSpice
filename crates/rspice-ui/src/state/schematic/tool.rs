...
mod tests {
    ...
    fn test_tool_display_name() {
        assert_eq!(Tool::Select.display_name(), "Select");
        assert_eq!(Tool::Wire.display_name(), "Wire");
        assert_eq!(Tool::Probe.display_name(), "Probe");
        assert_eq!(
            Tool::Place(ComponentType::Resistor).display_name(),
            "Resistor"
        );
    }
    ...
    fn test_tool_shortcut() {
        assert_eq!(Tool::Select.shortcut(), Some('s'));
        assert_eq!(Tool::Wire.shortcut(), Some('w'));
        assert_eq!(Tool::Place(ComponentType::Ground).shortcut(), Some('g'));
    }

    #[test]
    fn test_tool_is_place() {
        assert!(!Tool::Select.is_place_tool());
        assert!(!Tool::Wire.is_place_tool());
        assert!(Tool::Place(ComponentType::Resistor).is_place_tool());
    }

    #[test]
    fn test_tool_placing() {
        assert_eq!(Tool::Select.placing(), None);
        assert_eq!(
            Tool::Place(ComponentType::Capacitor).placing(),
    ...
    fn test_tool_cursor() {
        ...
        assert_eq!(Tool::Wire.cursor(), "crosshair");
        assert_eq!(Tool::Place(ComponentType::Resistor).cursor(), "copy");
        assert_eq!(Tool::Label.cursor(), "text");
    }

    #[test]
    fn test_tool_is_select() {
        assert!(Tool::Select.is_select());
        assert!(!Tool::Wire.is_select());
    }

    #[test]
    fn test_tool_is_wire() {
        assert!(!Tool::Select.is_wire());
        assert!(Tool::Wire.is_wire());
    }
}
