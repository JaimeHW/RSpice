...
mod tests {
    ...
    #[test]
    fn test_selection_rect_default() {
        let rect = SelectionRect::new();
        assert!(!rect.is_active());
        assert_eq!(rect.start, Point::new(0, 0));
        assert_eq!(rect.current, Point::new(0, 0));
    }

    #[test]
    fn test_selection_rect_start_at() {
        let mut rect = SelectionRect::new();
        rect.start_at(Point::new(10, 20));
        assert!(rect.is_active());
        assert_eq!(rect.start, Point::new(10, 20));
        assert_eq!(rect.current, Point::new(10, 20));
    }

    #[test]
    fn test_selection_rect_update() {
        let mut rect = SelectionRect::new();
        rect.start_at(Point::new(10, 20));
        rect.update(Point::new(50, 60));
        assert_eq!(rect.current, Point::new(50, 60));
        assert_eq!(rect.start, Point::new(10, 20)); // Start unchanged
    }

    #[test]
    fn test_selection_rect_update_inactive() {
        let mut rect = SelectionRect::new();
        rect.update(Point::new(50, 60));
    ...
    #[test]
    fn test_selection_rect_bounds_normalized() {
        let mut rect = SelectionRect::new();
        // Drag from top-right to bottom-left
        rect.start_at(Point::new(100, 100));
        rect.update(Point::new(10, 20));
    ...
    #[test]
    fn test_selection_rect_finish_valid() {
        let mut rect = SelectionRect::new();
        rect.start_at(Point::new(10, 20));
        rect.update(Point::new(50, 60));

        let bounds = rect.finish();
        assert!(bounds.is_some());
        let (min_x, min_y, max_x, max_y) = bounds.unwrap();
        assert_eq!((min_x, min_y, max_x, max_y), (10, 20, 50, 60));
        assert!(!rect.is_active());
    }

    #[test]
    fn test_selection_rect_finish_zero_size() {
        let mut rect = SelectionRect::new();
        rect.start_at(Point::new(10, 20));
        // Don't move - same start and end
        let bounds = rect.finish();
        assert!(bounds.is_none()); // Zero size returns None
    }

    #[test]
    fn test_selection_rect_finish_inactive() {
        let mut rect = SelectionRect::new();
        let bounds = rect.finish();
    ...
    fn test_selection_rect_contains_point() {
        let mut rect = SelectionRect::new();
        rect.start_at(Point::new(10, 10));
        rect.update(Point::new(50, 50));
        ...
        assert!(!rect.contains(Point::new(60, 30)));
    }

    #[test]
    fn test_selection_rect_intersects_rect() {
        let mut rect = SelectionRect::new();
        rect.start_at(Point::new(10, 10));
        rect.update(Point::new(50, 50));

        // Fully inside
        assert!(rect.intersects_rect((20, 20, 40, 40)));
        // Overlapping
        assert!(rect.intersects_rect((0, 0, 20, 20)));
        // Fully outside
        assert!(!rect.intersects_rect((60, 60, 100, 100)));
        // Edge touching
        assert!(rect.intersects_rect((50, 50, 100, 100)));
    }
}
