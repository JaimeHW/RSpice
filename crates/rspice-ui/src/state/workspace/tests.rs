...
/// A persisted source binding keeps its absolute identity on every host.
///
/// The two spellings cover each other: on Windows the Unix path is the one
/// `Path::is_absolute` rejects, and on macOS and Linux it is the Windows path.
/// Whichever host runs this, one binding exercises the syntax-only branch, so a
/// regression that goes back to asking the running host fails here rather than
/// on one runner in CI. Neither may ever be called non-absolute; the foreign
/// one is reported as unavailable on this host instead, without the filesystem
/// probe that would answer about an unrelated local file.
#[cfg(not(target_arch = "wasm32"))]
#[test]
fn either_desktop_binding_syntax_keeps_an_absolute_source_identity() {
    let mut foreign_diagnostics = 0;
    let mut local_diagnostics = 0;

    for source_path in [
        PathBuf::from(r"C:\rspice-portable-binding\amp.cir"),
        PathBuf::from("/rspice-portable-binding/amp.cir"),
    ] {
        let mut workspace = ProjectWorkspace::default();
        let mut libraries = LibraryManager::default();
        workspace.ensure_library_model(&mut libraries);
        ...
        let mut library = Library::new("models");
        let mut cell = Cell::new("amp");
        cell.add_view(View::new("spice", ViewType::Spice).with_path(source_path.clone()));
        library.add_cell(cell);
        libraries.add_library(library);
        ...
        let diagnostic = resolution
            .bindings
            .iter()
            .find_map(|row| row.diagnostic.as_deref())
            .unwrap_or_default()
            .to_owned();
        assert!(
            !diagnostic.contains("does not have an absolute source identity"),
            "{} is an absolute source identity: {diagnostic}",
            source_path.display()
        );
        if diagnostic.contains("foreign-platform source identity") {
            foreign_diagnostics += 1;
        } else {
            assert!(
                diagnostic.contains("cannot read"),
                "{} is reachable syntax on this host: {diagnostic}",
                source_path.display()
            );
            local_diagnostics += 1;
        }
    ...
    assert_eq!(foreign_diagnostics, 1);
    assert_eq!(local_diagnostics, 1);
}
