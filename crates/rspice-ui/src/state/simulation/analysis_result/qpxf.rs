...
pub(super) fn validate_display(
    ...
    let traces = AnalysisResultPayload::qpxf_display_traces(response)?;
    let expected_count: usize = traces
        .iter()
        .map(|t| if t.imaginary.is_some() { 2 } else { 1 })
        .sum();
    if waveforms.len() != expected_count {
        return Err("QPXF saved display differs from its transfer and delay traces".into());
    }
    for trace in traces {
        if let Some(imaginary) = trace.imaginary {
            let magnitude = waveforms
                .iter()
                .find(|w| w.name == format!("|{}|", trace.name))
                .ok_or("QPXF magnitude trace is missing")?;
            let phase = waveforms
                .iter()
                .find(|w| w.name == format!("phase({})", trace.name))
                .ok_or("QPXF phase trace is missing")?;
            let complex = magnitude
                .complex
                .as_ref()
                .ok_or("QPXF complex transfer is missing")?;
            if *magnitude.x != trace.frequencies
                || magnitude.x != phase.x
                || magnitude.unit.as_deref() != Some(trace.unit)
                || phase.unit.as_deref() != Some("°")
                || phase.complex.is_some()
                || complex.source_name != trace.name
                || *complex.real != trace.real
                || *complex.imag != imaginary
                || magnitude.y.len() != trace.real.len()
                || phase.y.len() != trace.real.len()
            {
                return Err("QPXF saved complex trace shape, axis or unit differs".into());
            }
            for (i, (real, imag)) in trace.real.iter().zip(imaginary).enumerate() {
                if magnitude.y[i] != real.hypot(imag)
                    || phase.y[i] != imag.atan2(*real).to_degrees()
                {
                    return Err("QPXF saved projection differs from its complex transfer".into());
                }
            }
        } else {
            let saved = waveforms
                .iter()
                .find(|w| w.name == trace.name)
                .ok_or("QPXF delay segment is missing")?;
            if *saved.x != trace.frequencies
                || *saved.y != trace.real
                || saved.unit.as_deref() != Some("s")
                || saved.complex.is_some()
            {
                return Err(
                    "QPXF saved delay segment differs from its finite phase derivative".into(),
                );
            }
        }
    }
