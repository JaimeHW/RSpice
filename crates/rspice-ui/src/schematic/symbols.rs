...
impl SymbolLibrary {
    ...
    pub fn load_embedded() -> Result<Self, SymbolError> {
        ...
        // Map ComponentType to (SVG filename, Name)
        // Target dimensions are derived from ComponentType::symbol_dimensions()
        // to ensure consistency between terminal positions and symbol rendering.
        let symbol_mappings: &[(ComponentType, &str, &str)] = &[
            // Passive components
        ...
        for (component_type, filename, name) in symbol_mappings {
            ...
            // Get dimensions from ComponentType - this ensures consistency
            // between terminal positions and symbol rendering
