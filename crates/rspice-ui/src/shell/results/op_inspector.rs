...
pub fn show(ui: &mut Ui, state: &mut AppState) {
    ...
    let filter = state.shell.results.op_filter.clone();
    let sort = state.shell.results.op_sort.clone();
    ...
    if let Some(key) = clicked_sort {
        let next = match &state.shell.results.op_sort {
            Some((current, ascending)) if *current == key => Some((key, !*ascending)),
            _ => Some((key, false)),
        };
        state.shell.results.op_sort = next;
    }
}
