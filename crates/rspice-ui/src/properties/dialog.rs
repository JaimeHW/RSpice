...
impl PropertyEditorState {
    ...
    pub fn validate(&mut self) -> bool {
        ...
        if !self.edited.value.is_empty() {
            if let Err(e) = parse_engineering_value(&self.edited.value) {
...
pub fn parse_engineering_value(input: &str) -> Result<f64, String> {
    ...
    let normalized_suffix = suffix_lower
        .replace('\u{00B5}', "u") // MICRO SIGN
        .replace('\u{03BC}', "u"); // GREEK SMALL LETTER MU
...
mod tests {
    ...
    #[test]
    fn test_property_editor_state_default() {
        let state = PropertyEditorState::new();
        assert!(!state.open);
        assert!(state.component_id.is_none());
    }

    #[test]
    fn test_property_editor_open_for() {
        let mut state = PropertyEditorState::new();
        let props = EditedProperties {
            name: "R1".to_string(),
            value: "1k".to_string(),
            model: String::new(),
            parameters: vec![],
        ...
        state.open_for(42, props.clone());

        assert!(state.open);
        assert_eq!(state.component_id, Some(42));
        assert_eq!(state.edited, props);
        assert_eq!(state.original, props);
    }

    #[test]
    fn test_property_editor_cancel() {
        let mut state = PropertyEditorState::new();
        state.open = true;
    ...
    #[test]
    fn test_property_editor_has_changes() {
        let mut state = PropertyEditorState::new();
        let props = EditedProperties {
            name: "R1".to_string(),
            value: "1k".to_string(),
        ...
        state.open_for(42, props);

        assert!(!state.has_changes());

        state.edited.value = "2k".to_string();
        assert!(state.has_changes());
    }

    #[test]
    fn test_property_editor_validate_valid() {
        let mut state = PropertyEditorState::new();
        state.edited.name = "R1".to_string();
        state.edited.value = "1k".to_string();
    ...
    #[test]
    fn test_property_editor_validate_empty_name() {
        let mut state = PropertyEditorState::new();
        state.edited.name = "".to_string();
    ...
    #[test]
    fn test_property_editor_validate_invalid_name_start() {
        let mut state = PropertyEditorState::new();
        state.edited.name = "1R".to_string();

        assert!(!state.validate());
        assert!(state.error_message.unwrap().contains("start with a letter"));
    }

    #[test]
    fn test_property_editor_validate_invalid_value() {
        let mut state = PropertyEditorState::new();
        state.edited.name = "R1".to_string();
        state.edited.value = "invalid".to_string();

        assert!(!state.validate());
    }

    #[test]
    fn test_property_editor_revert() {
        let mut state = PropertyEditorState::new();
        let props = EditedProperties {
            name: "R1".to_string(),
            value: "1k".to_string(),
        ...
        state.edited.value = "999k".to_string();
        state.revert();

        assert_eq!(state.edited.value, "1k");
    }

    // =========================================================================
    // Engineering Value Parser Tests
    // =========================================================================

    /// Helper for floating-point approximate equality (relative tolerance)
    ...
    fn test_parse_engineering_basic() {
        assert_eq!(parse_engineering_value("1").unwrap(), 1.0);
        assert_eq!(parse_engineering_value("100").unwrap(), 100.0);
        assert_eq!(parse_engineering_value("3.3").unwrap(), 3.3);
    }

    #[test]
    fn test_parse_engineering_kilo() {
        assert_eq!(parse_engineering_value("1k").unwrap(), 1000.0);
        assert_eq!(parse_engineering_value("4.7k").unwrap(), 4700.0);
        assert_eq!(parse_engineering_value("10K").unwrap(), 10000.0);
    }

    #[test]
    fn test_parse_engineering_mega() {
        assert_eq!(parse_engineering_value("1meg").unwrap(), 1e6);
        assert_eq!(parse_engineering_value("2.2MEG").unwrap(), 2.2e6);
    }

    #[test]
    fn test_parse_engineering_milli() {
        assert_eq!(parse_engineering_value("1m").unwrap(), 1e-3);
        assert_eq!(parse_engineering_value("100m").unwrap(), 0.1);
    }

    #[test]
    fn test_parse_engineering_micro() {
        assert_approx_eq!(parse_engineering_value("1u").unwrap(), 1e-6);
        assert_approx_eq!(parse_engineering_value("10u").unwrap(), 10e-6);
        assert_approx_eq!(parse_engineering_value("4.7µ").unwrap(), 4.7e-6);
    }

    #[test]
    fn test_parse_engineering_nano() {
        assert_approx_eq!(parse_engineering_value("1n").unwrap(), 1e-9);
        assert_approx_eq!(parse_engineering_value("100n").unwrap(), 100e-9);
    }

    #[test]
    fn test_parse_engineering_pico() {
        assert_eq!(parse_engineering_value("1p").unwrap(), 1e-12);
        assert_eq!(parse_engineering_value("47p").unwrap(), 47e-12);
    }

    #[test]
    fn test_parse_engineering_femto() {
        assert_eq!(parse_engineering_value("1f").unwrap(), 1e-15);
    }

    #[test]
    fn test_parse_engineering_giga_tera() {
        assert_eq!(parse_engineering_value("1g").unwrap(), 1e9);
        assert_eq!(parse_engineering_value("1t").unwrap(), 1e12);
    }

    #[test]
    fn test_parse_engineering_invalid() {
        assert!(parse_engineering_value("").is_err());
        assert!(parse_engineering_value("abc").is_err());
        assert!(parse_engineering_value("1xyz").is_err());
    }

    #[test]
    fn test_parse_engineering_scientific() {
        // Note: scientific notation in the base number works
        assert_eq!(parse_engineering_value("1e3").unwrap(), 1000.0);
        assert_eq!(parse_engineering_value("1.5e-6").unwrap(), 1.5e-6);
    }

    #[test]
    fn test_format_engineering_basic() {
        assert_eq!(format_engineering_value(1000.0), "1k");
        assert_eq!(format_engineering_value(1e6), "1Meg");
        assert_eq!(format_engineering_value(1e-3), "1m");
        assert_eq!(format_engineering_value(1e-6), "1u");
        assert_eq!(format_engineering_value(1e-9), "1n");
        assert_eq!(format_engineering_value(1e-12), "1p");
    }

    #[test]
    fn test_format_engineering_fractional() {
        assert_eq!(format_engineering_value(4700.0), "4.7k");
        assert_eq!(format_engineering_value(2.2e6), "2.2Meg");
    }

    #[test]
    fn test_format_engineering_roundtrip() {
        let values = [1.0, 1e3, 4.7e3, 1e6, 1e-3, 1e-6, 47e-12];
        for &v in &values {
            let formatted = format_engineering_value(v);
            let parsed = parse_engineering_value(&formatted).unwrap();
            assert!(
