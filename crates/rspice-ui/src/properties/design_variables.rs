...
mod tests {
    ...
    // =========================================================================
    // Engineering Number Parsing Tests
    // =========================================================================

    #[test]
    fn test_parse_engineering_basic() {
        assert_eq!(parse_engineering_number("1").unwrap(), 1.0);
        assert_eq!(parse_engineering_number("3.3").unwrap(), 3.3);
        assert_eq!(parse_engineering_number("100").unwrap(), 100.0);
    }

    #[test]
    fn test_parse_engineering_kilo() {
        assert_eq!(parse_engineering_number("1k").unwrap(), 1e3);
        assert_eq!(parse_engineering_number("4.7k").unwrap(), 4.7e3);
    }

    #[test]
    fn test_parse_engineering_micro() {
        let result = parse_engineering_number("1u").unwrap();
        assert!((result - 1e-6).abs() < 1e-15);
    }

    #[test]
    fn test_parse_engineering_nano() {
        let result = parse_engineering_number("180n").unwrap();
        assert!((result - 180e-9).abs() < 1e-18);
    }

    #[test]
    fn test_parse_engineering_meg() {
        assert_eq!(parse_engineering_number("1meg").unwrap(), 1e6);
    }
    ...
    fn test_evaluate_simple_number() {
        let ctx = HashMap::new();
        assert_eq!(evaluate_expression("1k", &ctx).unwrap(), 1e3);
    }

    #[test]
    fn test_evaluate_variable_reference() {
        let mut ctx = HashMap::new();
        ctx.insert("W1".to_string(), 1e-6);

        assert_eq!(evaluate_expression("W1", &ctx).unwrap(), 1e-6);
    }
    ...
    fn test_evaluate_multiplication() {
        ...
        let result = evaluate_expression("2*W1", &ctx).unwrap();
        assert!((result - 2e-6).abs() < 1e-15);
    }

    #[test]
    fn test_evaluate_addition() {
        let ctx = HashMap::new();
        let result = evaluate_expression("1k+500", &ctx).unwrap();
        assert_eq!(result, 1500.0);
    }

    // =========================================================================
    // Design Variables State Tests
    // =========================================================================

    #[test]
    fn test_add_variable() {
        let mut state = DesignVariablesState::new();
        state
            .add_variable("W1".to_string(), "1u".to_string())
            .unwrap();

        assert_eq!(state.variables.len(), 1);
        assert_eq!(state.variables[0].name, "W1");
        assert_eq!(state.get_value("W1"), Some(1e-6));
    }

    #[test]
    fn test_add_duplicate_variable_fails() {
        let mut state = DesignVariablesState::new();
        state
            .add_variable("W1".to_string(), "1u".to_string())
            .unwrap();
        let result = state.add_variable("W1".to_string(), "2u".to_string());
    ...
    fn test_variable_dependency() {
        ...
        state
            ...
            .unwrap();
        state
            .add_variable("W2".to_string(), "2*W1".to_string())
            .unwrap();

        assert_eq!(state.get_value("W1"), Some(1e-6));
        let w2 = state.get_value("W2").unwrap();
        assert!((w2 - 2e-6).abs() < 1e-15);
    }
    ...
    fn test_remove_variable() {
        ...
        state
            ...
            .unwrap();
        state
            .add_variable("W2".to_string(), "2u".to_string())
            .unwrap();

        state.remove_variable(0);

        assert_eq!(state.variables.len(), 1);
        assert_eq!(state.variables[0].name, "W2");
    }
    ...
    fn test_update_expression() {
        let mut state = DesignVariablesState::new();
        state
            .add_variable("R1".to_string(), "1k".to_string())
            .unwrap();

        state.update_expression(0, "2k".to_string());

        assert_eq!(state.get_value("R1"), Some(2e3));
    }
    ...
    fn test_get_context() {
        ...
        state
            ...
            .unwrap();
        state
            .add_variable("L1".to_string(), "180n".to_string())
            .unwrap();

        let ctx = state.get_context();

        assert_eq!(ctx.len(), 2);
        assert_eq!(ctx.get("W1"), Some(&1e-6));
    }
    ...
    fn test_format_engineering() {
        ...
        assert_eq!(format_engineering(180e-9), "180n");
    }
}
