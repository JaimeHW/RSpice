...
mod tests {
    ...
    fn default_plot() -> Rect {
        Rect::from_min_max(Pos2::new(0.0, 0.0), Pos2::new(260.0, 140.0))
    }

    #[test]
    fn test_single_label_avoids_its_anchor_line() {
        let plot = default_plot();
        let requests = [VerticalLabelRequest {
            anchor_x: 120.0,
            size: Vec2::new(44.0, 14.0),
        }];
        let config = VerticalLabelLayoutConfig::default();
        let placements = place_vertical_line_labels(plot, &requests, &[120.0], &[], config);

        assert_eq!(placements.len(), 1);
        let rect = placements[0].rect;
        assert!(rect.min.x >= plot.min.x && rect.max.x <= plot.max.x);
        assert!(!(120.0 >= rect.min.x && 120.0 <= rect.max.x));
    }

    #[test]
    fn test_adjacent_labels_do_not_overlap_or_cross_lines() {
        let plot = default_plot();
        let requests = [
        ...
        let config = VerticalLabelLayoutConfig::default();
        let lines = [120.0, 132.0];
        let placements = place_vertical_line_labels(plot, &requests, &lines, &[], config);
        ...
        for placement in &placements {
            for line in lines {
                assert!(
                    !(line >= placement.rect.min.x && line <= placement.rect.max.x),
                    "line {} should not pass through {:?}",
                    line,
                    placement.rect
                );
            }
    ...
    #[test]
    fn test_soft_obstacles_can_push_label_to_lower_row() {
        let plot = default_plot();
        let requests = [VerticalLabelRequest {
            anchor_x: 130.0,
        ...
        let config = VerticalLabelLayoutConfig {
            preferred_rows: 4,
            ..VerticalLabelLayoutConfig::default()
        };
        // Occupy the entire first lane to force lower-row placement.
        let obstacles = [Rect::from_min_max(
            Pos2::new(plot.min.x, 0.0),
            Pos2::new(plot.max.x, 20.0),
        )];
        let placements = place_vertical_line_labels(plot, &requests, &[130.0], &obstacles, config);

        assert_eq!(placements.len(), 1);
        assert!(
            placements[0].rect.min.y > plot.min.y + config.top_margin,
            "expected label to move below first row to avoid strong top-band obstacles"
        );
    }

    #[test]
    fn test_anchor_near_edge_stays_inside_plot() {
        let plot = default_plot();
        let requests = [VerticalLabelRequest {
            anchor_x: 4.0,
            size: Vec2::new(70.0, 14.0),
        }];
        let placements = place_vertical_line_labels(
            plot,
            &requests,
            &[4.0],
            &[],
        ...
        assert_eq!(placements.len(), 1);
        assert!(placements[0].rect.min.x >= plot.min.x);
        assert!(placements[0].rect.max.x <= plot.max.x);
    }
}
