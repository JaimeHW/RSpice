...
mod tests {
    ...
    fn test_mode_display_names() {
        assert_eq!(SmithChartMode::Impedance.display_name(), "Impedance (Z)");
        assert_eq!(SmithChartMode::Admittance.display_name(), "Admittance (Y)");
        assert_eq!(SmithChartMode::Combined.display_name(), "Combined (Z+Y)");
    }

    #[test]
    fn test_mode_all() {
        let modes = SmithChartMode::all();
    ...
    #[test]
    fn test_sparam_point() {
        let s = Complex::new(0.5, 0.5);
        let point = SParamPoint::new(1e9, s);
        assert_eq!(point.frequency, 1e9);
        assert!((point.magnitude_db() - (-3.01)).abs() < 0.1);
    }

    #[test]
    fn test_sparam_trace() {
        let mut trace = SParamTrace::new("S11");
        trace.add_point(1e9, Complex::new(0.1, 0.1));
        trace.add_point(2e9, Complex::new(0.2, -0.1));

        assert_eq!(trace.points.len(), 2);

        let range = trace.frequency_range().unwrap();
        assert_eq!(range.0, 1e9);
        assert_eq!(range.1, 2e9);
    }

    #[test]
    fn test_sparam_find_by_frequency() {
        let mut trace = SParamTrace::new("S11");
        trace.add_point(1e9, Complex::new(0.1, 0.0));
        trace.add_point(2e9, Complex::new(0.2, 0.0));
        trace.add_point(3e9, Complex::new(0.3, 0.0));

        let point = trace.find_by_frequency(1.8e9).unwrap();
        assert_eq!(point.frequency, 2e9);
    }
    ...
    #[test]
    fn test_marker_impedance() {
        let mut marker = SmithChartMarker::default();
        marker.active = true;
        marker.x = 0.0;
        marker.y = 0.0;

        let z = marker.impedance(50.0);
        assert!(approx_eq(z.r, 50.0));
        assert!(approx_eq(z.x, 0.0));
    }

    #[test]
    fn test_marker_vswr_at_center() {
        let mut marker = SmithChartMarker::default();
        marker.x = 0.0;
        marker.y = 0.0;
        assert!(approx_eq(marker.vswr(), 1.0));
    }

    #[test]
    fn test_marker_vswr_at_edge() {
        let mut marker = SmithChartMarker::default();
        marker.x = 0.5;
        marker.y = 0.0;
        // |Γ| = 0.5, VSWR = (1+0.5)/(1-0.5) = 3
        assert!((marker.vswr() - 3.0).abs() < 0.01);
    }
    ...
    #[test]
    fn test_vswr_circle_radius() {
        let circle = VswrCircle::new(2.0);
        // VSWR=2 -> |Γ| = 1/3
        assert!((circle.radius() - 1.0 / 3.0).abs() < 0.001);
    }

    #[test]
    fn test_vswr_circle_unity() {
        let circle = VswrCircle::new(1.0);
    ...
    fn test_state_default() {
        ...
        assert_eq!(state.mode, SmithChartMode::Impedance);
        assert_eq!(state.z0, 50.0);
        assert!(state.traces.is_empty());
    }

    #[test]
    fn test_state_set_z0() {
        let mut state = SmithChartState::new();
        state.set_z0(75.0);
        assert_eq!(state.z0, 75.0);

        // Invalid Z0 should be ignored
        state.set_z0(-10.0);
        assert_eq!(state.z0, 75.0);
    }

    #[test]
    fn test_state_place_marker() {
        let mut state = SmithChartState::new();
        state.place_marker(0.3, 0.4);

        assert!(state.marker.active);
        assert!(approx_eq(state.marker.x, 0.3));
        assert!(approx_eq(state.marker.y, 0.4));
    }

    #[test]
    fn test_state_place_marker_clamped() {
        let mut state = SmithChartState::new();
        state.place_marker(2.0, 0.0);

        // Should be clamped to unit circle
        let r = (state.marker.x * state.marker.x + state.marker.y * state.marker.y).sqrt();
        assert!(r <= 1.001);
    }

    #[test]
    fn test_state_load_sparam() {
        let mut state = SmithChartState::new();
        let freqs = vec![1e9, 2e9, 3e9];
        let s_re = vec![0.1, 0.2, 0.3];
        let s_im = vec![0.0, 0.1, -0.1];

        state.load_sparam_data("S11", &freqs, &s_re, &s_im);

        assert_eq!(state.traces.len(), 1);
        assert_eq!(state.traces[0].points.len(), 3);
    }

    // =========================================================================
    // Circle Calculation Tests
    // =========================================================================

    #[test]
    fn test_constant_r_zero() {
        let (cx, cy, r) = constant_r_circle(0.0);
    ...
    #[test]
    fn test_constant_r_one() {
        let (cx, cy, r) = constant_r_circle(1.0);
        assert!(approx_eq(cx, 0.5));
        assert!(approx_eq(cy, 0.0));
        assert!(approx_eq(r, 0.5));
    }

    #[test]
    fn test_constant_r_infinity() {
        // As r -> ∞, center -> (1, 0), radius -> 0
        let (cx, cy, r) = constant_r_circle(1000.0);
        assert!(cx > 0.99);
        assert!(approx_eq(cy, 0.0));
        assert!(r < 0.01);
    }
    ...
    #[test]
    fn test_constant_x_one() {
        let (cx, cy, r) = constant_x_circle(1.0).unwrap();
        assert!(approx_eq(cx, 1.0));
        assert!(approx_eq(cy, 1.0));
        assert!(approx_eq(r, 1.0));
    }

    #[test]
    fn test_constant_x_negative() {
        let (cx, cy, r) = constant_x_circle(-1.0).unwrap();
        assert!(approx_eq(cx, 1.0));
        assert!(approx_eq(cy, -1.0));
        assert!(approx_eq(r, 1.0));
    ...
    #[test]
    fn test_constant_x_zero_returns_none() {
        assert!(constant_x_circle(0.0).is_none());
    }

    #[test]
    fn test_standard_values() {
        assert!(standard_r_values().len() >= 5);
        assert!(standard_x_values().len() >= 5);
    }
}
