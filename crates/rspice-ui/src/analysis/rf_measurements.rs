...
mod tests {
    ...
    #[test]
    fn test_s_param_from_mag_phase() {
        let s = SParameter::from_mag_phase(0.5, 45.0);
        assert!((s.magnitude() - 0.5).abs() < 1e-10);
        assert!((s.phase_deg() - 45.0).abs() < 1e-10);
    }

    #[test]
    fn test_s_param_from_db_phase() {
        let s = SParameter::from_db_phase(-6.0, 0.0);
        // -6 dB ≈ 0.5 linear
        assert!((s.magnitude() - 0.5012).abs() < 0.001);
    }

    #[test]
    fn test_s_param_return_loss() {
        let s = SParameter::from_db_phase(-10.0, 0.0);
        assert!((s.return_loss() - 10.0).abs() < 0.001);
    }

    #[test]
    fn test_s_param_vswr() {
        let s = SParameter::from_db_phase(-20.0, 0.0);
        // |Γ| = 0.1, VSWR = 1.22
        assert!((s.vswr() - 1.22).abs() < 0.01);
    }

    // =========================================================================
    // LargeSigSParams Tests
    // =========================================================================

    #[test]
    fn test_large_sig_creation() {
        let lss = LargeSigSParams::new(1e9, -10.0);
        assert_eq!(lss.frequency, 1e9);
        assert_eq!(lss.input_power, -10.0);
    }

    #[test]
    fn test_large_sig_gain() {
        let mut lss = LargeSigSParams::new(1e9, -10.0);
        lss.s21 = SParameter::from_db_phase(10.0, 0.0);

        assert!((lss.gain_db() - 10.0).abs() < 0.1);
    }
    ...
    #[test]
    fn test_power_sweep_creation() {
        let sweep = PowerSweep::new(1e9);
        assert_eq!(sweep.frequency, 1e9);
        assert!(sweep.pin.is_empty());
    }

    #[test]
    fn test_power_sweep_add_point() {
        let mut sweep = PowerSweep::new(1e9);
        sweep.add_point(-20.0, -10.0); // 10 dB gain

        assert_eq!(sweep.pin.len(), 1);
        assert_eq!(sweep.gain[0], 10.0);
    }
    ...
    fn test_p1db() {
        ...
        let p1db = sweep.p1db_in().unwrap();
        assert!((p1db - 0.0).abs() < 0.5);
    }

    // =========================================================================
    // IntermodMeasurement Tests
    // =========================================================================

    #[test]
    fn test_intermod_creation() {
        let im = IntermodMeasurement::new(1e9, 1.001e9, -20.0);
        assert_eq!(im.f1, 1e9);
        assert_eq!(im.input_power, -20.0);
    }

    #[test]
    fn test_iip3_calculation() {
        let im = IntermodMeasurement::new(1e9, 1.001e9, -20.0).with_powers(0.0, -60.0);

        // C/I = 60 dB
        // IIP3 = -20 + 60/2 = 10 dBm
        assert!((im.iip3() - 10.0).abs() < 0.1);
    }

    #[test]
    fn test_oip3_calculation() {
        let im = IntermodMeasurement::new(1e9, 1.001e9, -20.0).with_powers(0.0, -60.0);

        // OIP3 = 0 + 60/2 = 30 dBm
        assert!((im.oip3() - 30.0).abs() < 0.1);
    }

    // =========================================================================
    // MixerMeasurements Tests
    // =========================================================================

    #[test]
    fn test_mixer_creation() {
        let mixer = MixerMeasurements::new(1e9, 2.4e9);
        assert_eq!(mixer.f_if, 1.4e9);
        assert!(mixer.is_downconvert());
    }

    #[test]
    fn test_mixer_sfdr() {
        let mut mixer = MixerMeasurements::new(1e9, 2.4e9);
        mixer.iip3 = Some(10.0);
        mixer.noise_figure = Some(10.0);

        let sfdr = mixer.sfdr().unwrap();
        // SFDR = (2/3) * (10 + 174 - 10) = 116 dB
        assert!((sfdr - 116.0).abs() < 0.5);
    }
}
