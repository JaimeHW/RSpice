...
mod tests {
    ...
    #[test]
    fn test_open_model_browser_enables_browse_only_mode() {
        let mut state = AppState::default();
        state.model_browser_state.open = false;
        state.model_browser_state.browse_only = false;

        open_model_browser(&mut state);

        assert!(state.model_browser_state.open);
        assert!(state.model_browser_state.browse_only);
    }

    #[test]
    fn test_open_optimization_engine_opens_simulation_dialog_on_optimizer_tab() {
        let mut state = AppState::default();
        state.dialogs.simulation_dialog = false;
        state.dialogs.sim_active_tab = simulation_analysis_tabs::TAB_DC_OP;

        open_optimization_engine(&mut state);

        assert!(state.dialogs.simulation_dialog);
        assert_eq!(
            state.dialogs.sim_active_tab,
            simulation_analysis_tabs::TAB_OPTIMIZATION
        );
    }

    #[test]
    fn test_enable_cross_probe_opens_signal_browser_and_logs_message() {
        let mut state = AppState::default();
        state.panels.signal_browser = false;

        enable_cross_probe(&mut state);

        assert!(state.panels.signal_browser);
        assert!(
            state
                .console_messages
                .iter()
                .any(|msg| msg.message.contains("Cross-probe enabled")),
            "expected cross-probe confirmation message"
        );
    }

    #[test]
    fn test_run_design_rule_check_populates_dialog_results() {
        let mut state = AppState::default();
        state.dialogs.drc_dialog = false;
        state.dialogs.drc_results = None;

        run_design_rule_check(&mut state);

        assert!(state.dialogs.drc_dialog);
        assert!(state.dialogs.drc_results.is_some());
        assert!(
            state
                .console_messages
                .iter()
                .any(|msg| msg.message.contains("DRC")),
            "expected DRC summary message"
        );
    }

    #[test]
    fn test_run_electrical_rule_check_populates_dialog_results() {
        let mut state = AppState::default();
        state.dialogs.drc_dialog = false;
        state.dialogs.drc_results = None;

        run_electrical_rule_check(&mut state);
        ...
        assert!(
            state
                .console_messages
                .iter()
                .any(|msg| msg.message.contains("ERC")),
            "expected ERC summary message"
        );
    }
}
