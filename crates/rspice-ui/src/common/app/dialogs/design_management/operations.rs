...
impl RSpiceApp {
    pub(super) fn commit_design_management_subflow(
                ...
                == MoveHierarchyEffect::CreateChildCell
        {
            ...
            if !crate::common::app::dialogs::hierarchy::create::create_hierarchy_available(
                &self.state,
            ) {
                return Err(
            ...
            crate::common::app::dialogs::hierarchy::create::open_create_hierarchy_dialog(
                &mut self.state,
            );
            return Ok(());
        }
        ...
        let (tab, receipt) = match page {
            ...
            DesignManagementPage::MoveSelection => {
                ...
                let connectivity =
                    crate::common::app::dialogs::hierarchy::create::selected_component_sheet_move_plan(
                        &self.state,
                    )?;
