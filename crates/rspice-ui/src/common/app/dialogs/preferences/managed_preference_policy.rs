...
fn resolved_policy_rows(category: PreferenceCategory, state: &AppState) -> Vec<ResolvedPolicyRow> {
    match category {
        ...
        PreferenceCategory::Schematic => {
            ...
            let net_naming = rows
                .iter()
                .position(|row| row.setting == "Net naming policy")
                .expect("schematic policy projection includes net naming");
            rows.insert(
                net_naming,
                toggle_policy_row(
                    state,
                    "Edit-in-place hierarchy",
                    TogglePreference::HierarchicalEditInPlace,
                    "User default",
                    "hierarchy edit context",
                ),
            );
            rows
        }
...
mod tests {
    ...
    fn policy_projection_is_truthful_without_claiming_an_organization_overlay() {
        ...
        let edit_in_place = schematic
            .iter()
            .find(|row| row.setting == "Edit-in-place hierarchy")
            .expect("runtime-backed hierarchy preference is projected");
        assert_eq!(edit_in_place.resolved_value, "Enabled");
        assert_eq!(edit_in_place.source, "User default");
        assert_eq!(edit_in_place.reason, "hierarchy edit context");
        assert!(!edit_in_place.locked);
        let selection = schematic
            .iter()
            .position(|row| row.setting == "Selection crossing policy")
            .expect("selection crossing policy");
        let edit_in_place = schematic
            .iter()
            .position(|row| row.setting == "Edit-in-place hierarchy")
            .expect("edit-in-place hierarchy policy");
        let net_naming = schematic
            .iter()
            .position(|row| row.setting == "Net naming policy")
            .expect("net naming policy");
        assert!(selection < edit_in_place && edit_in_place < net_naming);
    ...
    #[test]
    fn hierarchy_policy_projection_tracks_the_runtime_toggle() {
        ...
        let rows = resolved_policy_rows(PreferenceCategory::Schematic, &state);
        let hierarchy = rows
            .iter()
            .find(|row| row.setting == "Edit-in-place hierarchy")
            .expect("hierarchy preference policy");
        assert_eq!(hierarchy.resolved_value, "Disabled");
    }
