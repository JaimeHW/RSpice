...
thread_local! {
    static BROWSER_SAVED_VIEW_IMPORT: std::cell::RefCell<Option<(
        crate::common::browser_file_import::TextImportToken,
        Result<Option<crate::common::browser_file_import::PickedTextFile>, String>,
...
impl RSpiceApp {
    pub(in crate::common::app) fn render_engineering_table_dialog(&mut self, ctx: &Context) {
        #[cfg(target_arch = "wasm32")]
        ...
        let result = (|| -> Result<(), String> {
            ...
            match format {
                EngineeringTableExportFormat::CsvSchema => {
                    ...
                    #[cfg(target_arch = "wasm32")]
                    {
                        let bytes = crate::common::export_workflow::deterministic_stored_zip(&[
                ...
                EngineeringTableExportFormat::Tsv => {
                    ...
                    if include_metadata {
                        ...
                        {
                            let bytes =
                                crate::common::export_workflow::deterministic_stored_zip(&[
    ...
    fn import_engineering_saved_view(&mut self) {
        ...
        #[cfg(target_arch = "wasm32")]
        {
            ...
            let token = match crate::common::browser_file_import::try_begin_text_import(
                crate::common::browser_file_import::BrowserTextImportKind::EngineeringTableView,
            ) {
                Ok(token) => token,
                Err(error) => {
                    self.state.dialogs.engineering_table.error = Some(error);
    ...
    fn consume_engineering_view_import(&mut self) {
        ...
        if self.state.dialogs.engineering_table.saved_import_token != Some(token)
            || !crate::common::browser_file_import::finish_text_import(token)
        {
            return;
        }
