...
use crate::common::project_workflow::{SaveRequestOutcome, save_active_for_continuation};
use crate::state::{
...
use crate::common::app::dialogs::check_and_save_validation::CheckAndSaveValidationReport;
use crate::common::app::dialogs::operation_primitives::{
    CONTEXT_WIDTH, SURFACE_HEIGHT, TRANSACTION_HEIGHT, ellipsized_text, operation_steps,
    paint_body_dividers,
};
use crate::common::app::dialogs::review_primitives::{input_field, purpose_line, read_only_field};
use crate::common::app::dialogs::schematic_command::{DISCARD_DETAIL, DISCARD_TITLE};
...
impl RSpiceApp {
    pub(in crate::common::app) fn render_check_and_save_dialog(&mut self, ctx: &Context) {
    ...
    fn commit_check_and_save(&mut self) {
        ...
        if original_journal.is_empty()
            && let Some(accepted) =
                crate::common::project_lifecycle::accepted_active_schematic(&self.state)
    ...
    #[cfg(target_arch = "wasm32")]
    pub(in crate::common::app) fn handle_check_and_save_continuation(
        &mut self,
        ...
        match event {
            crate::common::project_workflow::SaveContinuationEvent::Saved(_) => {
            ...
            crate::common::project_workflow::SaveContinuationEvent::SavedWithNewerChanges(_) => {
                if let Some(id) = revision_id {
            ...
            crate::common::project_workflow::SaveContinuationEvent::Cancelled(_) => {
                rollback_pending_validated_save(
                    &mut self.state,
                    "The browser save was cancelled. No validated revision was committed.",
                );
            }
            crate::common::project_workflow::SaveContinuationEvent::Conflict(_) => {
                rollback_pending_validated_save(
                    &mut self.state,
            ...
            crate::common::project_workflow::SaveContinuationEvent::Failed(_, error) => {
                rollback_pending_validated_save(
                    &mut self.state,
                    &format!("The validated revision could not be saved: {error}"),
                );
            }
            crate::common::project_workflow::SaveContinuationEvent::PublishedButNotAdopted(
                _,
                error,
            ) => {
                rollback_pending_validated_save(
...
fn rollback_validated_save(
    ...
    let detail = match rollback_result {
        Ok(()) => match crate::common::project_lifecycle::refresh_registry(state) {
            Ok(()) => message.to_owned(),
            Err(error) => format!(
...
fn checks_review_description(report: &CheckAndSaveValidationReport) -> String {
    ...
    for finding in report.blockers().iter().chain(report.advisories()) {
        ...
        detail.push_str(match finding.level {
            crate::common::app::dialogs::check_and_save_validation::CheckAndSaveFindingLevel::Blocker => "BLOCKER",
            crate::common::app::dialogs::check_and_save_validation::CheckAndSaveFindingLevel::Advisory => "ADVISORY",
        });
        detail.push_str(" · ");
