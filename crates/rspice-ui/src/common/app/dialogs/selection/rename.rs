...
use crate::common::app::{AppState, ConsoleMessage, RSpiceApp, RenameSelectionTarget};
...
fn selected_rename_target(state: &AppState) -> Option<RenameSelectionTarget> {
    ...
    if let Some(target) = crate::common::app::selected_named_net_target(state) {
...
impl RSpiceApp {
    pub(in crate::common::app) fn render_rename_selection_dialog(&mut self, ctx: &Context) {
...
fn validate_draft(
    ...
    match target {
        ...
        RenameSelectionTarget::NamedNet(expected) => {
            let candidate =
                match crate::common::app::validate_named_net_rename(schematic, expected, candidate)
                {
