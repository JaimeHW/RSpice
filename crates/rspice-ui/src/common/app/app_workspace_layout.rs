...
mod tests {
    ...
    #[test]
    fn test_compute_bottom_tab_width_enforces_minimum() {
        let width = compute_bottom_tab_width(0.0);
        assert_eq!(width, BOTTOM_TAB_MIN_WIDTH);
    }

    #[test]
    fn test_compute_bottom_tab_width_scales_with_text_width() {
        let width = compute_bottom_tab_width(100.0);
        assert_eq!(width, 100.0 + BOTTOM_TAB_PADDING * 2.0);
    }

    #[test]
    fn test_should_show_project_browser_respects_results_precedence() {
        let mut panels = PanelVisibility::default();
        panels.project_browser = true;
        panels.results_browser = false;
        assert!(should_show_project_browser(&panels));

        panels.results_browser = true;
        assert!(!should_show_project_browser(&panels));
    }

    #[test]
    fn test_separator_line_bounds_extends_edges() {
        let rect = egui::Rect::from_min_max(egui::pos2(10.0, 5.0), egui::pos2(30.0, 20.0));
