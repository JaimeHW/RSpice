...
impl RSpiceApp {
    ...
    pub(in crate::common::app) fn handle_image_export(&mut self, ctx: &Context) {
    ...
    fn save_browser_canvas_png(&mut self, ctx: &Context) {
        ...
        match result {
            Ok(()) => {
                ...
                self.state
                    .push_user_message(crate::common::app::ConsoleMessage::info(message));
            }
            Err(error) => {
                let message = format!("PNG export failed: {error}");
                self.state
                    ...
                    .error_with_title(ctx, "PNG export failed", error.to_string());
                self.state
                    .push_user_message(crate::common::app::ConsoleMessage::error(message));
            }
        }
    }
}
