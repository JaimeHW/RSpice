...
use super::app_check_and_save_validation::CheckAndSaveValidationReport;
use super::app_mockup_operation::{
    CONTEXT_WIDTH, SURFACE_HEIGHT, TRANSACTION_HEIGHT, ellipsized_text, operation_steps,
    paint_body_dividers,
};
use super::app_mockup_review::{input_field, purpose_line, read_only_field};
...
impl RSpiceApp {
    pub(super) fn render_check_and_save_dialog(&mut self, ctx: &Context) {
    ...
    #[cfg(target_arch = "wasm32")]
    pub(super) fn handle_check_and_save_continuation(
        &mut self,
...
fn checks_review_description(report: &CheckAndSaveValidationReport) -> String {
    ...
    for finding in report.blockers().iter().chain(report.advisories()) {
        ...
        detail.push_str(match finding.level {
            super::app_check_and_save_validation::CheckAndSaveFindingLevel::Blocker => "BLOCKER",
            super::app_check_and_save_validation::CheckAndSaveFindingLevel::Advisory => "ADVISORY",
        });
        detail.push_str(" · ");
