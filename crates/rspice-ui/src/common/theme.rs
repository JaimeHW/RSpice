...
mod tests {
    ...
    fn test_dark_theme_creation() {
        ...
        assert_eq!(theme.name, "RSpice Dark");
        assert!(theme.is_dark);
        assert_eq!(theme.canvas_bg, background_colors::CANVAS);
    }

    #[test]
    fn test_light_theme_creation() {
        let theme = RSpiceTheme::light();
        assert_eq!(theme.name, "RSpice Light");
        assert!(!theme.is_dark);
    }

    #[test]
    fn test_high_contrast_theme_creation() {
        let theme = RSpiceTheme::high_contrast();
        assert_eq!(theme.name, "High Contrast");
        assert!(theme.is_dark);
        assert_eq!(theme.canvas_bg, Color32::BLACK);
    }

    #[test]
    fn test_wire_colors_are_distinct() {
        // Wire colors must be visually distinct for usability
        assert_ne!(wire_colors::DEFAULT, wire_colors::SELECTED);
        assert_ne!(wire_colors::DEFAULT, wire_colors::HIGHLIGHTED);
        assert_ne!(wire_colors::POWER, wire_colors::GROUND);
    }

    #[test]
    fn test_ui_colors_contrast() {
        // Text must have sufficient contrast with backgrounds
        // (simplified check - real accessibility would use WCAG calculations)
        let text = ui_colors::TEXT_PRIMARY;
        let bg = background_colors::PANEL;

        let text_brightness = (text.r() as u32 + text.g() as u32 + text.b() as u32) / 3;
        let bg_brightness = (bg.r() as u32 + bg.g() as u32 + bg.b() as u32) / 3;

        // Text should be significantly brighter than background in dark mode
        assert!(text_brightness > bg_brightness + 100);
    }

    #[test]
    fn test_default_theme_is_dark() {
        let theme = RSpiceTheme::default();
        assert!(theme.is_dark, "EDA tools should default to dark mode");
    }

    #[test]
    fn test_accent_color_visibility() {
        // Accent color should be visible against both canvas and panel backgrounds
        let accent = ui_colors::ACCENT;
        let canvas = background_colors::CANVAS;
        let panel = background_colors::PANEL;

        // Simple brightness difference check
        let accent_bright = (accent.r() as i32 + accent.g() as i32 + accent.b() as i32) / 3;
        let canvas_bright = (canvas.r() as i32 + canvas.g() as i32 + canvas.b() as i32) / 3;
        let panel_bright = (panel.r() as i32 + panel.g() as i32 + panel.b() as i32) / 3;

        assert!(
            (accent_bright - canvas_bright).abs() > 50,
            "Accent must contrast with canvas"
        );
        assert!(
            (accent_bright - panel_bright).abs() > 50,
            "Accent must contrast with panel"
        );
    }
}
