...
fn generate_component_line(
    ...
    let line = match comp.kind {
        ...
        // Power Electronics - VDMOS (same as MOSFET format)
        ComponentType::NVdmos | ComponentType::PVdmos => {
            if nodes.len() >= 4 {
                let model = if comp.params.is_empty() {
                    if comp.kind == ComponentType::NVdmos {
                        "NVDMOS"
                    } else {
                        "PVDMOS"
                    }
                ...
                return Err(format!(
                    "{}: VDMOS needs at least 3 terminals (D, G, S)",
                    comp.name
                ));
            }
        }

        // Saturable Inductor - same format as regular inductor
        ComponentType::SaturableInductor => {
            if nodes.len() >= 2 {
                format!("{} {} {} {}", comp.name, nodes[0], nodes[1], comp.value)
            } else {
                return Err(format!(
                    "{}: Saturable inductor needs 2 terminals",
                    comp.name
                ));
            }
        }
