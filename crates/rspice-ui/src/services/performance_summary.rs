...
mod tests {
    ...
    fn test_metric_creation() {
        let metric = PerformanceMetric::new(MetricType::DcGain);
        assert_eq!(metric.name, "DC Gain");
        assert_eq!(metric.unit, "dB");
        assert_eq!(metric.status, MetricStatus::Pending);
    }

    #[test]
    fn test_metric_with_value() {
        let metric = PerformanceMetric::new(MetricType::PhaseMargin)
            .with_value(65.0)
            .with_min(45.0);

        assert_eq!(metric.value, Some(65.0));
        assert_eq!(metric.status, MetricStatus::Measured);
    }

    #[test]
    fn test_spec_check_pass() {
        let mut metric = PerformanceMetric::new(MetricType::PhaseMargin)
            .with_value(65.0)
            .with_min(45.0);
    ...
    #[test]
    fn test_spec_check_fail() {
        let mut metric = PerformanceMetric::new(MetricType::PhaseMargin)
            .with_value(30.0)
            .with_min(45.0);

        metric.check_spec();
        assert_eq!(metric.status, MetricStatus::Fail);
    }

    #[test]
    fn test_summary_init() {
        let mut summary = PerformanceSummary::new("Amplifier");
        summary.init_amplifier_metrics();

        assert_eq!(summary.len(), 10);
        assert!(summary.get(MetricType::DcGain).is_some());
        assert!(summary.get(MetricType::PhaseMargin).is_some());
    }

    #[test]
    fn test_engineering_format() {
        assert_eq!(format_engineering(1.5e-6, "V"), "1.500μV");
        assert_eq!(format_engineering(10.5e6, "Hz"), "10.50MHz");
        assert_eq!(format_engineering(65.0, "°"), "65.00°");
    }

    #[test]
    fn test_bandwidth_extraction() {
        let freq = vec![1e3, 1e4, 1e5, 1e6, 1e7];
        let mag_db = vec![60.0, 60.0, 57.0, 40.0, 20.0];

        let bw = extract_bandwidth_3db(&freq, &mag_db);
        assert!(bw.is_some());
        let bw = bw.unwrap();
        // Should be between 1e4 and 1e5 (around 57dB = 60-3)
        assert!(bw > 1e4 && bw < 1e6);
    }

    #[test]
    fn test_ugbw_extraction() {
        let freq = vec![1e3, 1e4, 1e5, 1e6, 1e7];
        let mag_db = vec![60.0, 40.0, 20.0, 0.0, -20.0];

        let ugbw = extract_ugbw(&freq, &mag_db);
        assert!(ugbw.is_some());
        let ugbw = ugbw.unwrap();
        // Should be around 1MHz where gain = 0dB
        assert!(ugbw >= 1e5 && ugbw <= 1e7);
    }

    #[test]
    fn test_corner_metrics() {
        let mut summary = PerformanceSummary::new("Corner Analysis");

        summary.add_corner_metric(
            "TT",
            PerformanceMetric::new(MetricType::DcGain).with_value(60.0),
        );
        summary.add_corner_metric(
            "FF",
            PerformanceMetric::new(MetricType::DcGain).with_value(62.0),
        );
        summary.add_corner_metric(
            "SS",
            PerformanceMetric::new(MetricType::DcGain).with_value(58.0),
        );

        assert_eq!(summary.corners().len(), 3);
        assert!(summary.corner_metrics("TT").is_some());
    }
}
