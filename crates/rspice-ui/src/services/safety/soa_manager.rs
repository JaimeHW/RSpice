...
mod tests {
    ...
    #[test]
    fn test_soa_limit_checking() {
        let mut manager = SoAManager::new();

        let mut nmos_limit = SoADefinition::new();
        nmos_limit.add_limit(SoALimit {
            parameter: SoAParameter::Vds,
            max_value: 3.3,
            min_value: None,
            max_duration: None,
            unit: "V".to_string(),
            description: "Max Drain-Source Voltage".to_string(),
        });

        manager.register_device("M1", nmos_limit);

        let mut values = HashMap::new();
        let mut m1_values = HashMap::new();

        // Test normal
        m1_values.insert(SoAParameter::Vds, 1.8);
        values.insert("M1".to_string(), m1_values.clone());
        manager.check_point(0.0, &values);
        assert_eq!(manager.violations().len(), 0);

        // Test warning (>90%)
        m1_values.insert(SoAParameter::Vds, 3.1);
        values.insert("M1".to_string(), m1_values.clone());
        manager.check_point(1e-9, &values);
        assert_eq!(manager.violations().len(), 1);
        assert_eq!(manager.violations()[0].severity, ViolationSeverity::Warning);

        // Test violation (>100%)
        m1_values.insert(SoAParameter::Vds, 3.5);
        values.insert("M1".to_string(), m1_values.clone());
