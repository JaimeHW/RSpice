...
fn reliability_output_ownership_restore_preserves_effective_controls_and_history() {
    ...
    let (dynamic, _) = plan.insert(AnalysisKind::Reliability).unwrap();
    let (empty, _) = plan.insert(AnalysisKind::Reliability).unwrap();
    let (transient, _) = plan.insert(AnalysisKind::Transient).unwrap();
    for (id, enabled) in [(dc, false), (dynamic, true), (empty, false)] {
        plan.edit(id, |draft| {
            let AnalysisDraft::Reliability(setup) = draft else {
                unreachable!()
            };
            setup.study.transient_stress = enabled;
        })
        .unwrap();
    }
    let legacy: AnalysisNumericOverride = serde_json::from_str(
    ...
    .unwrap();
    for id in [dc, dynamic, transient] {
        let index = plan.index_of(id).unwrap();
        plan.instances[index].numeric_override = Some(legacy.clone());
    }
    let index = plan.index_of(empty).unwrap();
    plan.instances[index].numeric_override =
        Some(serde_json::from_str(r#"{"output_time_points":[0.000001]}"#).unwrap());
    let mut restored: SimulationPlan = serde_json::from_str(&snapshot(&plan)).unwrap();
    let receipts = serde_json::to_value(&restored.receipts).unwrap();
    let drafts: Vec<_> = restored
        .instances
        .iter()
        .map(|instance| serde_json::to_value(&instance.draft).unwrap())
        .collect();
    restored.prepare_after_restore();
    for id in [dc, dynamic] {
        let record = restored.instance(id).unwrap().numeric_override().unwrap();
        assert!(record.value(NumericOverrideOption::Reltol).is_some());
        assert_eq!(
            record.value(NumericOverrideOption::Itl4).is_some(),
            id == dynamic
        );
        for option in [
            NumericOverrideOption::StrobeInterval,
    ...
    let error = restored
        .edit(dynamic, |draft| {
            ...
            setup.study.transient_stress = false;
        })
        .unwrap_err();
    assert!(error.to_string().contains("Transient stress"));
    assert_eq!(snapshot(&restored), once, "incompatible edits are atomic");
    restored.set_numeric_override(dynamic, None).unwrap();
    restored
        .edit(dynamic, |draft| {
            let AnalysisDraft::Reliability(setup) = draft else {
    ...
    restored
        .edit(dc, |_| ())
        .expect("restored constant-stress mission remains editable");
}
