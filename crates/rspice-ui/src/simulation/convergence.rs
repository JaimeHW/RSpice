...
mod tests {
    ...
    // =========================================================================
    // ConvergenceAlgorithm Tests
    // =========================================================================

    #[test]
    fn test_algorithm_display() {
        assert_eq!(
            ConvergenceAlgorithm::Newton.display_name(),
            "Standard Newton-Raphson"
        );
        assert_eq!(
            ConvergenceAlgorithm::NewtonGmin.display_name(),
            "Newton with GMIN Stepping"
        );
    }
    ...
    // =========================================================================
    // ToleranceSettings Tests
    // =========================================================================

    #[test]
    fn test_tolerance_default() {
        let tol = ToleranceSettings::default();
        assert_eq!(tol.reltol, 1e-3);
        assert_eq!(tol.abstol, 1e-12);
    }

    #[test]
    fn test_tolerance_tight() {
        let tol = ToleranceSettings::tight();
        assert!(tol.reltol < ToleranceSettings::default().reltol);
    }

    #[test]
    fn test_tolerance_loose() {
        let tol = ToleranceSettings::loose();
        assert!(tol.reltol > ToleranceSettings::default().reltol);
    }

    #[test]
    fn test_tolerance_scale() {
        let mut tol = ToleranceSettings::default();
        let original_reltol = tol.reltol;
        tol.scale_by(0.1);
        assert!((tol.reltol - original_reltol * 0.1).abs() < 1e-15);
    }
    ...
    #[test]
    fn test_gmin_default() {
        let cfg = GminConfig::default();
        assert!(cfg.enabled);
        assert!(cfg.gmin_initial > cfg.gmin_final);
    }

    #[test]
    fn test_gmin_steps_needed() {
        let cfg = GminConfig {
            gmin_initial: 1e-3,
            gmin_final: 1e-12,
            reduction_factor: 10.0,
        ...
        // 1e-3 to 1e-12 with 10x reduction = 9 steps
        assert_eq!(cfg.steps_needed(), 9);
    }

    #[test]
    fn test_gmin_at_step() {
        let cfg = GminConfig::default();
        assert_eq!(cfg.gmin_at_step(0), cfg.gmin_initial);

        let step1 = cfg.gmin_at_step(1);
        assert!(step1 < cfg.gmin_initial);
    }

    // =========================================================================
    // SourceSteppingConfig Tests
    // =========================================================================

    #[test]
    fn test_source_stepping_default() {
        let cfg = SourceSteppingConfig::default();
        assert_eq!(cfg.start_fraction, 0.0);
        assert!(cfg.enabled);
    }

    #[test]
    fn test_source_stepping_fraction() {
        let cfg = SourceSteppingConfig {
            start_fraction: 0.0,
            step_size: 0.25,
            max_steps: 10,
            enabled: true,
        ...
        assert_eq!(cfg.fraction_at_step(0), 0.0);
        assert_eq!(cfg.fraction_at_step(2), 0.5);
        assert_eq!(cfg.fraction_at_step(4), 1.0);
        assert_eq!(cfg.fraction_at_step(10), 1.0); // Capped
    }

    #[test]
    fn test_source_stepping_steps_to_full() {
        let cfg = SourceSteppingConfig {
            start_fraction: 0.0,
            step_size: 0.25,
            max_steps: 10,
        ...
        assert_eq!(cfg.steps_to_full(), 4);
    }

    // =========================================================================
    // InitialCondition Tests
    // =========================================================================

    #[test]
    fn test_ic_creation() {
        let ic = InitialCondition::ic("vout", 1.8);
        assert_eq!(ic.node, "vout");
        assert_eq!(ic.value, 1.8);
        assert_eq!(ic.ic_type, IcType::Ic);
    }

    #[test]
    fn test_nodeset_creation() {
        let ns = InitialCondition::nodeset("vbias", 0.9);
        assert_eq!(ns.ic_type, IcType::Nodeset);
    }

    // =========================================================================
    // ConvergenceOptions Tests
    // =========================================================================

    #[test]
    fn test_options_default() {
        let opts = ConvergenceOptions::new();
        assert_eq!(opts.algorithm, ConvergenceAlgorithm::Newton);
        assert!(!opts.fallback_algorithms.is_empty());
    }

    #[test]
    fn test_options_high_precision() {
        let opts = ConvergenceOptions::high_precision();
        assert!(opts.tolerances.reltol < ConvergenceOptions::default().tolerances.reltol);
        assert!(opts.max_iterations > ConvergenceOptions::default().max_iterations);
    }

    #[test]
    fn test_options_add_ic() {
        let mut opts = ConvergenceOptions::new();
        opts.add_ic(InitialCondition::ic("vout", 1.8));
        opts.add_nodeset("vbias", 0.9);

        assert_eq!(opts.initial_conditions.len(), 2);
    }

    #[test]
    fn test_options_active_ics() {
        let mut opts = ConvergenceOptions::new();
        opts.add_ic(InitialCondition::ic("v1", 1.0));

        let mut disabled_ic = InitialCondition::ic("v2", 2.0);
        disabled_ic.enabled = false;
        opts.add_ic(disabled_ic);

        assert_eq!(opts.active_ics().len(), 1);
    }

    #[test]
    fn test_options_to_spice() {
        let mut opts = ConvergenceOptions::new();
        opts.add_nodeset("vbias", 0.9);

        let spice = opts.to_spice_options();
        assert!(spice.contains("RELTOL"));
        assert!(spice.contains("ABSTOL"));
        assert!(spice.contains(".NODESET"));
    }

    #[test]
    fn test_options_custom() {
        let mut opts = ConvergenceOptions::new();
        opts.set_option("METHOD", "GEAR");

        assert_eq!(opts.custom_options.get("METHOD"), Some(&"GEAR".to_string()));
    }
}
