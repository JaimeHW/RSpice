...
#[test]
fn reliability_mission_saved_selection_keeps_primary_stress_and_restores_deferred_curves() {
    let original = SimulationResult::reliability_mission_retained_test_fixture();
    assert!(original.success, "{:?}", original.error_message);
    let Some(AnalysisResultPayload::ReliabilityMission { response }) = &original.result_payload
    else {
        panic!("wrong family");
    };
    let r = &response.stress.request;
    ...
    let trace = original
        .waveforms
        .iter()
        .find(|w| w.name.starts_with("Shift("))
        .unwrap();
    let mut output = output(
        SavedOutputKind::DerivedExpression,
        "Saved aging",
        &serde_json::to_string(&trace.name).unwrap(),
    );
    let contract = prepared(&output, &spec, AnalysisInstanceId::new());
    let mut retained = original.clone();
    apply_saved_output_policy(
        &mut retained,
        policy(OutputSelectionMode::ExplicitOnly),
    ...
    retained.validate_retained_evidence().unwrap();
    assert_eq!(retained.waveforms.len(), 1);
    assert_eq!(retained.waveforms[0].y, trace.y);
    assert_eq!(retained.result_payload, original.result_payload);
    let mut retained = reload(retained);
    output.id = crate::product::SavedOutputId::new();
    output.name = "Deferred aging".into();
    output.save_policy = SavedOutputPolicy::OnDemandFromRetainedState;
    let contract = prepared(&output, &spec, AnalysisInstanceId::new());
    materialize_saved_outputs(&mut retained, &[contract]);
    retained.validate_retained_evidence().unwrap();
    let receipt = retained.saved_output_receipts.len() - 1;
    materialize_deferred_saved_output(&mut retained, receipt).unwrap();
    retained.validate_retained_evidence().unwrap();
    assert!(
        retained
            .waveforms
            .iter()
            .any(|w| w.name == "Deferred aging" && w.y == trace.y)
    );
}
