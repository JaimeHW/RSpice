...
mod tests {
    ...
    #[test]
    fn test_default() {
        let c = PzConfig::default();
        assert_eq!(c.input_pos, "IN");
    }
    #[test]
    fn test_new() {
        let c = PzConfig::new("vin", "vout");
        assert_eq!(c.input_pos, "VIN");
    }
    #[test]
    fn test_with_type() {
        let c = PzConfig::default().with_type(PzAnalysisType::PolesOnly);
        assert_eq!(c.analysis_type, PzAnalysisType::PolesOnly);
    }
    #[test]
    fn test_with_transfer() {
        let c = PzConfig::default().with_transfer(PzTransferType::Current);
        assert_eq!(c.transfer_type, PzTransferType::Current);
    }
    #[test]
    fn test_validate_ok() {
    ...
    #[test]
    fn test_to_spice() {
        let s = PzConfig::default().to_spice();
        assert!(s.contains(".pz"));
    }
    #[test]
    fn test_analysis_all() {
        assert_eq!(PzAnalysisType::all().len(), 3);
    }
    #[test]
    fn test_transfer_all() {
        assert_eq!(PzTransferType::all().len(), 2);
    }
    #[test]
    fn test_dialog_roundtrip() {
        let s = PzDialogState::from_config(&PzConfig::default());
        assert!(s.to_config().is_ok());
    }
    #[test]
    fn test_reset() {
        let mut c = PzConfig::new("X", "Y");
        c.reset();
        assert_eq!(c.input_pos, "IN");
    }
}
