...
mod tests {
    ...
    #[test]
    fn test_default() {
        let c = XfConfig::default();
        assert_eq!(c.input_source, "VIN");
    }
    #[test]
    fn test_new() {
        let c = XfConfig::new(1e6, 1e9, 20);
        assert_eq!(c.start_freq, 1e6);
    }
    #[test]
    fn test_with_input() {
        let c = XfConfig::default().with_input("vsrc");
        assert_eq!(c.input_source, "VSRC");
    }
    #[test]
    fn test_with_output() {
        let c = XfConfig::default().with_output("vx");
        assert_eq!(c.output_node, "VX");
    }
    #[test]
    fn test_with_group_delay() {
        let c = XfConfig::default().with_group_delay(true);
        assert!(c.group_delay);
    }
    #[test]
    fn test_validate_ok() {
    ...
    fn test_total_points_decade() {
        let c = XfConfig::new(1.0, 1e9, 10);
        assert!(c.total_points() >= 90);
    }
    #[test]
    fn test_total_points_linear() {
        let mut c = XfConfig::default();
        c.sweep_type = XfSweepType::Linear;
        c.num_points = 100;
        assert_eq!(c.total_points(), 100);
    }
    #[test]
    fn test_to_spice() {
        let s = XfConfig::default().to_spice();
        assert!(s.contains(".xf"));
        assert!(s.contains("input=VIN"));
    }
    #[test]
    fn test_to_spice_group_delay() {
        let s = XfConfig::default().with_group_delay(true).to_spice();
        assert!(s.contains("groupdelay=yes"));
    }
    #[test]
    fn test_sweep_types() {
        assert_eq!(XfSweepType::all().len(), 3);
    }
    #[test]
    fn test_dialog_roundtrip() {
        let s = XfDialogState::from_config(&XfConfig::default());
        assert!(s.to_config().is_ok());
    }
    #[test]
    fn test_dialog_ensure_init() {
        let mut s = XfDialogState::default();
        s.ensure_initialized();
        assert!(s.initialized);
    }
    #[test]
    fn test_reset() {
        let mut c = XfConfig::new(1e9, 10e9, 50);
        c.reset();
        assert_eq!(c.start_freq, 1.0);
    }
}
