...
mod tests {
    ...
    #[test]
    fn test_default() {
        let c = TempConfig::default();
        assert_eq!(c.temp_start, -40.0);
    }
    #[test]
    fn test_new() {
        let c = TempConfig::new(0.0, 100.0, 10.0);
        assert_eq!(c.temp_stop, 100.0);
    }
    #[test]
    fn test_with_base() {
        let c = TempConfig::default().with_base(TempBaseAnalysis::Ac);
        assert_eq!(c.base_analysis, TempBaseAnalysis::Ac);
    }
    #[test]
    fn test_with_corner() {
        let c = TempConfig::default().with_corner_temps();
        assert!(c.corner_temps);
        assert_eq!(c.specific_temps.len(), 4);
    }
    #[test]
    fn test_validate_ok() {
    ...
    fn test_validate_cold() {
        let mut c = TempConfig::default();
        c.temp_start = -300.0;
        assert!(c.validate().is_err());
    }
    #[test]
    fn test_validate_step_wrong_sign() {
        let c = TempConfig::new(0.0, 100.0, -10.0);
        assert!(c.validate().is_err());
    }
    #[test]
    fn test_to_spice_range() {
        let s = TempConfig::default().to_spice();
        assert!(s.contains(".temp"));
    }
    #[test]
    fn test_to_spice_corners() {
        let s = TempConfig::default().with_corner_temps().to_spice();
        assert!(s.contains("-40"));
        assert!(s.contains("125"));
    }
    #[test]
    fn test_num_temps() {
        let c = TempConfig::new(0.0, 100.0, 25.0);
        assert_eq!(c.num_temps(), 5);
    }
    #[test]
    fn test_num_temps_corners() {
        let c = TempConfig::default().with_corner_temps();
        assert_eq!(c.num_temps(), 4);
    }
    #[test]
    fn test_base_all() {
        assert_eq!(TempBaseAnalysis::all().len(), 4);
    }
    #[test]
    fn test_dialog_roundtrip() {
        let s = TempDialogState::from_config(&TempConfig::default());
        assert!(s.to_config().is_ok());
    }
    #[test]
    fn test_reset() {
        let mut c = TempConfig::new(50.0, 150.0, 10.0);
        c.reset();
        assert_eq!(c.temp_start, -40.0);
    }
}
