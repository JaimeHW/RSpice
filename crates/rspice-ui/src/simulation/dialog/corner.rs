...
impl CornerDialogState {
    ...
    pub fn render(&mut self, ui: &mut Ui) {
        ...
        ui.heading("Corner Analysis");
        ui.add_space(4.0);
        ui.label(
            egui::RichText::new("PVT (Process, Voltage, Temperature) variation analysis").weak(),
        );
        ui.add_space(12.0);

        // Process Corners
        ui.group(|ui| {
            ui.label(egui::RichText::new("Process Corners").strong());
            ui.add_space(4.0);

            ui.horizontal(|ui| {
                ui.checkbox(&mut self.process_tt, "TT");
                ui.checkbox(&mut self.process_ss, "SS");
                ui.checkbox(&mut self.process_ff, "FF");
            });
            ui.horizontal(|ui| {
                ui.checkbox(&mut self.process_sf, "SF");
                ui.checkbox(&mut self.process_fs, "FS");
            });
        });
        ...
        ui.group(|ui| {
            ui.checkbox(
                &mut self.enable_voltage_sweep,
                egui::RichText::new("Voltage Corners").strong(),
            );
            ui.add_space(4.0);

            if self.enable_voltage_sweep {
                egui::Grid::new("corner_voltage_grid")
                    .num_columns(3)
                    .spacing([15.0, 4.0])
                    .show(ui, |ui| {
                        ui.label("Min:");
                        ui.add(
                            egui::TextEdit::singleline(&mut self.voltage_min)
                                .desired_width(60.0)
                                .hint_text("0.9"),
                        );
                        ui.label("V");
                        ...
                        ui.label("Nom:");
                        ui.add(
                            egui::TextEdit::singleline(&mut self.voltage_nom)
                                .desired_width(60.0)
                                .hint_text("1.0"),
                        );
                        ui.label("V");
                        ...
                        ui.label("Max:");
                        ui.add(
                            egui::TextEdit::singleline(&mut self.voltage_max)
                                .desired_width(60.0)
                                .hint_text("1.1"),
                        );
                        ui.label("V");
        ...
        ui.group(|ui| {
            ui.checkbox(
                &mut self.enable_temp_sweep,
                egui::RichText::new("Temperature Corners").strong(),
            );
            ui.add_space(4.0);

            if self.enable_temp_sweep {
                egui::Grid::new("corner_temp_grid")
                    .num_columns(3)
                    .spacing([15.0, 4.0])
                    .show(ui, |ui| {
                        ui.label("Cold:");
                        ui.add(
                            egui::TextEdit::singleline(&mut self.temp_cold)
                                .desired_width(60.0)
                                .hint_text("-40"),
                        );
                        ui.label("°C");
                        ...
                        ui.label("Room:");
                        ui.add(
                            egui::TextEdit::singleline(&mut self.temp_room)
                                .desired_width(60.0)
                                .hint_text("25"),
                        );
                        ui.label("°C");
                        ...
                        ui.label("Hot:");
                        ui.add(
                            egui::TextEdit::singleline(&mut self.temp_hot)
                                .desired_width(60.0)
                                .hint_text("125"),
                        );
                        ui.label("°C");
        ...
        ui.group(|ui| {
            ...
            ui.checkbox(&mut self.full_matrix, "Full Matrix (all PVT combinations)");

            ui.add_space(4.0);
            ui.horizontal(|ui| {
                ui.label("Base Analysis:");
                let analyses = ["Transient", "AC", "DC", "Operating Point"];
                egui::ComboBox::from_id_salt("corner_base")
                    .selected_text(analyses[self.base_analysis_idx])
                    .show_ui(ui, |ui| {
                        for (idx, name) in analyses.iter().enumerate() {
                            if ui
                                .selectable_label(self.base_analysis_idx == idx, *name)
                                .clicked()
                            {
                                self.base_analysis_idx = idx;
                            }
                        }
                    });
            });
...
mod tests {
    ...
    fn test_process_corner_names() {
        assert_eq!(ProcessCorner::TT.short_name(), "TT");
        assert_eq!(ProcessCorner::SS.short_name(), "SS");
        assert_eq!(ProcessCorner::FF.short_name(), "FF");
        assert_eq!(ProcessCorner::SF.short_name(), "SF");
        assert_eq!(ProcessCorner::FS.short_name(), "FS");
    }

    #[test]
    fn test_process_corner_display_names() {
        assert!(ProcessCorner::TT.display_name().contains("Typical"));
        assert!(ProcessCorner::SS.display_name().contains("Slow"));
    }
    ...
    fn test_standard_five() {
        let five = ProcessCorner::standard_five();
        assert_eq!(five.len(), 5);
        assert!(five.contains(&ProcessCorner::TT));
        assert!(five.contains(&ProcessCorner::SS));
    }

    #[test]
    fn test_speed_corners() {
        let speed = ProcessCorner::speed_corners();
        assert_eq!(speed.len(), 3);
        assert!(speed.contains(&ProcessCorner::TT));
        assert!(speed.contains(&ProcessCorner::SS));
        assert!(speed.contains(&ProcessCorner::FF));
    }
    ...
    fn test_config_default() {
        ...
        assert_eq!(cfg.process_corners.len(), 1);
        assert_eq!(cfg.voltages.len(), 1);
        assert_eq!(cfg.temperatures.len(), 1);
    }

    #[test]
    fn test_config_standard_corners() {
        let cfg = CornerConfig::standard_corners();
        assert_eq!(cfg.process_corners.len(), 5);
    }

    #[test]
    fn test_config_commercial_pvt() {
        let cfg = CornerConfig::commercial_pvt();
        assert_eq!(cfg.process_corners.len(), 3);
        assert_eq!(cfg.voltages.len(), 3);
    ...
    fn test_config_builder_chain() {
        let cfg = CornerConfig::default()
            .with_process_corners(ProcessCorner::speed_corners())
    ...
    fn test_config_diagonal() {
        let cfg = CornerConfig::default().diagonal();
        assert!(!cfg.full_matrix);
    }

    #[test]
    fn test_config_with_base_analysis() {
        let cfg = CornerConfig::default().with_base_analysis(CornerBaseAnalysis::Ac);
        assert_eq!(cfg.base_analysis, CornerBaseAnalysis::Ac);
    }
    ...
    #[test]
    fn test_num_corners_full_matrix() {
        let cfg = CornerConfig::commercial_pvt();
        // 3 process * 3 voltage * 3 temp = 27
        assert_eq!(cfg.num_corners(), 27);
    }

    #[test]
    fn test_num_corners_diagonal() {
        let cfg = CornerConfig::commercial_pvt().diagonal();
        // max(3, 3, 3) = 3
        assert_eq!(cfg.num_corners(), 3);
    }

    #[test]
    fn test_num_corners_single() {
        let cfg = CornerConfig::default();
        assert_eq!(cfg.num_corners(), 1);
    }
    ...
    #[test]
    fn test_corner_names_format() {
        let cfg = CornerConfig::default();
        let names = cfg.corner_names();
        assert_eq!(names.len(), 1);
        assert!(names[0].contains("TT"));
        assert!(names[0].contains("V"));
        assert!(names[0].contains("C"));
    }

    #[test]
    fn test_corner_names_full_matrix() {
        let cfg = CornerConfig::commercial_pvt();
        let names = cfg.corner_names();
    ...
    #[test]
    fn test_validate_no_process() {
        let mut cfg = CornerConfig::default();
        cfg.process_corners.clear();
        assert!(cfg.validate().is_err());
    }

    #[test]
    fn test_validate_no_voltage() {
        let mut cfg = CornerConfig::default();
        cfg.voltages.clear();
        assert!(cfg.validate().is_err());
    }
    ...
    #[test]
    fn test_validate_negative_voltage() {
        let mut cfg = CornerConfig::default();
        cfg.voltages = vec![-1.0];
        assert!(cfg.validate().is_err());
    }

    #[test]
    fn test_validate_no_temp() {
        let mut cfg = CornerConfig::default();
    ...
    #[test]
    fn test_validate_below_absolute_zero() {
        let mut cfg = CornerConfig::default();
        cfg.temperatures = vec![-300.0]; // Below absolute zero
        assert!(cfg.validate().is_err());
    }
    ...
    fn test_config_reset() {
        let mut cfg = CornerConfig::commercial_pvt();
        cfg.reset();
        assert_eq!(cfg.process_corners.len(), 1);
    }

    // =========================================================================
    // CornerBaseAnalysis Tests
    // =========================================================================

    #[test]
    fn test_base_analysis_names() {
        assert_eq!(CornerBaseAnalysis::Transient.display_name(), "Transient");
        assert_eq!(CornerBaseAnalysis::Ac.display_name(), "AC");
    }

    #[test]
    fn test_base_analysis_all() {
        assert_eq!(CornerBaseAnalysis::all().len(), 4);
    }
    ...
    fn test_dialog_state_from_config() {
        let cfg = CornerConfig::commercial_pvt();
        let state = CornerDialogState::from_config(&cfg);

        assert!(state.initialized);
        assert!(state.process_tt);
        assert!(state.process_ss);
        assert!(state.process_ff);
        assert!(!state.process_sf);
        assert!(state.enable_voltage_sweep);
        assert!(state.enable_temp_sweep);
    }

    #[test]
    fn test_dialog_state_to_config() {
        let mut state = CornerDialogState::from_config(&CornerConfig::default());
        state.process_ss = true;

        let result = state.to_config();
        assert!(result.is_ok());

        let cfg = result.unwrap();
        assert!(cfg.process_corners.contains(&ProcessCorner::SS));
    }

    #[test]
    fn test_dialog_state_no_process_selected() {
        let mut state = CornerDialogState::from_config(&CornerConfig::default());
        state.process_tt = false;
    ...
    fn test_dialog_state_ensure_initialized() {
        let mut state = CornerDialogState::default();
        assert!(!state.initialized);
    ...
    #[test]
    fn test_military_temp_range() {
        let cfg = CornerConfig::default().with_temperatures(vec![-55.0, 25.0, 150.0]);
        assert!(cfg.validate().is_ok());
    }

    #[test]
    fn test_automotive_temp_range() {
        let cfg = CornerConfig::default().with_temperatures(vec![-40.0, 25.0, 175.0]);
        assert!(cfg.validate().is_ok());
    }

    #[test]
    fn test_fine_voltage_sweep() {
        let cfg =
            CornerConfig::default().with_voltages(vec![0.85, 0.90, 0.95, 1.00, 1.05, 1.10, 1.15]);
        assert!(cfg.validate().is_ok());
        assert_eq!(cfg.voltages.len(), 7);
    }
}
