...
mod tests {
    ...
    #[test]
    fn test_new() {
        let cfg = EnvelopeConfig::new(5e9, 1e-6);
        assert_eq!(cfg.fundamental_freq, 5e9);
    }
    #[test]
    fn test_validate_ok() {
    ...
    fn test_validate_zero_freq() {
        let mut c = EnvelopeConfig::default();
        c.fundamental_freq = 0.0;
        assert!(c.validate().is_err());
    }
    #[test]
    fn test_validate_zero_stop() {
        let mut c = EnvelopeConfig::default();
        c.stop_time = 0.0;
        assert!(c.validate().is_err());
    }
    ...
    #[test]
    fn test_carrier_period() {
        let c = EnvelopeConfig::new(1e9, 1e-6);
        assert!((c.carrier_period() - 1e-9).abs() < 1e-18);
    }
    #[test]
    fn test_num_cycles() {
        let c = EnvelopeConfig::new(1e9, 10e-6);
        assert_eq!(c.num_cycles(), 10000);
    }
    #[test]
    fn test_to_spice() {
        let s = EnvelopeConfig::default().to_spice();
        assert!(s.contains(".envlp"));
    }
    #[test]
    fn test_modulation_all() {
        assert_eq!(ModulationType::all().len(), 4);
    }
    #[test]
    fn test_dialog_roundtrip() {
        let s = EnvelopeDialogState::from_config(&EnvelopeConfig::default());
        assert!(s.to_config().is_ok());
    }
    #[test]
    fn test_reset() {
        let mut c = EnvelopeConfig::new(5e9, 100e-6);
        c.reset();
        assert_eq!(c.fundamental_freq, 2.4e9);
    }
}
