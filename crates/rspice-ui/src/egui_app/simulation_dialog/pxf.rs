...
mod tests {
    ...
    fn test_config_default() {
        let cfg = PxfConfig::default();
        assert_eq!(cfg.start_freq, 1e3);
        assert_eq!(cfg.output_sideband, 1);
    }

    #[test]
    fn test_config_new() {
        let cfg = PxfConfig::new(1e6, 1e9, 20);
    ...
    fn test_config_builder() {
        let cfg = PxfConfig::new(1e6, 1e9, 10)
            .with_output("VIF", 2)
            .with_input("VRF")
            .with_sidebands(7);
        assert_eq!(cfg.output_node, "VIF");
        assert_eq!(cfg.output_sideband, 2);
    ...
    fn test_to_spice() {
        ...
        assert!(spice.starts_with(".pxf"));
        assert!(spice.contains("output=VOUT"));
        assert!(spice.contains("outsideband=1"));
        assert!(spice.contains("input=VIN"));
    }

    #[test]
    fn test_total_points_decade() {
        let cfg = PxfConfig::new(1e3, 1e9, 10);
        assert!(cfg.total_points() >= 60);
    }

    #[test]
    fn test_sweep_types() {
        assert_eq!(PxfSweepType::all().len(), 3);
        assert_eq!(PxfSweepType::Decade.spice_keyword(), "dec");
    }

    #[test]
    fn test_dialog_state_roundtrip() {
        let cfg = PxfConfig::default();
        let state = PxfDialogState::from_config(&cfg);
    ...
    fn test_dialog_state_ensure_initialized() {
        let mut state = PxfDialogState::default();
    ...
    #[test]
    fn test_reset() {
        let mut cfg = PxfConfig::new(1e9, 10e9, 50);
        cfg.reset();
        assert_eq!(cfg.start_freq, 1e3);
    }

    #[test]
    fn test_negative_sideband_output() {
        let cfg = PxfConfig::default().with_output("VIF", -1);
        assert_eq!(cfg.output_sideband, -1);
