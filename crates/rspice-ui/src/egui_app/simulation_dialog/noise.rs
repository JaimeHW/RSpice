...
mod tests {
    ...
    fn test_config_default() {
        ...
        assert_eq!(cfg.output_node, "out"); // Default valid output
        assert_eq!(cfg.input_source, "VIN"); // Default valid input source
        assert_eq!(cfg.source_filter, NoiseSourceType::All);
        assert!(cfg.validate().is_ok()); // Defaults should be valid
    }

    #[test]
    fn test_config_new() {
        let cfg = NoiseConfig::new("out", "VIN");
        assert_eq!(cfg.output_node, "out");
        assert_eq!(cfg.input_source, "VIN");
    }

    #[test]
    fn test_with_freq_range() {
        let cfg = NoiseConfig::new("out", "VIN").with_freq_range(100.0, 1e6, 20);
    ...
    fn test_validate_no_output() {
        let cfg = NoiseConfig::new("", "VIN");
        assert!(cfg.validate().is_err());
    }

    #[test]
    fn test_validate_no_source() {
        let cfg = NoiseConfig::new("out", "");
        assert!(cfg.validate().is_err());
    }

    #[test]
    fn test_validate_bad_freq() {
        let mut cfg = NoiseConfig::new("out", "VIN");
        cfg.start_freq = -1.0;
        assert!(cfg.validate().is_err());
    }
    ...
    fn test_to_spice() {
        ...
        assert!(spice.contains("V(out)"));
        assert!(spice.contains("VIN"));
    }

    #[test]
    fn test_to_spice_with_reference() {
        let mut cfg = NoiseConfig::new("out", "VIN");
        cfg.reference_node = "gnd".to_string();
        let spice = cfg.to_spice();
        assert!(spice.contains("V(out,gnd)"));
    }

    #[test]
    fn test_noise_source_type_names() {
        assert!(!NoiseSourceType::All.display_name().is_empty());
        assert!(!NoiseSourceType::ResistorOnly.display_name().is_empty());
    }

    #[test]
    fn test_noise_source_type_all() {
        let all = NoiseSourceType::all();
        assert_eq!(all.len(), 3);
    }
    ...
    fn test_dialog_tab_name() {
        let cfg = NoiseConfig::default();
        assert_eq!(cfg.name(), "Noise");
    }

    #[test]
    fn test_dialog_tab_reset() {
        let mut cfg = NoiseConfig::new("custom_out", "VTEST");
        cfg.reset();
        assert_eq!(cfg.output_node, "out"); // Reset to default
        assert_eq!(cfg.input_source, "VIN"); // Reset to default
    }
}
