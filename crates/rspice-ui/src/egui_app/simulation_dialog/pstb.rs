...
mod tests {
    ...
    fn test_config_default() {
        let cfg = PstbConfig::default();
        assert_eq!(cfg.probe, "LPROBE");
        assert_eq!(cfg.max_harmonics, 10);
        assert!(cfg.annotate);
    }

    #[test]
    fn test_config_new() {
        let cfg = PstbConfig::new("myprobe");
        assert_eq!(cfg.probe, "MYPROBE");
    }

    #[test]
    fn test_config_builder() {
        let cfg = PstbConfig::new("lp1")
            .with_harmonics(20)
            .with_multipliers(15)
            .with_annotate(false);
        assert_eq!(cfg.max_harmonics, 20);
        assert_eq!(cfg.num_multipliers, 15);
        assert!(!cfg.annotate);
    }
    ...
    fn test_validate_empty_probe() {
        let mut cfg = PstbConfig::default();
        cfg.probe = String::new();
    ...
    #[test]
    fn test_validate_zero_multipliers() {
        let mut cfg = PstbConfig::default();
        cfg.num_multipliers = 0;
        assert!(cfg.validate().is_err());
    }
    ...
    fn test_to_spice() {
        ...
        assert!(spice.starts_with(".pstb"));
        assert!(spice.contains("probe=LPROBE"));
        assert!(spice.contains("maxharm=10"));
        assert!(spice.contains("annotate=yes"));
    }

    #[test]
    fn test_dialog_state_roundtrip() {
        let cfg = PstbConfig::default();
        let state = PstbDialogState::from_config(&cfg);
    ...
    fn test_dialog_state_ensure_initialized() {
        let mut state = PstbDialogState::default();
    ...
    #[test]
    fn test_reset() {
        let mut cfg = PstbConfig::new("custom").with_harmonics(50);
        cfg.reset();
        assert_eq!(cfg.probe, "LPROBE");
        assert_eq!(cfg.max_harmonics, 10);
    }
