...
pub fn render_waveform_viewer(ui: &mut Ui, app_state: &mut AppState) {
    // Get or initialize viewer state
    // For now we use the simulation waveforms directly
    let waveforms = &app_state.simulation.waveforms;

    // Create a temporary viewer state for rendering
    // In a full implementation, this would be persisted in AppState
    let mut viewer_state = WaveformViewerState::new();
    viewer_state.load_from_simulation(waveforms);
    ...
    // Render each section
    render_header(ui, &layout, &mut viewer_state, app_state);
    render_y_axis(ui, &layout, &viewer_state);
    render_plot_area(ui, &layout, &mut viewer_state, app_state);
    render_x_axis(ui, &layout, &viewer_state);
    render_legend(ui, &layout, &mut viewer_state, app_state);
}
    ...
    ui.allocate_ui_at_rect(header_rect, |ui| {
        ui.horizontal(|ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                // Close button
                if ui.small_button("✕").clicked() {
                    app_state.panels.waveform = false;
                }
...
fn handle_plot_interactions(
    ...
    let view = &mut viewer_state.view;
    ...
    if response.hovered() {
        ...
        if scroll_delta.y.abs() > 0.1 {
            ...
            if let Some(pos) = response.hover_pos() {
                ...
                if modifiers.shift {
                    view.zoom_x_only(factor as f64, x_frac);
                } else if modifiers.ctrl {
                    view.zoom_y_only(factor as f64, y_frac);
                } else {
                    view.zoom(factor as f64, x_frac, y_frac);
                }
            }
        }
    }
    ...
    if response.clicked() && !view.did_drag {
        if let Some(pos) = response.hover_pos() {
            let x_frac = (pos.x - layout.plot.min.x) / layout.plot.width();
            let data_x = view.x_min + x_frac as f64 * view.x_range();
        ...
        if modifiers.shift {
            ...
            if !viewer_state.box_selection.is_selecting {
                if let Some(pos) = response.hover_pos() {
                    ...
                    let data_x = view.x_min + x_frac as f64 * view.x_range();
                    let data_y = view.y_max - y_frac as f64 * view.y_range();
            ...
            let data_dx = -(delta.x as f64 / view.plot_width) * view.x_range();
            let data_dy = (delta.y as f64 / view.plot_height) * view.y_range();
        ...
        ui.vertical(|ui| {
            ...
            for (i, trace) in viewer_state.traces.iter().enumerate() {
                ...
                ui.horizontal(|ui| {
                    ...
                    // Trace name (clickable to toggle)
                    ...
                    if name_response.clicked() {
                        // Toggle visibility - need to get mutable access
                        // This is handled outside the loop
                    }
