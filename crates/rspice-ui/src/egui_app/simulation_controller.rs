...
use super::simulation::config::{
    AcAnalysisConfig, AcSweepType, DcSweepConfig, TransientAnalysisConfig,
};
use super::simulation::{
    AnalysisConfig, NetlistGenerator, SimulationRunner, SimulationStatus, WaveformData,
};
...
impl SimulationController {
    ...
    fn start_simulation(&mut self, state: &mut AppState) {
        ...
        let mut generator = NetlistGenerator::new(&state.schematic);
        ...
        if let Err(e) = self.runner.start(config, netlist) {
            state
                .console_messages
                .push(super::app::ConsoleMessage::error(format!(
    ...
    fn update_waveforms(&self, state: &mut AppState, result: &super::simulation::SimulationResult) {
