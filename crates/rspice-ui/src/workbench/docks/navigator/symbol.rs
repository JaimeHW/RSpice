...
use crate::state::{PortSpec, SymbolDocument, SymbolShape};
...
pub(super) fn show(ui: &mut Ui, app: &mut RSpiceApp) {
    ...
    let Ok(document) = app.state.load_active_symbol_document() else {
    ...
    ScrollArea::vertical()
        ...
        .show(ui, |ui| {
            pins_section(ui, app, &document, &ports);
            body_section(ui, app, &document);
            labels_section(ui, &document);
...
fn pins_section(ui: &mut Ui, app: &mut RSpiceApp, document: &SymbolDocument, ports: &[PortSpec]) {
    section_header(
        ui,
        "Pins · netlist order",
    ...
    if document.pins.is_empty() {
        super::empty_navigator_row(ui, "No pins declared by the interface");
        return;
    }
...
fn shape_anchor(shape: &SymbolShape) -> crate::state::Point {
    match shape {
        SymbolShape::Polyline { points, .. } => {
            points.first().copied().unwrap_or_else(crate::state::Point::origin)
        }
...
fn body_section(ui: &mut Ui, app: &mut RSpiceApp, document: &SymbolDocument) {
    ...
    if document.body.is_empty() {
        super::empty_navigator_row(
            ui,
            "No body shapes. Draw with the polyline, circle, arc, arrow or dot tools.",
        );
        return;
    }
...
/// Where the placed instance draws its name and value. These are anchors on
/// the symbol, not editable schematic attributes, so they are reported
/// rather than offered as rows to select.
fn labels_section(ui: &mut Ui, document: &SymbolDocument) {
    section_header(ui, "Label anchors", Some("2"));
    property_row(
        ui,
        "Instance name",
        &format!("{}, {}", document.name_anchor.x, document.name_anchor.y),
    );
    property_row(
        ui,
        "Value",
        &format!("{}, {}", document.value_anchor.x, document.value_anchor.y),
    );
    property_row(
        ui,
        "Origin",
        &format!("{}, {}", document.origin.x, document.origin.y),
    );
}
