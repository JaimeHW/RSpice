        ...
        if plan.setup().render().format() == OutputFormat::BrowserPrintDocument {
            begin_publication_authentication(
                ...
                PendingNativePublication {
                    ...
                    destination: NativePublicationDestination::DesktopPrintHandoff,
                },
            )?;
...
mod execution;
#[cfg(target_arch = "wasm32")]
use execution::start_browser_publication;
#[cfg(not(target_arch = "wasm32"))]
use execution::start_native_publication;

pub(super) use execution::poll_publication;
#[cfg(not(target_arch = "wasm32"))]
    ...
    let completion = match result {
        ...
        Ok(completion) => match (publication.destination, completion.payload) {
            ...
            (
                ...
                super::finalize::FinalizationPayload::DesktopPrintHandoff { outcome, accepted },
            ) => (|| {
                let HardcopyOutcome::DesktopPrintHandoffAccepted { pages_accepted, .. } = &outcome
                else {
                    return Err(
                        "Desktop hardcopy finalization returned an inconsistent handoff outcome."
                            .to_owned(),
                    );
                };
                if *pages_accepted as usize != accepted {
                    return Err(
                        "Desktop hardcopy finalization returned an inconsistent page count."
                            .to_owned(),
                    );
                }
                let receipt =
                ...
                commit_print_mapping_persistence(app, publication.staged_mapping)?;
                Ok(format!(
                    "Opened a print-ready document containing {accepted} page{} in the system browser. Use the browser's Print command to continue.",
                    if accepted == 1 { "" } else { "s" }
                ))
            })(),
            (
