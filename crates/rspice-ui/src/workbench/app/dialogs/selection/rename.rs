...
//! undo transaction. Components, net labels, logical named-net authorities,
//! and declared buses retain their IDs; bus dependencies are rebased through
//! the schematic's semantic bus transaction rather than by rewriting display
//! text.
...
fn object_summary(target: &RenameSelectionTarget) -> String {
    match target {
        ...
        RenameSelectionTarget::NamedNet(net) => format!(
            "net {} \u{00b7} {} stable naming authorit{}",
            net.name,
            net.authority_count(),
            if net.authority_count() == 1 {
                "y"
            } else {
                "ies"
            }
...
fn reference_summary(schematic: &SchematicState, target: &RenameSelectionTarget) -> String {
    match target {
        ...
        RenameSelectionTarget::NamedNet(net) => format!(
            "{} label ID(s) + {} interface-port ID(s) \u{00b7} {} conductor segment(s) \u{00b7} one atomic logical-net transaction",
            net.labels.len(),
            net.ports.len(),
            net.wire_ids.len()
        ),
...
fn validate_draft(
    ...
    match target {
        ...
        RenameSelectionTarget::NamedNet(expected) => {
            ...
            RenameValidation::Valid(Box::new(RenameCommit::NamedNet {
                expected: expected.clone(),
                name: candidate,
            }))
        }
...
fn apply_commit(state: &mut AppState, commit: RenameCommit) -> Result<bool, String> {
...
mod tests {
    use super::*;
    use crate::state::{Bus, ComponentType, NetLabel, Point, Wire};
    ...
    #[test]
    fn named_wire_and_interface_port_open_the_net_scoped_rename_target() {
        let mut app = RSpiceApp::test_instance();
        app.state
            .schematic
            .wires
        ...
        assert!(matches!(
            ...
            Some(RenameSelectionTarget::NamedNet(ref target))
                if target.name == "sense" && target.labels[0].id == 82
        ));
        ...
        assert!(matches!(
            app.state.dialogs.rename_selection.target,
            Some(RenameSelectionTarget::NamedNet(ref target))
                if target.name == "sense"
                    && target.ports.iter().any(|port| port.id == 83)
        ));
    }
    ...
    fn saved_output(name: &str, expression: &str) -> crate::state::SavedOutput {
        ...
        .expect("a raw probe is a valid saved output")
    }
    ...
    fn instance_rename_remaps_saved_outputs() {
        ...
        app.state.workspace.simulation_plan_payloads.push(
            crate::state::SimulationPlanPayloadRecord {
                ...
                payload: crate::state::SimulationPlanPayload {
                    saved_outputs: vec![
                        saved_output("device current", "I(R1)"),
                        saved_output("internal node", "V(/R1/mid)"),
                        saved_output("unrelated node", "V(out)"),
                    ],
                    ..Default::default()
                },
            },
        ...
        let expressions = app
            ...
            .expect("the payload this test owns")
            .payload
            .saved_outputs
            .iter()
            .map(|output| output.source_expression.clone())
            .collect::<Vec<_>>();
        assert_eq!(
            expressions,
