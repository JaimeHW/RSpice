...
pub(crate) fn run_preflight_block_reason(
    ...
    blocking_drc: Option<&DrcResult>,
        ...
        let Some(configured_schematic) = workspace
            .simulation_root_schematic(&workspace.active_view, schematic)
        else {
            return Some(format!(
        ...
        if let Err(error) = model_library
            .reference_process_model_cards(sim_setup.reference_pvt.process)
        {
            return Some(error);
        }
