...
fn qualification_evidence_summary(
...
fn append_typed_metadata_comparison_rows(
...
fn append_named_contract_rows<T, Key, Summary>(
    rows: &mut Vec<ModelReleaseComparisonRow>,
    category: &str,
    released: &[T],
    candidate: &[T],
...
fn append_numerical_evidence_rows(
    rows: &mut Vec<ModelReleaseComparisonRow>,
...
fn required_authoring_text(label: &str, value: &str) -> Result<String, String> {
...
fn parse_content_digest(label: &str, value: &str) -> Result<ContentDigest, String> {
    value
        .trim()
        .parse::<ContentDigest>()
...
fn document_reference(label: &str, id: &str, digest: &str) -> Result<DocumentReference, String> {
...
fn optional_document_reference(
    label: &str,
    id: &str,
    digest: &str,
...
fn parse_object_revision(label: &str, value: &str) -> Result<ObjectRevision, String> {
    let parsed = value
        .trim()
        .parse::<u64>()
...
fn synchronize_parameter_overrides(
...
fn parse_optional_finite_bound(
    value: &str,
    field: String,
    diagnostics: &mut Vec<ModelFieldDiagnostic>,
) -> Option<FiniteF64> {
...
fn parse_frequency_axis(value: &str) -> Result<Vec<FiniteValue>, String> {
    let tokens = value
...
fn parse_sample_index(value: &str, label: &str) -> Result<usize, String> {
    value
        .trim()
        .parse::<usize>()
...
fn parse_non_negative(value: &str, label: &str) -> Result<f64, String> {
...
fn start_qualification_execution_for(
    app: &mut RSpiceApp,
