...
pub(super) fn action_file_new(state: &mut AppState) {
    crate::workbench::file_workflow::create_new_schematic(state);
}

pub(super) fn action_file_open_with_io(state: &mut AppState, io: &(impl FileWorkflowIo + ?Sized)) {
    crate::workbench::file_workflow::open_schematic_from_dialog_with_io(state, io);
}
...
pub(super) fn action_file_save_with_io(
    ...
    crate::workbench::file_workflow::save_schematic_with_io(state, io)
}
