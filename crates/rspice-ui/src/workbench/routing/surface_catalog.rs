        ...
        impl SurfaceId {
            ...
            pub const fn owner_workspace(self) -> Option<Workspace> {
                match self {
                    ...
                    | Self::LibraryCellviewManager => Some(Workspace::Models),
                    _ => self.workspace(),
                }
            }
...
define_surface_catalog! {
    ...
    ReleaseCockpit => { id: "release-cockpit", label: "Release closure", archetype: SpecialistWorkspace, tier: Preview, status: Preview, deep_link: "?surface=release-cockpit" },
    MultiTestStudio => { id: "multi-test-studio", label: "Multi-test verification plans", archetype: SpecialistWorkspace, tier: ReleaseTarget, status: Preview, deep_link: "?surface=multi-test-studio" },
    ...
    LayoutWorkbench => { id: "layout-workbench", label: "Physical layout", archetype: SpecialistWorkspace, tier: Preview, status: Preview, deep_link: "?surface=layout-workbench" },
    ...
    ReportAuthoring => { id: "report-authoring", label: "Verification report", archetype: SpecialistWorkspace, tier: ReleaseTarget, status: ReleaseScope, deep_link: "?surface=report-authoring" },
    ...
    ModelExtraction => { id: "model-extraction", label: "Model extraction", archetype: SpecialistWorkspace, tier: Preview, status: Preview, deep_link: "?surface=model-extraction" },
    ...
    JobDiagnostics => { id: "job-diagnostics", label: "Failure diagnostics", archetype: SpecialistWorkspace, tier: ReleaseTarget, status: Preview, deep_link: "?surface=job-diagnostics" },
    RegressionOrchestration => { id: "regression-orchestration", label: "Regression orchestration", archetype: SpecialistWorkspace, tier: Preview, status: Preview, deep_link: "?surface=regression-orchestration" },
    ...
    HeterogeneousWorkbench => { id: "heterogeneous-workbench", label: "Heterogeneous assembly", archetype: SpecialistWorkspace, tier: QualifiedExternalFirst, status: ExternalFirst, deep_link: "?surface=heterogeneous-workbench" },
    LibraryCharacterization => { id: "library-characterization", label: "Cell-library characterization", archetype: SpecialistWorkspace, tier: Preview, status: Preview, deep_link: "?surface=library-characterization" },
    PdkTechnologyAdmin => { id: "pdk-technology-admin", label: "PDK technology administration", archetype: SpecialistWorkspace, tier: Preview, status: Preview, deep_link: "?surface=pdk-technology-admin" },
...
mod tests {
    ...
    #[test]
    fn models_and_pdk_commercial_tiers_match_the_governed_mockup_contract() {
        for surface in [
            ...
            assert_eq!(surface.canonical_tier(), CanonicalTier::Preview);
            assert_eq!(surface.release_status(), ReleaseStatus::Preview);
        }
        for surface in [SurfaceId::ModelEditor, SurfaceId::LibraryCellviewManager] {
            assert_eq!(surface.canonical_tier(), CanonicalTier::ReleaseTarget);
            assert_eq!(surface.release_status(), ReleaseStatus::ReleaseScope);
        }
        assert_eq!(
            SurfaceId::SolverQualificationCenter.canonical_tier(),
            CanonicalTier::Internal
        );
        assert_eq!(
            SurfaceId::SolverQualificationCenter.release_status(),
            ReleaseStatus::InternalOnly
        );
    }
    ...
    #[test]
    fn library_cellview_manager_keeps_specialist_identity_with_models_ownership() {
        assert_eq!(SurfaceId::LibraryCellviewManager.workspace(), None);
        assert_eq!(
            SurfaceId::LibraryCellviewManager.owner_workspace(),
            Some(Workspace::Models)
        );
        assert_eq!(
            Workspace::try_from(SurfaceId::LibraryCellviewManager),
            Err(NonPrimarySurface {
                surface_id: SurfaceId::LibraryCellviewManager,
            })
        );
        assert_eq!(
            SurfaceId::LibraryCellviewManager.deep_link(),
            "?surface=library-cellview-manager"
        );
    }
