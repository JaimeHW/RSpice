...
impl WaveformWriter {
    ...
    /// Write Touchstone two-port S-parameter data (RI format).
    fn write_touchstone(&self, dataset: &WaveformDataset, path: &Path) -> Result<(), String> {
        let (frequencies, s11, s21, s12, s22) = Self::extract_touchstone_2port(dataset)?;
        let z0 = dataset
            .metadata
            .get("z0")
        ...
        if version >= 2 {
            ...
            writeln!(writer, "[Number of Ports] 2").map_err(|e| format!("Write error: {}", e))?;
        ...
        for idx in 0..frequencies.len() {
            let (s11r, s11i) = s11[idx];
            let (s21r, s21i) = s21[idx];
            let (s12r, s12i) = s12[idx];
            let (s22r, s22i) = s22[idx];
            writeln!(
                writer,
                "{:.12e} {:.12e} {:.12e} {:.12e} {:.12e} {:.12e} {:.12e} {:.12e} {:.12e}",
                frequencies[idx], s11r, s11i, s21r, s21i, s12r, s12i, s22r, s22i
            )
            .map_err(|e| format!("Write error: {}", e))?;
        }
    ...
    > {
        ...
        let s11 = Self::extract_complex_pair(dataset, "S11", n)?;
        let s21 = Self::extract_complex_pair(dataset, "S21", n)?;
        let s12 = Self::extract_complex_pair(dataset, "S12", n)?;
        let s22 = Self::extract_complex_pair(dataset, "S22", n)?;

        Ok((x.data.clone(), s11, s21, s12, s22))
    }

    fn extract_complex_pair(
        dataset: &WaveformDataset,
        base_name: &str,
        expected_len: usize,
    ) -> Result<Vec<(f64, f64)>, String> {
        let base = Self::normalize_signal_name(base_name);
        let real_names = [
            format!("{}RE", base),
            format!("{}REAL", base),
            format!("{}R", base),
        ...
        let real = Self::find_signal_by_names(dataset, &real_names)
            .ok_or_else(|| format!("Missing Touchstone real component for {}", base_name))?;
        let imag = Self::find_signal_by_names(dataset, &imag_names)
            .ok_or_else(|| format!("Missing Touchstone imag component for {}", base_name))?;

        if real.data.len() != expected_len || imag.data.len() != expected_len {
            return Err(format!(
                "Touchstone signal length mismatch for {} (freq={}, real={}, imag={})",
                base_name,
                expected_len,
                real.data.len(),
                imag.data.len()
    ...
    fn find_signal_by_names<'a>(
        dataset: &'a WaveformDataset,
        normalized_names: &[String],
    ) -> Option<&'a WaveformSignal> {
        dataset.signals.iter().find(|signal| {
            let normalized = Self::normalize_signal_name(&signal.name);
            normalized_names.iter().any(|name| name == &normalized)
        })
    }
...
mod tests {
    ...
    fn test_write_touchstone_requires_complete_matrix_components() {
        ...
        assert!(err.contains("Missing Touchstone"));
    }
