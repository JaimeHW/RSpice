...
mod tests {
    ...
    #[test]
    fn test_si_prefix_zero() {
        let (scale, prefix) = si_prefix_for_value(0.0);
        assert_eq!(scale, 1.0);
        assert_eq!(prefix, "");
    }

    #[test]
    fn test_si_prefix_unity() {
        let (scale, prefix) = si_prefix_for_value(1.0);
        assert_eq!(scale, 1.0);
        assert_eq!(prefix, "");
    }

    #[test]
    fn test_si_prefix_femto() {
        let (scale, prefix) = si_prefix_for_value(1e-15);
        assert_eq!(prefix, "f");
        assert!((1e-15 * scale - 1.0).abs() < 0.01);
    }
    ...
    #[test]
    fn test_si_prefix_negative() {
        // Negative values should use same prefix as positive
        let (scale_pos, prefix_pos) = si_prefix_for_value(1e-6);
        let (scale_neg, prefix_neg) = si_prefix_for_value(-1e-6);
        assert_eq!(scale_pos, scale_neg);
        assert_eq!(prefix_pos, prefix_neg);
    }
    ...
    fn test_format_with_si_prefix() {
        let result = format_with_si_prefix(0.000001, "s", 1);
    ...
    #[test]
    fn test_calculate_ticks_basic() {
        let ticks = calculate_ticks(0.0, 10.0, 5);

        // Should have reasonable number of major ticks
        assert!(!ticks.major_ticks.is_empty());
        assert!(ticks.major_ticks.len() >= 3);
        assert!(ticks.major_ticks.len() <= 15);
    ...
    #[test]
    fn test_calculate_ticks_negative_range() {
        let ticks = calculate_ticks(-5.0, 5.0, 5);

        // Should include zero
        assert!(ticks.major_ticks.iter().any(|&t| t.abs() < 0.01));
    }

    #[test]
    fn test_calculate_ticks_small_range() {
        let ticks = calculate_ticks(0.0, 1e-9, 5);

        assert!(!ticks.major_ticks.is_empty());
        assert_eq!(ticks.prefix, "n");
    }

    #[test]
    fn test_calculate_ticks_large_range() {
        let ticks = calculate_ticks(0.0, 1e6, 5);

        assert!(!ticks.major_ticks.is_empty());
        assert_eq!(ticks.prefix, "M");
    }

    #[test]
    fn test_calculate_ticks_interval_1_2_5() {
        // Verify intervals are from the 1-2-5 sequence
        let ticks = calculate_ticks(0.0, 100.0, 5);

        // Interval should be 10, 20, or 50 (or scaled versions)
        let interval = ticks.major_interval;
        let normalized = interval / 10.0_f64.powf(interval.log10().floor());

        // Should be close to 1, 2, or 5
        let is_1_2_5 = (normalized - 1.0).abs() < 0.01
            || (normalized - 2.0).abs() < 0.01
            || (normalized - 5.0).abs() < 0.01
            || (normalized - 10.0).abs() < 0.01; // 10 is also valid (1 * 10)

        assert!(
            is_1_2_5,
            "Interval {} normalized to {} should be 1, 2, 5, or 10",
            interval, normalized
        );
    }

    #[test]
    fn test_calculate_ticks_minor_subdivisions() {
        let ticks = calculate_ticks(0.0, 10.0, 5);

        // Should have minor ticks
        assert!(!ticks.minor_ticks.is_empty());

        // Minor ticks should not coincide with major ticks
        let epsilon = ticks.major_interval * 1e-6;
        for &minor in &ticks.minor_ticks {
            let at_major = ticks
                .major_ticks
                .iter()
                .any(|&m| (m - minor).abs() < epsilon);
            assert!(!at_major, "Minor tick {} coincides with major tick", minor);
        }
    }

    #[test]
    fn test_calculate_ticks_invalid_range() {
        let ticks = calculate_ticks(10.0, 0.0, 5); // min > max
        assert!(ticks.major_ticks.is_empty());

        let ticks2 = calculate_ticks(f64::NAN, 10.0, 5);
        assert!(ticks2.major_ticks.is_empty());
    }

    #[test]
    fn test_calculate_precision() {
        assert_eq!(calculate_precision(10.0), 0);
        assert_eq!(calculate_precision(1.0), 0);
        assert_eq!(calculate_precision(0.5), 1);
        assert_eq!(calculate_precision(0.1), 1);
        assert_eq!(calculate_precision(0.05), 2);
        assert_eq!(calculate_precision(0.01), 2);
        assert_eq!(calculate_precision(0.001), 3);
        assert_eq!(calculate_precision(0.0001), 4);
    }
    ...
    #[test]
    fn test_generate_tick_labels() {
        let ticks = vec![0.0, 1.0, 2.0, 3.0];
        let labels = generate_tick_labels(&ticks, 1.0, 0);
    ...
    #[test]
    fn test_generate_tick_labels_scaled() {
        let ticks = vec![0.0, 1e-6, 2e-6];
        let labels = generate_tick_labels(&ticks, 1e6, 0);
    ...
    #[test]
    fn test_generate_tick_labels_with_precision() {
        let ticks = vec![0.0, 0.5, 1.0];
        let labels = generate_tick_labels(&ticks, 1.0, 1);
    ...
    #[test]
    fn test_format_axis_unit() {
        assert_eq!(format_axis_unit("s", "µ"), "µs");
        assert_eq!(format_axis_unit("V", "m"), "mV");
        assert_eq!(format_axis_unit("Hz", "k"), "kHz");
        assert_eq!(format_axis_unit("s", ""), "s");
    }
    ...
    #[test]
    fn test_generate_grid_lines() {
        let ticks = calculate_ticks(0.0, 10.0, 5);
        let grid = generate_grid_lines(&ticks);

        // Should have both major and minor lines
        let major_count = grid
            .iter()
            .filter(|g| g.line_type == GridLineType::Major)
            .count();
        let minor_count = grid
            .iter()
            .filter(|g| g.line_type == GridLineType::Minor)
            .count();
    ...
    #[test]
    fn test_grid_lines_sorted() {
        let ticks = calculate_ticks(0.0, 10.0, 5);
        let grid = generate_grid_lines(&ticks);

        // Verify lines are sorted by position
        for i in 1..grid.len() {
            assert!(grid[i].position >= grid[i - 1].position);
        }
    }

    // =========================================================================
    // Time/Voltage Formatting Tests
    // =========================================================================

    #[test]
    fn test_format_time() {
        let result = format_time(1e-9);
        assert!(result.contains("n") && result.contains("s"));

        let result2 = format_time(1e-3);
        assert!(result2.contains("m") && result2.contains("s"));
    }

    #[test]
    fn test_format_time_delta() {
        let result = format_time_delta(500e-9);
        assert!(result.contains("Δt"));
        assert!(result.contains("ns"));
    }

    #[test]
    fn test_format_frequency() {
        let result = format_frequency(1e6);
        assert!(result.contains("MHz") || (result.contains("M") && result.contains("Hz")));
    }

    #[test]
    fn test_period_to_frequency() {
        assert!((period_to_frequency(1e-3) - 1000.0).abs() < 1e-10);
        assert!((period_to_frequency(1e-6) - 1e6).abs() < 1.0);
        assert_eq!(period_to_frequency(0.0), 0.0);
        assert_eq!(period_to_frequency(-1.0), 0.0);
    }

    #[test]
    fn test_format_voltage() {
        let result = format_voltage(0.001);
        assert!(result.contains("mV"));

        let result2 = format_voltage(5.0);
        assert!(result2.contains("V"));
    }

    #[test]
    fn test_format_voltage_delta() {
        let result = format_voltage_delta(0.5);
        assert!(result.contains("Δv"));
    }

    #[test]
    fn test_format_current() {
        let result = format_current(0.001);
        assert!(result.contains("mA"));
    }
}
