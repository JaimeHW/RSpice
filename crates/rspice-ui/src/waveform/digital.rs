...
mod tests {
    ...
    #[test]
    fn test_logic_state_from_voltage() {
        let config = DigitalWaveformConfig::cmos_3v3();

        assert_eq!(LogicState::from_voltage(0.0, &config), LogicState::Low);
        assert_eq!(LogicState::from_voltage(0.5, &config), LogicState::Low);
        assert_eq!(LogicState::from_voltage(1.5, &config), LogicState::Unknown);
        assert_eq!(LogicState::from_voltage(2.5, &config), LogicState::High);
        assert_eq!(LogicState::from_voltage(3.3, &config), LogicState::High);
    }

    #[test]
    fn test_digital_signal_from_analog() {
        let config = DigitalWaveformConfig::cmos_3v3();

        // Simple clock signal: 0 -> 3.3 -> 0 -> 3.3
        let times = vec![0.0, 1.0, 2.0, 3.0, 4.0];
        let voltages = vec![0.0, 3.3, 0.0, 3.3, 0.0];

        let signal = DigitalSignal::from_analog("CLK", &times, &voltages, &config);

        assert_eq!(signal.name, "CLK");
        assert_eq!(signal.initial_state, LogicState::Low);
    ...
    #[test]
    fn test_digital_signal_state_at() {
        let config = DigitalWaveformConfig::cmos_3v3();

        let times = vec![0.0, 1.0, 2.0];
        let voltages = vec![0.0, 3.3, 0.0];

        let signal = DigitalSignal::from_analog("DATA", &times, &voltages, &config);

        assert_eq!(signal.state_at(0.5), LogicState::Low);
        assert_eq!(signal.state_at(1.5), LogicState::High);
        assert_eq!(signal.state_at(2.5), LogicState::Low);
    }

    #[test]
    fn test_digital_bus_value() {
        let config = DigitalWaveformConfig::cmos_3v3();

        // Create 4-bit bus
        let times = vec![0.0, 1.0];
        let signals = vec![
            DigitalSignal::from_analog("D0", &times, &[3.3, 3.3], &config), // 1
            DigitalSignal::from_analog("D1", &times, &[0.0, 0.0], &config), // 0
            DigitalSignal::from_analog("D2", &times, &[3.3, 3.3], &config), // 1
            DigitalSignal::from_analog("D3", &times, &[0.0, 0.0], &config), // 0
        ];

        let bus = DigitalBus::new("DATA", signals);

        // Value should be 0b0101 = 5
        assert_eq!(bus.value_at(0.5), Some(5));
        assert_eq!(bus.format_value(5), "5"); // Hex format, 1 digit
    }
