...
Progress at `f03724409`: authored optimization terms, scoring, retained observations, and their exact evidence checks now live in `results::optimization`; persisted family metadata and result digests import that owner directly. The executor retains an internal re-export for callers until R04 removes it. The guard now measures `services → simulation` 27/8 and `state → simulation` 11/9; `io → simulation` remains 15/13. Native unit-test compilation, 39 optimization tests, browser-app compilation, and browser-worker compilation pass. R00 and R01 remain open.
