...
pub fn drafts() -> Vec<CaseDraft> {
    ...
    drafts.push(rc_low_pass(RcCase {
        id: "repro.rc-charge.004",
        primary: "deterministic_reproducibility",
        extra: vec!["transient"],
        repetitions: 3,
        pulse: step_pulse(0.0, 5.5, 9e-5, 1e-6),
        r: 2.7e3,
        c: 3.3e-7,
        stop: 4.59e-3,
        tmax: 3e-6,
    ...
    drafts.push(rl_decay(RlCase {
        id: "repro.rl-step.003",
        primary: "deterministic_reproducibility",
        extra: vec!["transient"],
        repetitions: 3,
        pulse: step_pulse(0.0, 6.0, 4e-6, 1e-7),
        r: 1.5e2,
        l: 3e-3,
        stop: 6.4e-5,
        tmax: 2e-7,
    ...
    drafts.push(rc_low_pass(RcCase {
        id: "repro.rc-settle.002",
        primary: "deterministic_reproducibility",
        extra: vec!["transient"],
        repetitions: 3,
        pulse: step_pulse(0.0, 2.4, 6e-5, 1e-6),
        r: 9.1e2,
        c: 2.2e-7,
        stop: 2.464e-3,
        tmax: 4e-6,
    ...
    for (id, pulse, r, c, stop, tmax, absolute, relative) in [
        ...
        (
            // Probed mid-decay after three edge-heavy cycles: the error
            // here is set by the step controller's truncation tolerance,
            // not the ceiling, so the claim is half a percent.
            "mixed.square-lp.006",
            Pulse {
                initial: 0.0,
                pulsed: 6.0,
        ...
        (
            // Probed a fifth of a time constant after a fall that itself
            // interrupted a mid-charge ramp: the most step-controller-
            // limited point in the family, so the claim is one percent.
            "mixed.square-lp.008",
            Pulse {
                initial: 0.0,
                pulsed: 10.0,
