...
def test_every_family_publishes_the_shared_document(results):
    ...
    for kind, result in results:
        ...
        # Version 5 adds explicit sensitivity availability; older readers must
        # reject it before trying to decode the new payload fields.
        assert document["schemaVersion"] == 5, kind
...
@pytest.mark.parametrize("expression, expected_dc", [
    ...
    ("exp(-1000000*(ln(2+cos(2*pi*64meg*time+0.1))+0.5*ln(2+cos(2*(2*pi*64meg*time+0.1)))-1.5)^2)", 0.001042689086571716),
    ("exp(-1000000*(2.302585092994046*(log10(2+cos(2*pi*64meg*time+0.1))+0.5*log10(2+cos(2*(2*pi*64meg*time+0.1))))-1.5)^2)", 0.001042689086571716),
    ("exp(-1000000*(sqrt(2+cos(2*pi*64meg*time+0.1))+0.5*sqrt(2+cos(2*(2*pi*64meg*time+0.1)))-2.4)^2)", 0.001069489245389935),
    ("exp(-1000000*(abs(cos(2*pi*64meg*time+0.1))+0.5*abs(cos(2*(2*pi*64meg*time+0.1)))-0.75)^2)", 0.04282819034132224),
    ("exp(-1000000*(max(cos(2*pi*64meg*time+0.1),-cos(2*pi*64meg*time+0.1))+0.5*max(cos(2*(2*pi*64meg*time+0.1)),-cos(2*(2*pi*64meg*time+0.1)))-0.75)^2)", 0.04282819034132224),
    ("exp(-1000000*(min(cos(2*pi*64meg*time+0.1),-cos(2*pi*64meg*time+0.1),1)+0.5*min(cos(2*(2*pi*64meg*time+0.1)),-cos(2*(2*pi*64meg*time+0.1)),1)+0.75)^2)", 0.04282819034132224),
    ("exp(-1000000*(atan(cos(2*pi*64meg*time+0.1))+0.5*atan(cos(2*(2*pi*64meg*time+0.1)))-0.25)^2)", 0.00040382551583931654),
    ("exp(-1000000*(sinh(cos(2*pi*64meg*time+0.1))+0.5*sinh(cos(2*(2*pi*64meg*time+0.1)))-0.25)^2)", 0.00028901630323141536),
    ("exp(-1000000*(cosh(cos(2*pi*64meg*time+0.1))+0.5*cosh(cos(2*(2*pi*64meg*time+0.1)))-1.7)^2)", 0.01350698003904257),
    ("exp(-1000000*(tanh(cos(2*pi*64meg*time+0.1))+0.5*tanh(cos(2*(2*pi*64meg*time+0.1)))-0.25)^2)", 0.00041029391299753326),
...
def test_pss_nonlinear_time_features_survive_python_and_pickle(expression, expected_dc):
    ...
    for waveform in (result, restored):
        ...
        if bounded_composition:
            ...
            elif expression.startswith("atan("):
                low, high = (-0.006425394680575902, 0.012850531334919424)
            else:
