...
pub struct SensConfig {
    ...
    pub sweep: Option<crate::config::SensitivitySweep>,
}
...
impl SensConfig {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        crate::config::SensitivityConfig {
...
impl SensDialogState {
    pub fn from_config(config: &SensConfig) -> Self {
        Self {
            ...
            ac_sweep_idx: config.sweep.map_or(0, |sweep| match sweep.variation {
                crate::config::AcSweepType::Decade => 0,
                crate::config::AcSweepType::Octave => 1,
                crate::config::AcSweepType::Linear => 2,
            }),
            initialized: true,
        }
    }
    ...
    pub fn to_config(&self) -> Result<SensConfig, String> {
        ...
        let freq = if sens_type == SensType::Ac {
            crate::options::parse_si_value(&self.ac_freq)
        ...
        let sweep = if sens_type == SensType::Ac && !self.ac_stop.trim().is_empty() {
            let stop_frequency = crate::options::parse_si_value(&self.ac_stop)
            ...
            let variation = match self.ac_sweep_idx {
                0 => crate::config::AcSweepType::Decade,
                1 => crate::config::AcSweepType::Octave,
                2 => crate::config::AcSweepType::Linear,
                _ => return Err("Select a valid sweep kind (dec, oct or lin)".to_owned()),
            };
            Some(crate::config::SensitivitySweep {
                stop_frequency,
                points,
                variation,
            })
        ...
        let config = SensConfig {
            ...
            sens_type,
            ac_freq: freq,
            filter: crate::config::canonical_sensitivity_filter(&self.filter),
            sweep,
        };
        config.validate()?;
        Ok(config)
...
mod tests {
    ...
    fn a_swept_sensitivity_card_carries_its_sweep() {
        use crate::config::{AcSweepType, SensitivityConfig};
