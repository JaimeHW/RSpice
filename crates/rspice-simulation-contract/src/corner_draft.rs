//! Portable Corner draft and legacy run-set migration.

use crate::corner_config::{CornerBaseAnalysis, CornerConfig};
...
struct CornerDialogStateRepr {
    ...
    #[serde(default)]
    base_analysis: Option<rspice_app_types::product::AnalysisInstanceId>,
    #[serde(default)]
    process_tt: bool,
    #[serde(default)]
    process_ss: bool,
...
fn migrate_legacy_space(repr: &CornerDialogStateRepr) -> Option<RunSetState> {
    ...
    state.composition.mode = if repr.full_matrix {
        crate::run_set::RunSetCompositionMode::Cartesian
    } else {
        crate::run_set::RunSetCompositionMode::Zipped
    };
