...
use crate::fourier_output::{fourier_card_output, split_fourier_output};
...
impl FourierConfig {
    ...
    pub fn outputs(&self) -> Vec<String> {
        ...
        if !self.output_node.trim().is_empty() {
            outputs.push(fourier_card_output(&self.output_node, &self.output_ref));
        }
        outputs.extend(
...
mod tests {
    ...
    fn the_fourier_card_carries_every_output_it_was_given() {
        ...
        assert_eq!(
            spelled,
            vec!["V(OUT,REF)", "V(MID,OUT)", "I(V1)", "V(IN)"],
            "{outputs:?}"
        );
    }
