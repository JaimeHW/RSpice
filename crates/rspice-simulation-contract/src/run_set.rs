//! Persisted run-set dimensions, authored values, budgets, and composition.
...
fn canonical_value(lexical: &str, kind: RunSetDimensionKind) -> Option<f64> {
    ...
    match kind {
        ...
        RunSetDimensionKind::Parameter | RunSetDimensionKind::Source => {
            let value = crate::options::parse_si_value(text).ok()?;
        ...
        RunSetDimensionKind::Temperature => {
            ...
            let stripped = text
                ...
                .trim_end_matches('°')
                .trim();
            let value = crate::options::parse_si_value(stripped).ok()?;
