...
fn a_monte_carlo_card_publishes_the_selected_mean_confidence() {
    ...
    let response = job.execute(
        "MC confidence\n.param rload=1k\nV1 in 0 1\nR1 in out {rload}\nR2 out 0 1k\n.mc 8 START 37 SEED 7 UNIFORM 0.1 CONFIDENCE 90 CI BOOTSTRAP RESAMPLES 257 BOOTSEED 18446744073709551615\n.end\n",
        "monte_carlo",
    );
    assert_eq!(response["status"], "succeeded", "{response}");
    let document = typed_result(&job, &response, "mc-001.result.json");
    ...
    assert_eq!(
        ...
        Some([37, 38, 39, 40, 41, 42, 43, 44].as_slice())
    );
    assert_eq!(
        scalar_value(&document, "first_trial"),
