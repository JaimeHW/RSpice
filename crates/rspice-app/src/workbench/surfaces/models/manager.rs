...
/// gate is a sibling of this module and reaches them through here.
#[cfg(test)]
pub(super) use bindings::CONSUMER_INDEX_BUILDS;
#[cfg(test)]
pub(super) use drift::SOURCE_DRIFT_HASHES;
...
enum ManagerAction {
    ...
    /// Rehash every pinned source and record what no longer matches. Queued
    /// from a frame and run outside it: the frame decides *that* the corpus
...
pub(super) fn show(ui: &mut Ui, app: &mut RSpiceApp) {
    ...
    if drift::needs_scan(&app.state) {
        pending_actions.push(ManagerAction::ScanSourceDrift);
    }
    ...
    if page == ModelsPage::Qualification {
        qualification_page::show(ui, app);
    } else {
...

    {
        ...
        dialogs::render_dialog(ui, &mut render, &hub_catalog);
    }
                    ...
                    ScrollArea::horizontal()
                        ...
                        .show(ui, |ui| {
                            match app.state.workbench.models_view.catalog_scope {
                                ...
                                ModelsCatalogScope::InstalledPacks => {
                                    // One rail for the ledger, in the bar where
                                    // every other scope keeps its own. The
                                    // shipped-corpus table below the ledger
                                    // carries its own chips beside itself,
