...
use pvt_preparation::{
    expand_pvt_point_tasks, materialize_spec_run_environment_source, operating_point_config,
    operating_point_spec, point_is_nominal, prepare_pvt_point_source,
    validate_retained_operating_point_contract,
};
...
pub(in crate::simulation) struct PreparedTask {
    ...
    /// Which points of the global Run Set the authored analysis declared this
    /// task at. Consumed by the expansion below and by nothing after it: it
...
impl PreparedTask {
    ...
    /// Declare which points of the global Run Set this task runs at.
    ///
    /// Only the plan-backed builder calls it. A manual deck carries its own
    /// `.step` and `.temp` directives and has no global Run Set to participate
    /// in, so it keeps the default.
    pub(in crate::simulation) fn with_run_at(
        mut self,
        run_at: crate::simulation::run_set::AnalysisRunAt,
    ) -> Self {
        self.run_at = run_at;
    ...
    /// Which points of the global Run Set this task's analysis declared, for
    /// the gates that must price the queue before it is expanded.
...
impl PreparedRunSnapshot {
    ...
    pub(in crate::simulation) fn new(mut parts: SnapshotParts) -> Result<Self, PreparationError> {
        ...
        parts.tasks = if global_run_set_has_axes {
            // Resolved before anything is minted, and from the run set's own
        ...
        // Preflight/dispatch parity, enforced where every caller passes.
        //
        // A snapshot exists to be dispatched, and dispatch's first act is to
        // seal a receipt from it. A snapshot whose receipt would be refused is
        // therefore not a runnable plan — and constructing it here means the
        // refusal reaches preflight, campaign preparation and manual-deck
        // validation as a named blocker instead of surfacing after a run was
        // reported queued. The receipt itself is rebuilt at dispatch from the
        // same sealer over the same facts, so this is a check, not a cache.
        snapshot.prepared_run_receipt()?;
...
fn validate_prepared_task_integrity(
    ...
    saved_outputs::validate_task(task)
}
...
fn derive_pvt_points(
    ...
    if let Some(run_set) = run_set {
        ...
        for point in points {
            let run_set_point_key = point.point_key();
            ...
            prepared.push(PreparedPvtPoint {
                ...
                run_set_point_key: Some(run_set_point_key),
            });
        }
