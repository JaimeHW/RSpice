...
impl WaveformWriter {
    ...
    fn touchstone_text(dataset: &WaveformDataset) -> Result<String, String> {
        ...
        let z0 = z0_by_port[0];
        let uniform_reference = z0_by_port.iter().all(|value| *value == z0);
        ...
        if version >= 2 {
            // Touchstone v2 fixes the opening of the file: `[Version]`, then
            // the option line, then `[Number of Ports]` as the first keyword
            // after it. `[Two-Port Data Order]` is required of a two-port file
            // and permitted of no other (Touchstone 2.0 specification, "File
            // Format Description", "[Number of Ports]" and "[Two-Port Data
            // Order]").
            contents.push_str("[Version] 2.0\n");
            contents.push_str(&format!("# Hz S RI R {}\n", z0));
            contents.push_str(&format!("[Number of Ports] {}\n", num_ports));
            if num_ports == 2 {
                contents.push_str("[Two-Port Data Order] 21_12\n");
            }
            contents.push_str(&format!("[Number of Frequencies] {}\n", frequencies.len()));
            if !noise.is_empty() {
                contents.push_str(&format!("[Number of Noise Frequencies] {}\n", noise.len()));
            }
        ...
        if !noise.is_empty() {
            contents.push_str("! Noise parameters referenced to a 290 K source\n");
            if version >= 2 {
                contents.push_str("[Noise Data]\n");
            }
            for [frequency, db, magnitude, angle, resistance] in noise {
                contents.push_str(&format!(
                    "{frequency:e} {db:e} {magnitude:e} {angle:e} {resistance:e}\n"
                ));
            }
        }
        if version >= 2 {
    ...
    fn extract_touchstone_matrix(
        ...
        for row in 1..=max_port {
            for col in 1..=max_port {
                ...
                if real.data.len() != expected_len
                    || imag.data.len() != expected_len
                    || real.x_values.as_deref().is_some_and(|axis| axis != x.data)
                    || imag.x_values.as_deref().is_some_and(|axis| axis != x.data)
                {
                    return Err(format!(
...
mod tests {
    ...
    /// which Touchstone v2 requires be declared by `[Two-Port Data Order]`. The
    /// v2 keywords are written in the order the specification prescribes: the
    /// option line follows `[Version]` and precedes `[Number of Ports]`, which
    /// is the first keyword after it (Touchstone 2.0 specification, "File
    /// Format Description" and "[Number of Ports]").
    #[test]
    ...
    /// `[Two-Port Data Order]` is permitted only on a 2-port file, so a wider
    /// export must not carry it.
    #[test]
    fn touchstone_text_export_omits_two_port_data_order_on_wider_networks() {
        let text = WaveformWriter::new(WaveformFormat::Touchstone)
            .write_text(&positional_touchstone_dataset(3, 2))
            .expect("touchstone text serializes");

        assert!(!text.contains("Two-Port Data Order"), "{text}");
        assert!(text.contains("[Number of Ports] 3\n"), "{text}");
    }
    ...
    #[test]
    fn touchstone_export_rejects_oversized_matrices_and_unsupported_versions() {
        let mut dataset = sample_touchstone_dataset();
        dataset.add_signal(WaveformSignal::new("S1000000_1_RE", SignalType::SParameter));
        assert!(
        ...
        let mut dataset = sample_touchstone_dataset();
        dataset.x_signal.as_mut().unwrap().data.reverse();
        assert!(
            WaveformWriter::new(WaveformFormat::Touchstone)
                .write_text(&dataset)
                .unwrap_err()
        ...
        for version in ["0", "3", "unknown"] {
            ...
            dataset
                .metadata
                .insert("touchstone_version".into(), version.into());
            assert!(
                WaveformWriter::new(WaveformFormat::Touchstone)
                    .write_text(&dataset)
                    .unwrap_err()
