...
impl Instance {
	pub fn evaluate_noise_processes_at_frequency(&self, ctx: &GeneratedEvalContext<'_>, frequency_hz: f64, visitor: &mut dyn GeneratedNoiseProcessVisitor) -> Result<(), GeneratedNoiseEvaluationError> {
		...
		let process_0_table = [];
		let process_0_gain_0 = GeneratedNoiseComplex::scaled_transfer(DL, 0.0, frequency_hz, -1.0 * self.multiplicity.sqrt());
		...
		let process_1_table = [];
		let process_1_gain_0 = GeneratedNoiseComplex::scaled_transfer(DM, 0.0, frequency_hz, -1.0 * self.multiplicity.sqrt());
