...
impl Parameters {
    fn new_box() -> Box<Self> {
        ...
        unsafe {
            ...
            std::ptr::copy_nonoverlapping(DEFAULTS_0.as_ptr(), (*ptr).values.as_mut_ptr().add(0), 50);
            let params = &*ptr;
...
pub(crate) const PARAMETER_MODEL_FLAGS: [bool; 50] = [
    false, false, false, false, false, false, false, false, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true,
...
fn parameter_computed_max_bound(parameters: &Parameters, index: usize) -> Result<Option<ParameterBound>, String> {
    let params = parameters;
    let bound: Option<ParameterBound> = match index {
        29 => Some(ParameterBound { value: (1.0 - params[27]), label: "computed upper-bound expression" }),
        _ => None,
    };
    if let Some(bound) = bound {
        validate_finite_parameter(bound.label, bound.value)?;
...
impl Instance {
    ...
    pub const CHECKPOINT_MODEL_IDENTITY: &'static str = "e1b89db5e163d83e4fb4a8f06ae021b279304fa728b015a4b6a99ba9a6012ef3";
