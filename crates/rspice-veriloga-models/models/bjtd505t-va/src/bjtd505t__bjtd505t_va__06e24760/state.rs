	...
	fn new_box_with_simulation_parameters(simulation_parameters: &GeneratedSimulationParameters) -> Box<Self> {
		...
		unsafe {
			...
			const DEFAULTS: [f64; 147] = [
				...
				0.0, 2.0, 400.0, 1e-40, 1e-40, 0.0, 0.0, 1.0,
				0.0, 0.16, 0.0,
			];
...
pub struct Instance {
	...
	pub(crate) canonical_reactive: Box<[f64; 110]>,
}
...
impl Instance {
	...
	pub const PARAMETER_DESCRIPTORS: [P; 147] = [
		...
		P::model("swnlsh", Some(0.0)).integer().minimum(B::inclusive(0.0)).maximum(B::inclusive(1.0)),
		P::model("rth", Some(300.0)).minimum(B::exclusive(0.0)),
		P::model("cth", Some(3.0000000000000004e-9)).minimum(B::inclusive(0.0)),
		P::model("ath", Some(0.0)),
	...
	pub const VARIABLE_COUNT: usize = 589;
	pub const DDT_STATE_COUNT: usize = 10;
	pub const IDT_STATE_COUNT: usize = 0;
	pub const IDTMOD_STATE_COUNT: usize = 0;
	pub const ACCEPTED_STATE_SHAPE_IDENTITY: GeneratedVerilogAAcceptedStateShapeIdentity = GeneratedVerilogAAcceptedStateShapeIdentity::from_bytes([207, 63, 156, 23, 159, 120, 101, 96, 243, 147, 209, 14, 173, 50, 28, 64, 152, 170, 46, 37, 251, 132, 252, 240, 223, 211, 57, 193, 153, 105, 149, 121]);
	...
	pub const CHECKPOINT_MODEL_IDENTITY: &'static str = "1f6443acfe299ec1a3de9b4440e997571d748b351751daab1e7755e74305499c";
	...
	pub fn restore_rollback_state(&mut self, state: &GeneratedVerilogARollbackState) {
		debug_assert_eq!(state.values.len(), 54);
